...
| `cninfo_c_class_mappers.py` | **Phase 4**：C 类 basic + security + executive + share_capital profile mapper 草案 | 是 |
...
- `fixtures/c_class/` — known-company fixtures（12 条）+ probe records + basic_profile（2）+ security_profile（3）+ executive_profile（6）+ **share_capital_profile（6）**
...
5. **下一步**：shareholder_profile mapper draft（top + float）；E/F 暂缓。
