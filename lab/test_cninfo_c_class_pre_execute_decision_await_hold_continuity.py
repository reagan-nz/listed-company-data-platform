"""
CNINFO C-class — Pre-EXECUTE decision-await hold continuity 单测（离线 · CNINFO=0 · C-FM-11）。

运行：
    python3 lab/test_cninfo_c_class_pre_execute_decision_await_hold_continuity.py
"""
...
from cninfo_c_class_pre_execute_decision_await_hold_continuity import (  # noqa: E402
    ...
    assert_decision_await_continuity_output_root,
    build_fm01_to_10_gate_battery_rows,
    fingerprint_continuity_matrix,
    recompute_readiness_fingerprint,
    run_pre_execute_decision_await_hold_continuity,
)

_RUNNER = os.path.join(
    _LAB_DIR, "run_cninfo_c_class_pre_execute_decision_await_hold_continuity.py"
)
_TEST_SUMMARY_REL = (
    "outputs/validation/"
    "cninfo_c_class_pre_execute_decision_await_hold_continuity_test_summary_20260715.md"
)
...
class TestPreExecuteDecisionAwaitHoldContinuity(unittest.TestCase):
    def test_output_root_requires_mock_and_not_fm06_to_fm10(self) -> None:
        ...
        with self.assertRaises(RuntimeError):
            assert_decision_await_continuity_output_root(FM06_MOCK_ROOT_REL)
        with self.assertRaises(RuntimeError):
            assert_decision_await_continuity_output_root(FM07_MOCK_ROOT_REL)
        with self.assertRaises(RuntimeError):
            assert_decision_await_continuity_output_root(FM08_MOCK_ROOT_REL)
        with self.assertRaises(RuntimeError):
            assert_decision_await_continuity_output_root(FM09_MOCK_ROOT_REL)
        with self.assertRaises(RuntimeError):
            assert_decision_await_continuity_output_root(FM10_MOCK_ROOT_REL)
        norm = assert_decision_await_continuity_output_root(
            DEFAULT_MOCK_OUTPUT_ROOT_REL
        )
        self.assertIn("_mock_c_fm11", norm)
    ...
    def test_fm_battery_requires_all_ten(self) -> None:
        good = {
            "gate": "PASS_OFFLINE",
            "cninfo_calls": 0,
            "execute_production_snapshot_rebuild": False,
        ...
        bad = dict(good)
        bad["gate"] = "FAIL"
        _rows, checks = build_fm01_to_10_gate_battery_rows(
            fm01=good,
            fm02=good,
            fm03=good,
            fm04=good,
        ...
        self.assertFalse(checks["fm01_to_10_battery_all_pass"])

    def test_recompute_matches_frozen_readiness(self) -> None:
        frozen_file_fp, recomputed_fp = recompute_readiness_fingerprint(
            probe_output_root_rel=(
                "outputs/validation/_mock_c_fm11_unit_readiness_probe"
            )
        )
        self.assertEqual(
            frozen_file_fp["fingerprint_sha256"], FROZEN_READINESS_FP_SHA256
        )
        self.assertEqual(
            recomputed_fp["fingerprint_sha256"], FROZEN_READINESS_FP_SHA256
        )
        self.assertEqual(frozen_file_fp["fail_count"], 0)

    def test_continuity_matrix_fingerprint_stable(self) -> None:
        rows = [
            {
                "check_id": "a",
        ...
        fp1 = fingerprint_continuity_matrix(rows)
        fp2 = fingerprint_continuity_matrix(rows)
        self.assertEqual(fp1["fingerprint_sha256"], fp2["fingerprint_sha256"])

    def test_full_continuity_pass_isolated_mock(self) -> None:
        result = run_pre_execute_decision_await_hold_continuity(
            paths=DecisionAwaitContinuityPaths(
                output_root_rel="outputs/validation/_mock_c_fm11_cli_test_tmp"
            )
        )
        ...
        self.assertEqual(
            result["continuity_packet"]["decision_status"],
            "AWAITING_HUMAN_EXECUTE_DECISION",
        )
        self.assertTrue(
            result["continuity_packet"]["idle_not_required_while_awaiting"]
        )
        self.assertTrue(result["continuity_packet"]["ready_for_commit"])
        self.assertFalse(result["continuity_packet"]["ready_for_execute"])
        self.assertEqual(
            result["continuity_packet"]["controller_action"],
            "DECISION_AWAIT_HOLD_CONTINUITY_ONLY",
        )
        self.assertEqual(
            result["frozen_readiness_fingerprint_sha256"],
            FROZEN_READINESS_FP_SHA256,
        )
        self.assertTrue(
        ...
        self.assertTrue(
            os.path.isfile(
                os.path.join(BASE_DIR, result["continuity_packet_path"])
            )
        )
        self.assertTrue(
        ...
        self.assertEqual(battery["fm11_gate"], "PASS_OFFLINE")
        # 确认未覆盖 MOCK8 / MOCK10 / MOCK11 / MOCK12
        ...
        fm10_fp = os.path.join(
            BASE_DIR, FM10_MOCK_ROOT_REL, "readiness_fingerprint.json"
        )
        with open(fm10_fp, encoding="utf-8") as fh:
            rfp = json.load(fh)
        self.assertEqual(rfp["task_id"], "C-FM-10")
        self.assertEqual(
            rfp["fingerprint"]["fingerprint_sha256"], FROZEN_READINESS_FP_SHA256
        )
    ...
    def test_cninfo_not_called(self) -> None:
        with mock.patch("requests.get") as get_mock, mock.patch(
            ...
            run_pre_execute_decision_await_hold_continuity(
                paths=DecisionAwaitContinuityPaths(
                    output_root_rel=(
                        "outputs/validation/_mock_c_fm11_unit_cninfo_probe"
                    )
                )
            )
...
if __name__ == "__main__":
    ...
    suite = loader.loadTestsFromTestCase(TestPreExecuteDecisionAwaitHoldContinuity)
    result = unittest.TextTestRunner(verbosity=2).run(suite)
    cases = [
        {
            "case": "test_output_root_requires_mock_and_not_fm06_to_fm10",
            "result": "PASS",
        },
        {"case": "test_auth_index_write_still_forbidden", "result": "PASS"},
        {"case": "test_fm_battery_requires_all_ten", "result": "PASS"},
        {"case": "test_recompute_matches_frozen_readiness", "result": "PASS"},
        {"case": "test_continuity_matrix_fingerprint_stable", "result": "PASS"},
        {
            "case": "test_full_continuity_pass_isolated_mock",
            "result": "PASS",
        },
