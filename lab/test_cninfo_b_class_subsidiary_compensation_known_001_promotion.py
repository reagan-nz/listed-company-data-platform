"""
B-FM-39：子公司管理制度 / 薪酬与考核方案 known-document 晋升锁测（离线）。

覆盖：
- subsidiary_management_system_known_001 与
  compensation_assessment_plan_known_001 已为 ready
- title_pattern 与裸「管理制度」/「薪酬」可区分
- harvest 标题经硬化路由预测 announcement → general（非 other）
- 既有 LIVE_PASS 路径不回退（含 B-FM-38 monetary / external_guarantee）
- 不重开 monetary / external_guarantee / ined / gm 等 LIVE_PASS

无 CNINFO · 无 live · 不造 §7 FP。

运行：
    python lab/test_cninfo_b_class_subsidiary_compensation_known_001_promotion.py
"""
...
HARVEST_SUB = "分、子公司管理制度（2025年6月）"
HARVEST_COMP = "利尔化学股份有限公司2025年度经营团队薪酬与考核方案"
PATTERN_SUB = "子公司管理制度"
PATTERN_COMP = "薪酬与考核方案"
PATTERN_BARE = "管理制度"
BARE_SALARY = "某经营团队薪酬方案"
...
class TestSubsidiaryCompensationKnown001Promotion(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.config = routing._load_yaml(routing.DEFAULT_CATEGORIES)
        cls.known = _load_cases(KNOWN_YAML)

    def test_subsidiary_system_known_001_ready_fields(self) -> None:
        c = _by_id(self.known, "subsidiary_management_system_known_001")
        self.assertEqual(c["case_status"], "ready")
        self.assertEqual(c["company_code"], "300261")
        self.assertEqual(c["company_name"], "雅本化学")
        self.assertEqual(c["title_pattern"], PATTERN_SUB)
        ...
        self.assertIn(PATTERN_SUB, HARVEST_SUB)
        self.assertNotEqual(c["title_pattern"], "管理制度")
        self.assertNotEqual(c["title_pattern"], "制度")

    def test_compensation_plan_known_001_ready_fields(self) -> None:
        c = _by_id(self.known, "compensation_assessment_plan_known_001")
        self.assertEqual(c["case_status"], "ready")
        self.assertEqual(c["company_code"], "002258")
        self.assertEqual(c["company_name"], "利尔化学")
        self.assertEqual(c["title_pattern"], PATTERN_COMP)
        self.assertEqual(c["date_start"], "2025-06-09")
        self.assertEqual(c["date_end"], "2025-06-12")
        self.assertEqual(c["expected_document_type"], "announcement")
        self.assertEqual(c["expected_route_to"], "cninfo_general_announcement_pdf")
        self.assertIn(PATTERN_COMP, HARVEST_COMP)
        self.assertNotEqual(c["title_pattern"], "薪酬")
        self.assertFalse(retrieval._title_matches(BARE_SALARY, PATTERN_COMP))

    def test_patterns_mutually_distinct(self) -> None:
        """两案 pattern 与裸管理制度互斥。"""
        self.assertNotEqual(PATTERN_SUB, PATTERN_COMP)
        self.assertNotEqual(PATTERN_SUB, PATTERN_BARE)
        self.assertFalse(retrieval._title_matches(HARVEST_SUB, PATTERN_COMP))
        self.assertFalse(retrieval._title_matches(HARVEST_COMP, PATTERN_SUB))
        self.assertTrue(retrieval._title_matches(HARVEST_SUB, PATTERN_SUB))
        self.assertTrue(retrieval._title_matches(HARVEST_COMP, PATTERN_COMP))

    def test_harvest_titles_route_announcement(self) -> None:
        r_s = routing.route_title(HARVEST_SUB, self.config)
        self.assertEqual(r_s.predicted_document_type, "announcement")
        self.assertNotEqual(r_s.predicted_document_type, "other")
        self.assertEqual(r_s.predicted_route_to, "cninfo_general_announcement_pdf")
        r_c = routing.route_title(HARVEST_COMP, self.config)
        self.assertEqual(r_c.predicted_document_type, "announcement")
        self.assertNotEqual(r_c.predicted_document_type, "other")
    ...
    def test_prior_paths_not_regressed(self) -> None:
        ...
        r_g = routing.route_title("海量数据对外担保管理制度", self.config)
    ...
    def test_closed_live_pass_cases_still_ready(self) -> None:
        """已 LIVE_PASS 案仍为 ready（不降级）；含 B-FM-38 两案。"""
        for case_id in (
