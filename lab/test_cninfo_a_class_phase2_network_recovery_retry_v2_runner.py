"""
A-class Phase 2 network recovery retry v2 runner 测试（无 CNINFO · 无 live 执行）。

运行：
    python lab/test_cninfo_a_class_phase2_network_recovery_retry_v2_runner.py
"""
...
RETRY_V2_UNIVERSE = runner.DEFAULT_RETRY_V2_UNIVERSE_CSV
RETRY_V2_OUTPUT_ROOT = runner.DEFAULT_RETRY_V2_OUTPUT_ROOT
RETRY_V2_DRYRUN_REPORT = os.path.join(
    RETRY_V2_OUTPUT_ROOT, "reports", "a_class_phase2_retry_v2_dryrun_report.csv"
)
RETRY_V2_DRYRUN_SUMMARY = os.path.join(
    RETRY_V2_OUTPUT_ROOT, "reports", "a_class_phase2_retry_v2_dryrun_summary.md"
)
...
class TestAClassPhase2NetworkRecoveryRetryV2Runner(unittest.TestCase):
    def test_dry_run_retry_v2_calls_cninfo_zero_times(self) -> None:
        with mock.patch("requests.get") as get_mock, mock.patch("requests.post") as post_mock:
            result = _run(V2_DRYRUN_ARGS)
            self.assertEqual(result.returncode, 0, msg=result.stderr + result.stdout)
            get_mock.assert_not_called()
            post_mock.assert_not_called()
        self.assertIn("cninfo_calls=0", result.stdout)

    def test_retry_v2_universe_size_must_equal_8(self) -> None:
        result = _run(V2_DRYRUN_ARGS + ["--limit", "3"])
    ...
    def test_retry_v2_include_column_accepted(self) -> None:
        cases = runner.load_retry_universe(RETRY_V2_UNIVERSE)
        self.assertEqual(len(cases), 8)
        for case in cases:
            self.assertEqual(case.retry_include, "yes")

    def test_report_period_column_accepted_as_expected_period_alias(self) -> None:
        cases = runner.load_retry_universe(RETRY_V2_UNIVERSE)
    ...
    def test_input_universe_csv_not_mutated(self) -> None:
        before = _file_sha256(RETRY_V2_UNIVERSE)
        with open(RETRY_V2_UNIVERSE, newline="", encoding="utf-8") as f:
            header = next(csv.reader(f))
        self.assertIn("retry_v2_include", header)
        self.assertIn("report_period", header)
        self.assertNotIn("retry_include", header)
        result = _run(V2_DRYRUN_ARGS)
        self.assertEqual(result.returncode, 0, msg=result.stderr + result.stdout)
        after = _file_sha256(RETRY_V2_UNIVERSE)
        self.assertEqual(before, after)

    def test_retry_v2_output_root_accepted(self) -> None:
        ok, err = runner.validate_retry_v2_output_root(RETRY_V2_OUTPUT_ROOT)
    ...
    def test_live_mode_requires_retry_v2_approval_flag(self) -> None:
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertIn(runner.RETRY_V2_APPROVAL_REQUIRED, result.stderr)
    ...
    def test_ocr_extraction_blocked(self) -> None:
        for flag, err in (
            ...
            result = _run(V2_DRYRUN_ARGS + [flag])
            self.assertNotEqual(result.returncode, 0, msg=flag)
    ...
    def test_dry_run_retry_v2_report_generated(self) -> None:
        result = _run(V2_DRYRUN_ARGS)
        self.assertEqual(result.returncode, 0, msg=result.stderr + result.stdout)
        self.assertTrue(os.path.isfile(RETRY_V2_DRYRUN_REPORT))
        self.assertTrue(os.path.isfile(RETRY_V2_DRYRUN_SUMMARY))
        with open(RETRY_V2_DRYRUN_REPORT, newline="", encoding="utf-8") as f:
            rows = list(csv.DictReader(f))
        self.assertEqual(len(rows), runner.RETRY_REQUIRED_UNIVERSE_SIZE)
        self.assertEqual(set(rows[0].keys()), set(runner.RETRY_V2_DRYRUN_COLUMNS))
        case_ids = {row["case_id"] for row in rows}
        self.assertEqual(case_ids, runner.RETRY_ALLOWED_CASE_IDS)
        for row in rows:
            self.assertEqual(row["retry_v2_include"], "yes")
