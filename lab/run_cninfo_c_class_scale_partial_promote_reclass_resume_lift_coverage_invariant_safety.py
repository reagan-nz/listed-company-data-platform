...
residual lift denial + coverage invariant lock runner（离线 · C-FM-29）。

Usage:
    python3 lab/run_cninfo_c_class_scale_partial_promote_reclass_resume_lift_coverage_invariant_safety.py
    python3 lab/run_cninfo_c_class_scale_partial_promote_reclass_resume_lift_coverage_invariant_safety.py \\
      --output-root outputs/validation/_mock_c_fm29_scale_partial_promote_reclass_resume_lift_coverage_invariant_safety
"""
...
REPORT_ROOT_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_partial_promote_reclass_resume_lift_coverage_invariant_safety"
)
REPORT_MD_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_partial_promote_reclass_resume_lift_coverage_invariant_safety_20260715.md"
)
REPORT_JSON_REL = (
    "outputs/validation/"
...
TASK_REPORT_REL = (
    "outputs/validation/"
    "cninfo_c_class_fm29_scale_partial_promote_reclass_resume_lift_coverage_invariant_safety_20260715.md"
)
...
def write_reports(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    for key, name in (
        ("promote_reclass_path", "partial_promote_reclass_denial_ledger.json"),
        ("resume_same_path", "resume_same_hold_write_boundary_ledger.json"),
        ("residual_lift_path", "residual_lift_denial_ledger.json"),
        ("coverage_lock_path", "coverage_invariant_lock_ledger.json"),
    ):
    ...
    json_payload = {
        ...
        "promote_reclass_path": payload["promote_reclass_path"],
        "resume_same_path": payload["resume_same_path"],
        "residual_lift_path": payload["residual_lift_path"],
        "coverage_lock_path": payload["coverage_lock_path"],
    ...
    md_lines = [
        ...
        *layer_lines,
        ...
        "## Scale / safety gain",
        "",
        "- FM28 连续：unique=**2249** · status=**2134/106/9** · delta=**2** · "
        "coverage=**117** · membership/write-boundary fps",
        "- partial promote/reclass denial：**106** 码 · promote→complete 与跨带 "
        "reclass 拒绝",
        "- resume-same hold write-boundary：**301212** · harvest/force_improve/"
        "promote 拒绝",
        ...
        "",
        "```",
        f"c_fm_29_scale_partial_promote_reclass_resume_lift_coverage_invariant_safety_gate = "
        f"{payload['gate']}",
        "execute_production_snapshot_rebuild = false",
        "approved_for_snapshot_rebuild = false",
        "cninfo_calls = 0",
    ...
    task_lines = [
        ...
        "## Task",
        "",
        "在 C-FM-28 已 commit 且 EXECUTE 仍 human-held 之上，继续 **非 seal** "
        "规模/安全能力（非 extension↔drift / seal-chain）：**partial promote/"
        "reclass denial（106）**、**resume-same hold write-boundary（301212）**、"
        "**residual lift denial（9+2）**、**coverage invariant lock"
        "（117）**、**FM28 连续 + MOCK31 隔离**；产物写入隔离 MOCK31"
        "（不覆盖 MOCK3–30）。",
        "",
        "## Capability gain",
        "",
    ...
    with open(readme, "w", encoding="utf-8") as fh:
        fh.write(
            "\n".join(
                [
                    "# C-FM-29 mock scale partial promote/reclass resume lift coverage invariant safety root",
                    "",
                    f"gate: `{payload['gate']}`",
                    "seal_chain_extended: false",
                    f"scale_tier_count: {payload['scale_tier_count']}",
                    ...
                    f"surface_harvest_delta: {payload['surface_harvest_delta_n']}",
                    f"resume_same: {payload['resume_same']}",
                    f"partial_risk_bands: {bands}",
                    f"residual_safety_coverage: {payload['residual_safety_coverage']}",
                    "does_not_overwrite: _mock_c_fm02.._mock_c_fm28 / standard dryrun",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "C-FM-29 scale partial promote/reclass/resume-lift/coverage "
            "invariant safety (offline)"
        )
    )
    parser.add_argument(
        "--output-root",
    ...
    if args.execute:
        print(
            ...
            "C-FM-29 is offline QA only; KEEP_EXECUTE_FALSE",
            file=sys.stderr,
        )
        return 2

    payload = run_scale_partial_promote_reclass_resume_lift_coverage_invariant_safety(
        paths=PromoteReclassResumeLiftPaths(output_root_rel=args.output_root)
    )
