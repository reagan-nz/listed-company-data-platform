...
fence + partial risk-band rollup runner（离线 · C-FM-27）。

Usage:
    python3 lab/run_cninfo_c_class_scale_residual_disposition_quarantine_pending_fence_safety.py
    python3 lab/run_cninfo_c_class_scale_residual_disposition_quarantine_pending_fence_safety.py \\
      --output-root outputs/validation/_mock_c_fm27_scale_residual_disposition_quarantine_pending_fence_safety
"""
...
REPORT_ROOT_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_residual_disposition_quarantine_pending_fence_safety"
)
REPORT_MD_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_residual_disposition_quarantine_pending_fence_safety_20260715.md"
)
REPORT_JSON_REL = (
    "outputs/validation/"
...
TASK_REPORT_REL = (
    "outputs/validation/"
    "cninfo_c_class_fm27_scale_residual_disposition_quarantine_pending_fence_safety_20260715.md"
)
...
def write_reports(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    for key, name in (
        ("failed_disposition_path", "failed_disposition_quarantine_ledger.json"),
        ("surface_fence_path", "surface_delta_pending_fence_ledger.json"),
        ("partial_band_path", "partial_risk_band_rollup_ledger.json"),
    ):
    ...
    json_payload = {
        ...
        "failed_disposition_path": payload["failed_disposition_path"],
        "surface_fence_path": payload["surface_fence_path"],
        "partial_band_path": payload["partial_band_path"],
    ...
    md_lines = [
        ...
        *layer_lines,
        ...
        "",
        "```",
        f"c_fm_27_scale_residual_disposition_quarantine_pending_fence_safety_gate = "
        f"{payload['gate']}",
        "execute_production_snapshot_rebuild = false",
        "approved_for_snapshot_rebuild = false",
        "cninfo_calls = 0",
    ...
    task_lines = [
        ...
        "## Task",
        "",
        "在 C-FM-26 已 commit 且 EXECUTE 仍 human-held 之上，继续 **非 seal** "
        "规模/安全能力（非 extension↔drift / seal-chain）：**failed residual "
        "disposition quarantine（9）**、**surface−harvest delta pending fence"
        "（Δ2）**、**partial risk-band rollup（106）**、**FM26 连续 + MOCK29 隔离**；"
        "产物写入隔离 MOCK29（不覆盖 MOCK3–28）。",
        "",
        "## Capability gain",
        "",
    ...
    with open(readme, "w", encoding="utf-8") as fh:
        fh.write(
            "\n".join(
                [
                    "# C-FM-27 mock scale residual disposition quarantine pending fence safety root",
                    "",
                    f"gate: `{payload['gate']}`",
                    "seal_chain_extended: false",
                    f"scale_tier_count: {payload['scale_tier_count']}",
                    ...
                    "does_not_overwrite: _mock_c_fm02.._mock_c_fm26 / standard dryrun",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "C-FM-27 scale residual disposition/pending-fence/risk-band safety "
            "(offline)"
        )
    )
    parser.add_argument(
    ...
    if args.execute:
        print(
            ...
            "C-FM-27 is offline QA only; KEEP_EXECUTE_FALSE",
            file=sys.stderr,
        )
        return 2

    payload = run_scale_residual_disposition_quarantine_pending_fence_safety(
        paths=DispositionQuarantinePaths(output_root_rel=args.output_root)
    )
