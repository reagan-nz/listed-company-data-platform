"""
CNINFO C-class — 规模 union/overlap/surface/stable-wall-meta-bundle
单测（离线 · CNINFO=0 · C-FM-43）。

运行：
    python3 lab/test_cninfo_c_class_scale_union_overlap_delta_stable_wall_meta_bundle_safety.py
"""
...
from cninfo_c_class_scale_union_overlap_delta_stable_wall_meta_bundle_safety import (  # noqa: E402
    ...
    UnionOverlapDeltaStableWallMetaBundlePaths,
    assert_fm43_output_root,
    build_cross_stable_metrics_wall_meta_bundle_identity_lock_rows,
    build_fm42_continuity_rows,
    ...
    run_scale_union_overlap_delta_stable_wall_meta_bundle_safety,
)
...
_RUNNER = os.path.join(
    _LAB_DIR,
    "run_cninfo_c_class_scale_union_overlap_delta_stable_wall_meta_bundle_safety.py",
)
_TEST_SUMMARY_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_union_overlap_delta_stable_wall_meta_bundle_safety_test_summary_20260715.md"
)
...
def _write_test_summary(cases: list) -> None:
    ...
    lines = [
        "# C-FM-43 Scale Union/Overlap/Surface/Stable-Wall-Meta-Bundle — Test Summary",
        "",
        "_offline · CNINFO=0_",
        "",
        "| case | result |",
    ...
    lines.extend(
        [
            "",
            "```",
            "c_fm_43_scale_union_overlap_delta_stable_wall_meta_bundle_safety_test_gate = "
            "PASS_OFFLINE",
            "cninfo_calls = 0",
            ...
            f"cross_stable_metrics_wall_meta_bundle_sha256 = "
            f"{EXPECTED_CROSS_STABLE_METRICS_WALL_META_BUNDLE_SHA256}",
            "```",
            "",
        ]
    )
...
class TestCFm43UnionOverlapDeltaStableWallMetaBundle(unittest.TestCase):
    def test_constants_frozen(self) -> None:
        ...
        self.assertEqual(EXPECTED_UNION_STATUS_FORMULA, "2134/106/9")
        self.assertEqual(EXPECTED_OVERLAP_DELTA_FORMULA, "overlap_delta=12")
        self.assertEqual(EXPECTED_SURFACE_DELTA_FORMULA, "surface_delta=2")
        self.assertEqual(EXPECTED_RESUME_FORMULA, "28+1+0=29")
        self.assertEqual(len(EXPECTED_CROSS_STABLE_METRICS_WALL_META_BUNDLE), 5)
    ...
    def test_union_status_composition_mutation_denied(self) -> None:
        ...
        self.assertTrue(checks["union_status_composition_identity_lock_all_pass"])
        self.assertEqual(
            meta["fingerprint"], FROZEN_UNION_STATUS_COMPOSITION_IDENTITY_LOCK_FP_SHA256
        )
        fp, _doc = fingerprint_union_status_composition_identity_lock()
        self.assertEqual(fp, FROZEN_UNION_STATUS_COMPOSITION_IDENTITY_LOCK_FP_SHA256)

    def test_overlap_delta_composition_mutation_denied(self) -> None:
        mut = evaluate_overlap_delta_composition_mutation(
            proposed_overlap=13,
            proposed_unique=2249,
            proposed_additive=2261,
        )
        self.assertIs(mut["mutation_allowed"], False)
    ...
    def test_wall_meta_bundle_mutation_denied(self) -> None:
        ...
        mut = evaluate_cross_stable_metrics_wall_meta_bundle_mutation(
        ...
        self.assertTrue(
            checks["cross_stable_metrics_wall_meta_bundle_identity_lock_all_pass"]
        )
        self.assertEqual(
            meta["fingerprint"],
            FROZEN_CROSS_STABLE_METRICS_WALL_META_BUNDLE_IDENTITY_LOCK_FP_SHA256,
    ...
    def test_fm42_continuity(self) -> None:
        rows, checks = build_fm42_continuity_rows(
            UnionOverlapDeltaStableWallMetaBundlePaths()
        )
        self.assertTrue(checks.get("fm42_continuity_all_pass"), rows[-1])
    ...
    def test_output_root_guards(self) -> None:
        ...
        ok = assert_fm43_output_root(DEFAULT_MOCK_OUTPUT_ROOT_REL)
        self.assertTrue(ok)

    def test_full_offline_run(self) -> None:
        result = run_scale_union_overlap_delta_stable_wall_meta_bundle_safety()
        ...
        self.assertEqual(result["surface_delta_formula"], "surface_delta=2")
        self.assertEqual(result["additive_formula"], "2249+12=2261")
        self.assertEqual(result["overlap_delta_formula"], "overlap_delta=12")
        self.assertEqual(
            result["cross_stable_metrics_wall_meta_bundle_sha256"],
            EXPECTED_CROSS_STABLE_METRICS_WALL_META_BUNDLE_SHA256,
        )
        packet = os.path.join(BASE_DIR, result["packet_path"])
        with open(packet, encoding="utf-8") as fh:
            doc = json.load(fh)
        self.assertEqual(doc["task_id"], "C-FM-43")
        self.assertFalse(doc["approved_for_snapshot_rebuild"])

    def test_cli_runner_offline(self) -> None:
        tmp = os.path.join(
            BASE_DIR,
            "outputs/validation/_mock_c_fm43_cli_test_tmp",
        )
        proc = subprocess.run(
...
if __name__ == "__main__":
    ...
    suite = loader.loadTestsFromTestCase(
        TestCFm43UnionOverlapDeltaStableWallMetaBundle
    )
