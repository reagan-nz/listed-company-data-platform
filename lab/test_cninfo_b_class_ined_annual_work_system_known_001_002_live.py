"""
B-FM-44：独立董事年报工作制度 known_001/002 live allow-list 锁测（离线；不请求 CNINFO）。

覆盖：
- allow-list 仅含 independent_director_annual_report_work_system_known_001 与
  known_002（category 空）
- 字段与 fixtures ready 对齐
- mock live 路径可对两案得到 pass（predicted_document_type=announcement；
  非 annual_report / other）
- 不包含 independent_ned / continuous_supervision / company_articles 等已
  LIVE_PASS 案

无真实 CNINFO · 不造 §7 FP。

运行：
    python lab/test_cninfo_b_class_ined_annual_work_system_known_001_002_live.py
"""
...
LIVE_DIR = os.path.join(
    ...
    "outputs",
    "validation",
    "cninfo_b_class_ined_annual_work_system_known_001_002_live_20260715",
)
...
ANN_001 = "1223314160"
ANN_002 = "1223763870"
# 2025-04-25 / 2025-06-04 00:00:00 UTC
ANN_TS_001_MS = 1745539200000
ANN_TS_002_MS = 1748995200000
...
class TestInedAnnualWorkSystemKnown001002LiveAllowList(unittest.TestCase):
