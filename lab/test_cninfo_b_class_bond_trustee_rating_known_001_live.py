"""
B-FM-29：债券受托管理事务报告 / 跟踪评级报告 known-document live allow-list 锁测（离线；不请求 CNINFO）。

覆盖：
- allow-list 仅含 bond_trustee_report_known_001 与 tracking_rating_report_known_001（category 空）
- 字段与 fixtures ready 对齐
- mock live 路径可对两案得到 pass（predicted_document_type=announcement；非 other）
- 不包含 listing_sponsor / equity_change / verification / legal / supervisory / shareholder 已 LIVE_PASS 案

无真实 CNINFO · 不造 §7 FP。

运行：
    python lab/test_cninfo_b_class_bond_trustee_rating_known_001_live.py
"""
...
LIVE_DIR = os.path.join(
    ...
    "outputs",
    "validation",
    "cninfo_b_class_bond_trustee_rating_known_001_live_20260715",
)
...
ANN_TRUSTEE = "1223979550"
ANN_RATING = "1224013532"
# 2025-06-25 / 2025-06-27 00:00:00 UTC
ANN_TS_TRUSTEE_MS = 1750809600000
...
class TestBondTrusteeRatingKnown001LiveAllowList(unittest.TestCase):
    def test_allowlist_scope_exactly_two_known_cases(self) -> None:
        ...
        self.assertEqual(
            ...
            ["bond_trustee_report_known_001", "tracking_rating_report_known_001"],
        )
