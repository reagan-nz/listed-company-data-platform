...
# -*- coding: utf-8 -*-
"""
CNINFO D 类 equity_pledge — D-FM-41 next-slice offline planning smoke。
...
EP_CASE_IDS = ("DEP101", "DEP102", "DEP103", "DEP104", "DEP105")
EXCLUDED_CODES = {"688671", "301259"}
FORBIDDEN_EP_TDATE = {"2026-07-03"}
EXPECTED_ANCHOR = "2026-07-02"

PLANNING_MD = (
    BASE_DIR / "plans" / "cninfo_d_class_equity_pledge_next_slice_planning_20260715.md"
)
MATRIX_CSV = (
    BASE_DIR
    / "outputs"
    / "validation"
    / "cninfo_d_class_equity_pledge_es_shareholder_change_next_slice_candidate_matrix_20260715.csv"
)
SKETCH_CSV = (
    BASE_DIR
    / "outputs"
...
RECOMMENDATION_MD = (
    ...
    / "outputs"
    / "validation"
    / "cninfo_d_class_equity_pledge_next_slice_recommendation_20260715.md"
)
SUMMARY_MD = (
    BASE_DIR
    / "outputs"
    / "validation"
    / "cninfo_d_class_equity_pledge_next_slice_planning_summary_20260715.md"
)
NEXT_STEP_MD = (
    BASE_DIR
    / "outputs"
    / "validation"
    / "cninfo_d_class_equity_pledge_next_slice_next_step_recommendation_20260715.md"
)
CAVEAT_LEDGER_CSV = (
    BASE_DIR
    / "outputs"
    / "validation"
    / "cninfo_d_class_equity_pledge_next_slice_final_caveat_ledger.csv"
)
EVIDENCE_MD = (
    BASE_DIR
    / "outputs"
    / "validation"
    / "cninfo_d_class_equity_pledge_dfm41_next_slice_planning_20260715.md"
)
...
class EquityPledgeNextSlicePlanningOfflineTests(unittest.TestCase):
    """D-FM-41：equity_pledge next-slice sketch + frozen-root sha256。"""
    ...
    def test_universe_sketch_five_cases_dense_day(self) -> None:
        ...
        self.assertEqual(tuple(r["case_id"] for r in rows), EP_CASE_IDS)
        for row in rows:
            self.assertEqual(row["component"], "equity_pledge")
            ...
            self.assertNotIn(row["anchor_tdate"], FORBIDDEN_EP_TDATE)
    ...
    def test_sample_raw_matches_dense_day_cite(self) -> None:
        payload = json.loads(SAMPLE_RAW.read_text(encoding="utf-8"))
        self.assertEqual(payload["source_id"], "equity_pledge")
        self.assertEqual(payload["query_params"]["tdate"], EXPECTED_ANCHOR)
        self.assertEqual(payload["raw_record"]["SECCODE"], "000001")

    def test_candidate_matrix_ranks_ep_primary_and_forbids_live(self) -> None:
        rows = _load_csv(MATRIX_CSV)
        by_id = {r["option_id"]: r for r in rows}
        self.assertEqual(
            by_id["EQUITY_PLEDGE_NEXT_SLICE_OFFLINE"]["recommendation_rank"], "1"
        )
        self.assertEqual(
            by_id["EQUITY_PLEDGE_NEXT_SLICE_OFFLINE"]["status"], "primary_executed"
        )
        self.assertEqual(by_id["ES_NEXT_SLICE_OFFLINE"]["status"], "deferred")
        self.assertEqual(
        ...
        self.assertEqual(
            by_id["EQUITY_PLEDGE_FIRST_SLICE_RELIVE"]["status"], "forbidden"
        )
        self.assertEqual(
            by_id["FIA_AT_SD_FROZEN_ROOT_MUTATE"]["status"], "forbidden"
        )
        self.assertEqual(
            by_id["FIA_FURTHER_SCALE_BOUNDED_LIVE"]["status"], "forbidden_this_round"
        )
    ...
    def test_planning_and_gates_text(self) -> None:
        ...
        self.assertIn("DEP101", planning)
        self.assertIn("2026-07-02", planning)
        self.assertIn(
        ...
        self.assertIn("primary_component = equity_pledge", recommendation)
    ...
    def test_caveat_ledger_has_ep_ns_entries(self) -> None:
        rows = _load_csv(CAVEAT_LEDGER_CSV)
        ids = {r["caveat_id"] for r in rows}
        self.assertIn("CAV-EP-NS-001", ids)
        self.assertIn("CAV-EP-NS-002", ids)
        self.assertIn("CAV-EP-NS-003", ids)
        self.assertIn("CAV-EP-NS-007", ids)

    def test_checklist_blocks_live_and_runner(self) -> None:
        rows = _load_csv(CHECKLIST_CSV)
        by_id = {r["item_id"]: r for r in rows}
        self.assertEqual(by_id["EP-NS-STUB-05"]["status"], "forbidden_this_round")
        self.assertEqual(by_id["EP-NS-GATE-02"]["status"], "NOT_APPROVED")
        self.assertEqual(by_id["EP-NS-GATE-01"]["status"], "READY_FOR_APPROVAL")
        self.assertEqual(by_id["EP-NS-SAFE-14"]["status"], "policy")

    def test_registry_equity_pledge_still_present(self) -> None:
        self.assertTrue(REGISTRY.is_file())
        text = REGISTRY.read_text(encoding="utf-8")
        self.assertIn("source_id: equity_pledge", text)
        self.assertIn("equityPledge/list", text)

    def test_ep_first_slice_live_report_still_present(self) -> None:
        rows = _load_csv(EP_FIRST_LIVE_REPORT)
        self.assertEqual(len(rows), 5)
        self.assertEqual(rows[0]["case_id"], "DEP001")
