"""
CNINFO C-class — 非 seal Cross-FM mock cohort 扩展单测（离线 · CNINFO=0 · C-FM-13）。

运行：
    python3 lab/test_cninfo_c_class_nonseal_cross_fm_mock_cohort_extension.py
"""
...
_RUNNER = os.path.join(
    _LAB_DIR, "run_cninfo_c_class_nonseal_cross_fm_mock_cohort_extension.py"
)
_TEST_SUMMARY_REL = (
    "outputs/validation/"
    "cninfo_c_class_nonseal_cross_fm_mock_cohort_extension_test_summary_20260715.md"
)
...
def _write_test_summary(cases: list) -> None:
    ...
    lines = [
        "# C-FM-13 Non-seal Cross-FM Mock Cohort Extension — Test Summary",
        "",
        "_offline · CNINFO=0_",
        "",
        "| case | result |",
    ...
    lines.extend(
        [
            "",
            "```",
            "c_fm_13_nonseal_cross_fm_mock_cohort_extension_test_gate = PASS_OFFLINE",
            "cninfo_calls = 0",
...
class TestNonsealCrossFmMockCohortExtension(unittest.TestCase):
    def test_output_root_requires_mock_and_not_frozen(self) -> None:
        with self.assertRaises(RuntimeError):
            assert_fm13_output_root("outputs/validation/cninfo_c_class_not_mock")
        with self.assertRaises(RuntimeError):
            assert_fm13_output_root(AUTHORITATIVE_DUAL_LAYER_INDEX_ROOT_REL)
        ...
        norm = assert_fm13_output_root(DEFAULT_MOCK_OUTPUT_ROOT_REL)
        self.assertIn("_mock_c_fm13", norm)
    ...
    def test_nonseal_specs_include_fm05_and_fm12(self) -> None:
        ...
        self.assertIn("fm03_harvest_exclusion_consistency", ids)
        self.assertEqual(len(specs), 6)

    def test_harvest_exclusion_consistency_pass(self) -> None:
        _rows, checks = build_harvest_exclusion_consistency_rows(ExtensionPaths())
        self.assertTrue(checks.get("harvest_excl_fm03_fingerprint"))
        self.assertTrue(checks.get("harvest_excl_fm03_gate"))
        self.assertTrue(checks.get("harvest_exclusion_consistency_all_pass"))

    def test_frozen_isolation_blocks_mock14_allows_mock15(self) -> None:
        rows, checks = build_frozen_mock_isolation_rows(
            ExtensionPaths(output_root_rel=DEFAULT_MOCK_OUTPUT_ROOT_REL)
        )
        self.assertTrue(checks.get("frozen_block_C-ROOT-MOCK14"))
        self.assertTrue(checks.get("frozen_block_C-ROOT-MOCK8"))
        self.assertTrue(checks.get("frozen_allow_mock15"))
    ...
    def test_fingerprint_matrix_stable(self) -> None:
        ...
        fp1 = fingerprint_extension_matrix(rows)
        fp2 = fingerprint_extension_matrix(rows)
        self.assertEqual(fp1["fingerprint_sha256"], fp2["fingerprint_sha256"])

    def test_full_extension_pass_isolated_mock(self) -> None:
        result = run_nonseal_cross_fm_mock_cohort_extension(
            paths=ExtensionPaths(
                output_root_rel="outputs/validation/_mock_c_fm13_cli_test_tmp"
            )
        )
    ...
    def test_cninfo_not_called(self) -> None:
        with mock.patch("requests.get") as get_mock, mock.patch(
            ...
            run_nonseal_cross_fm_mock_cohort_extension(
                paths=ExtensionPaths(
                    output_root_rel=(
                        "outputs/validation/_mock_c_fm13_unit_cninfo_probe"
                    )
                )
            )
...
if __name__ == "__main__":
    ...
    suite = loader.loadTestsFromTestCase(TestNonsealCrossFmMockCohortExtension)
    ...
    cases = [
        ...
        {
            "case": "test_nonseal_specs_include_fm05_and_fm12",
            "result": "PASS",
        },
        {
            "case": "test_harvest_exclusion_consistency_pass",
            "result": "PASS",
        },
        {
            "case": "test_frozen_isolation_blocks_mock14_allows_mock15",
            "result": "PASS",
        },
