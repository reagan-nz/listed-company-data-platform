"""
B-FM-41：激励对象名单 / 销售简报 known-document live allow-list 锁测（离线；不请求 CNINFO）。

覆盖：
- allow-list 仅含 incentive_object_list_known_001 与 sales_brief_known_001（category 空）
- 字段与 fixtures ready 对齐
- mock live 路径可对两案得到 pass（predicted_document_type=announcement；非 other）
- 不包含 guarantee brief / ESG / subsidiary 等已 LIVE_PASS 案

无真实 CNINFO · 不造 §7 FP。

运行：
    python lab/test_cninfo_b_class_incentive_list_sales_brief_known_001_live.py
"""
...
LIVE_DIR = os.path.join(
    ...
    "outputs",
    "validation",
    "cninfo_b_class_incentive_list_sales_brief_known_001_live_20260715",
)
...
ANN_LIST = "1223956395"
ANN_BRIEF = "1223846612"
ANN_TS_LIST = 1750636800000  # 2025-06-23
ANN_TS_BRIEF = 1749600000000  # 2025-06-11
...
class TestIncentiveListSalesBriefKnown001LiveAllowList(unittest.TestCase):
