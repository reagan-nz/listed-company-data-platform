"""
CNINFO C-class — 规模 residual_formula composition identity lock +
resume_taxonomy composition identity lock + risk_band_status composition
identity lock + cross_residual_resume_risk_coverage_wall_meta_bundle
identity lock（离线 · C-FM-44）。

在 C-FM-43（union_status composition identity lock（2134/106/9）+
overlap_delta composition identity lock（Δ12）+ surface_delta composition
identity lock（Δ2）+ cross_stable_metrics_wall_meta_bundle identity lock）已
commit 且 EXECUTE 仍 human-held 之上，继续非 seal 规模/安全能力（不新增 seal /
decision-await / commit-boundary；非 extension↔drift 循环）：
  1) FM43 packet / fingerprint / gate / union·overlap·surface·wall ledgers 零漂移连续
  ...
  5) cross_residual_resume_risk_coverage_wall_meta_bundle_identity_lock：残差/resume/风险/覆盖墙元捆绑
  6) output-root：MOCK3–45 冻结 · MOCK46 放行
  7) FM-01..05 + FM-12..43 gate battery（跳过 seal FM06–11）

禁止：CNINFO live · production EXECUTE · 覆盖 MOCK3–45 / 权威 dual-layer 索引 ·
...
# C-FM-44 本包冻结指纹（运行后由 fingerprint_* 锁定）
...
FROZEN_RESUME_TAXONOMY_COMPOSITION_IDENTITY_LOCK_FP_SHA256 = (
...
FROZEN_CROSS_RESIDUAL_RESUME_RISK_COVERAGE_WALL_META_BUNDLE_IDENTITY_LOCK_FP_SHA256 = (
...
THIS_TASK_ROOT_ID = "C-ROOT-MOCK46"
PRIOR_TASK_ROOT_ID = "C-ROOT-MOCK45"
RESUME_HARVEST_ROOT_ID = "C-ROOT-002"

FROZEN_ROOT_IDS_MUST_BLOCK = tuple(f"C-ROOT-MOCK{i}" for i in range(3, 46))
...
class ResidualResumeRiskCoverageWallMetaBundlePaths:
    """只读输入与隔离写根路径规格。"""
...
def assert_fm44_output_root(
    output_root: str, *, base_dir: str = BASE_DIR
) -> str:
    """
    C-FM-44 写根：须 validation/_mock_*，不得覆盖 MOCK3–45，
    不得写权威 dual-layer 索引；允许本任务 MOCK46 或未登记 ephemeral。
    """
    ...
    if root_id is not None and root_id != THIS_TASK_ROOT_ID:
        raise RuntimeError(
            ...
            f"(C-FM-44 only writes {THIS_TASK_ROOT_ID} or ephemeral _mock_*)"
        )
...
def fingerprint_residual_formula_composition_identity_lock() -> Tuple[str, Dict[str, Any]]:
    """residual_formula composition identity lock 指纹（106+9+2=117）。"""
    doc = {
        "kind": "residual_formula_composition_identity_lock",
...
def fingerprint_resume_taxonomy_composition_identity_lock() -> Tuple[str, Dict[str, Any]]:
    """resume_taxonomy composition identity lock 指纹（28/1/0）。"""
    doc = {
        "kind": "resume_taxonomy_composition_identity_lock",
...
def fingerprint_cross_residual_resume_risk_coverage_wall_meta_bundle_identity_lock() -> Tuple[str, Dict[str, Any]]:
    """cross_residual_resume_risk_coverage_wall_meta_bundle identity lock 指纹。"""
    doc = {
        "kind": "cross_residual_resume_risk_coverage_wall_meta_bundle_identity_lock",
        "compositions": {
            k: dict(v)
            for k, v in EXPECTED_CROSS_RESIDUAL_RESUME_RISK_COVERAGE_WALL_META_BUNDLE.items()
        },
...
def evaluate_residual_formula_composition_mutation(
    *,
    ...
    """评估 residual_formula 组成变异；一律拒绝。"""
    formula_ok = (
        proposed_partial + proposed_failed + proposed_delta_n == proposed_coverage
    )
    matches = (
        ...
        and proposed_coverage == EXPECTED_RESIDUAL_SAFETY_COVERAGE
        and formula_ok
    )
    return {
        "proposed": {
        ...
        "reason": "residual_formula_composition_identity_frozen_pre_execute",
        "hold": "KEEP_EXECUTE_FALSE",
    }


def evaluate_resume_taxonomy_composition_mutation(
    *,
    ...
    matches = (
        ...
        and proposed_total == EXPECTED_RESUME_TOTAL
        and formula_ok
    )
    return {
        "proposed": {
        ...
        "matches_frozen": matches,
        "mutation_allowed": False,
        "reason": "resume_taxonomy_composition_identity_frozen_pre_execute",
        "hold": "KEEP_EXECUTE_FALSE",
    }
...
def evaluate_risk_band_status_composition_mutation(
    *,
    ...
    return {
        "proposed": {"bands": proposed, "partial": proposed_partial},
        "frozen": {"bands": frozen, "partial": EXPECTED_UNION_PARTIAL},
...
def evaluate_cross_residual_resume_risk_coverage_wall_meta_bundle_mutation(
    *, proposed_compositions: Dict[str, Any]
) -> Dict[str, Any]:
    """评估 cross residual/resume/risk/coverage wall meta bundle 变异；一律拒绝。"""
    frozen = {
        k: dict(v)
        for k, v in EXPECTED_CROSS_RESIDUAL_RESUME_RISK_COVERAGE_WALL_META_BUNDLE.items()
    }
    ...
    return {
        ...
        "mutation_allowed": False,
        "reason": (
            "cross_residual_resume_risk_coverage_wall_meta_bundle_identity_frozen_pre_execute"
        ),
        "hold": "KEEP_EXECUTE_FALSE",
    }


def build_residual_formula_composition_identity_lock_rows(
    paths: ResidualResumeRiskCoverageWallMetaBundlePaths,
    *,
    base_dir: str = BASE_DIR,
) -> Tuple[List[Dict[str, str]], Dict[str, bool], Dict[str, Any]]:
    """residual_formula composition identity lock（106+9+2=117）。"""
    del paths, base_dir
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    fp, doc = fingerprint_residual_formula_composition_identity_lock()
    ...
    identity_ok = (
        ...
        and comp_sha == EXPECTED_RESIDUAL_FORMULA_COMPOSITION_SHA256
        and doc["composition_sha256"] == EXPECTED_RESIDUAL_FORMULA_COMPOSITION_SHA256
    )
    checks["residual_formula_composition_identity"] = identity_ok
    rows.append(
        _row(
            check_id="residual_formula_composition_identity",
            layer="residual_formula_composition_identity_lock",
            expected="106+9+2=117",
            observed=f"formula={EXPECTED_RESIDUAL_FORMULA};sha={comp_sha[:12]}",
            ok=identity_ok,
            notes="ok" if identity_ok else "residual_formula_composition_drift",
        )
    )
    ...
    sample_denials = [
        evaluate_residual_formula_composition_mutation(
            ...
            proposed_coverage=EXPECTED_RESIDUAL_SAFETY_COVERAGE + 1,
        ),
        evaluate_residual_formula_composition_mutation(
            proposed_partial=EXPECTED_UNION_PARTIAL,
            proposed_failed=8,
    ...
    checks["residual_formula_composition_mutation_denied"] = mut_ok
    rows.append(
        _row(
            check_id="residual_formula_composition_mutation_denied",
            layer="residual_formula_composition_identity_lock",
    ...
    flags_ok = (
        ...
        and fp == FROZEN_RESIDUAL_FORMULA_COMPOSITION_IDENTITY_LOCK_FP_SHA256
    )
    checks["residual_formula_composition_flags"] = flags_ok
    rows.append(
        _row(
            check_id="residual_formula_composition_flags",
            layer="residual_formula_composition_identity_lock",
            expected=FROZEN_RESIDUAL_FORMULA_COMPOSITION_IDENTITY_LOCK_FP_SHA256,
            observed=fp,
            ok=flags_ok,
            notes="ok" if flags_ok else "residual_formula_fp_drift",
        )
    )

    identity_lock_ok = all(checks.values())
    checks["residual_formula_composition_identity_lock_all_pass"] = identity_lock_ok
    rows.append(
        _row(
            check_id="residual_formula_composition_identity_lock_all_pass",
            layer="residual_formula_composition_identity_lock",
            expected="identity+mutation_denied+fp",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=identity_lock_ok,
            notes="ok" if identity_lock_ok else "residual_formula_lock_incomplete",
        )
    )
    meta = {
        "fingerprint": fp,
...
def build_resume_taxonomy_composition_identity_lock_rows(
    paths: ResidualResumeRiskCoverageWallMetaBundlePaths,
    *,
    base_dir: str = BASE_DIR,
) -> Tuple[List[Dict[str, str]], Dict[str, bool], Dict[str, Any]]:
    """resume_taxonomy composition identity lock（28/1/0）。"""
    del paths, base_dir
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    fp, doc = fingerprint_resume_taxonomy_composition_identity_lock()
    ...
    identity_ok = (
        ...
        + EXPECTED_RESUME_WORSE
        == EXPECTED_RESUME_TOTAL
        and EXPECTED_RESUME_TAXONOMY_FORMULA == "28/1/0"
        and EXPECTED_RESUME_FORMULA == "28+1+0=29"
        and comp_sha == EXPECTED_RESUME_TAXONOMY_COMPOSITION_SHA256
        and doc["composition_sha256"] == EXPECTED_RESUME_TAXONOMY_COMPOSITION_SHA256
    )
    checks["resume_taxonomy_composition_identity"] = identity_ok
    rows.append(
        _row(
            check_id="resume_taxonomy_composition_identity",
            layer="resume_taxonomy_composition_identity_lock",
            expected="28/1/0",
            observed=f"formula={EXPECTED_RESUME_TAXONOMY_FORMULA};sha={comp_sha[:12]}",
            ok=identity_ok,
            notes="ok" if identity_ok else "resume_taxonomy_composition_drift",
        )
    )

    sample_denials = [
        evaluate_resume_taxonomy_composition_mutation(
            proposed_improved=29,
    ...
    mut_ok = all(d["mutation_allowed"] is False for d in sample_denials)
    checks["resume_taxonomy_composition_mutation_denied"] = mut_ok
    rows.append(
        _row(
            check_id="resume_taxonomy_composition_mutation_denied",
            layer="resume_taxonomy_composition_identity_lock",
            expected="mutation_allowed=false",
            observed=f"samples={len(sample_denials)};all_denied={mut_ok}",
            ok=mut_ok,
    ...
    flags_ok = (
        ...
        and doc["deny_deflate"] is True
        and doc["hold"] == "KEEP_EXECUTE_FALSE"
        and fp == FROZEN_RESUME_TAXONOMY_COMPOSITION_IDENTITY_LOCK_FP_SHA256
    )
    checks["resume_taxonomy_composition_flags"] = flags_ok
    rows.append(
        _row(
            check_id="resume_taxonomy_composition_flags",
            layer="resume_taxonomy_composition_identity_lock",
            expected=FROZEN_RESUME_TAXONOMY_COMPOSITION_IDENTITY_LOCK_FP_SHA256,
            observed=fp,
            ok=flags_ok,
            notes="ok" if flags_ok else "resume_taxonomy_fp_drift",
        )
    )

    identity_lock_ok = all(checks.values())
    checks["resume_taxonomy_composition_identity_lock_all_pass"] = identity_lock_ok
    rows.append(
        _row(
            check_id="resume_taxonomy_composition_identity_lock_all_pass",
            layer="resume_taxonomy_composition_identity_lock",
            expected="identity+mutation_denied+fp",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=identity_lock_ok,
            notes="ok" if identity_lock_ok else "resume_taxonomy_lock_incomplete",
        )
    )
    meta = {
        "fingerprint": fp,
...
def build_risk_band_status_composition_identity_lock_rows(
    ...
    *,
    ...
    identity_ok = (
        ...
        and EXPECTED_RISK_BAND_STATUS_FORMULA == "75/14/12/5"
        and EXPECTED_RISK_BAND_FORMULA == "75+14+12+5=106"
        and comp_sha == EXPECTED_RISK_BAND_STATUS_COMPOSITION_SHA256
        and doc["composition_sha256"] == EXPECTED_RISK_BAND_STATUS_COMPOSITION_SHA256
    )
    ...
    rows.append(
        _row(
            ...
            expected="75/14/12/5",
            observed=f"formula={EXPECTED_RISK_BAND_STATUS_FORMULA};sha={comp_sha[:12]}",
            ok=identity_ok,
    ...
    inflated["p35_heavy"] = inflated["p35_heavy"] + 1
    deflated = dict(EXPECTED_PARTIAL_RISK_BANDS)
    deflated["fu_light"] = deflated["fu_light"] - 1
    sample_denials = [
        evaluate_risk_band_status_composition_mutation(
            proposed_bands=inflated,
            proposed_partial=EXPECTED_UNION_PARTIAL + 1,
        ),
        evaluate_risk_band_status_composition_mutation(
            proposed_bands=deflated,
            proposed_partial=EXPECTED_UNION_PARTIAL - 1,
        ),
        evaluate_risk_band_status_composition_mutation(
            proposed_bands={"p35_heavy": 70, "p3_mid": 16, "p2_mid": 12, "fu_light": 8},
    ...
    rows.append(
        _row(
            ...
            observed=fp,
            ok=flags_ok,
            notes="ok" if flags_ok else "risk_band_status_fp_drift",
        )
    )

    identity_lock_ok = all(checks.values())
    checks["risk_band_status_composition_identity_lock_all_pass"] = identity_lock_ok
    rows.append(
        _row(
            ...
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=identity_lock_ok,
            notes="ok" if identity_lock_ok else "risk_band_status_lock_incomplete",
        )
    )
    meta = {
        "fingerprint": fp,
...
def build_cross_residual_resume_risk_coverage_wall_meta_bundle_identity_lock_rows(
    paths: ResidualResumeRiskCoverageWallMetaBundlePaths,
    *,
    base_dir: str = BASE_DIR,
) -> Tuple[List[Dict[str, str]], Dict[str, bool], Dict[str, Any]]:
    """cross residual/resume/risk/coverage wall meta bundle identity lock。"""
    del paths, base_dir
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    fp, doc = fingerprint_cross_residual_resume_risk_coverage_wall_meta_bundle_identity_lock()
    ...
    identity_ok = (
        bundle_sha == EXPECTED_CROSS_RESIDUAL_RESUME_RISK_COVERAGE_WALL_META_BUNDLE_SHA256
        and len(EXPECTED_CROSS_RESIDUAL_RESUME_RISK_COVERAGE_WALL_META_BUNDLE) == 5
        and EXPECTED_CROSS_RESIDUAL_RESUME_RISK_COVERAGE_WALL_META_BUNDLE[
            "residual_formula_composition"
        ]["formula"]
    ...
    checks["cross_residual_resume_risk_coverage_wall_meta_bundle_identity"] = identity_ok
    rows.append(
        _row(
            check_id="cross_residual_resume_risk_coverage_wall_meta_bundle_identity",
            layer="cross_residual_resume_risk_coverage_wall_meta_bundle_identity_lock",
            expected="5_compositions_wall_meta_bundle",
            observed=(
                f"n={len(EXPECTED_CROSS_RESIDUAL_RESUME_RISK_COVERAGE_WALL_META_BUNDLE)};"
                f"sha={bundle_sha[:12]}"
            ),
            ok=identity_ok,
    ...
    deflated = {
        k: dict(v)
        for k, v in EXPECTED_CROSS_RESIDUAL_RESUME_RISK_COVERAGE_WALL_META_BUNDLE.items()
        if k != "risk_band_status_composition"
    }
    mutated = {
        k: dict(v)
        for k, v in EXPECTED_CROSS_RESIDUAL_RESUME_RISK_COVERAGE_WALL_META_BUNDLE.items()
    }
    mutated["residual_formula_composition"] = {
        "formula": "106+9+2=116",
        "composition_sha256": EXPECTED_RESIDUAL_FORMULA_COMPOSITION_SHA256,
    }
    sample_denials = [
        evaluate_cross_residual_resume_risk_coverage_wall_meta_bundle_mutation(
    ...
    mut_ok = all(d["mutation_allowed"] is False for d in sample_denials)
    checks["cross_residual_resume_risk_coverage_wall_meta_bundle_mutation_denied"] = mut_ok
    rows.append(
        _row(
            check_id="cross_residual_resume_risk_coverage_wall_meta_bundle_mutation_denied",
            layer="cross_residual_resume_risk_coverage_wall_meta_bundle_identity_lock",
            expected="mutation_allowed=false",
            observed=f"samples={len(sample_denials)};all_denied={mut_ok}",
            ok=mut_ok,
    ...
    checks["cross_residual_resume_risk_coverage_wall_meta_bundle_flags"] = flags_ok
    rows.append(
        _row(
            check_id="cross_residual_resume_risk_coverage_wall_meta_bundle_flags",
            layer="cross_residual_resume_risk_coverage_wall_meta_bundle_identity_lock",
            expected=FROZEN_CROSS_RESIDUAL_RESUME_RISK_COVERAGE_WALL_META_BUNDLE_IDENTITY_LOCK_FP_SHA256,
            observed=fp,
            ok=flags_ok,
    ...
    identity_lock_ok = all(checks.values())
    checks[
        "cross_residual_resume_risk_coverage_wall_meta_bundle_identity_lock_all_pass"
    ] = identity_lock_ok
    rows.append(
        _row(
            check_id="cross_residual_resume_risk_coverage_wall_meta_bundle_identity_lock_all_pass",
            layer="cross_residual_resume_risk_coverage_wall_meta_bundle_identity_lock",
            expected="identity+mutation_denied+fp",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=identity_lock_ok,
...
def build_fm43_continuity_rows(
    paths: ResidualResumeRiskCoverageWallMetaBundlePaths, *, base_dir: str = BASE_DIR
) -> Tuple[List[Dict[str, str]], Dict[str, bool]]:
    """FM43 packet / fingerprint / gate / 四 ledger 零漂移。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}

    packet = load_json(_abs(paths.fm43_packet_rel, base_dir=base_dir))
    fp_doc = load_json(_abs(paths.fm43_fingerprint_rel, base_dir=base_dir))
    gate_doc = load_json(_abs(paths.fm43_gate_json_rel, base_dir=base_dir))
    us_led = load_json(_abs(paths.fm43_union_status_rel, base_dir=base_dir))
    od_led = load_json(_abs(paths.fm43_overlap_delta_rel, base_dir=base_dir))
    sd_led = load_json(_abs(paths.fm43_surface_delta_rel, base_dir=base_dir))
    wall_led = load_json(_abs(paths.fm43_wall_meta_rel, base_dir=base_dir))
    ...
    pkt_ok = (
        ...
        and packet.get("union_status_composition_sha256")
        == EXPECTED_UNION_STATUS_COMPOSITION_SHA256
        and packet.get("overlap_delta_composition_sha256")
        == EXPECTED_OVERLAP_DELTA_COMPOSITION_SHA256
        and packet.get("surface_delta_composition_sha256")
        == EXPECTED_SURFACE_DELTA_COMPOSITION_SHA256
        and packet.get("cross_stable_metrics_wall_meta_bundle_sha256")
        == EXPECTED_CROSS_STABLE_METRICS_WALL_META_BUNDLE_SHA256
    ...
    checks["fm43_packet_zero_drift"] = pkt_ok
    rows.append(
        _row(
            check_id="fm43_packet_zero_drift",
            layer="fm43_continuity",
            path=paths.fm43_packet_rel,
            expected="PASS_OFFLINE+stable_metrics+compositions",
            observed=f"gate={packet.get('gate')};unique={packet.get('harvest_unique_union')}",
            ok=pkt_ok,
            notes="ok" if pkt_ok else "fm43_packet_drift",
        )
    )
    ...
    fp_ok = (
        ...
        and fp_doc.get("union_status_composition_sha256")
        == EXPECTED_UNION_STATUS_COMPOSITION_SHA256
        and fp_doc.get("overlap_delta_composition_sha256")
        == EXPECTED_OVERLAP_DELTA_COMPOSITION_SHA256
        and fp_doc.get("surface_delta_composition_sha256")
        == EXPECTED_SURFACE_DELTA_COMPOSITION_SHA256
        and fp_doc.get("cross_stable_metrics_wall_meta_bundle_sha256")
        == EXPECTED_CROSS_STABLE_METRICS_WALL_META_BUNDLE_SHA256
        and fp_doc.get("seal_chain_extended") is False
    )
    checks["fm43_fingerprint_zero_drift"] = fp_ok
    rows.append(
        _row(
            check_id="fm43_fingerprint_zero_drift",
            layer="fm43_continuity",
            path=paths.fm43_fingerprint_rel,
            expected="stable_metrics+fm43_fps",
            observed=f"unique={fp_doc.get('harvest_unique_union')}",
            ok=fp_ok,
            notes="ok" if fp_ok else "fm43_fingerprint_drift",
        )
    )
    ...
    gate_ok = (
        ...
        and gate_doc.get("cross_stable_metrics_wall_meta_bundle_sha256")
        == EXPECTED_CROSS_STABLE_METRICS_WALL_META_BUNDLE_SHA256
        and gate_doc.get("seal_chain_extended") is False
    )
    checks["fm43_gate_zero_drift"] = gate_ok
    rows.append(
        _row(
            check_id="fm43_gate_zero_drift",
            layer="fm43_continuity",
            path=paths.fm43_gate_json_rel,
            expected="PASS_OFFLINE+KEEP_EXECUTE_FALSE",
            observed=f"gate={gate_doc.get('gate')}",
            ok=gate_ok,
            notes="ok" if gate_ok else "fm43_gate_drift",
        )
    )

    us_ok = (
        us_led.get("fingerprint_sha256") == FM43_FROZEN_UNION_STATUS_FP
        and us_led.get("composition_sha256") == EXPECTED_UNION_STATUS_COMPOSITION_SHA256
    )
    checks["fm43_union_status_ledger_zero_drift"] = us_ok
    rows.append(
        _row(
            check_id="fm43_union_status_ledger_zero_drift",
            layer="fm43_continuity",
            path=paths.fm43_union_status_rel,
            expected=FM43_FROZEN_UNION_STATUS_FP,
            observed=str(us_led.get("fingerprint_sha256")),
            ok=us_ok,
            notes="ok" if us_ok else "fm43_union_status_ledger_drift",
        )
    )

    od_ok = (
        od_led.get("fingerprint_sha256") == FM43_FROZEN_OVERLAP_DELTA_FP
        and od_led.get("composition_sha256")
        == EXPECTED_OVERLAP_DELTA_COMPOSITION_SHA256
    )
    checks["fm43_overlap_delta_ledger_zero_drift"] = od_ok
    rows.append(
        _row(
            check_id="fm43_overlap_delta_ledger_zero_drift",
            layer="fm43_continuity",
            path=paths.fm43_overlap_delta_rel,
            expected=FM43_FROZEN_OVERLAP_DELTA_FP,
            observed=str(od_led.get("fingerprint_sha256")),
            ok=od_ok,
            notes="ok" if od_ok else "fm43_overlap_delta_ledger_drift",
        )
    )

    sd_ok = (
        sd_led.get("fingerprint_sha256") == FM43_FROZEN_SURFACE_DELTA_FP
        and sd_led.get("composition_sha256")
        == EXPECTED_SURFACE_DELTA_COMPOSITION_SHA256
    )
    checks["fm43_surface_delta_ledger_zero_drift"] = sd_ok
    rows.append(
        _row(
            check_id="fm43_surface_delta_ledger_zero_drift",
            layer="fm43_continuity",
            path=paths.fm43_surface_delta_rel,
            expected=FM43_FROZEN_SURFACE_DELTA_FP,
            observed=str(sd_led.get("fingerprint_sha256")),
            ok=sd_ok,
            notes="ok" if sd_ok else "fm43_surface_delta_ledger_drift",
        )
    )

    wall_ok = (
        wall_led.get("fingerprint_sha256") == FM43_FROZEN_WALL_META_FP
        and wall_led.get("bundle_sha256")
        == EXPECTED_CROSS_STABLE_METRICS_WALL_META_BUNDLE_SHA256
    )
    checks["fm43_wall_meta_ledger_zero_drift"] = wall_ok
    rows.append(
        _row(
            check_id="fm43_wall_meta_ledger_zero_drift",
            layer="fm43_continuity",
            path=paths.fm43_wall_meta_rel,
            expected=FM43_FROZEN_WALL_META_FP,
            observed=str(wall_led.get("fingerprint_sha256")),
            ok=wall_ok,
            notes="ok" if wall_ok else "fm43_wall_meta_ledger_drift",
        )
    )

    all_ok = all(checks.values()) if checks else False
    checks["fm43_continuity_all_pass"] = all_ok
    rows.append(
        _row(
            check_id="fm43_continuity_all_pass",
            layer="fm43_continuity",
            expected="packet+fp+gate+4ledgers",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
            notes="ok" if all_ok else "fm43_continuity_incomplete",
        )
    )
    return rows, checks
...
def build_output_root_protection_rows(
    ...
    """output-root 保护：resume/harvest 写拒绝 + MOCK46 放行。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    ...
    out_detail = ""
    try:
        assert_fm44_output_root(paths.output_root_rel, base_dir=base_dir)
        out_ok = True
        out_detail = "allowed"
    except RuntimeError as exc:
    ...
    rows.append(
        _row(
            ...
            expected="MOCK46_or_ephemeral_allowed",
            observed=out_detail,
            ok=out_ok,
    ...
    rows.append(
        _row(
            ...
            layer="output_root_protection",
            expected="harvest+resume_refused;mock46_ok",
            observed=(
...
def build_frozen_mock_isolation_rows(
    ...
    """冻结 mock cohort 写隔离：MOCK3–45 拒绝 · MOCK46 放行。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    load_frozen_mock_cohort_roots.cache_clear()
    ...
    # 先验 MOCK45（C-FM-43）写根必须仍冻结
    mock45_rel = FM43_MOCK_ROOT_REL
    mock45_blocked = False
    try:
        assert_fm44_output_root(mock45_rel, base_dir=base_dir)
    except RuntimeError as exc:
        mock45_blocked = FROZEN_MOCK_COHORT_WRITE_FORBIDDEN in str(exc)
    checks["mock45_still_frozen"] = mock45_blocked
    rows.append(
        _row(
            check_id="mock45_still_frozen",
            layer="frozen_mock_isolation",
            root_id=PRIOR_TASK_ROOT_ID,
            path=mock45_rel,
            expected="write_forbidden",
            observed="blocked" if mock45_blocked else "allowed",
            ok=mock45_blocked,
            notes="ok" if mock45_blocked else "mock45_write_leak",
        )
    )
    ...
    try:
        ...
        allow_ok = False
    checks["frozen_allow_mock46"] = allow_ok
    rows.append(
        _row(
            check_id="frozen_allow_mock46",
            layer="frozen_mock_isolation",
    ...
    rows.append(
        _row(
            ...
            layer="frozen_mock_isolation",
            expected="MOCK3-45_block+MOCK46_allow",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
...
def build_protected_csv_registry_rows(
    *,
    ...
    """protected_output_roots.csv：MOCK46 已登记。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    ...
    mock46 = by_id.get(THIS_TASK_ROOT_ID) or {}
    path_ok = DEFAULT_MOCK_OUTPUT_ROOT_REL in str(mock46.get("path_pattern") or "")
    checks["protected_csv_mock46_path"] = path_ok
    rows.append(
        _row(
            check_id="protected_csv_mock46_path",
            layer="protected_csv_registry",
            root_id=THIS_TASK_ROOT_ID,
            expected=DEFAULT_MOCK_OUTPUT_ROOT_REL,
            observed=str(mock46.get("path_pattern") or ""),
            ok=path_ok,
            notes="ok" if path_ok else "mock46_path_mismatch",
        )
    )
    ...
    rows.append(
        _row(
            ...
            layer="protected_csv_registry",
            expected="MOCK3–45+MOCK46+resume+auth+registered",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
...
def build_fm_gate_battery_rows(
    *, gates: Dict[str, Dict[str, Any]]
    ...
    """FM-01..05 + FM-12..43 gate battery（跳过 seal FM06–11）。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    specs = [
    ...
    seal_skip_keys = {
        ...
        "fm36", "fm37", "fm38", "fm39", "fm40", "fm41", "fm42", "fm43",
    }
    for check_id, key in specs:
        ...
        if key in (
            ...
            "fm33", "fm34", "fm35", "fm36", "fm37", "fm38", "fm39", "fm40", "fm41", "fm42", "fm43",
        ):
            ok = (
                ok
                and payload.get("harvest_unique_union") == EXPECTED_HARVEST_UNIQUE_UNION
        ...
        if key in (
            ...
            "fm34", "fm35", "fm36", "fm37", "fm38", "fm39", "fm40", "fm41", "fm42", "fm43",
        ):
            ok = (
                ok
    ...
    rows.append(
        _row(
            check_id="fm01_05_12_43_battery_all_pass",
            layer="fm_gate_battery",
    ...
    checks["fm01_05_12_43_battery_all_pass"] = all_ok
    return rows, checks
...
def ensure_protected_roots_csv_fm44(
    *,
    csv_rel: str = PROTECTED_ROOTS_CSV_REL,
    base_dir: str = BASE_DIR,
) -> None:
    """注册 C-ROOT-MOCK46；加固 C-ROOT-002 union/overlap/surface/wall-meta 说明。"""
    ...
    _upsert(
        RESUME_HARVEST_ROOT_ID,
        {
            ...
            "notes": (
                ...
                "C-FM-24..43 scale/safety freezes + C-FM-44 residual/resume/risk/"
                "coverage-wall-meta-bundle; 只读直至人批重跑"
            ),
        },
    )
    _upsert(
        THIS_TASK_ROOT_ID,
        {
            ...
            "notes": (
                "C-FM-44 scale residual_formula composition identity lock (106+9+2=117) + "
                "resume_taxonomy composition identity lock (28/1/0) + "
                "risk_band_status composition identity lock (75/14/12/5) + "
                "cross_residual_resume_risk_coverage_wall_meta_bundle identity lock + "
                "FM43 continuity; never production EXECUTE; must not overwrite MOCK3-45; "
...
def run_scale_residual_resume_risk_coverage_wall_meta_bundle_safety(
    *,
    paths: ResidualResumeRiskCoverageWallMetaBundlePaths | None = None,
    base_dir: str = BASE_DIR,
) -> Dict[str, Any]:
    """执行 C-FM-44 规模 residual/resume/risk/coverage-wall-meta-bundle 离线 QA。"""
    paths = paths or ResidualResumeRiskCoverageWallMetaBundlePaths()
    generated_at = _utc_now_iso()
    ensure_protected_roots_csv_fm44(
        csv_rel=paths.protected_roots_csv_rel, base_dir=base_dir
    )
    out_root = assert_fm44_output_root(paths.output_root_rel, base_dir=base_dir)

    matrix: List[Dict[str, str]] = []
    cont_rows, cont_checks = build_fm43_continuity_rows(paths, base_dir=base_dir)
    matrix.extend(cont_rows)
    rf_rows, rf_checks, rf_meta = build_residual_formula_composition_identity_lock_rows(
        paths, base_dir=base_dir
    )
    matrix.extend(rf_rows)
    rt_rows, rt_checks, rt_meta = (
        build_resume_taxonomy_composition_identity_lock_rows(paths, base_dir=base_dir)
    )
    matrix.extend(rt_rows)
    rb_rows, rb_checks, rb_meta = (
        build_risk_band_status_composition_identity_lock_rows(paths, base_dir=base_dir)
    )
    matrix.extend(rb_rows)
    bundle_rows, bundle_checks, bundle_meta = (
        build_cross_residual_resume_risk_coverage_wall_meta_bundle_identity_lock_rows(
            paths, base_dir=base_dir
        )
    )
    ...
    layer_gates = {
        "fm43_continuity": (
            "PASS_OFFLINE"
            if cont_checks.get("fm43_continuity_all_pass")
            else "FAIL_OFFLINE"
        ),
        "residual_formula_composition_identity_lock": (
            "PASS_OFFLINE"
            if rf_checks.get("residual_formula_composition_identity_lock_all_pass")
            else "FAIL_OFFLINE"
        ),
        "resume_taxonomy_composition_identity_lock": (
            "PASS_OFFLINE"
            if rt_checks.get("resume_taxonomy_composition_identity_lock_all_pass")
            else "FAIL_OFFLINE"
        ),
        "risk_band_status_composition_identity_lock": (
            "PASS_OFFLINE"
            if rb_checks.get("risk_band_status_composition_identity_lock_all_pass")
            else "FAIL_OFFLINE"
        ),
        "cross_residual_resume_risk_coverage_wall_meta_bundle_identity_lock": (
            "PASS_OFFLINE"
        ...
        "fm_gate_battery": (
            "PASS_OFFLINE"
            if bat_checks.get("fm01_05_12_43_battery_all_pass")
            else "FAIL_OFFLINE"
        ),
        "execute_hold_seal": (
            "PASS_OFFLINE"
    ...
    rf_rel = _write_ledger(
        "residual_formula_composition_identity_lock_ledger.json",
        {
            "generated_at": generated_at,
            "task_id": TASK_ID,
            "fingerprint_sha256": rf_meta["fingerprint"],
            "composition_sha256": rf_meta["composition_sha256"],
            "residual_formula": rf_meta["residual_formula"],
            "coverage_formula": rf_meta["coverage_formula"],
            "sample_denials": rf_meta["sample_denials"],
            "doc": rf_meta["doc"],
        },
    )
    rt_rel = _write_ledger(
        "resume_taxonomy_composition_identity_lock_ledger.json",
        {
            "generated_at": generated_at,
            "task_id": TASK_ID,
            "fingerprint_sha256": rt_meta["fingerprint"],
            "composition_sha256": rt_meta["composition_sha256"],
            "resume_taxonomy_formula": rt_meta["resume_taxonomy_formula"],
            "resume_formula": rt_meta["resume_formula"],
    ...
    rb_rel = _write_ledger(
        "risk_band_status_composition_identity_lock_ledger.json",
        {
            ...
            "risk_band_status_formula": rb_meta["risk_band_status_formula"],
    ...
    observed_fps = {
        "residual_formula_composition_identity_lock": rf_meta["fingerprint"],
        "resume_taxonomy_composition_identity_lock": rt_meta["fingerprint"],
        "risk_band_status_composition_identity_lock": rb_meta["fingerprint"],
        "cross_residual_resume_risk_coverage_wall_meta_bundle_identity_lock": bundle_meta[
            "fingerprint"
        ],
        "scale_matrix": fp,
    }
    ...
    packet_rel = _write_ledger(
        "scale_packet.json",
        {
            ...
            "notes": (
                ...
                "risk_band_status composition identity lock (75/14/12/5) + "
                "cross_residual_resume_risk_coverage_wall_meta_bundle identity lock + "
                "FM43 continuity + MOCK46; EXECUTE remains human-held; "
                "does not overwrite MOCK3-45"
            ),
        },
    )
    ...
    with open(_abs(readme_rel, base_dir=base_dir), "w", encoding="utf-8") as fh:
        fh.write(
            "\n".join(
                [
                    "# C-FM-44 mock scale residual/resume/risk/coverage-wall-meta-bundle safety root",
                    "",
                    f"task_id: {TASK_ID}",
                    f"gate: {overall}",
                    "cninfo_calls: 0",
                    ...
                    f"coverage_formula: {EXPECTED_COVERAGE_FORMULA}",
                    f"residual_formula_composition_sha256: {EXPECTED_RESIDUAL_FORMULA_COMPOSITION_SHA256}",
                    f"resume_taxonomy_composition_sha256: {EXPECTED_RESUME_TAXONOMY_COMPOSITION_SHA256}",
                    f"risk_band_status_composition_sha256: {EXPECTED_RISK_BAND_STATUS_COMPOSITION_SHA256}",
                    f"cross_residual_resume_risk_coverage_wall_meta_bundle_sha256: {EXPECTED_CROSS_RESIDUAL_RESUME_RISK_COVERAGE_WALL_META_BUNDLE_SHA256}",
                    "",
                    "Isolation: MOCK3-45 frozen; this root is C-ROOT-MOCK46.",
    ...
    return {
        ...
        "resume_taxonomy_path": rt_rel,
        "risk_band_status_path": rb_rel,
        "cross_residual_resume_risk_coverage_wall_meta_bundle_path": bundle_rel,
        "battery_path": battery_rel,
        "packet_path": packet_rel,
        "observed_fps": observed_fps,
        "inputs": {
            "fm43_packet": paths.fm43_packet_rel,
            "fm43_fingerprint": paths.fm43_fingerprint_rel,
            "fm43_gate": paths.fm43_gate_json_rel,
