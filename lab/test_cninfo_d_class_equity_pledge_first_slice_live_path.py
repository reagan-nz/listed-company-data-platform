"""
D-class equity_pledge first-slice live path 测试（mock CNINFO · 不执行真实 live）。

运行：
    python lab/test_cninfo_d_class_equity_pledge_first_slice_live_path.py
"""
...
def _ep_row_from_dict(row: dict[str, str]) -> runner.EquityPledgeFirstSliceRow:
    return runner.EquityPledgeFirstSliceRow(
        ...
        exclude_flags=row.get("exclude_flags", ""),
        notes=row.get("notes", ""),
        dlc005_reference=row.get("dlc005_reference", ""),
    )


def _mock_cninfo_equity_pledge(
    _session: Any,
    _source_cfg: dict,
    ...
    code = runner.EQUITY_PLEDGE_FIRST_SLICE_EXPECTED_COMPANY_CODES.get(case_id, "000895")
    return (
        {
            "data": {
                "records": [
                    {"SECCODE": code, "SECNAME": "mock", "TDATE": "2026-07-03"}
                ]
            }
        },
        200,
...
def _run_live_inprocess(
    ...
    with mock.patch.object(
        runner, "_cninfo_request", side_effect=_mock_cninfo_equity_pledge
    ) as mock_req, mock.patch("time.sleep"):
        rc = runner.main(_live_args(root, extra=extra))
    return rc, mock_req, root


class TestEquityPledgeFirstSliceLivePath(unittest.TestCase):
    @classmethod
    ...
    def test_live_without_approval_flag_rejected_before_cninfo(self) -> None:
        ...
        self.assertIn(runner.EQUITY_PLEDGE_FIRST_SLICE_APPROVAL_REQUIRED, result.stderr)
    ...
    def test_wrong_approval_flag_rejected_before_cninfo(self) -> None:
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertIn(runner.EQUITY_PLEDGE_FIRST_SLICE_WRONG_APPROVAL_FLAG, result.stderr)
    ...
    def test_live_path_function_exists_and_wired(self) -> None:
        ...
        self.assertEqual(called_cases, runner.EQUITY_PLEDGE_FIRST_SLICE_ALLOWED_CASE_IDS)
    ...
    def test_live_stub_removed(self) -> None:
        ...
        self.assertEqual(rc, 0)
        self.assertNotEqual(
            runner.execute_equity_pledge_first_slice_live.__doc__ or "",
            "equity_pledge 第一切片 live 尚未实现；须单独 live-path 任务。",
        )
    ...
    def test_universe_size_must_equal_5(self) -> None:
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertIn(runner.EQUITY_PLEDGE_FIRST_SLICE_UNIVERSE_SIZE_MISMATCH, result.stderr)
    ...
    def test_only_dep001_through_dep005_allowed(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = _run(_live_args(OUTPUT_ROOT, extra=["--universe-csv", path]))
        self.assertNotEqual(result.returncode, 0)
        self.assertIn(runner.EQUITY_PLEDGE_FIRST_SLICE_FORBIDDEN_CASE_ID, result.stderr)
    ...
    def test_component_must_be_equity_pledge(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = _run(_live_args(OUTPUT_ROOT, extra=["--universe-csv", path]))
        self.assertNotEqual(result.returncode, 0)
        self.assertIn(runner.EQUITY_PLEDGE_FIRST_SLICE_WRONG_COMPONENT, result.stderr)
    ...
    def test_688671_rejected(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = _run(_live_args(OUTPUT_ROOT, extra=["--universe-csv", path]))
        self.assertNotEqual(result.returncode, 0)
        self.assertIn(runner.EQUITY_PLEDGE_FIRST_SLICE_FORBIDDEN_COMPANY_CODE, result.stderr)
    ...
    def test_output_root_isolation_enforced(self) -> None:
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertIn(runner.EQUITY_PLEDGE_FIRST_SLICE_V1_OUTPUT_ROOT_WRITE_BLOCKED, result.stderr)
    ...
    def test_request_cap_lte_20_in_planning(self) -> None:
        rows = _read_universe_rows()
        total = sum(
            runner.compute_equity_pledge_first_slice_planned_requests(_ep_row_from_dict(r))
            for r in rows
        )
        self.assertLessEqual(total, runner.EQUITY_PLEDGE_FIRST_SLICE_TOTAL_MAX_REQUESTS)
    ...
    def test_execution_gate_pass_with_caveat_at_three_of_five(self) -> None:
        rows = [
            ...
            if r["case_id"] in ("DEP001", "DEP002", "DEP003")
        ]
        summaries = {
            "DEP001": {
        ...
        gate = runner.compute_equity_pledge_first_slice_execution_gate(rows, summaries)
        self.assertEqual(gate, runner.EQUITY_PLEDGE_FIRST_SLICE_EXECUTION_GATE_PASS)

    def test_empty_but_valid_semantics_dep001(self) -> None:
        row = _ep_row_from_dict(_read_universe_rows()[0])
        summary = {
            "retrieval_status": "empty_but_valid",
            "quality_status": "pass",
            "record_count": "0",
        }
        self.assertTrue(runner.is_equity_pledge_first_slice_acceptable(row, summary))

    def test_captured_normal_or_empty_but_valid_found(self) -> None:
        row = _ep_row_from_dict(
            next(r for r in _read_universe_rows() if r["case_id"] == "DEP002")
        )
        summary = {
            "retrieval_status": "found",
            "quality_status": "pass",
    ...
    def test_captured_normal_or_needs_review_found(self) -> None:
        row = _ep_row_from_dict(
            next(r for r in _read_universe_rows() if r["case_id"] == "DEP004")
        )
        summary = {
            "retrieval_status": "found",
    ...
    def test_needs_review_retrieval_status_acceptable_for_dep004(self) -> None:
        row = _ep_row_from_dict(
            next(r for r in _read_universe_rows() if r["case_id"] == "DEP004")
        )
        summary = {
    ...
    def test_disclosure_only_notes_does_not_upgrade_captured_normal(self) -> None:
        row = runner.EquityPledgeFirstSliceRow(
            case_id="DEP002",
            company_code="000895",
            company_name="双汇发展",
        ...
        self.assertFalse(runner.is_equity_pledge_first_slice_acceptable(row, summary))
