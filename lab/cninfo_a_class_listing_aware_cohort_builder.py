...
1. 源：company_basic_profile ∩ full_market_2024 名称/交易所
   （S7/S8/S9/S10/S11/S12/S13/S14/S15/S16 默认使用 A 轨 coverage overlay，合并 latent harvest profiles）
...
5. 可选 prefix_concentration：同一 3 位码前缀在本片最多 N 个
   （S7/S8/S9/S10/S11/S12/S13/S14/S15/S16 默认 N=25；避免 mono-prefix 批处理再现 S6 首轮 timeout 窗）
...
- S16：AD2E1301–1350（默认 · `--slice s16` · overlay + prefix cap）

禁止：CNINFO live、伪造上市日、mutate 封闭 S1–S15 live 根、静默改写 expected_period。
"""
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    """CLI：生成 listing-aware universe（offline · 默认 S16）。"""
    import argparse
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "s14=AD2E1201-1250；s15=AD2E1251-1300；s16=AD2E1301-1350（默认）"
        ),
    )
