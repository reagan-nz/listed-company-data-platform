#!/usr/bin/env python3
"""
CNINFO C-class — 规模 risk-band membership + write-boundary denial +
residual safety cross-matrix runner（离线 · C-FM-28）。

Usage:
    python3 lab/run_cninfo_c_class_scale_risk_band_membership_write_boundary_cross_matrix_safety.py
    python3 lab/run_cninfo_c_class_scale_risk_band_membership_write_boundary_cross_matrix_safety.py \\
      --output-root outputs/validation/_mock_c_fm28_scale_risk_band_membership_write_boundary_cross_matrix_safety
"""
...
REPORT_ROOT_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_risk_band_membership_write_boundary_cross_matrix_safety"
)
REPORT_MD_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_risk_band_membership_write_boundary_cross_matrix_safety_20260715.md"
)
REPORT_JSON_REL = (
    "outputs/validation/"
...
TASK_REPORT_REL = (
    "outputs/validation/"
    "cninfo_c_class_fm28_scale_risk_band_membership_write_boundary_cross_matrix_safety_20260715.md"
)
...
def write_reports(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    for key, name in (
        ("membership_path", "partial_risk_band_membership_ledger.json"),
        ("quarantine_write_path", "quarantine_write_boundary_denial_ledger.json"),
        ("fence_absorb_path", "fence_absorb_denial_ledger.json"),
        ("cross_matrix_path", "residual_safety_cross_matrix_ledger.json"),
    ):
    ...
    json_payload = {
        ...
        "membership_path": payload["membership_path"],
        "quarantine_write_path": payload["quarantine_write_path"],
        "fence_absorb_path": payload["fence_absorb_path"],
        "cross_matrix_path": payload["cross_matrix_path"],
    ...
    md_lines = [
        ...
        *layer_lines,
        ...
        "## Scale / safety gain",
        "",
        "- FM27 连续：unique=**2249** · status=**2134/106/9** · delta=**2** · "
        "bands=**p35_heavy=75…**",
        ...
        "",
        "```",
        f"c_fm_28_scale_risk_band_membership_write_boundary_cross_matrix_safety_gate = "
        f"{payload['gate']}",
        "execute_production_snapshot_rebuild = false",
        "approved_for_snapshot_rebuild = false",
        "cninfo_calls = 0",
    ...
    task_lines = [
        ...
        "## Task",
        "",
        "在 C-FM-27 已 commit 且 EXECUTE 仍 human-held 之上，继续 **非 seal** "
        "规模/安全能力（非 extension↔drift / seal-chain）：**partial risk-band "
        "membership（106）**、**quarantine write-boundary denial（9）**、"
        "**fence absorb denial（Δ2）**、**residual safety cross-matrix"
        "（coverage=117）**、**FM27 连续 + MOCK30 隔离**；产物写入隔离 MOCK30"
        "（不覆盖 MOCK3–29）。",
        "",
        "## Capability gain",
        "",
    ...
    with open(readme, "w", encoding="utf-8") as fh:
        fh.write(
            "\n".join(
                [
                    "# C-FM-28 mock scale risk-band membership write-boundary cross-matrix safety root",
                    "",
                    f"gate: `{payload['gate']}`",
                    "seal_chain_extended: false",
                    f"scale_tier_count: {payload['scale_tier_count']}",
                    ...
                    f"residual_safety_coverage: {payload['residual_safety_coverage']}",
                    "does_not_overwrite: _mock_c_fm02.._mock_c_fm27 / standard dryrun",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "C-FM-28 scale risk-band membership/write-boundary/cross-matrix "
            "safety (offline)"
        )
    )
    parser.add_argument(
    ...
    if args.execute:
        print(
            ...
            "C-FM-28 is offline QA only; KEEP_EXECUTE_FALSE",
            file=sys.stderr,
        )
        return 2

    payload = run_scale_risk_band_membership_write_boundary_cross_matrix_safety(
        paths=RiskBandMembershipPaths(output_root_rel=args.output_root)
    )
