...
union_partition_formula identity lock runner（离线 · C-FM-37）。

Usage:
    python3 lab/run_cninfo_c_class_scale_improved_partial_surface_delta_union_partition_formula_safety.py
    python3 lab/run_cninfo_c_class_scale_improved_partial_surface_delta_union_partition_formula_safety.py \\
      --output-root outputs/validation/_mock_c_fm37_scale_improved_partial_surface_delta_union_partition_formula_safety
"""
...
REPORT_ROOT_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_improved_partial_surface_delta_union_partition_formula_safety"
)
REPORT_MD_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_improved_partial_surface_delta_union_partition_formula_safety_20260715.md"
)
REPORT_JSON_REL = (
    "outputs/validation/"
...
TASK_REPORT_REL = (
    "outputs/validation/"
    "cninfo_c_class_fm37_scale_improved_partial_surface_delta_union_partition_formula_safety_20260715.md"
)
...
def write_reports(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    for key, name in (
        ("resume_improved_path", "resume_improved_membership_freeze_ledger.json"),
        ("partial_codes_path", "partial_codes_membership_freeze_ledger.json"),
        (
            "surface_delta_path",
            "surface_harvest_delta_membership_freeze_ledger.json",
        ),
        (
            "union_formula_path",
            "union_partition_formula_identity_lock_ledger.json",
        ),
    ):
    ...
    json_payload = {
        ...
        "partial_codes_path": payload["partial_codes_path"],
        "surface_delta_path": payload["surface_delta_path"],
        "union_formula_path": payload["union_formula_path"],
    ...
    md_lines = [
        ...
        *layer_lines,
        ...
        "## Scale / safety gain",
        "",
        "- FM36 连续：unique=**2249** · dryrun=**1053** · Δ2 · status=**2134/106/9** · "
        "resume=**28/1/0** · residual=**117=106+9+2** · failed/same-worse/residual/hold fps",
        ...
        "",
        "```",
        f"c_fm_37_scale_improved_partial_surface_delta_union_partition_formula_safety_gate = "
        f"{payload['gate']}",
        "execute_production_snapshot_rebuild = false",
        "approved_for_snapshot_rebuild = false",
        "cninfo_calls = 0",
    ...
    task_lines = [
        ...
        "## Task",
        "",
        "在 C-FM-36 已 commit 且 EXECUTE 仍 human-held 之上，继续 **非 seal** "
        "规模/安全能力（非 extension↔drift / seal-chain）：**resume_improved membership "
        "freeze（28）**、**partial_codes membership freeze（106）**、"
        "**surface_harvest_delta membership freeze（Δ2）**、**union_partition_formula "
        "identity lock（2249=2134+106+9；2251=2249+2）**、**FM36 连续 + MOCK39 隔离**；"
        "产物写入隔离 MOCK39（不覆盖 MOCK3–38）。",
        "",
        "## Capability gain",
        "",
        "1. FM36 packet/fingerprint/gate/ledger 零漂移连续（unique=2249 · 1053 · "
        ...
        "## Next",
        "",
        "- Controller 可 commit 本包（improved/partial/delta membership + "
        "union-formula only）",
    ...
    with open(readme, "w", encoding="utf-8") as fh:
        fh.write(
            "\n".join(
                [
                    "# C-FM-37 mock scale improved/partial/surface-delta/union-formula safety root",
                    "",
                    f"gate: `{payload['gate']}`",
                    "seal_chain_extended: false",
                    f"scale_tier_count: {payload['scale_tier_count']}",
                    ...
                    f"resume_taxonomy: {payload['resume_improved']}/"
                    f"{payload['resume_same']}/{payload['resume_worse']}",
                    "does_not_overwrite: _mock_c_fm02.._mock_c_fm36 / standard dryrun",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "C-FM-37 scale improved/partial/surface-delta membership + "
            "union-partition formula safety (offline)"
        )
    )
    parser.add_argument(
        "--output-root",
    ...
    if args.execute:
        print(
            ...
            "C-FM-37 is offline QA only; KEEP_EXECUTE_FALSE",
            file=sys.stderr,
        )
        return 2

    payload = run_scale_improved_partial_surface_delta_union_partition_formula_safety(
        paths=ImprovedPartialSurfaceDeltaUnionPartitionFormulaPaths(
