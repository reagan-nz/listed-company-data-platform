"""
B 类 category routing — 货币资金管理制度 / 对外担保管理制度边角锁测（B-FM-38）。

覆盖 harvest 已见但旧逻辑落 other 的标题：
- 「…货币资金管理制度…」→ announcement（非 other；窄；勿裸「管理制度」）
- 「…对外担保管理制度…」→ announcement（非 other；窄；≠对外担保情况简报）
- 裸「管理制度」/分子公司管理制度不得因本包泛化抬成 announcement
- 薪酬 / 名单 / 简报 / ESG 仍落 other
- 公司章程 / 募资管理制度 / 独立非执行董事工作制度 / 总经理工作细则不回退

离线 only · 无 CNINFO · 无 live · 不造 §7 FP · 不重开 LIVE_PASS 包。

运行：
    python lab/test_cninfo_b_class_category_routing_monetary_external_guarantee_system_edge.py
"""
...
MONETARY = "货币资金管理制度"  # BD2E586
GUARANTEE = "海量数据对外担保管理制度"  # BD2E790
BARE_SYS = "某事项管理制度"  # 不得因裸「管理制度」抬成 announcement
SUBSIDIARY = "分、子公司管理制度（2025年6月）"  # BD2E342；本包不推
...
class TestMonetaryExternalGuaranteeSystemRoutingEdge(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.config = routing._load_yaml(CATEGORIES)

    def test_monetary_funds_system_announcement_not_other(self) -> None:
        """BD2E586：货币资金管理制度 → announcement（闭合 other）。"""
        r = routing.route_title(MONETARY, self.config)
        ...
        self.assertIn("货币资金管理制度", r.matched_patterns)

    def test_external_guarantee_system_announcement_not_other(self) -> None:
        """BD2E790：对外担保管理制度 → announcement（闭合 other）。"""
        r = routing.route_title(GUARANTEE, self.config)
        self.assertEqual(r.predicted_document_type, "announcement")
        self.assertEqual(r.predicted_route_to, "cninfo_general_announcement_pdf")
        self.assertNotEqual(r.predicted_document_type, "other")
        self.assertEqual(r.classification_status, "classified_correctly")
        self.assertIn("对外担保管理制度", r.matched_patterns)

    def test_bare_or_subsidiary_system_not_lifted(self) -> None:
        """裸「管理制度」/分子公司管理制度不得因本包泛化抬成 announcement。"""
        ...
        self.assertEqual(r_sub.predicted_document_type, "other")

    def test_guarantee_brief_and_low_value_still_other(self) -> None:
        """对外担保情况简报与薪酬/名单/简报/ESG 仍落 other（本包不硬推）。"""
        for title in (GUARANTEE_BRIEF, XINCHOU, MINGDAN, JIANBAO, ESG):
