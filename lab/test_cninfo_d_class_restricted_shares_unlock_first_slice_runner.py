"""
D-class restricted_shares_unlock first-slice runner 测试（无 CNINFO · 无 live 执行）。

运行：
    python lab/test_cninfo_d_class_restricted_shares_unlock_first_slice_runner.py
"""
...
DRYRUN_REPORT = runner.RESTRICTED_SHARES_UNLOCK_FIRST_SLICE_DRYRUN_REPORT_CSV
DRYRUN_SUMMARY = runner.RESTRICTED_SHARES_UNLOCK_FIRST_SLICE_DRYRUN_SUMMARY_MD
...
class TestRestrictedSharesUnlockFirstSliceRunner(unittest.TestCase):
    ...
    def test_mixed_mode_with_block_trade_blocked(self) -> None:
        result = _run(BASE_ARGS + ["--block-trade-first-slice"])
        self.assertNotEqual(result.returncode, 0)
        self.assertIn(
            runner.RESTRICTED_SHARES_UNLOCK_FIRST_SLICE_MIXED_MODE_BLOCKED,
            result.stderr,
        )
    ...
    def test_live_path_execute_function_exists(self) -> None:
        ...
        self.assertTrue(callable(runner.execute_restricted_shares_unlock_first_slice_live))

    def test_wrong_approval_flag_rejected_before_cninfo(self) -> None:
        with mock.patch("requests.get") as get_mock, mock.patch("requests.post") as post_mock:
            result = _run(
                BASE_ARGS + ["--approve-d-class-block-trade-first-slice"]
            )
            get_mock.assert_not_called()
            post_mock.assert_not_called()
        self.assertNotEqual(result.returncode, 0)
