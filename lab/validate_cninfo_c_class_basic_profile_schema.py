"""
CNINFO C-class basic_profile fixture schema validation (Era C Phase 4).

Validates fixtures/c_class/basic_profile/basic_profile_fixtures.jsonl against
schemas/c_class/c_company_basic_profile.schema.json.

No network, no database, no verified.

Usage:
    python lab/validate_cninfo_c_class_basic_profile_schema.py
"""
...
DEFAULT_SCHEMA = os.path.join(
    BASE_DIR, "schemas", "c_class", "c_company_basic_profile.schema.json"
)
DEFAULT_CSV = os.path.join(
    BASE_DIR, "outputs", "validation", "cninfo_c_class_basic_profile_schema_validation_report.csv"
)
DEFAULT_MD = os.path.join(
    BASE_DIR, "outputs", "validation", "cninfo_c_class_basic_profile_schema_validation_summary.md"
)
...
def write_summary_md(
    ...
    lines = [
        ...
        f"| 脚本 | `lab/validate_cninfo_c_class_basic_profile_schema.py` |",
        "",
        "## 3. 总体结果",
        "",
        f"| 指标 | 数值 |",
    ...
    lines.extend([
        ...
        "## 附录",
        "",
        "详见 [cninfo_c_class_basic_profile_schema_validation_report.csv]"
        "(cninfo_c_class_basic_profile_schema_validation_report.csv)。",
        "",
    ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate C-class basic_profile fixtures")
