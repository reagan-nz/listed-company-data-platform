...
# -*- coding: utf-8 -*-
"""
D 类 abnormal_trading further-scale（~50）孤立根 runner。

共享 denser-day 探针 + 离线 secCode 过滤；默认 rows=200 覆盖 2026-07-02 全日截面。
不写入 AT next-slice / ESH / SC / EP / RSU / FIA 冻结根。

用法：
  .venv/bin/python lab/run_cninfo_d_class_abnormal_trading_further_scale.py --build-universe-lock
  .venv/bin/python lab/run_cninfo_d_class_abnormal_trading_further_scale.py --dry-run \\
  ...
  .venv/bin/python lab/run_cninfo_d_class_abnormal_trading_further_scale.py --live \\
...
SHARED_PROBE_KEY = "single_day_paged_2026-07-02_rows200"
SHARED_ROWS = 200
EXPECTED_SIZE = 50
PASS_THRESHOLD = 40  # ≥40/50 acceptable → PASS_WITH_CAVEAT
...
MARKETLIST_CITE_JSON = os.path.join(
    VALIDATION,
    "cninfo_d_class_abnormal_trading_further_scale_marketlist_cite_20260716.json",
)

FORBIDDEN_CODES = {"688671", "301259"}
# next-slice 已证不在 denser-day 截面内的空控候选
EMPTY_CONTROL_CODES = [
    ("000895", "双汇发展"),
    ("601988", "中国银行"),
]
...
def validate_universe(rows: Sequence[Dict[str, str]]) -> List[str]:
    ...
    if len(rows) != EXPECTED_SIZE:
        issues.append(
            f"abnormal_trading_further_scale_universe_size_must_equal_{EXPECTED_SIZE}:"
            f"got={len(rows)}"
        )
    seen_cases: Set[str] = set()
    seen_codes: Set[str] = set()
    expected_ids = {f"DAT{i}" for i in range(201, 251)}
    for row in rows:
        ...
        if case_id not in expected_ids:
            issues.append(f"forbidden_case_id_in_abnormal_trading_further_scale:{case_id}")
        ...
        if anchor != ANCHOR_TDATE:
            issues.append(f"anchor_tdate_mismatch:{case_id}:{anchor}")
    ...
    if seen_cases != expected_ids:
        issues.append("case_id_set_must_be_DAT201_DAT250")
    return issues


def build_universe_lock_from_cite() -> int:
    """一次 CNINFO denser-day cite → marketList 快照 + DAT201–250 lock。"""
    ...
    payload, http_status, last_error = core._cninfo_request(
        session, component_cfg, params, stats, "at_fs_marketlist_cite"
    )
    ...
    for rec in records:
        ...
        if not code or code in FORBIDDEN_CODES or code in EXCLUDED_NEXT_SLICE_CODES:
            continue
        code = code.zfill(6)
    ...
    if len(found_codes) < 48:
        print(
            f"ERROR: denser-day cite too thin for 48 found slots: got={len(found_codes)}",
            file=sys.stderr,
        )
        return 2

    selected_found = found_codes[:48]
    rows: List[Dict[str, str]] = []
    for idx, code in enumerate(selected_found, start=201):
        rec = by_code[code]
        name = str(rec.get("secName") or rec.get("SECNAME") or "").strip() or code
        rows.append(
            {
                "case_id": f"DAT{idx}",
                "probe_key": f"at_fs_shared_dense_{code}",
                "company_code": code,
                "company_name": name,
                "component": COMPONENT,
                "market": _infer_market(code),
                "query_mode": "single_day_paged",
                "anchor_tdate": ANCHOR_TDATE,
                ...
                "notes": (
                    "further-scale found-path candidate from denser-day marketList cite; "
                    "NOT verified"
                ),
                "evidence_cite": "D-FM-03_marketlist_cite_20260702",
                "universe_lock_status": "locked",
                "lock_date": "2026-07-16",
                "approval_task_id": "D-FM-03",
                "per_case_request_budget": "1",
                "total_request_cap": "50",
                "shared_probe_prefer": "1",
                "shared_rows": str(SHARED_ROWS),
                "dense_day_cite_strength": "live_marketlist_cite_total_rows",
            }
        )

    for j, (code, name) in enumerate(EMPTY_CONTROL_CODES):
        case_id = f"DAT{249 + j}"  # DAT249, DAT250
        rows.append(
            {
                "case_id": case_id,
                "probe_key": f"at_fs_empty_control_{code}",
                "company_code": code,
                "company_name": name,
                "component": COMPONENT,
                "market": _infer_market(code),
                ...
                "exclude_flags": (
                    "exclude_688671;exclude_301259;"
                    "exclude_sparse_day_20260703_sole_found_anchor;"
                    "empty_control_not_in_denser_day_marketlist;"
                    "detail_nested_deferred"
                ),
                "notes": (
                    "empty control retained; prior next-slice live proved absent "
                    "from denser-day marketList"
                ),
                "evidence_cite": "D-FM-03_empty_control_from_DAT101_105_live",
                "universe_lock_status": "locked",
                "lock_date": "2026-07-16",
                "approval_task_id": "D-FM-03",
                "per_case_request_budget": "1",
    ...
    ordered = [rows_by_id[f"DAT{i}"] for i in range(201, 251)]
    ...
    print(
        f"mode=abnormal_trading_further_scale_universe_lock "
        f"cases={len(ordered)} cninfo_calls={stats.cninfo_requests} "
        f"marketlist_total={total} selected_found=48 empty_controls=2"
    )
        ...
        dry.append(
            {
                ...
                "notes": (
                    f"anchor={ANCHOR_TDATE}; shared_probes=1; rows={SHARED_ROWS}; "
...
def write_dryrun_summary(
    ...
    lines = [
        ...
        f"- shared probe: **sdate=edate={ANCHOR_TDATE}** · page=1 · rows={SHARED_ROWS}",
        ...
        "```",
        "",
        f"**Future acceptance threshold: ≥{PASS_THRESHOLD}/{EXPECTED_SIZE} acceptable → PASS_WITH_CAVEAT**",
        "",
    ]
    path = os.path.join(
        output_paths["reports"],
...
def compute_gate(universe_rows: Sequence[Dict[str, str]], summaries: Dict[str, Dict[str, str]]) -> str:
    acceptable = 0
...
def execute_live(
    ...
    quality_rows = [
        {k: r[k] for k in QUALITY_COLUMNS}
        for r in live_rows
    ]
    ...
    summary_path = os.path.join(
        output_paths["reports"],
        "d_class_abnormal_trading_further_scale_live_summary.md",
    )
    lines = [
        "# CNINFO D 类 abnormal_trading Further-Scale Live Summary",
        "",
        f"_生成时间：{datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S')} UTC_",
        "",
        "> **性质：** abnormal_trading further-scale live · **R19 standing bounded** · **NOT verified**",
        "",
        "## Counts",
        "",
        "| 指标 | 值 |",
        ...
        "d_class_abnormal_trading_further_scale_live_path_gate = READY_FOR_APPROVAL",
        "d_class_abnormal_trading_further_scale_live_gate = NOT_APPROVED",
        f"d_class_abnormal_trading_further_scale_execution_gate = {gate}",
    ...
    print(
        f"mode=abnormal_trading_further_scale_live cases={len(live_rows)} "
        f"cninfo_calls={stats.cninfo_requests} acceptable={acceptable}/{len(live_rows)} "
        f"found={found} empty={empty}"
    )
    print(f"gate=d_class_abnormal_trading_further_scale_execution_gate={gate}")
...
def run_dry_run(args: argparse.Namespace) -> int:
    ...
    print(
        f"mode=abnormal_trading_further_scale_dry_run cases={len(dry_rows)} "
        f"planned_request_count_total=1 cninfo_calls=0"
    )
    print("gate=d_class_abnormal_trading_further_scale_runner_extension_gate=READY_FOR_APPROVAL")
...
def run_live(args: argparse.Namespace) -> int:
    if not args.approve_d_class_abnormal_trading_further_scale:
        print(
            "ERROR: approve_d_class_abnormal_trading_further_scale_required",
            file=sys.stderr,
        )
        return 2
    ok_root, root_err = validate_output_root(args.output_root)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="D-class abnormal_trading further-scale (~50) runner"
    )
