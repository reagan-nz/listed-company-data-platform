"""
B-FM-52：董事会决议 known_004 / known_005 晋升锁测（离线）。

覆盖：
- board_resolution_known_004 已为 ready（BD2E658 届次董事会第十四次）
- board_resolution_known_005 已为 ready（BD2E730 届次董事会第十三次）
- title_pattern 与 board known_001–003、监事会届次案可区分
- harvest 标题经既有路由预测为 board_resolution
- 既有 LIVE_PASS 路径不回退（含 B-FM-51 supervisory/board）
- 本包不改路由；不重开已 LIVE_PASS 案；拒绝 audit 年报陷阱

无 CNINFO · 无 live · 不造 §7 FP。

运行：
    python lab/test_cninfo_b_class_board_resolution_known_004_005_promotion.py
"""
...
PATTERN_BOARD_004 = "第三届董事会第十四次会议决议公告"
PATTERN_BOARD_005 = "第三届董事会第十三次会议决议公告"
...
class TestBoardResolutionKnown004005Promotion(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.config = routing._load_yaml(routing.DEFAULT_CATEGORIES)
        cls.known = _load_cases(KNOWN_YAML)

    def test_board_resolution_known_004_ready_fields(self) -> None:
        c = _by_id(self.known, "board_resolution_known_004")
        self.assertEqual(c["case_status"], "ready")
        self.assertEqual(c["company_code"], "300998")
        self.assertEqual(c["company_name"], "宁波方正")
        self.assertEqual(c["title_pattern"], PATTERN_BOARD_004)
        ...
        self.assertIn(PATTERN_BOARD_004, HARVEST_BOARD_004)
        self.assertNotEqual(c["title_pattern"], PATTERN_BOARD_001)
        self.assertNotEqual(c["title_pattern"], PATTERN_BOARD_002)
        self.assertNotEqual(c["title_pattern"], PATTERN_BOARD_003)
        self.assertNotEqual(c["title_pattern"], PATTERN_BOARD_005)
        self.assertNotEqual(c["title_pattern"], "决议公告")
        self.assertTrue(retrieval._title_matches(HARVEST_BOARD_004, PATTERN_BOARD_004))
        self.assertFalse(retrieval._title_matches(HARVEST_BOARD_002, PATTERN_BOARD_004))
        self.assertFalse(retrieval._title_matches(HARVEST_BOARD_005, PATTERN_BOARD_004))
        self.assertFalse(retrieval._title_matches(HARVEST_BOARD_003, PATTERN_BOARD_004))
        self.assertFalse(retrieval._title_matches(HARVEST_SB_004, PATTERN_BOARD_004))
        # known_002「第四次」不得误抬「第十四次」
        self.assertFalse(retrieval._title_matches(HARVEST_BOARD_004, PATTERN_BOARD_002))

    def test_board_resolution_known_005_ready_fields(self) -> None:
        c = _by_id(self.known, "board_resolution_known_005")
        self.assertEqual(c["case_status"], "ready")
        self.assertEqual(c["company_code"], "301511")
        self.assertEqual(c["company_name"], "德福科技")
        self.assertEqual(c["title_pattern"], PATTERN_BOARD_005)
        self.assertEqual(c["date_start"], "2025-06-26")
        self.assertEqual(c["date_end"], "2025-06-29")
        self.assertEqual(c["expected_document_type"], "board_resolution")
        self.assertEqual(c["expected_route_to"], "cninfo_general_announcement_pdf")
        self.assertIn(PATTERN_BOARD_005, HARVEST_BOARD_005)
        self.assertNotEqual(c["title_pattern"], PATTERN_BOARD_001)
        self.assertNotEqual(c["title_pattern"], PATTERN_BOARD_002)
        self.assertNotEqual(c["title_pattern"], PATTERN_BOARD_003)
        self.assertNotEqual(c["title_pattern"], PATTERN_BOARD_004)
        self.assertTrue(retrieval._title_matches(HARVEST_BOARD_005, PATTERN_BOARD_005))
        self.assertFalse(retrieval._title_matches(HARVEST_BOARD_004, PATTERN_BOARD_005))
        self.assertFalse(retrieval._title_matches(HARVEST_BOARD_002, PATTERN_BOARD_005))
        self.assertFalse(retrieval._title_matches(HARVEST_SB_003, PATTERN_BOARD_005))
        self.assertFalse(retrieval._title_matches(HARVEST_BOARD_005, PATTERN_BOARD_002))
    ...
    def test_patterns_mutually_distinct(self) -> None:
        ...
        self.assertFalse(retrieval._title_matches(HARVEST_BOARD_005, PATTERN_BOARD_004))
        self.assertFalse(retrieval._title_matches(HARVEST_SB_004, PATTERN_BOARD_004))
        self.assertFalse(retrieval._title_matches(HARVEST_BOARD_004, "第四届监事会第三十次会议决议公告"))

    def test_harvest_titles_route_correctly(self) -> None:
        for title in (HARVEST_BOARD_004, HARVEST_BOARD_005):
    ...
    def test_prior_paths_not_regressed(self) -> None:
        for title, expected in (
            ...
            (HARVEST_BOARD_003, "board_resolution"),
