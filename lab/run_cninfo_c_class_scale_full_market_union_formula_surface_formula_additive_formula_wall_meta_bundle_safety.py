...
identity lock runner（离线 · C-FM-53）。

Usage:
    python3 lab/run_cninfo_c_class_scale_full_market_union_formula_surface_formula_additive_formula_wall_meta_bundle_safety.py
    python3 lab/run_cninfo_c_class_scale_full_market_union_formula_surface_formula_additive_formula_wall_meta_bundle_safety.py \\
      --output-root outputs/validation/_mock_c_fm53_scale_full_market_union_formula_surface_formula_additive_formula_wall_meta_bundle_safety
"""
...
REPORT_ROOT_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_full_market_union_formula_surface_formula_additive_formula_wall_meta_bundle_safety"
)
REPORT_MD_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_full_market_union_formula_surface_formula_additive_formula_wall_meta_bundle_safety_20260716.md"
)
REPORT_JSON_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_full_market_union_formula_surface_formula_additive_formula_wall_meta_bundle_safety_20260716.json"
)
TASK_REPORT_REL = (
    "outputs/validation/"
    "cninfo_c_class_fm53_scale_full_market_union_formula_surface_formula_additive_formula_wall_meta_bundle_safety_20260716.md"
)
...
def write_reports(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    for key, name in (
        ...
        (
            "cross_full_market_union_formula_surface_formula_additive_formula_wall_meta_bundle_path",
    ...
    json_payload = {
        ...
        "cross_full_market_union_formula_surface_formula_additive_formula_wall_meta_bundle_sha256": payload[
        ...
        "cross_full_market_union_formula_surface_formula_additive_formula_wall_meta_bundle_path": payload[
            "cross_full_market_union_formula_surface_formula_additive_formula_wall_meta_bundle_path"
        ],
    ...
    md_lines = [
        ...
        *layer_lines,
        ...
        "union/surface/additive 公式墙元捆绑身份锁",
        "- MOCK3–54 冻结 · MOCK55 放行",
        "",
        "## Hold",
        "",
        "```",
        f"c_fm_53_scale_full_market_union_formula_surface_formula_additive_formula_wall_meta_bundle_safety_gate = "
        f"{payload['gate']}",
        "execute_production_snapshot_rebuild = false",
        "approved_for_snapshot_rebuild = false",
        "cninfo_calls = 0",
    ...
    task_lines = [
        ...
        "## Task",
        "",
        "在 C-FM-52 已 commit 且 EXECUTE 仍 human-held 之上，继续 **非 seal** "
        ...
        "lock（surface_delta=2）**、**cross_full_market_union_formula_surface_formula_additive_formula_wall_meta_bundle "
        "identity lock（union/surface/additive 公式墙元捆绑）**、**FM51 连续 + MOCK55 隔离**；"
        "产物写入隔离 MOCK55（不覆盖 MOCK3–54）。",
        "",
        "## Capability gain",
        "",
        "1. FM52 packet/fingerprint/gate/ledger 零漂移连续（unique=2249 · 1053 · "
        ...
        "5. cross_full_market_union_formula_surface_formula_additive_formula_wall_meta_bundle_identity_lock："
        "union/surface/additive 公式墙元捆绑 · 组成变异拒绝",
        "6. output-root：MOCK3–54 冻结 · MOCK55 放行；harvest/resume 写拒绝",
        ...
        "| 路径 | 变更 |",
        "|------|------|",
        "| `lab/cninfo_c_class_scale_full_market_union_formula_surface_formula_additive_formula_wall_meta_bundle_safety.py` | **新增** 核心 |",
        "| `lab/run_cninfo_c_class_scale_full_market_union_formula_surface_formula_additive_formula_wall_meta_bundle_safety.py` | **新增** runner |",
        "| `lab/test_cninfo_c_class_scale_full_market_union_formula_surface_formula_additive_formula_wall_meta_bundle_safety.py` | **新增** 测试 |",
        "| `outputs/validation/cninfo_c_class_erad_protected_output_roots.csv` | C-ROOT-MOCK55 + C-ROOT-002 说明 |",
        ...
        "| 允许 | 禁止 |",
        "|------|------|",
        "| validation/_mock_* union_formula/surface_formula/additive_formula-wall ledger / battery / packet | 生产 snapshot EXECUTE |",
        "| 只读 FM01–05 / FM12–52 gate JSON / harvest / resume / dryrun / exclusion | CNINFO live |",
        "| offline QA · union_formula/surface_formula/additive_formula-wall 重算（不覆盖 MOCK3–54） | 覆盖 MOCK3–54 |",
        ...
        "## Next",
        "",
        "- Controller 可 commit 本包（union_formula/surface_formula/additive_formula-wall-meta-bundle only）",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "C-FM-53 scale union_formula/surface_formula/additive_formula-wall-meta-bundle "
            "safety (offline)"
        )
    )
    parser.add_argument(
    ...
    if args.execute:
        print(
            ...
            "C-FM-53 is offline QA only; KEEP_EXECUTE_FALSE",
            file=sys.stderr,
        )
        return 2
