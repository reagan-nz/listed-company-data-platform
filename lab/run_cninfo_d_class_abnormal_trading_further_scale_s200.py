...
# -*- coding: utf-8 -*-
"""
D 类 abnormal_trading further-scale S200（~200）孤立根 runner。

组成策略：denser-day（2026-07-02）全量可用码 + 邻近日（2026-07-01）增量码拼合至 ~200，
外加少量 empty-control。共享探针按日各 1 次；离线 secCode 过滤。

不写入 AT further_scale S50 / next_slice / ESH / SC / EP / RSU / FIA 冻结根。

用法：
  .venv/bin/python lab/run_cninfo_d_class_abnormal_trading_further_scale_s200.py --build-universe-lock
  .venv/bin/python lab/run_cninfo_d_class_abnormal_trading_further_scale_s200.py --dry-run \\
  ...
  .venv/bin/python lab/run_cninfo_d_class_abnormal_trading_further_scale_s200.py --live \\
...
DEFAULT_UNIVERSE_CSV = os.path.join(
    VALIDATION,
    "cninfo_d_class_abnormal_trading_further_scale_s200_universe_lock_20260716.csv",
)
MARKETLIST_CITE_JSON = os.path.join(
    VALIDATION,
    "cninfo_d_class_abnormal_trading_further_scale_s200_marketlist_cite_20260716.json",
)

FORBIDDEN_CODES = {"688671", "301259"}
# 空控候选池：仅当两日 marketList 均不出现时才锁定；优先保留 S50 已证空控
...
def _probe_key(tdate: str) -> str:
    return f"single_day_paged_{tdate}_rows{SHARED_ROWS}"
...
def validate_output_root(output_root: str) -> Tuple[bool, str]:
    ...
    if root != expected:
        return False, (
            "abnormal_trading_further_scale_s200_output_root_must_be_"
...
def validate_universe(rows: Sequence[Dict[str, str]]) -> List[str]:
    ...
    if len(rows) != EXPECTED_SIZE:
        issues.append(
            f"abnormal_trading_further_scale_s200_universe_size_must_equal_{EXPECTED_SIZE}:"
            f"got={len(rows)}"
        )
    ...
    for row in rows:
        ...
        if case_id not in expected_ids:
            issues.append(f"forbidden_case_id_in_abnormal_trading_further_scale_s200:{case_id}")
        ...
        if anchor not in allowed_anchors:
            issues.append(f"anchor_tdate_not_in_s200_compose:{case_id}:{anchor}")
    ...
    if seen_cases != expected_ids:
        issues.append("case_id_set_must_be_DAT301_DAT500")
    return issues
...
def build_universe_lock_from_cite() -> int:
    """两次 CNINFO cite（denser + adjacent）→ 拼合 DAT301–500 lock。"""
    ...
    primary = _cite_day(
        session, component_cfg, stats, PRIMARY_TDATE, "at_s200_cite_primary"
    )
    adjacent = _cite_day(
        session, component_cfg, stats, ADJACENT_TDATE, "at_s200_cite_adjacent"
    )
    ...
    empty_budget = len(selected_empty)
    found_budget = EXPECTED_SIZE - empty_budget
        ...
        rows.append(
            {
                ...
                "probe_key": f"at_s200_shared_{tdate}_{code}",
                "company_code": code,
                "company_name": name,
                "component": COMPONENT,
                ...
                "exclude_flags": (
                    ...
                    "exclude_at_further_scale_s50_root;"
                    "detail_nested_deferred"
                ),
                "notes": (
                    f"s200 found-path from {cite_tag} marketList cite; NOT verified"
                ),
                "evidence_cite": f"D-FM-04_{cite_tag}_{tdate.replace('-', '')}",
                "universe_lock_status": "locked",
                "lock_date": "2026-07-16",
                "approval_task_id": "D-FM-04",
                "per_case_request_budget": "1",
                "total_request_cap": str(EXPECTED_SIZE),
                "shared_probe_prefer": "1",
                "shared_rows": str(SHARED_ROWS),
                "dense_day_cite_strength": "multi_day_compose_primary_plus_adjacent",
    ...
    for code, name in selected_empty:
        rows.append(
            {
                "case_id": f"DAT{case_idx}",
                "probe_key": f"at_s200_empty_control_{code}",
                "company_code": code,
                "company_name": name,
                "component": COMPONENT,
                "market": _infer_market(code),
                ...
                "exclude_flags": (
                    "exclude_688671;exclude_301259;"
                    "exclude_sparse_day_20260703_sole_found_anchor;"
                    "empty_control_not_in_s200_compose_marketlist;"
                    "detail_nested_deferred"
                ),
                "notes": (
                    "empty control for s200 compose; cite-verified absent from "
                    f"primary={PRIMARY_TDATE} and adjacent={ADJACENT_TDATE}"
                ),
                "evidence_cite": "D-FM-04_empty_control_compose",
                "universe_lock_status": "locked",
                "lock_date": "2026-07-16",
                "approval_task_id": "D-FM-04",
                "per_case_request_budget": "1",
    ...
    cite_payload = {
        ...
        "primary": {
            ...
            "selected_found_codes": selected_primary,
        },
        "adjacent": {
            "tdate": ADJACENT_TDATE,
            ...
            "adjacent_only_available": len(adjacent_only),
            "selected_found_codes": selected_adjacent,
        },
        "compose": {
            "expected_size": EXPECTED_SIZE,
            "selected_primary_found": len(selected_primary),
            "selected_adjacent_found": len(selected_adjacent),
            "empty_controls": [c for c, _ in selected_empty],
            "case_id_range": f"DAT{CASE_ID_START}-DAT{CASE_ID_END}",
        },
        "sample_records_primary": primary["records"][:3],
        "sample_records_adjacent": adjacent["records"][:3],
    }
    ...
    print(
        f"mode=abnormal_trading_further_scale_s200_universe_lock "
        f"cases={len(ordered)} cninfo_calls={stats.cninfo_requests} "
        f"primary_total={primary['total']} adjacent_total={adjacent['total']} "
        f"selected_primary={len(selected_primary)} "
        f"selected_adjacent={len(selected_adjacent)} "
    ...
    return 0 if ok_http and stats.cninfo_requests == 2 else 2
        ...
        dry.append(
            {
                ...
                "shared_probe_key": _probe_key(tdate),
                "planned_output_root": output_root,
                "planned_endpoint": ENDPOINT,
                "cninfo_call_planned": "shared_per_day",
                "pdf_download": "no",
                "ocr": "no",
                "extraction": "no",
                "db_write": "no",
                ...
                "notes": (
                    f"anchor={tdate}; shared_probes_per_unique_day={len(unique_days)}; "
                    f"rows={SHARED_ROWS}; company_filter_offline=secCode; "
                    f"forbidden_anchor={FORBIDDEN_ANCHOR}; "
                    f"compose=primary_{PRIMARY_TDATE}+adjacent_{ADJACENT_TDATE}; "
...
def write_planned_snapshots(
    ...
    for row in rows:
        ...
        payload = {
            ...
            "query_mode": row["query_mode"],
            "shared_probe_key": _probe_key(tdate),
            "planned_requests": [_probe_key(tdate)],
            ...
            "compose_note": f"primary={PRIMARY_TDATE}; adjacent={ADJACENT_TDATE}",
        }
...
def write_dryrun_report(
    ...
    path = os.path.join(
        ...
        "d_class_abnormal_trading_further_scale_s200_dryrun_report.csv",
    )
...
def write_dryrun_summary(
    ...
    lines = [
        ...
        f"| planned_shared_cninfo_requests | **{len(unique_days)}**（按日共享） |",
        ...
        f"- compose: primary=`{PRIMARY_TDATE}` + adjacent=`{ADJACENT_TDATE}`",
        f"- shared probes: **{len(unique_days)}** day(s) · page=1 · rows={SHARED_ROWS}",
    ...
    path = os.path.join(
        output_paths["reports"],
        "d_class_abnormal_trading_further_scale_s200_dryrun_summary.md",
    )
...
def assess_case(
    ...
    tdate = row["anchor_tdate"]
    notes = [
        f"shared_probe={_probe_key(tdate)}",
        ...
        json.dump(
            {
                ...
                "shared_probe": _probe_key(row["anchor_tdate"]),
                "cninfo_called": True,
                "detail_nested_deferred": True,
            },
            f,
...
def execute_live(
    ...
    # 按日共享探针：每个唯一 anchor_tdate 一次 CNINFO
    ...
    if stats.cninfo_requests != len(unique_days):
        print(
            f"ERROR: shared_cninfo_plan_mismatch:got={stats.cninfo_requests}"
            f":expected={len(unique_days)}",
            file=sys.stderr,
        )
        return 2
    ...
    quality_path = os.path.join(
        output_paths["reports"],
        "d_class_abnormal_trading_further_scale_s200_quality_report.csv",
    )
    ...
    summary_path = os.path.join(
        output_paths["reports"],
        "d_class_abnormal_trading_further_scale_s200_live_summary.md",
    )
    lines = [
        ...
        "",
        "```text",
        f"d_class_abnormal_trading_further_scale_s200_execution_gate = {gate}",
        ...
        "## Caveats",
        "",
        f"- multi_day_compose_not_full_market：primary `{PRIMARY_TDATE}` + adjacent "
        f"`{ADJACENT_TDATE}` 拼合子集，非全市场 abnormal_trading 覆盖。",
    ...
    print(
        f"mode=abnormal_trading_further_scale_s200_live cases={len(live_rows)} "
    ...
    print(f"gate=d_class_abnormal_trading_further_scale_s200_execution_gate={gate}")
...
def run_dry_run(args: argparse.Namespace) -> int:
    ...
    unique_days = sorted({r["anchor_tdate"] for r in dry_rows})
    print(
        f"mode=abnormal_trading_further_scale_s200_dry_run cases={len(dry_rows)} "
        f"planned_shared_days={len(unique_days)} cninfo_calls=0"
    )
    print("gate=d_class_abnormal_trading_further_scale_s200_dryrun_gate=PASS_OFFLINE")
...
def run_live(args: argparse.Namespace) -> int:
    if not args.approve_d_class_abnormal_trading_further_scale_s200:
        print(
            "ERROR: approve_d_class_abnormal_trading_further_scale_s200_required",
            file=sys.stderr,
        )
        return 2
    ok_root, root_err = validate_output_root(args.output_root)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="D-class abnormal_trading further-scale S200 (~200) runner"
    )
