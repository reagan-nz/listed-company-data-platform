"""
D-class block_trade first-slice runner 测试（无 CNINFO · 无 live 执行）。

运行：
    python lab/test_cninfo_d_class_block_trade_first_slice_runner.py
"""
...
UNIVERSE_CSV = runner.DEFAULT_BLOCK_TRADE_FIRST_SLICE_UNIVERSE_CSV
OUTPUT_ROOT = runner.DEFAULT_BLOCK_TRADE_FIRST_SLICE_OUTPUT_ROOT
DRYRUN_REPORT = runner.BLOCK_TRADE_FIRST_SLICE_DRYRUN_REPORT_CSV
DRYRUN_SUMMARY = runner.BLOCK_TRADE_FIRST_SLICE_DRYRUN_SUMMARY_MD
...
MARGIN_OUTPUT = runner.DEFAULT_MARGIN_TRADING_FIRST_SLICE_OUTPUT_ROOT
...
def _block_row_from_dict(r: dict[str, str]) -> runner.BlockTradeFirstSliceRow:
    return runner.BlockTradeFirstSliceRow(
        case_id=r["case_id"],
        ...
        reason=r.get("reason", ""),
        dlc002_reference=r.get("dlc002_reference", ""),
    )


class TestBlockTradeFirstSliceRunner(unittest.TestCase):
    ...
    def test_universe_size_must_equal_5(self) -> None:
        ...
        self.assertIn(runner.BLOCK_TRADE_FIRST_SLICE_UNIVERSE_SIZE_MISMATCH, result.stderr)

    def test_only_dbt001_through_dbt005_allowed(self) -> None:
        rows = _read_universe_rows()
        rows[0] = {**rows[0], "case_id": "DBT999"}
        with tempfile.TemporaryDirectory() as tmp:
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertIn(runner.BLOCK_TRADE_FIRST_SLICE_FORBIDDEN_CASE_ID, result.stderr)

    def test_component_must_be_block_trade(self) -> None:
        rows = _read_universe_rows()
        rows = [{**r, "component": "margin_trading"} if r["case_id"] == "DBT002" else r for r in rows]
        with tempfile.TemporaryDirectory() as tmp:
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertIn(runner.BLOCK_TRADE_FIRST_SLICE_WRONG_COMPONENT, result.stderr)

    def test_688671_rejected(self) -> None:
        rows = _read_universe_rows()
        rows = [{**r, "company_code": "688671"} if r["case_id"] == "DBT003" else r for r in rows]
        with tempfile.TemporaryDirectory() as tmp:
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertIn(runner.BLOCK_TRADE_FIRST_SLICE_FORBIDDEN_COMPANY_CODE, result.stderr)

    def test_301259_rejected(self) -> None:
        rows = _read_universe_rows()
        rows = [{**r, "company_code": "301259"} if r["case_id"] == "DBT004" else r for r in rows]
        with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_output_root_isolation_enforced(self) -> None:
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertIn(runner.BLOCK_TRADE_FIRST_SLICE_V1_OUTPUT_ROOT_WRITE_BLOCKED, result.stderr)

    def test_known_event_margin_disclosure_roots_write_blocked(self) -> None:
        for bad_root, token in [
            (V2_OUTPUT_ROOT, runner.BLOCK_TRADE_FIRST_SLICE_V2_OUTPUT_ROOT_WRITE_BLOCKED),
            (
                REPLACEMENT_OUTPUT_ROOT,
                runner.BLOCK_TRADE_FIRST_SLICE_REPLACEMENT_OUTPUT_ROOT_WRITE_BLOCKED,
            ),
            (
                TARGETED_OUTPUT_ROOT,
                runner.BLOCK_TRADE_FIRST_SLICE_TARGETED_PROBE_OUTPUT_ROOT_WRITE_BLOCKED,
            ),
            (
                MARGIN_OUTPUT_ROOT,
                runner.BLOCK_TRADE_FIRST_SLICE_MARGIN_OUTPUT_ROOT_WRITE_BLOCKED,
            ),
            (
                DISCLOSURE_OUTPUT_ROOT,
                runner.BLOCK_TRADE_FIRST_SLICE_DISCLOSURE_OUTPUT_ROOT_WRITE_BLOCKED,
            ),
        ]:
    ...
    def test_live_without_approval_rejected_before_cninfo(self) -> None:
        with mock.patch("requests.get") as get_mock, mock.patch("requests.post") as post_mock:
            ...
            post_mock.assert_not_called()
        self.assertNotEqual(result.returncode, 0)
        self.assertIn(runner.BLOCK_TRADE_FIRST_SLICE_APPROVAL_REQUIRED, result.stderr)

    def test_live_path_execute_function_exists(self) -> None:
        self.assertTrue(hasattr(runner, "execute_block_trade_first_slice_live"))
        self.assertTrue(callable(runner.execute_block_trade_first_slice_live))

    def test_wrong_approval_flag_rejected_before_cninfo(self) -> None:
        with mock.patch("requests.get") as get_mock, mock.patch("requests.post") as post_mock:
            result = _run(
                BASE_ARGS + ["--approve-d-class-margin-trading-first-slice"]
            )
            get_mock.assert_not_called()
            post_mock.assert_not_called()
        self.assertNotEqual(result.returncode, 0)
        self.assertIn(runner.BLOCK_TRADE_FIRST_SLICE_WRONG_APPROVAL_FLAG, result.stderr)

    def test_planned_request_count_lte_20(self) -> None:
        load_rows = _read_universe_rows()
        total = sum(
            runner.compute_block_trade_first_slice_planned_requests(
                _block_row_from_dict(r)
            )
            for r in load_rows
        )
        self.assertLessEqual(total, runner.BLOCK_TRADE_FIRST_SLICE_TOTAL_MAX_REQUESTS)
    ...
    def test_disclosure_only_captured_normal_upgrade_blocked(self) -> None:
        row = _block_row_from_dict(_read_universe_rows()[0])
        row = runner.BlockTradeFirstSliceRow(
            case_id=row.case_id,
            ...
            reason="disclosure-only evidence must not promote",
            dlc002_reference=row.dlc002_reference,
        )
        summary = {
        ...
        self.assertFalse(runner.is_block_trade_first_slice_acceptable(row, summary))
