"""
CNINFO C-class — Pre-EXECUTE controller commit-boundary / seal-chain 就绪账本（离线 · C-FM-08）。

在 C-FM-07（墙冻结漂移复核 seal）之上，补齐：
  1) FM-01..07 gate battery 只读聚合（含 FM-07 漂移 seal gate）
  2) seal-chain 连续性：MOCK8 墙冻结 + MOCK9 漂移 seal 产物与指纹锚点对齐
  3) EXECUTE hold seal：FM-06 人批包与 FM-07 drift seal 均 KEEP_EXECUTE_FALSE
  4) controller commit-boundary readiness packet（offline 链 PASS · EXECUTE 仍阻断）
  5) protected_output_roots.csv 注册一致性（MOCK3–10 · AUTH1）

禁止：CNINFO live · production EXECUTE · 覆盖权威 dual-layer 索引 ·
      覆盖 MOCK8/MOCK9 · verified 声称 · 翻转 approved_for_snapshot_rebuild ·
      commit/push。
"""
...
DEFAULT_MOCK_OUTPUT_ROOT_REL = (
    "outputs/validation/_mock_c_fm08_pre_execute_controller_commit_boundary"
)
...
FM07_DRIFT_MATRIX_REL = f"{FM07_MOCK_ROOT_REL}/drift_matrix.csv"
FM07_DRIFT_FP_REL = f"{FM07_MOCK_ROOT_REL}/drift_fingerprint.json"
FM07_BATTERY_REL = f"{FM07_MOCK_ROOT_REL}/fm_gate_battery.json"
...
class CommitBoundaryPaths:
    ...
    fm07_drift_matrix_rel: str = FM07_DRIFT_MATRIX_REL
    fm07_drift_fp_rel: str = FM07_DRIFT_FP_REL
    fm07_battery_rel: str = FM07_BATTERY_REL
...
def assert_commit_boundary_output_root(
    output_root: str, *, base_dir: str = BASE_DIR
) -> str:
    """commit-boundary 产物写根：必须 validation/_mock_*，并拒绝权威索引与 MOCK8/9。"""
    ...
    for label, frozen_rel in (
        ...
        if out_rel == frozen_rel or out_rel.startswith(frozen_rel + "/"):
            raise RuntimeError(
                f"C_FM08_MUST_NOT_OVERWRITE_{label}: "
...
def build_fm01_to_07_gate_battery_rows(
    *,
    fm01: Dict[str, Any],
    fm02: Dict[str, Any],
    fm03: Dict[str, Any],
    ...
    """FM-01..07 既有 gate 只读聚合（不重跑；含 FM-07 漂移 seal gate）。"""
    rows, checks = build_fm01_to_06_gate_battery_rows(
        fm01=fm01, fm02=fm02, fm03=fm03, fm04=fm04, fm05=fm05, fm06=fm06
    )
    rows = [r for r in rows if r.get("check_id") != "fm01_to_06_battery_all_pass"]
    checks.pop("fm01_to_06_battery_all_pass", None)

    gate = str(fm07.get("gate") or "").strip()
    cninfo = fm07.get("cninfo_calls", None)
    execute = fm07.get("execute_production_snapshot_rebuild", None)
    approved = fm07.get("approved_for_snapshot_rebuild", None)
    ok = (
        gate == "PASS_OFFLINE"
        and cninfo == 0
    ...
    rows.append(
        _row(
            check_id="fm07_pre_execute_wall_freeze_drift_recheck",
            layer="fm_gate_battery",
            path="fm07_pre_execute_wall_freeze_drift_recheck",
            ...
            ok=ok,
            notes="ok" if ok else "fm07_gate_not_pass",
        )
    )
    checks["fm07_pre_execute_wall_freeze_drift_recheck"] = ok
    ...
    rows.append(
        _row(
            check_id="fm01_to_07_battery_all_pass",
            layer="fm_gate_battery",
            expected="all_prior_fm_gates_PASS_OFFLINE",
            observed=f"pass={sum(1 for v in checks.values() if v)}/7",
            ok=prior_ok,
            notes="ok" if prior_ok else "battery_incomplete",
        )
    )
    checks["fm01_to_07_battery_all_pass"] = prior_ok
    return rows, checks


def build_seal_chain_continuity_rows(
    *,
    paths: CommitBoundaryPaths,
    base_dir: str = BASE_DIR,
) -> Tuple[List[Dict[str, str]], Dict[str, bool], Dict[str, Any], Dict[str, Any]]:
    """MOCK8 墙冻结 + MOCK9 漂移 seal 连续性（不覆盖冻结根）。"""
    ...
    presence_specs = [
        ...
        ("fm07_drift_matrix_exists", paths.fm07_drift_matrix_rel, "file"),
        ("fm07_drift_fp_exists", paths.fm07_drift_fp_rel, "file"),
        ("fm07_battery_exists", paths.fm07_battery_rel, "file"),
    ]
    ...
    # 与 C-FM-07 一致：优先 fingerprint.fingerprint_sha256
    excl_sha = str(
    ...
    seal_path = _abs(paths.fm07_drift_seal_rel, base_dir=base_dir)
    ...
    seal_kind = str(drift_seal.get("packet_kind") or "").strip()
    drift_detected = drift_seal.get("drift_detected", None)
    seal_wall = str(drift_seal.get("frozen_wall_fingerprint_sha256") or "").strip()
    seal_excl = str(
        drift_seal.get("frozen_exclusion_fingerprint_sha256") or ""
    ).strip()
    re_wall = str(drift_seal.get("recomputed_wall_fingerprint_sha256") or "").strip()
    re_excl = str(
        drift_seal.get("recomputed_exclusion_fingerprint_sha256") or ""
    ).strip()

    task_ok = seal_task == "C-FM-07"
    checks["seal_chain_fm07_task_id"] = task_ok
    rows.append(
        _row(
            check_id="seal_chain_fm07_task_id",
            layer="seal_chain_continuity",
    ...
    kind_ok = seal_kind == "pre_execute_wall_freeze_drift_recheck_seal"
    checks["seal_chain_fm07_packet_kind"] = kind_ok
    rows.append(
        _row(
            check_id="seal_chain_fm07_packet_kind",
            layer="seal_chain_continuity",
            expected="pre_execute_wall_freeze_drift_recheck_seal",
            observed=seal_kind or "missing",
            ok=kind_ok,
        )
    )

    gate_ok = seal_gate == "PASS_OFFLINE"
    checks["seal_chain_fm07_gate"] = gate_ok
    rows.append(
        _row(
            check_id="seal_chain_fm07_gate",
            layer="seal_chain_continuity",
    ...
    checks["seal_chain_zero_drift"] = no_drift_ok
    rows.append(
        _row(
            check_id="seal_chain_zero_drift",
            layer="seal_chain_continuity",
    ...
    chain_fp_ok = (
        seal_wall == FROZEN_WALL_FP_SHA256
        and seal_excl == FROZEN_EXCLUSION_FP_SHA256
        and re_wall == FROZEN_WALL_FP_SHA256
        and re_excl == FROZEN_EXCLUSION_FP_SHA256
        and wall_sha == FROZEN_WALL_FP_SHA256
        and excl_sha == FROZEN_EXCLUSION_FP_SHA256
    )
    ...
    rows.append(
        _row(
            ...
            layer="seal_chain_continuity",
            expected="wall+exclusion anchors continuous across MOCK8/MOCK9",
            observed=(
                f"wall={wall_sha[:12]}…;seal_wall={seal_wall[:12]}…;"
                f"re_wall={re_wall[:12]}…;excl_ok={excl_ok}"
            ),
            ok=chain_fp_ok,
        )
    )

    mock8_isolated = is_allowed_mock_test_cleanup_path(
        _abs(paths.fm06_mock_root_rel, base_dir=base_dir), base_dir=base_dir
    )
    mock9_isolated = is_allowed_mock_test_cleanup_path(
        _abs(paths.fm07_mock_root_rel, base_dir=base_dir), base_dir=base_dir
    )
    iso_ok = mock8_isolated and mock9_isolated
    checks["seal_chain_mock8_9_isolated"] = iso_ok
    rows.append(
        _row(
            check_id="seal_chain_mock8_9_isolated",
            layer="seal_chain_continuity",
            expected="both_isolated_mock",
            observed=f"mock8={mock8_isolated};mock9={mock9_isolated}",
            ok=iso_ok,
        )
    )
...
def build_dual_hold_seal_rows(
    *,
    ...
    """FM-06 人批包与 FM-07 drift seal 双层 KEEP_EXECUTE_FALSE。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    packet: Dict[str, Any] = {}
    ...
    self_ok = True  # 本包硬常量：execute=false · approved=false
    checks["fm08_self_constants_hold"] = self_ok
    rows.append(
        _row(
            check_id="fm08_self_constants_hold",
            layer="execute_hold_seal",
            expected="execute=false;approved=false",
            observed="execute=false;approved=false",
            ok=self_ok,
            notes="c_fm08_hard_constants",
        )
    )
...
def build_controller_readiness_rows(
    *,
    ...
    # commit boundary ready ≠ execute ready
    ...
    checks["readiness_human_gate_intact"] = human_ok
    rows.append(
        _row(
            check_id="readiness_human_gate_intact",
            layer="controller_readiness",
            expected="human_action_required_for_execute=true",
            observed=f"fm06={fm06_packet.get('human_action_required_for_execute')};"
    ...
    packet = {
        "packet_kind": "pre_execute_controller_commit_boundary",
        "task_id": TASK_ID,
        ...
        "notes": (
            "本包聚合 FM-01..07 seal-chain 并产出 controller commit-boundary 就绪账本；"
            "KEEP_EXECUTE_FALSE；不覆盖 MOCK8/MOCK9；不翻转 approved_for_snapshot_rebuild；"
...
def build_protected_csv_registry_rows(
    *,
    ...
    """protected_output_roots.csv 注册一致性（含 MOCK10）。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    ...
    mock9_ok = mock9_path == FM07_MOCK_ROOT_REL
    ...
    mock10_ok = mock10_path == DEFAULT_MOCK_OUTPUT_ROOT_REL
    ...
    rows.append(
        _row(
            ...
            layer="protected_csv_registry",
            expected="MOCK3-10+AUTH1_registered",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
        )
    )
...
def run_pre_execute_controller_commit_boundary(
    *,
    paths: CommitBoundaryPaths = CommitBoundaryPaths(),
    base_dir: str = BASE_DIR,
) -> Dict[str, Any]:
    """执行 C-FM-08 pre-EXECUTE controller commit-boundary（CNINFO=0）。"""
    generated_at = _utc_now_iso()
    out_root = assert_commit_boundary_output_root(
    ...
    bat_rows, bat_checks = build_fm01_to_07_gate_battery_rows(
        fm01=fm01,
        fm02=fm02,
        fm03=fm03,
        fm04=fm04,
    ...
    chain_rows, chain_checks, wall_fp, drift_seal = build_seal_chain_continuity_rows(
        paths=paths, base_dir=base_dir
    )
    seal_rows, seal_checks, fm06_packet = build_dual_hold_seal_rows(
    ...
    matrix = bat_rows + chain_rows + seal_rows + ready_rows + csv_rows
    layer_gates = {
        "fm_gate_battery": (
    ...
    readiness_path = os.path.join(out_root, "controller_commit_boundary_packet.json")
