...
identity lock（离线 · C-FM-46）。

在 C-FM-45（unique_surface_additive composition identity lock（2249/2251/2261）+
tier_coverage_status composition identity lock（7/3314）+ combined_dryrun_status
composition identity lock（dryrun=1053）+ cross_unique_surface_additive_tier_dryrun_wall_meta_bundle
...
（不新增 seal / decision-await / commit-boundary；非 extension↔drift 循环）：
  1) FM45 packet / fingerprint / gate / unique·tier·dryrun·wall ledgers 零漂移连续
  ...
  5) cross_lineage_drift_protected_repro_wall_meta_bundle_identity_lock：lineage/drift/protected/repro 墙元捆绑
  6) output-root：MOCK3–47 冻结 · MOCK48 放行
  7) FM-01..05 + FM-12..45 gate battery（跳过 seal FM06–11）

禁止：CNINFO live · production EXECUTE · 覆盖 MOCK3–47 / 权威 dual-layer 索引 ·
...
FM45_PACKET_REL = f"{FM45_MOCK_ROOT_REL}/scale_packet.json"
FM45_FINGERPRINT_REL = f"{FM45_MOCK_ROOT_REL}/scale_fingerprint.json"
FM45_UNIQUE_SURFACE_ADDITIVE_REL = (
    f"{FM45_MOCK_ROOT_REL}/unique_surface_additive_composition_identity_lock_ledger.json"
)
FM45_TIER_COVERAGE_STATUS_REL = (
    f"{FM45_MOCK_ROOT_REL}/tier_coverage_status_composition_identity_lock_ledger.json"
)
FM45_COMBINED_DRYRUN_STATUS_REL = (
    f"{FM45_MOCK_ROOT_REL}/combined_dryrun_status_composition_identity_lock_ledger.json"
)
FM45_WALL_META_REL = (
    f"{FM45_MOCK_ROOT_REL}/"
    "cross_unique_surface_additive_tier_dryrun_wall_meta_bundle_identity_lock_ledger.json"
)

# C-FM-46 本包冻结指纹（运行后由 fingerprint_* 锁定）
...
# FM45 连续锚点（只读）
FROZEN_FM45_UNIQUE_SURFACE_ADDITIVE_COMPOSITION_IDENTITY_LOCK_FP_SHA256 = (
    "29a262c9ac163890eb717bd3d6a4d5dc0ee3592fe19347ee07835178dd58d181"
)
FROZEN_FM45_TIER_COVERAGE_STATUS_COMPOSITION_IDENTITY_LOCK_FP_SHA256 = (
    "b7328a3e04080fa88f2aea3048a0c02e3890819342b19b7ce15fcafeeec68af8"
)
FROZEN_FM45_COMBINED_DRYRUN_STATUS_COMPOSITION_IDENTITY_LOCK_FP_SHA256 = (
    "3ad93cd9a8f8a120a498b42de66490f6ec627e3b40f685cc8f979a9d10c167b1"
)
FROZEN_FM45_CROSS_UNIQUE_SURFACE_ADDITIVE_TIER_DRYRUN_WALL_META_BUNDLE_IDENTITY_LOCK_FP_SHA256 = (
...
THIS_TASK_ROOT_ID = "C-ROOT-MOCK48"
PRIOR_TASK_ROOT_ID = "C-ROOT-MOCK47"
RESUME_HARVEST_ROOT_ID = "C-ROOT-002"

FROZEN_ROOT_IDS_MUST_BLOCK = tuple(f"C-ROOT-MOCK{i}" for i in range(3, 48))
...
class LineageDriftProtectedReproWallMetaBundlePaths:
    """只读输入与隔离写根路径规格。"""
    ...
    fm45_packet_rel: str = FM45_PACKET_REL
    fm45_fingerprint_rel: str = FM45_FINGERPRINT_REL
    fm45_unique_surface_additive_rel: str = FM45_UNIQUE_SURFACE_ADDITIVE_REL
    fm45_tier_coverage_status_rel: str = FM45_TIER_COVERAGE_STATUS_REL
    fm45_combined_dryrun_status_rel: str = FM45_COMBINED_DRYRUN_STATUS_REL
    fm45_wall_meta_rel: str = FM45_WALL_META_REL
...
def assert_fm46_output_root(
    output_root: str, *, base_dir: str = BASE_DIR
) -> str:
    """
    C-FM-46 写根：须 validation/_mock_*，不得覆盖 MOCK3–47，
    不得写权威 dual-layer 索引；允许本任务 MOCK48 或未登记 ephemeral。
    """
    ...
    if root_id is not None and root_id != THIS_TASK_ROOT_ID:
        raise RuntimeError(
            ...
            f"(C-FM-46 only writes {THIS_TASK_ROOT_ID} or ephemeral _mock_*)"
        )
...
def fingerprint_lineage_winner_provenance_composition_identity_lock() -> Tuple[str, Dict[str, Any]]:
    """lineage_winner_provenance composition identity lock 指纹（winner/complete）。"""
    doc = {
        "kind": "lineage_winner_provenance_composition_identity_lock",
        "winner_map_sha256": EXPECTED_WINNER_MAP_SHA256,
        "complete_codes_sha256": EXPECTED_COMPLETE_CODES_SHA256,
        "composition_sha256": EXPECTED_LINEAGE_WINNER_PROVENANCE_COMPOSITION_SHA256,
        "lineage_winner_provenance_formula": EXPECTED_LINEAGE_WINNER_PROVENANCE_FORMULA,
...
def fingerprint_partition_codeset_composition_identity_lock() -> Tuple[str, Dict[str, Any]]:
    """partition_codeset composition identity lock 指纹（complete/partial/failed）。"""
    doc = {
        "kind": "partition_codeset_composition_identity_lock",
        "complete_codes_sha256": EXPECTED_COMPLETE_CODES_SHA256,
        "partial_codes_sha256": EXPECTED_PARTIAL_CODES_SHA256,
        "failed_codes_sha256": EXPECTED_FAILED_CODES_SHA256,
        "composition_sha256": EXPECTED_PARTITION_CODESET_COMPOSITION_SHA256,
        "partition_codeset_formula": EXPECTED_PARTITION_CODESET_FORMULA,
        "deny_composition_mutate": True,
        "deny_inflate": True,
        "deny_deflate": True,
        "hold": "KEEP_EXECUTE_FALSE",
...
def fingerprint_protected_root_registry_composition_identity_lock() -> Tuple[str, Dict[str, Any]]:
    """protected_root_registry composition identity lock 指纹。"""
    doc = {
        "kind": "protected_root_registry_composition_identity_lock",
        "frozen_mock_lo": 3,
        "frozen_mock_hi": 47,
        "frozen_mock_count": 45,
        "allow_mock_id": 48,
        "required_extra": ["C-ROOT-002", "C-ROOT-011", "C-ROOT-AUTH1"],
        "required_total": 49,
        "composition_sha256": EXPECTED_PROTECTED_ROOT_REGISTRY_COMPOSITION_SHA256,
        "protected_root_registry_formula": EXPECTED_PROTECTED_ROOT_REGISTRY_FORMULA,
        "deny_composition_mutate": True,
        "deny_inflate": True,
        "deny_deflate": True,
        "hold": "KEEP_EXECUTE_FALSE",
...
def fingerprint_cross_lineage_drift_protected_repro_wall_meta_bundle_identity_lock() -> Tuple[str, Dict[str, Any]]:
    """cross_lineage_drift_protected_repro_wall_meta_bundle identity lock 指纹。"""
    doc = {
        "kind": "cross_lineage_drift_protected_repro_wall_meta_bundle_identity_lock",
        "compositions": {
            k: dict(v)
...
def evaluate_lineage_winner_provenance_composition_mutation(
    *,
    proposed_winner_map_sha256: str,
    proposed_complete_codes_sha256: str,
) -> Dict[str, Any]:
    """评估 lineage winner/complete 组成变异；一律拒绝。"""
    matches = (
        proposed_winner_map_sha256 == EXPECTED_WINNER_MAP_SHA256
        and proposed_complete_codes_sha256 == EXPECTED_COMPLETE_CODES_SHA256
    )
    return {
        "proposed": {
            "winner_map_sha256": proposed_winner_map_sha256,
            "complete_codes_sha256": proposed_complete_codes_sha256,
        },
        "frozen": {
            "winner_map_sha256": EXPECTED_WINNER_MAP_SHA256,
            "complete_codes_sha256": EXPECTED_COMPLETE_CODES_SHA256,
        ...
        "reason": "lineage_winner_provenance_composition_identity_frozen_pre_execute",
        "hold": "KEEP_EXECUTE_FALSE",
    }


def evaluate_partition_codeset_composition_mutation(
    *,
    proposed_complete_codes_sha256: str,
    proposed_partial_codes_sha256: str,
    proposed_failed_codes_sha256: str,
) -> Dict[str, Any]:
    """评估 partition codeset 组成变异；一律拒绝。"""
    matches = (
        proposed_complete_codes_sha256 == EXPECTED_COMPLETE_CODES_SHA256
        and proposed_partial_codes_sha256 == EXPECTED_PARTIAL_CODES_SHA256
        and proposed_failed_codes_sha256 == EXPECTED_FAILED_CODES_SHA256
    )
    return {
        "proposed": {
            "complete_codes_sha256": proposed_complete_codes_sha256,
            "partial_codes_sha256": proposed_partial_codes_sha256,
            "failed_codes_sha256": proposed_failed_codes_sha256,
        },
        "frozen": {
            "complete_codes_sha256": EXPECTED_COMPLETE_CODES_SHA256,
            "partial_codes_sha256": EXPECTED_PARTIAL_CODES_SHA256,
        ...
        "matches_frozen": matches,
        "mutation_allowed": False,
        "reason": "partition_codeset_composition_identity_frozen_pre_execute",
        "hold": "KEEP_EXECUTE_FALSE",
    }


def evaluate_protected_root_registry_composition_mutation(
    *,
    proposed_frozen_hi: int,
    proposed_allow_mock_id: int,
    proposed_required_total: int,
) -> Dict[str, Any]:
    """评估 protected_root_registry 组成变异；一律拒绝。"""
    matches = (
        proposed_frozen_hi == 47
        and proposed_allow_mock_id == 48
        and proposed_required_total == 49
    )
    return {
        "proposed": {
            "frozen_mock_hi": proposed_frozen_hi,
            "allow_mock_id": proposed_allow_mock_id,
            "required_total": proposed_required_total,
        },
        "frozen": {
            "frozen_mock_hi": 47,
            "allow_mock_id": 48,
        ...
        "matches_frozen": matches,
        "mutation_allowed": False,
        "reason": "protected_root_registry_composition_identity_frozen_pre_execute",
        "hold": "KEEP_EXECUTE_FALSE",
    }


def evaluate_cross_lineage_drift_protected_repro_wall_meta_bundle_mutation(
    *, proposed_compositions: Dict[str, Any]
) -> Dict[str, Any]:
    """评估 cross lineage/drift/protected/repro wall meta bundle 变异；一律拒绝。"""
    frozen = {
        k: dict(v)
        for k, v in EXPECTED_CROSS_LINEAGE_DRIFT_PROTECTED_REPRO_WALL_META_BUNDLE.items()
    }
    ...
    return {
        ...
        "mutation_allowed": False,
        "reason": (
            "cross_lineage_drift_protected_repro_wall_meta_bundle_identity_frozen_pre_execute"
        ),
        "hold": "KEEP_EXECUTE_FALSE",
    }


def build_lineage_winner_provenance_composition_identity_lock_rows(
    paths: LineageDriftProtectedReproWallMetaBundlePaths,
    *,
    base_dir: str = BASE_DIR,
) -> Tuple[List[Dict[str, str]], Dict[str, bool], Dict[str, Any]]:
    """lineage_winner_provenance composition identity lock（winner/complete）。"""
    del paths, base_dir
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    fp, doc = fingerprint_lineage_winner_provenance_composition_identity_lock()
    ...
    identity_ok = (
        EXPECTED_LINEAGE_WINNER_PROVENANCE_FORMULA == "winner/complete"
        and len(EXPECTED_WINNER_MAP_SHA256) == 64
        and len(EXPECTED_COMPLETE_CODES_SHA256) == 64
        and comp_sha == EXPECTED_LINEAGE_WINNER_PROVENANCE_COMPOSITION_SHA256
        and doc["composition_sha256"] == EXPECTED_LINEAGE_WINNER_PROVENANCE_COMPOSITION_SHA256
    )
    checks["lineage_winner_provenance_composition_identity"] = identity_ok
    rows.append(
        _row(
            check_id="lineage_winner_provenance_composition_identity",
            layer="lineage_winner_provenance_composition_identity_lock",
            expected="winner/complete",
            observed=f"formula={EXPECTED_LINEAGE_WINNER_PROVENANCE_FORMULA};sha={comp_sha[:12]}",
            ok=identity_ok,
            notes="ok" if identity_ok else "lineage_winner_provenance_composition_drift",
        )
    )
    ...
    sample_denials = [
        ...
        evaluate_lineage_winner_provenance_composition_mutation(
            proposed_winner_map_sha256="a" * 64,
            proposed_complete_codes_sha256="b" * 64,
        ),
        evaluate_lineage_winner_provenance_composition_mutation(
            proposed_winner_map_sha256=EXPECTED_WINNER_MAP_SHA256,
            proposed_complete_codes_sha256=EXPECTED_COMPLETE_CODES_SHA256,
    ...
    checks["lineage_winner_provenance_composition_mutation_denied"] = mut_ok
    rows.append(
        _row(
            check_id="lineage_winner_provenance_composition_mutation_denied",
            layer="lineage_winner_provenance_composition_identity_lock",
            expected="mutation_allowed=false",
            observed=f"samples={len(sample_denials)};all_denied={mut_ok}",
            ok=mut_ok,
            notes="ok" if mut_ok else "lineage_winner_provenance_mutation_leak",
        )
    )
    ...
    flags_ok = (
        ...
        and fp == FROZEN_LINEAGE_WINNER_PROVENANCE_COMPOSITION_IDENTITY_LOCK_FP_SHA256
    )
    checks["lineage_winner_provenance_composition_flags"] = flags_ok
    rows.append(
        _row(
            check_id="lineage_winner_provenance_composition_flags",
            layer="lineage_winner_provenance_composition_identity_lock",
            expected=FROZEN_LINEAGE_WINNER_PROVENANCE_COMPOSITION_IDENTITY_LOCK_FP_SHA256,
            observed=fp,
            ok=flags_ok,
            notes="ok" if flags_ok else "lineage_winner_provenance_fp_drift",
        )
    )

    identity_lock_ok = all(checks.values())
    checks["lineage_winner_provenance_composition_identity_lock_all_pass"] = identity_lock_ok
    rows.append(
        _row(
            check_id="lineage_winner_provenance_composition_identity_lock_all_pass",
            layer="lineage_winner_provenance_composition_identity_lock",
            expected="identity+mutation_denied+fp",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=identity_lock_ok,
            notes="ok" if identity_lock_ok else "lineage_winner_provenance_lock_incomplete",
        )
    )
    meta = {
        "fingerprint": fp,
...
def build_partition_codeset_composition_identity_lock_rows(
    paths: LineageDriftProtectedReproWallMetaBundlePaths,
    *,
    base_dir: str = BASE_DIR,
) -> Tuple[List[Dict[str, str]], Dict[str, bool], Dict[str, Any]]:
    """partition_codeset composition identity lock（complete/partial/failed）。"""
    del paths, base_dir
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    fp, doc = fingerprint_partition_codeset_composition_identity_lock()
    ...
    identity_ok = (
        EXPECTED_PARTITION_CODESET_FORMULA == "complete/partial/failed"
        and comp_sha == EXPECTED_PARTITION_CODESET_COMPOSITION_SHA256
        and doc["composition_sha256"] == EXPECTED_PARTITION_CODESET_COMPOSITION_SHA256
    )
    checks["partition_codeset_composition_identity"] = identity_ok
    rows.append(
        _row(
            check_id="partition_codeset_composition_identity",
            layer="partition_codeset_composition_identity_lock",
            expected="complete/partial/failed",
            observed=f"formula={EXPECTED_PARTITION_CODESET_FORMULA};sha={comp_sha[:12]}",
            ok=identity_ok,
            notes="ok" if identity_ok else "partition_codeset_composition_drift",
        )
    )
    ...
    sample_denials = [
        evaluate_partition_codeset_composition_mutation(
            ...
            proposed_partial_codes_sha256=EXPECTED_PARTIAL_CODES_SHA256,
            proposed_failed_codes_sha256=EXPECTED_FAILED_CODES_SHA256,
        ),
        evaluate_partition_codeset_composition_mutation(
            proposed_complete_codes_sha256=EXPECTED_COMPLETE_CODES_SHA256,
    ...
    mut_ok = all(d["mutation_allowed"] is False for d in sample_denials)
    checks["partition_codeset_composition_mutation_denied"] = mut_ok
    rows.append(
        _row(
            check_id="partition_codeset_composition_mutation_denied",
            layer="partition_codeset_composition_identity_lock",
            expected="mutation_allowed=false",
            observed=f"samples={len(sample_denials)};all_denied={mut_ok}",
            ok=mut_ok,
    ...
    flags_ok = (
        ...
        and doc["deny_deflate"] is True
        and doc["hold"] == "KEEP_EXECUTE_FALSE"
        and fp == FROZEN_PARTITION_CODESET_COMPOSITION_IDENTITY_LOCK_FP_SHA256
    )
    checks["partition_codeset_composition_flags"] = flags_ok
    rows.append(
        _row(
            check_id="partition_codeset_composition_flags",
            layer="partition_codeset_composition_identity_lock",
            expected=FROZEN_PARTITION_CODESET_COMPOSITION_IDENTITY_LOCK_FP_SHA256,
            observed=fp,
            ok=flags_ok,
            notes="ok" if flags_ok else "partition_codeset_fp_drift",
        )
    )

    identity_lock_ok = all(checks.values())
    checks["partition_codeset_composition_identity_lock_all_pass"] = identity_lock_ok
    rows.append(
        _row(
            check_id="partition_codeset_composition_identity_lock_all_pass",
            layer="partition_codeset_composition_identity_lock",
            expected="identity+mutation_denied+fp",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=identity_lock_ok,
            notes="ok" if identity_lock_ok else "partition_codeset_lock_incomplete",
        )
    )
    meta = {
        "fingerprint": fp,
...
def build_protected_root_registry_composition_identity_lock_rows(
    paths: LineageDriftProtectedReproWallMetaBundlePaths,
    *,
    base_dir: str = BASE_DIR,
) -> Tuple[List[Dict[str, str]], Dict[str, bool], Dict[str, Any]]:
    """protected_root_registry composition identity lock。"""
    del paths, base_dir
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    fp, doc = fingerprint_protected_root_registry_composition_identity_lock()
    ...
    identity_ok = (
        EXPECTED_PROTECTED_ROOT_REGISTRY_FORMULA == "MOCK3-47+MOCK48+resume+auth"
        and len(FROZEN_ROOT_IDS_MUST_BLOCK) == 45
        and THIS_TASK_ROOT_ID == "C-ROOT-MOCK48"
        and comp_sha == EXPECTED_PROTECTED_ROOT_REGISTRY_COMPOSITION_SHA256
        and doc["composition_sha256"] == EXPECTED_PROTECTED_ROOT_REGISTRY_COMPOSITION_SHA256
    )
    checks["protected_root_registry_composition_identity"] = identity_ok
    rows.append(
        _row(
            check_id="protected_root_registry_composition_identity",
            layer="protected_root_registry_composition_identity_lock",
            expected="MOCK3-47+MOCK48+resume+auth",
            observed=f"formula={EXPECTED_PROTECTED_ROOT_REGISTRY_FORMULA};sha={comp_sha[:12]}",
            ok=identity_ok,
            notes="ok" if identity_ok else "protected_root_registry_composition_drift",
        )
    )
    ...
    sample_denials = [
        evaluate_protected_root_registry_composition_mutation(
            ...
            proposed_allow_mock_id=48,
            proposed_required_total=49,
        ),
        evaluate_protected_root_registry_composition_mutation(
            proposed_frozen_hi=47,
    ...
    mut_ok = all(d["mutation_allowed"] is False for d in sample_denials)
    checks["protected_root_registry_composition_mutation_denied"] = mut_ok
    rows.append(
        _row(
            check_id="protected_root_registry_composition_mutation_denied",
            layer="protected_root_registry_composition_identity_lock",
            expected="mutation_allowed=false",
            observed=f"samples={len(sample_denials)};all_denied={mut_ok}",
            ok=mut_ok,
            notes="ok" if mut_ok else "protected_root_registry_mutation_leak",
        )
    )
    ...
    flags_ok = (
        ...
        and doc["deny_deflate"] is True
        and doc["hold"] == "KEEP_EXECUTE_FALSE"
        and fp == FROZEN_PROTECTED_ROOT_REGISTRY_COMPOSITION_IDENTITY_LOCK_FP_SHA256
    )
    checks["protected_root_registry_composition_flags"] = flags_ok
    rows.append(
        _row(
            check_id="protected_root_registry_composition_flags",
            layer="protected_root_registry_composition_identity_lock",
            expected=FROZEN_PROTECTED_ROOT_REGISTRY_COMPOSITION_IDENTITY_LOCK_FP_SHA256,
            observed=fp,
            ok=flags_ok,
            notes="ok" if flags_ok else "protected_root_registry_fp_drift",
        )
    )

    identity_lock_ok = all(checks.values())
    checks["protected_root_registry_composition_identity_lock_all_pass"] = identity_lock_ok
    rows.append(
        _row(
            check_id="protected_root_registry_composition_identity_lock_all_pass",
            layer="protected_root_registry_composition_identity_lock",
            expected="identity+mutation_denied+fp",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=identity_lock_ok,
            notes="ok" if identity_lock_ok else "protected_root_registry_lock_incomplete",
        )
    )
    meta = {
        "fingerprint": fp,
...
def build_cross_lineage_drift_protected_repro_wall_meta_bundle_identity_lock_rows(
    paths: LineageDriftProtectedReproWallMetaBundlePaths,
    *,
    base_dir: str = BASE_DIR,
) -> Tuple[List[Dict[str, str]], Dict[str, bool], Dict[str, Any]]:
    ...
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    fp, doc = fingerprint_cross_lineage_drift_protected_repro_wall_meta_bundle_identity_lock()

    comps = EXPECTED_CROSS_LINEAGE_DRIFT_PROTECTED_REPRO_WALL_META_BUNDLE
    bundle_sha = hashlib.sha256(
        json.dumps(comps, ensure_ascii=False, sort_keys=True).encode("utf-8")
    ).hexdigest()
    identity_ok = (
        len(comps) == 5
        and comps["lineage_winner_provenance_composition"]["formula"]
        == EXPECTED_LINEAGE_WINNER_PROVENANCE_FORMULA
        and comps["partition_codeset_composition"]["formula"]
        == EXPECTED_PARTITION_CODESET_FORMULA
        and comps["protected_root_registry_composition"]["formula"]
        == EXPECTED_PROTECTED_ROOT_REGISTRY_FORMULA
        and comps["cross_unique_surface_additive_tier_dryrun_wall_meta_bundle"][
            "bundle_sha256"
        ]
        == EXPECTED_CROSS_UNIQUE_SURFACE_ADDITIVE_TIER_DRYRUN_WALL_META_BUNDLE_SHA256
        and bundle_sha == EXPECTED_CROSS_LINEAGE_DRIFT_PROTECTED_REPRO_WALL_META_BUNDLE_SHA256
        and doc["bundle_sha256"]
        == EXPECTED_CROSS_LINEAGE_DRIFT_PROTECTED_REPRO_WALL_META_BUNDLE_SHA256
    )
    checks["cross_lineage_drift_protected_repro_wall_meta_bundle_identity"] = identity_ok
    rows.append(
        _row(
            check_id="cross_lineage_drift_protected_repro_wall_meta_bundle_identity",
            layer="cross_lineage_drift_protected_repro_wall_meta_bundle_identity_lock",
            expected="5_compositions_wall_meta_bundle",
            observed=f"n={len(comps)};sha={bundle_sha[:12]}",
            ok=identity_ok,
            notes="ok" if identity_ok else "wall_meta_bundle_drift",
        )
    )

    mutated = {k: dict(v) for k, v in comps.items()}
    ...
    sample_denials = [
        ...
        evaluate_cross_lineage_drift_protected_repro_wall_meta_bundle_mutation(
            proposed_compositions={**comps, "extra": {"formula": "x"}}
        ),
        evaluate_cross_lineage_drift_protected_repro_wall_meta_bundle_mutation(
            proposed_compositions={k: dict(v) for k, v in comps.items()}
        ),
    ]
    mut_ok = all(d["mutation_allowed"] is False for d in sample_denials)
    checks["cross_lineage_drift_protected_repro_wall_meta_bundle_mutation_denied"] = mut_ok
    rows.append(
        _row(
            check_id="cross_lineage_drift_protected_repro_wall_meta_bundle_mutation_denied",
            layer="cross_lineage_drift_protected_repro_wall_meta_bundle_identity_lock",
            expected="mutation_allowed=false",
            observed=f"samples={len(sample_denials)};all_denied={mut_ok}",
            ok=mut_ok,
    ...
    checks["cross_lineage_drift_protected_repro_wall_meta_bundle_flags"] = flags_ok
    rows.append(
        _row(
            check_id="cross_lineage_drift_protected_repro_wall_meta_bundle_flags",
            layer="cross_lineage_drift_protected_repro_wall_meta_bundle_identity_lock",
            expected=FROZEN_CROSS_LINEAGE_DRIFT_PROTECTED_REPRO_WALL_META_BUNDLE_IDENTITY_LOCK_FP_SHA256,
            observed=fp,
            ok=flags_ok,
    ...
    rows.append(
        _row(
            check_id="cross_lineage_drift_protected_repro_wall_meta_bundle_identity_lock_all_pass",
            layer="cross_lineage_drift_protected_repro_wall_meta_bundle_identity_lock",
            expected="identity+mutation_denied+fp",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=identity_lock_ok,
...
def build_fm45_continuity_rows(
    paths: LineageDriftProtectedReproWallMetaBundlePaths, *, base_dir: str = BASE_DIR
) -> Tuple[List[Dict[str, str]], Dict[str, bool]]:
    """FM44 packet / fingerprint / gate / 四 ledger 零漂移。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}

    packet = load_json(_abs(paths.fm45_packet_rel, base_dir=base_dir))
    fp_doc = load_json(_abs(paths.fm45_fingerprint_rel, base_dir=base_dir))
    gate_doc = load_json(_abs(paths.fm45_gate_json_rel, base_dir=base_dir))
    us_led = load_json(_abs(paths.fm45_unique_surface_additive_rel, base_dir=base_dir))
    od_led = load_json(_abs(paths.fm45_tier_coverage_status_rel, base_dir=base_dir))
    sd_led = load_json(_abs(paths.fm45_combined_dryrun_status_rel, base_dir=base_dir))
    wall_led = load_json(_abs(paths.fm45_wall_meta_rel, base_dir=base_dir))
    ...
    pkt_ok = (
        ...
        and packet.get("unique_surface_additive_composition_sha256")
        == EXPECTED_UNIQUE_SURFACE_ADDITIVE_COMPOSITION_SHA256
        and packet.get("tier_coverage_status_composition_sha256")
        == EXPECTED_TIER_COVERAGE_STATUS_COMPOSITION_SHA256
        and packet.get("combined_dryrun_status_composition_sha256")
        == EXPECTED_COMBINED_DRYRUN_STATUS_COMPOSITION_SHA256
        and packet.get("cross_unique_surface_additive_tier_dryrun_wall_meta_bundle_sha256")
        == EXPECTED_CROSS_UNIQUE_SURFACE_ADDITIVE_TIER_DRYRUN_WALL_META_BUNDLE_SHA256
    ...
    checks["fm45_packet_zero_drift"] = pkt_ok
    rows.append(
        _row(
            check_id="fm45_packet_zero_drift",
            layer="fm45_continuity",
            path=paths.fm45_packet_rel,
            expected="PASS_OFFLINE+stable_metrics+compositions",
            observed=f"gate={packet.get('gate')};unique={packet.get('harvest_unique_union')}",
            ok=pkt_ok,
            notes="ok" if pkt_ok else "fm45_packet_drift",
        )
    )
    ...
    fp_ok = (
        ...
        and fp_doc.get("unique_surface_additive_composition_sha256")
        == EXPECTED_UNIQUE_SURFACE_ADDITIVE_COMPOSITION_SHA256
        and fp_doc.get("tier_coverage_status_composition_sha256")
        == EXPECTED_TIER_COVERAGE_STATUS_COMPOSITION_SHA256
        and fp_doc.get("combined_dryrun_status_composition_sha256")
        == EXPECTED_COMBINED_DRYRUN_STATUS_COMPOSITION_SHA256
        and fp_doc.get("cross_unique_surface_additive_tier_dryrun_wall_meta_bundle_sha256")
        == EXPECTED_CROSS_UNIQUE_SURFACE_ADDITIVE_TIER_DRYRUN_WALL_META_BUNDLE_SHA256
        and fp_doc.get("seal_chain_extended") is False
    )
    checks["fm45_fingerprint_zero_drift"] = fp_ok
    rows.append(
        _row(
            check_id="fm45_fingerprint_zero_drift",
            layer="fm45_continuity",
            path=paths.fm45_fingerprint_rel,
            expected="stable_metrics+fm44_fps",
            observed=f"unique={fp_doc.get('harvest_unique_union')}",
            ok=fp_ok,
            notes="ok" if fp_ok else "fm45_fingerprint_drift",
        )
    )
    ...
    gate_ok = (
        ...
        and gate_doc.get("cross_unique_surface_additive_tier_dryrun_wall_meta_bundle_sha256")
        == EXPECTED_CROSS_UNIQUE_SURFACE_ADDITIVE_TIER_DRYRUN_WALL_META_BUNDLE_SHA256
        and gate_doc.get("seal_chain_extended") is False
    )
    checks["fm45_gate_zero_drift"] = gate_ok
    rows.append(
        _row(
            check_id="fm45_gate_zero_drift",
            layer="fm45_continuity",
            path=paths.fm45_gate_json_rel,
            expected="PASS_OFFLINE+KEEP_EXECUTE_FALSE",
            observed=f"gate={gate_doc.get('gate')}",
            ok=gate_ok,
            notes="ok" if gate_ok else "fm45_gate_drift",
        )
    )
    ...
    checks["fm45_unique_surface_additive_ledger_zero_drift"] = us_ok
    rows.append(
        _row(
            check_id="fm45_unique_surface_additive_ledger_zero_drift",
            layer="fm45_continuity",
            path=paths.fm45_unique_surface_additive_rel,
            expected=FROZEN_FM45_UNIQUE_SURFACE_ADDITIVE_COMPOSITION_IDENTITY_LOCK_FP_SHA256,
            observed=str(us_led.get("fingerprint_sha256")),
            ok=us_ok,
            notes="ok" if us_ok else "fm45_unique_surface_additive_ledger_drift",
        )
    )
    ...
    checks["fm45_tier_coverage_status_ledger_zero_drift"] = od_ok
    rows.append(
        _row(
            check_id="fm45_tier_coverage_status_ledger_zero_drift",
            layer="fm45_continuity",
            path=paths.fm45_tier_coverage_status_rel,
            expected=FROZEN_FM45_TIER_COVERAGE_STATUS_COMPOSITION_IDENTITY_LOCK_FP_SHA256,
            observed=str(od_led.get("fingerprint_sha256")),
            ok=od_ok,
            notes="ok" if od_ok else "fm45_tier_coverage_status_ledger_drift",
        )
    )
    ...
    checks["fm45_combined_dryrun_status_ledger_zero_drift"] = sd_ok
    rows.append(
        _row(
            check_id="fm45_combined_dryrun_status_ledger_zero_drift",
            layer="fm45_continuity",
            path=paths.fm45_combined_dryrun_status_rel,
            expected=FROZEN_FM45_COMBINED_DRYRUN_STATUS_COMPOSITION_IDENTITY_LOCK_FP_SHA256,
            observed=str(sd_led.get("fingerprint_sha256")),
            ok=sd_ok,
            notes="ok" if sd_ok else "fm45_combined_dryrun_status_ledger_drift",
        )
    )
    ...
    checks["fm45_wall_meta_ledger_zero_drift"] = wall_ok
    rows.append(
        _row(
            check_id="fm45_wall_meta_ledger_zero_drift",
            layer="fm45_continuity",
            path=paths.fm45_wall_meta_rel,
            expected=FROZEN_FM45_CROSS_UNIQUE_SURFACE_ADDITIVE_TIER_DRYRUN_WALL_META_BUNDLE_IDENTITY_LOCK_FP_SHA256,
            observed=str(wall_led.get("fingerprint_sha256")),
            ok=wall_ok,
            notes="ok" if wall_ok else "fm45_wall_meta_ledger_drift",
        )
    )

    all_ok = all(checks.values()) if checks else False
    checks["fm45_continuity_all_pass"] = all_ok
    rows.append(
        _row(
            check_id="fm45_continuity_all_pass",
            layer="fm45_continuity",
            expected="packet+fp+gate+4ledgers",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
            notes="ok" if all_ok else "fm45_continuity_incomplete",
        )
    )
    return rows, checks
...
def build_output_root_protection_rows(
    ...
    """output-root 保护：resume/harvest 写拒绝 + MOCK48 放行。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    ...
    out_detail = ""
    try:
        assert_fm46_output_root(paths.output_root_rel, base_dir=base_dir)
        out_ok = True
        out_detail = "allowed"
    except RuntimeError as exc:
    ...
    rows.append(
        _row(
            ...
            expected="MOCK48_or_ephemeral_allowed",
            observed=out_detail,
            ok=out_ok,
...
def build_frozen_mock_isolation_rows(
    ...
    """冻结 mock cohort 写隔离：MOCK3–47 拒绝 · MOCK48 放行。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    load_frozen_mock_cohort_roots.cache_clear()
    ...
    mock47_rel = FM45_MOCK_ROOT_REL
    mock47_blocked = False
    try:
        assert_fm46_output_root(mock47_rel, base_dir=base_dir)
    except RuntimeError as exc:
        mock47_blocked = FROZEN_MOCK_COHORT_WRITE_FORBIDDEN in str(exc)
    checks["mock47_still_frozen"] = mock47_blocked
    rows.append(
        _row(
            check_id="mock47_still_frozen",
            layer="frozen_mock_isolation",
            root_id=PRIOR_TASK_ROOT_ID,
            path=mock47_rel,
            expected="write_forbidden",
            observed="blocked" if mock47_blocked else "allowed",
            ok=mock47_blocked,
            notes="ok" if mock47_blocked else "mock47_write_leak",
        )
    )
    ...
    try:
        ...
        allow_ok = False
    checks["frozen_allow_mock48"] = allow_ok
    rows.append(
        _row(
            check_id="frozen_allow_mock48",
            layer="frozen_mock_isolation",
    ...
    rows.append(
        _row(
            ...
            layer="frozen_mock_isolation",
            expected="MOCK3-47_block+MOCK48_allow",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
...
def build_protected_csv_registry_rows(
    *,
    ...
    """protected_output_roots.csv：MOCK48 已登记。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    ...
    mock48 = by_id.get(THIS_TASK_ROOT_ID) or {}
    path_ok = DEFAULT_MOCK_OUTPUT_ROOT_REL in str(mock48.get("path_pattern") or "")
    checks["protected_csv_mock48_path"] = path_ok
    rows.append(
        _row(
            check_id="protected_csv_mock48_path",
            layer="protected_csv_registry",
            root_id=THIS_TASK_ROOT_ID,
            expected=DEFAULT_MOCK_OUTPUT_ROOT_REL,
            observed=str(mock48.get("path_pattern") or ""),
            ok=path_ok,
            notes="ok" if path_ok else "mock48_path_mismatch",
        )
    )
    ...
    rows.append(
        _row(
            ...
            layer="protected_csv_registry",
            expected="MOCK3–47+MOCK48+resume+auth+registered",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
...
def build_fm_gate_battery_rows(
    *, gates: Dict[str, Dict[str, Any]]
    ...
    """FM-01..05 + FM-12..45 gate battery（跳过 seal FM06–11）。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    specs = [
    ...
    seal_skip_keys = {
        ...
        "fm36", "fm37", "fm38", "fm39", "fm40", "fm41", "fm42", "fm43", "fm44", "fm45", "fm45", "fm45", "fm45",
    }
    for check_id, key in specs:
        ...
        if key in (
            ...
            "fm33", "fm34", "fm35", "fm36", "fm37", "fm38", "fm39", "fm40", "fm41", "fm42", "fm43", "fm44", "fm45", "fm45", "fm45", "fm45",
        ):
            ok = (
                ok
                and payload.get("harvest_unique_union") == EXPECTED_HARVEST_UNIQUE_UNION
        ...
        if key in (
            ...
            "fm34", "fm35", "fm36", "fm37", "fm38", "fm39", "fm40", "fm41", "fm42", "fm43", "fm44", "fm45", "fm45", "fm45", "fm45",
        ):
            ok = (
                ok
        ...
        if key in (
            ...
            "fm35", "fm36", "fm37", "fm38", "fm39", "fm40", "fm41", "fm42", "fm43", "fm44", "fm45", "fm45", "fm45", "fm45",
        ):
            ok = (
                ok
    ...
    rows.append(
        _row(
            check_id="fm01_05_12_45_battery_all_pass",
            layer="fm_gate_battery",
    ...
    checks["fm01_05_12_45_battery_all_pass"] = all_ok
    return rows, checks
...
def ensure_protected_roots_csv_fm46(
    *,
    csv_rel: str = PROTECTED_ROOTS_CSV_REL,
    base_dir: str = BASE_DIR,
) -> None:
    """注册 C-ROOT-MOCK48；加固 C-ROOT-002 unique/surface/additive/tier/dryrun 说明。"""
    ...
    _upsert(
        RESUME_HARVEST_ROOT_ID,
        {
            ...
            "notes": (
                ...
                "C-FM-24..45 scale/safety freezes + C-FM-46 lineage/drift/protected/"
                "repro-wall-meta-bundle; 只读直至人批重跑"
            ),
        },
    )
    _upsert(
        THIS_TASK_ROOT_ID,
        {
            ...
            "notes": (
                "C-FM-46 scale lineage_winner_provenance composition identity lock "
                "(winner/complete) + partition_codeset composition identity lock "
                "(complete/partial/failed) + protected_root_registry composition identity lock "
                "(MOCK3-47+MOCK48+resume+auth) + cross_lineage_drift_protected_repro_wall_meta_bundle "
                "identity lock + FM45 continuity; never production EXECUTE; "
                "must not overwrite MOCK3-47; seal_chain_extended=false"
            ),
        },
    )
...
def run_scale_lineage_drift_protected_repro_wall_meta_bundle_safety(
    *,
    paths: LineageDriftProtectedReproWallMetaBundlePaths | None = None,
    base_dir: str = BASE_DIR,
) -> Dict[str, Any]:
    """执行 C-FM-46 规模 unique/surface/additive/tier/dryrun-wall-meta-bundle 离线 QA。"""
    paths = paths or LineageDriftProtectedReproWallMetaBundlePaths()
    generated_at = _utc_now_iso()
    ensure_protected_roots_csv_fm46(
        csv_rel=paths.protected_roots_csv_rel, base_dir=base_dir
    )
    out_root = assert_fm46_output_root(paths.output_root_rel, base_dir=base_dir)

    matrix: List[Dict[str, str]] = []
    cont_rows, cont_checks = build_fm45_continuity_rows(paths, base_dir=base_dir)
    matrix.extend(cont_rows)
    rf_rows, rf_checks, rf_meta = build_lineage_winner_provenance_composition_identity_lock_rows(
        paths, base_dir=base_dir
    )
    matrix.extend(rf_rows)
    rt_rows, rt_checks, rt_meta = (
        build_partition_codeset_composition_identity_lock_rows(paths, base_dir=base_dir)
    )
    matrix.extend(rt_rows)
    rb_rows, rb_checks, rb_meta = (
        build_protected_root_registry_composition_identity_lock_rows(paths, base_dir=base_dir)
    )
    ...
    layer_gates = {
        "fm45_continuity": (
            "PASS_OFFLINE"
            if cont_checks.get("fm45_continuity_all_pass")
            else "FAIL_OFFLINE"
        ),
        "lineage_winner_provenance_composition_identity_lock": (
            "PASS_OFFLINE"
            if rf_checks.get("lineage_winner_provenance_composition_identity_lock_all_pass")
            else "FAIL_OFFLINE"
        ),
        "partition_codeset_composition_identity_lock": (
            "PASS_OFFLINE"
            if rt_checks.get("partition_codeset_composition_identity_lock_all_pass")
            else "FAIL_OFFLINE"
        ),
        "protected_root_registry_composition_identity_lock": (
            "PASS_OFFLINE"
            if rb_checks.get("protected_root_registry_composition_identity_lock_all_pass")
            else "FAIL_OFFLINE"
        ),
        ...
        "fm_gate_battery": (
            "PASS_OFFLINE"
            if bat_checks.get("fm01_05_12_45_battery_all_pass")
            else "FAIL_OFFLINE"
        ),
        "execute_hold_seal": (
            "PASS_OFFLINE"
    ...
    rf_rel = _write_ledger(
        "lineage_winner_provenance_composition_identity_lock_ledger.json",
        {
            ...
            "lineage_winner_provenance_formula": rf_meta[
    ...
    rt_rel = _write_ledger(
        "partition_codeset_composition_identity_lock_ledger.json",
        {
            ...
            "partition_codeset_formula": rt_meta["partition_codeset_formula"],
    ...
    rb_rel = _write_ledger(
        "protected_root_registry_composition_identity_lock_ledger.json",
        {
            ...
            "protected_root_registry_formula": rb_meta[
    ...
    observed_fps = {
        "lineage_winner_provenance_composition_identity_lock": rf_meta["fingerprint"],
        "partition_codeset_composition_identity_lock": rt_meta["fingerprint"],
        "protected_root_registry_composition_identity_lock": rb_meta["fingerprint"],
        "cross_lineage_drift_protected_repro_wall_meta_bundle_identity_lock": bundle_meta[
            "fingerprint"
        ],
        "scale_matrix": fp,
    }
    ...
    packet_rel = _write_ledger(
        "scale_packet.json",
        {
            ...
            "notes": (
                "lineage_winner_provenance composition identity lock (winner/complete) + "
                "partition_codeset composition identity lock (complete/partial/failed) + "
                "protected_root_registry composition identity lock (MOCK3-47+MOCK48+resume+auth) + "
                "cross_lineage_drift_protected_repro_wall_meta_bundle identity lock + "
                "FM45 continuity + MOCK48; EXECUTE remains human-held; "
                "does not overwrite MOCK3-47"
            ),
        },
    )
    ...
    with open(_abs(readme_rel, base_dir=base_dir), "w", encoding="utf-8") as fh:
        fh.write(
            "\n".join(
                [
                    "# C-FM-46 mock scale lineage/drift/protected/repro-wall-meta-bundle safety root",
                    "",
                    f"task_id: {TASK_ID}",
                    f"gate: {overall}",
                    "cninfo_calls: 0",
                    ...
                    "",
                    "Isolation: MOCK3-47 frozen; this root is C-ROOT-MOCK48.",
    ...
    return {
        ...
        "fingerprint": fp,
        "lineage_winner_provenance_path": rf_rel,
        "partition_codeset_path": rt_rel,
        "protected_root_registry_path": rb_rel,
        "cross_lineage_drift_protected_repro_wall_meta_bundle_path": bundle_rel,
        "battery_path": battery_rel,
        "packet_path": packet_rel,
        "observed_fps": observed_fps,
        "inputs": {
            "fm45_packet": paths.fm45_packet_rel,
            "fm45_fingerprint": paths.fm45_fingerprint_rel,
            "fm45_gate": paths.fm45_gate_json_rel,
