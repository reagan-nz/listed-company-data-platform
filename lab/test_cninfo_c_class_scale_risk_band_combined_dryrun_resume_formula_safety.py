"""
CNINFO C-class — 规模 risk-band membership + combined-dryrun/resume formula
单测（离线 · CNINFO=0 · C-FM-39）。

运行：
    python3 lab/test_cninfo_c_class_scale_risk_band_combined_dryrun_resume_formula_safety.py
"""
...
from cninfo_c_class_scale_risk_band_combined_dryrun_resume_formula_safety import (  # noqa: E402
    ...
    assert_fm39_output_root,
    build_combined_dryrun_coverage_identity_lock_rows,
    build_fm38_continuity_rows,
    build_partial_risk_band_membership_freeze_rows,
    build_resume_formula_identity_lock_rows,
    build_risk_band_formula_identity_lock_rows,
    ...
    fingerprint_partial_risk_band_membership_freeze,
    load_frozen_partial_risk_band_membership,
...
_RUNNER = os.path.join(
    _LAB_DIR,
    "run_cninfo_c_class_scale_risk_band_combined_dryrun_resume_formula_safety.py",
)
_TEST_SUMMARY_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_risk_band_combined_dryrun_resume_formula_safety_test_summary_20260715.md"
)
...
def _write_test_summary(cases: list) -> None:
    ...
    lines = [
        "# C-FM-39 Scale Risk-Band/Combined-Dryrun/Resume Formula — Test Summary",
        "",
        "_offline · CNINFO=0_",
        "",
        "| case | result |",
    ...
    lines.extend(
        [
            "",
            "```",
            "c_fm_39_scale_risk_band_combined_dryrun_resume_formula_safety_test_gate = "
            "PASS_OFFLINE",
            "cninfo_calls = 0",
...
class TestCFm39RiskBandCombinedDryrunResumeFormula(unittest.TestCase):
    ...
    def test_partial_risk_band_membership_mutation_denied(self) -> None:
        mem = load_frozen_partial_risk_band_membership()
        inj = dict(mem)
        inj["999999"] = "fu_light"
        mut = evaluate_partial_risk_band_membership_mutation(
            proposed_membership=inj, action="inject"
        )
        self.assertIs(mut["mutation_allowed"], False)
        _rows, checks, meta = build_partial_risk_band_membership_freeze_rows(
            RiskBandCombinedDryrunResumeFormulaPaths()
        )
        self.assertTrue(checks["partial_risk_band_membership_freeze_all_pass"])
        self.assertEqual(
            meta["fingerprint"], FROZEN_PARTIAL_RISK_BAND_MEMBERSHIP_FREEZE_FP_SHA256
        )
        self.assertEqual(
            meta["membership_sha256"], EXPECTED_PARTIAL_RISK_BAND_MEMBERSHIP_SHA256
        )
        fp, _doc = fingerprint_partial_risk_band_membership_freeze()
        self.assertEqual(fp, FROZEN_PARTIAL_RISK_BAND_MEMBERSHIP_FREEZE_FP_SHA256)
    ...
    def test_combined_dryrun_coverage_mutation_denied(self) -> None:
        ...
        self.assertIs(mut["mutation_allowed"], False)
        _rows, checks, meta = build_combined_dryrun_coverage_identity_lock_rows(
            RiskBandCombinedDryrunResumeFormulaPaths()
        )
        self.assertTrue(checks["combined_dryrun_coverage_identity_lock_all_pass"])
        self.assertEqual(
            meta["fingerprint"], FROZEN_COMBINED_DRYRUN_COVERAGE_IDENTITY_LOCK_FP_SHA256
        )

    def test_risk_band_formula_mutation_denied(self) -> None:
        mut = evaluate_risk_band_formula_mutation(
            proposed_bands={"p35_heavy": 76, "p3_mid": 14, "p2_mid": 12, "fu_light": 5}
        )
        self.assertIs(mut["mutation_allowed"], False)
        _rows, checks, meta = build_risk_band_formula_identity_lock_rows(
            RiskBandCombinedDryrunResumeFormulaPaths()
        )
        self.assertTrue(checks["risk_band_formula_identity_lock_all_pass"])
        self.assertEqual(
            meta["fingerprint"], FROZEN_RISK_BAND_FORMULA_IDENTITY_LOCK_FP_SHA256
        )

    def test_resume_formula_mutation_denied(self) -> None:
        mut = evaluate_resume_formula_mutation(
            proposed_improved=29, proposed_same=1, proposed_worse=0
        )
        self.assertIs(mut["mutation_allowed"], False)
        _rows, checks, meta = build_resume_formula_identity_lock_rows(
            RiskBandCombinedDryrunResumeFormulaPaths()
        )
        self.assertTrue(checks["resume_formula_identity_lock_all_pass"])
        self.assertEqual(
            meta["fingerprint"], FROZEN_RESUME_FORMULA_IDENTITY_LOCK_FP_SHA256
        )

    def test_fm38_continuity(self) -> None:
        rows, checks = build_fm38_continuity_rows(
            RiskBandCombinedDryrunResumeFormulaPaths()
        )
        self.assertTrue(checks.get("fm38_continuity_all_pass"), rows[-1])
    ...
    def test_output_root_guards(self) -> None:
        ...
        ok = assert_fm39_output_root(DEFAULT_MOCK_OUTPUT_ROOT_REL)
        self.assertTrue(ok)

    def test_full_offline_run(self) -> None:
        result = run_scale_risk_band_combined_dryrun_resume_formula_safety()
...
if __name__ == "__main__":
    ...
    suite = loader.loadTestsFromTestCase(
        TestCFm39RiskBandCombinedDryrunResumeFormula
    )
    ...
    if result.wasSuccessful():
        for name in [
            ...
            "test_partial_risk_band_membership_mutation_denied",
            "test_combined_dryrun_coverage_mutation_denied",
            "test_risk_band_formula_mutation_denied",
            "test_resume_formula_mutation_denied",
            "test_fm38_continuity",
