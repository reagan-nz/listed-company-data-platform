...
# -*- coding: utf-8 -*-
"""
D 类 executive_shareholding further-scale（~50）孤立根 runner。
...
用法：
  .venv/bin/python lab/run_cninfo_d_class_executive_shareholding_further_scale.py --build-universe-lock
  .venv/bin/python lab/run_cninfo_d_class_executive_shareholding_further_scale.py --dry-run \\
  ...
  .venv/bin/python lab/run_cninfo_d_class_executive_shareholding_further_scale.py --live \\
...
LEADER_DETAIL_CITE_JSON = os.path.join(
    VALIDATION,
    "cninfo_d_class_executive_shareholding_further_scale_leader_detail_cite_20260716.json",
)
...
def validate_universe(rows: Sequence[Dict[str, str]]) -> List[str]:
    ...
    if len(rows) != EXPECTED_SIZE:
        issues.append(
            f"executive_shareholding_further_scale_universe_size_must_equal_{EXPECTED_SIZE}:"
            f"got={len(rows)}"
        )
    seen_cases: Set[str] = set()
    seen_codes: Set[str] = set()
    expected_ids = {f"DES{i}" for i in range(201, 251)}
    for row in rows:
        ...
        if case_id not in expected_ids:
            issues.append(
                f"forbidden_case_id_in_executive_shareholding_further_scale:{case_id}"
            )
    ...
    if seen_cases != expected_ids:
        issues.append("case_id_set_must_be_DES201_DES250")
    return issues
...
def build_universe_lock_from_cite() -> int:
    ...
    payload, http_status, last_error = core._cninfo_request(
        session, component_cfg, params, stats, "esh_fs_leader_detail_cite"
    )
    ...
    for rec in records:
        ...
        if not code:
            continue
        code = code.zfill(6)
        if code in FORBIDDEN_CODES or code in EXCLUDED_PRIOR_SLICE_CODES:
            continue
    ...
    if len(found_codes) < 48:
        print(
            f"ERROR: denser-mode cite too thin for 48 found slots: got={len(found_codes)}",
            file=sys.stderr,
        )
        return 2
    ...
    for idx, code in enumerate(selected_found, start=201):
        ...
        rows.append(
            {
                "case_id": f"DES{idx}",
                "probe_key": f"esh_fs_shared_threeMonth_b_{code}",
                "company_code": code,
                "company_name": name,
                "component": COMPONENT,
                ...
                "notes": (
                    "further-scale found-path candidate from denser-mode "
                    "leader/detail cite; NOT verified"
                ),
                "evidence_cite": "D-FM-06_leader_detail_cite_threeMonth_b",
                "universe_lock_status": "locked",
                "lock_date": "2026-07-16",
                "approval_task_id": "D-FM-06",
    ...
    for j, (code, name) in enumerate(EMPTY_CONTROL_CODES):
        case_id = f"DES{249 + j}"  # DES249, DES250
        rows.append(
            {
                "case_id": case_id,
                "probe_key": f"esh_fs_empty_control_{code}",
                "company_code": code,
                "company_name": name,
                "component": COMPONENT,
                "market": _infer_market(code),
    ...
    ordered = [rows_by_id[f"DES{i}"] for i in range(201, 251)]
    ...
    print(
        f"mode=executive_shareholding_further_scale_universe_lock "
        f"cases={len(ordered)} cninfo_calls={stats.cninfo_requests} "
        f"leader_detail_total={total} selected_found=48 empty_controls=2"
    )
        ...
        dry.append(
            {
                ...
                "notes": (
                    ...
                    f"exclude_esh_next_slice_DES101_105=yes"
                ),
            }
        )
    return dry
...
def execute_live(
    ...
    quality_path = os.path.join(
        output_paths["reports"],
        "d_class_executive_shareholding_further_scale_quality_report.csv",
    )
    ...
    summary_path = os.path.join(
        output_paths["reports"],
        "d_class_executive_shareholding_further_scale_live_summary.md",
    )
    lines = [
        ...
        "d_class_executive_shareholding_further_scale_live_path_gate = READY_FOR_APPROVAL",
        "d_class_executive_shareholding_further_scale_live_gate = NOT_APPROVED",
        f"d_class_executive_shareholding_further_scale_execution_gate = {gate}",
    ...
    print(
        f"mode=executive_shareholding_further_scale_live cases={len(live_rows)} "
    ...
    print(f"gate=d_class_executive_shareholding_further_scale_execution_gate={gate}")
...
def run_dry_run(args: argparse.Namespace) -> int:
    ...
    print(
        f"mode=executive_shareholding_further_scale_dry_run cases={len(dry_rows)} "
        f"planned_request_count_total=1 cninfo_calls=0"
    )
    print(
        "gate=d_class_executive_shareholding_further_scale_runner_extension_gate="
        "READY_FOR_APPROVAL"
    )
...
def run_live(args: argparse.Namespace) -> int:
    if not args.approve_d_class_executive_shareholding_further_scale:
        print(
            "ERROR: approve_d_class_executive_shareholding_further_scale_required",
            file=sys.stderr,
        )
        return 2
    ok_root, root_err = validate_output_root(args.output_root)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="D-class executive_shareholding further-scale (~50) runner"
    )
