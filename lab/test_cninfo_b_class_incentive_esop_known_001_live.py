"""
B-FM-35：激励买卖自查报告 / 员工持股计划 known-document live allow-list 锁测（离线；不请求 CNINFO）。

覆盖：
- allow-list 仅含 incentive_trading_self_inspection_known_001 与
  employee_stock_ownership_plan_known_001（category 空）
- 字段与 fixtures ready 对齐
- mock live 路径可对两案得到 pass（predicted_document_type=announcement；非 other）
- 不包含 asset_valuation / audit_report / independent_director 等已 LIVE_PASS 案

无真实 CNINFO · 不造 §7 FP。

运行：
    python lab/test_cninfo_b_class_incentive_esop_known_001_live.py
"""
...
LIVE_DIR = os.path.join(
    ...
    "outputs",
    "validation",
    "cninfo_b_class_incentive_esop_known_001_live_20260715",
)
...
ANN_SI = "1224017800"
ANN_ESOP = "1224016542"
# 2025-06-27 00:00:00 UTC 近似；mock 用固定 ms
ANN_TS = 1750982400000  # 2025-06-27
...
class TestIncentiveEsopKnown001LiveAllowList(unittest.TestCase):
