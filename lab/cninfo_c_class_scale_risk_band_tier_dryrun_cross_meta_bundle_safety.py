"""
CNINFO C-class — 规模 risk_band composition identity lock + tier_coverage
composition identity lock + combined_dryrun composition identity lock +
cross_formula_composition_meta_bundle identity lock（离线 · C-FM-42）。

在 C-FM-41（additive composition identity lock（2261=2249+12）+ residual
composition identity lock（117=106+9+2）+ resume composition identity lock
（29=28+1+0）+ cross_composition_bundle identity lock）已 commit 且 EXECUTE 仍
human-held 之上，继续非 seal 规模/安全能力（不新增 seal / decision-await /
commit-boundary；非 extension↔drift 循环）：
  1) FM41 packet / fingerprint / gate / additive·residual·resume·cross-
     composition ledgers 零漂移连续
  2) risk_band_composition_identity_lock：75+14+12+5=106 组成身份锁
  3) tier_coverage_composition_identity_lock：tiers=7;coverage_sum=3314
  4) combined_dryrun_composition_identity_lock：combined_dryrun=1053
  5) cross_formula_composition_meta_bundle_identity_lock：五组成元捆绑
  6) output-root：MOCK3–43 冻结 · MOCK44 放行
  7) FM-01..05 + FM-12..41 gate battery（跳过 seal FM06–11）

禁止：CNINFO live · production EXECUTE · 覆盖 MOCK3–43 / 权威 dual-layer 索引 ·
...
# C-FM-42 本包冻结指纹（运行后由 fingerprint_* 锁定）
FROZEN_RISK_BAND_COMPOSITION_IDENTITY_LOCK_FP_SHA256 = (
    "21eaa407c04b3feafb4a86a6d0815f70aa11d7ceebedf42ea1e25a0caf63d147"
)
FROZEN_TIER_COVERAGE_COMPOSITION_IDENTITY_LOCK_FP_SHA256 = (
    "485406d8361dfe10c60ffc89ff5697a10cdfbb152d8938e19d06e11481e7327f"
)
FROZEN_COMBINED_DRYRUN_COMPOSITION_IDENTITY_LOCK_FP_SHA256 = (
    "377a7751726206001db5efa01c10103b96e6820de244de62f6d5ea78d261c9ae"
)
FROZEN_CROSS_FORMULA_COMPOSITION_META_BUNDLE_IDENTITY_LOCK_FP_SHA256 = (
    "abc7ccf7b160ab2c7e082b2787adc0ab9d88d06eabab4602b28486597f3c2d6f"
)
...
EXPECTED_RISK_BAND_COMPOSITION_SHA256 = (
    "b1100df4a931733326acb0f6a1651d1daade08805daf7274cc8edb56c70e2f6c"
)
EXPECTED_TIER_COVERAGE_COMPOSITION_SHA256 = (
    "8dde1cb0a0d885aaffc6bbb9a57fd8114bce94d002d061b507c003c77964f8b0"
)
EXPECTED_COMBINED_DRYRUN_COMPOSITION_SHA256 = (
    "c9dc304d81cc6aad68062a89f3b69f76e594c79a91de548cbc6b516385cce383"
)
EXPECTED_CROSS_FORMULA_COMPOSITION_META_BUNDLE_SHA256 = (
    "1a95681ad05421037aa96867a2c3c7182791b4111a3924c1b9568aa4bff4cff2"
)
...
EXPECTED_CROSS_FORMULA_COMPOSITION_META_BUNDLE = {
    "cross_formula_bundle": {
        "bundle_sha256": EXPECTED_CROSS_FORMULA_BUNDLE_SHA256,
    },
    "cross_composition_bundle": {
        "bundle_sha256": EXPECTED_CROSS_COMPOSITION_BUNDLE_SHA256,
    },
    "risk_band_composition": {
        "formula": EXPECTED_RISK_BAND_FORMULA,
        "composition_sha256": EXPECTED_RISK_BAND_COMPOSITION_SHA256,
    },
    "tier_coverage_composition": {
        "formula": EXPECTED_TIER_COVERAGE_FORMULA,
        "composition_sha256": EXPECTED_TIER_COVERAGE_COMPOSITION_SHA256,
    },
    "combined_dryrun_composition": {
        "formula": EXPECTED_COMBINED_DRYRUN_FORMULA,
        "composition_sha256": EXPECTED_COMBINED_DRYRUN_COMPOSITION_SHA256,
    },
}
...
THIS_TASK_ROOT_ID = "C-ROOT-MOCK44"
PRIOR_TASK_ROOT_ID = "C-ROOT-MOCK44"
RESUME_HARVEST_ROOT_ID = "C-ROOT-002"

FROZEN_ROOT_IDS_MUST_BLOCK = tuple(f"C-ROOT-MOCK{i}" for i in range(3, 44))
...
class RiskBandTierDryrunCrossMetaBundlePaths:
    """只读输入与隔离写根路径规格。"""
...
def assert_fm42_output_root(
    output_root: str, *, base_dir: str = BASE_DIR
) -> str:
    """
    C-FM-42 写根：须 validation/_mock_*，不得覆盖 MOCK3–43，
    不得写权威 dual-layer 索引；允许本任务 MOCK44 或未登记 ephemeral。
    """
    ...
    if root_id is not None and root_id != THIS_TASK_ROOT_ID:
        raise RuntimeError(
            ...
            f"(C-FM-42 only writes {THIS_TASK_ROOT_ID} or ephemeral _mock_*)"
        )
...
def fingerprint_risk_band_composition_identity_lock() -> Tuple[str, Dict[str, Any]]:
    """risk_band composition identity lock 指纹（75+14+12+5=106）。"""
    doc = {
        "kind": "risk_band_composition_identity_lock",
...
def fingerprint_tier_coverage_composition_identity_lock() -> Tuple[str, Dict[str, Any]]:
    """tier_coverage composition identity lock 指纹（tiers=7;coverage_sum=3314）。"""
    doc = {
...
def fingerprint_cross_formula_composition_meta_bundle_identity_lock() -> Tuple[str, Dict[str, Any]]:
    """cross_formula_composition_meta_bundle identity lock 指纹（五组成元捆绑）。"""
    doc = {
        "kind": "cross_formula_composition_meta_bundle_identity_lock",
        "compositions": {
            k: dict(v) for k, v in EXPECTED_CROSS_FORMULA_COMPOSITION_META_BUNDLE.items()
        },
...
def evaluate_risk_band_composition_mutation(
    *,
    ...
    return {
        ...
        "reason": "risk_band_composition_identity_frozen_pre_execute",
        "hold": "KEEP_EXECUTE_FALSE",
    }
...
def evaluate_combined_dryrun_composition_mutation(
    *,
    proposed_combined_dryrun: int,
) -> Dict[str, Any]:
    """评估 combined_dryrun 组成变异；一律拒绝。"""
    matches = proposed_combined_dryrun == EXPECTED_COMBINED_DRYRUN_COVERAGE
    return {
        "proposed": {"combined_dryrun": proposed_combined_dryrun},
...
def evaluate_cross_formula_composition_meta_bundle_mutation(
    *, proposed_compositions: Dict[str, Any]
) -> Dict[str, Any]:
    """评估 cross_formula_composition_meta_bundle 变异；一律拒绝。"""
    frozen = {
        k: dict(v)
        for k, v in EXPECTED_CROSS_FORMULA_COMPOSITION_META_BUNDLE.items()
    }
    proposed = {
    ...
    return {
        ...
        "matches_frozen": matches,
        "mutation_allowed": False,
        "reason": "cross_formula_composition_meta_bundle_identity_frozen_pre_execute",
        "hold": "KEEP_EXECUTE_FALSE",
    }


def build_risk_band_composition_identity_lock_rows(
    paths: RiskBandTierDryrunCrossMetaBundlePaths,
    *,
    base_dir: str = BASE_DIR,
) -> Tuple[List[Dict[str, str]], Dict[str, bool], Dict[str, Any]]:
    """risk_band composition identity lock（75+14+12+5=106）。"""
    del paths, base_dir
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    fp, doc = fingerprint_risk_band_composition_identity_lock()

    identity_ok = (
        EXPECTED_PARTIAL_RISK_BANDS["p35_heavy"]
        + EXPECTED_PARTIAL_RISK_BANDS["p3_mid"]
        + EXPECTED_PARTIAL_RISK_BANDS["p2_mid"]
        + EXPECTED_PARTIAL_RISK_BANDS["fu_light"]
        == EXPECTED_UNION_PARTIAL
        and EXPECTED_RISK_BAND_FORMULA == "75+14+12+5=106"
        and doc["composition_sha256"] == EXPECTED_RISK_BAND_COMPOSITION_SHA256
    )
    checks["risk_band_composition_identity"] = identity_ok
    rows.append(
        _row(
            check_id="risk_band_composition_identity",
            layer="risk_band_composition_identity_lock",
            expected="75+14+12+5=106",
            observed=(
                f"formula={EXPECTED_RISK_BAND_FORMULA};"
                f"sha={EXPECTED_RISK_BAND_COMPOSITION_SHA256[:12]}"
            ),
            ok=identity_ok,
            notes="ok" if identity_ok else "risk_band_composition_drift",
        )
    )

    sample_denials = [
        evaluate_risk_band_composition_mutation(
            proposed_p35_heavy=EXPECTED_PARTIAL_RISK_BANDS["p35_heavy"] + 1,
        ...
        evaluate_risk_band_composition_mutation(
            proposed_p35_heavy=EXPECTED_PARTIAL_RISK_BANDS["p35_heavy"],
            proposed_p3_mid=EXPECTED_PARTIAL_RISK_BANDS["p3_mid"] - 1,
            proposed_p2_mid=EXPECTED_PARTIAL_RISK_BANDS["p2_mid"],
            proposed_fu_light=EXPECTED_PARTIAL_RISK_BANDS["fu_light"],
            proposed_partial=EXPECTED_UNION_PARTIAL,
        ),
    ...
    checks["risk_band_composition_mutation_denied"] = mut_ok
    rows.append(
        _row(
            check_id="risk_band_composition_mutation_denied",
            layer="risk_band_composition_identity_lock",
    ...
    checks["risk_band_composition_flags_locked"] = flags_ok
    rows.append(
        _row(
            check_id="risk_band_composition_flags_locked",
            layer="risk_band_composition_identity_lock",
    ...
    fp_ok = fp == FROZEN_RISK_BAND_COMPOSITION_IDENTITY_LOCK_FP_SHA256
    checks["risk_band_composition_fingerprint"] = fp_ok
    rows.append(
        _row(
            check_id="risk_band_composition_fingerprint",
            layer="risk_band_composition_identity_lock",
            expected=FROZEN_RISK_BAND_COMPOSITION_IDENTITY_LOCK_FP_SHA256,
            observed=fp,
            ok=fp_ok,
    ...
    checks["risk_band_composition_identity_lock_all_pass"] = all_ok
    rows.append(
        _row(
            check_id="risk_band_composition_identity_lock_all_pass",
            layer="risk_band_composition_identity_lock",
            expected="risk_band_composition_identity_lock+fp",
...
def build_tier_coverage_composition_identity_lock_rows(
    ...
    *,
    ...
    """tier_coverage composition identity lock（tiers=7;coverage_sum=3314）。"""
    del paths, base_dir
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    ...
    identity_ok = (
        ...
        and doc["composition_sha256"] == EXPECTED_TIER_COVERAGE_COMPOSITION_SHA256
    )
    ...
    rows.append(
        _row(
            ...
            observed=(
                f"formula={EXPECTED_TIER_COVERAGE_FORMULA};"
                f"sha={EXPECTED_TIER_COVERAGE_COMPOSITION_SHA256[:12]}"
            ),
            ok=identity_ok,
    ...
    sample_denials = [
        evaluate_tier_coverage_composition_mutation(
            proposed_tiers=EXPECTED_SCALE_TIER_COUNT + 1,
    ...
    checks["tier_coverage_composition_flags_locked"] = flags_ok
    rows.append(
        _row(
            check_id="tier_coverage_composition_flags_locked",
            layer="tier_coverage_composition_identity_lock",
            expected="deny_mutate/inflate/deflate+KEEP_EXECUTE_FALSE",
            observed=f"hold={doc['hold']}",
            ok=flags_ok,
    ...
    fp_ok = fp == FROZEN_TIER_COVERAGE_COMPOSITION_IDENTITY_LOCK_FP_SHA256
    checks["tier_coverage_composition_fingerprint"] = fp_ok
    rows.append(
        _row(
            check_id="tier_coverage_composition_fingerprint",
            layer="tier_coverage_composition_identity_lock",
    ...
    rows.append(
        _row(
            ...
            layer="tier_coverage_composition_identity_lock",
            expected="tier_coverage_composition_identity_lock+fp",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
...
def build_combined_dryrun_composition_identity_lock_rows(
    ...
    *,
    ...
    identity_ok = (
        EXPECTED_COMBINED_DRYRUN_COVERAGE == 1053
        and EXPECTED_COMBINED_DRYRUN_FORMULA == "combined_dryrun=1053"
        and doc["composition_sha256"] == EXPECTED_COMBINED_DRYRUN_COMPOSITION_SHA256
    )
    ...
    rows.append(
        _row(
            ...
            observed=(
                f"formula={EXPECTED_COMBINED_DRYRUN_FORMULA};"
                f"sha={EXPECTED_COMBINED_DRYRUN_COMPOSITION_SHA256[:12]}"
            ),
            ok=identity_ok,
    ...
    sample_denials = [
        evaluate_combined_dryrun_composition_mutation(
            proposed_combined_dryrun=EXPECTED_COMBINED_DRYRUN_COVERAGE + 1
        ),
        evaluate_combined_dryrun_composition_mutation(
            proposed_combined_dryrun=EXPECTED_COMBINED_DRYRUN_COVERAGE - 1
        ),
        evaluate_combined_dryrun_composition_mutation(
            proposed_combined_dryrun=0
        ),
        evaluate_combined_dryrun_composition_mutation(
            proposed_combined_dryrun=EXPECTED_COMBINED_DRYRUN_COVERAGE
        ),
    ]
    mut_ok = all(d["mutation_allowed"] is False for d in sample_denials)
    ...
    checks["combined_dryrun_composition_flags_locked"] = flags_ok
    rows.append(
        _row(
            check_id="combined_dryrun_composition_flags_locked",
            layer="combined_dryrun_composition_identity_lock",
            expected="deny_mutate/inflate/deflate+KEEP_EXECUTE_FALSE",
            observed=f"hold={doc['hold']}",
            ok=flags_ok,
    ...
    fp_ok = fp == FROZEN_COMBINED_DRYRUN_COMPOSITION_IDENTITY_LOCK_FP_SHA256
    checks["combined_dryrun_composition_fingerprint"] = fp_ok
    rows.append(
        _row(
            check_id="combined_dryrun_composition_fingerprint",
            layer="combined_dryrun_composition_identity_lock",
    ...
    rows.append(
        _row(
            ...
            layer="combined_dryrun_composition_identity_lock",
            expected="combined_dryrun_composition_identity_lock+fp",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
...
def build_cross_formula_composition_meta_bundle_identity_lock_rows(
    paths: RiskBandTierDryrunCrossMetaBundlePaths,
    *,
    base_dir: str = BASE_DIR,
) -> Tuple[List[Dict[str, str]], Dict[str, bool], Dict[str, Any]]:
    """cross_formula_composition_meta_bundle identity lock（五组成元捆绑）。"""
    del paths, base_dir
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    fp, doc = fingerprint_cross_formula_composition_meta_bundle_identity_lock()
    ...
    identity_ok = (
        bundle_sha == EXPECTED_CROSS_FORMULA_COMPOSITION_META_BUNDLE_SHA256
        and len(EXPECTED_CROSS_FORMULA_COMPOSITION_META_BUNDLE) == 5
        and EXPECTED_CROSS_FORMULA_COMPOSITION_META_BUNDLE[
            "risk_band_composition"
        ]["formula"]
        ...
        == EXPECTED_TIER_COVERAGE_FORMULA
        and EXPECTED_CROSS_FORMULA_COMPOSITION_META_BUNDLE[
            "combined_dryrun_composition"
        ]["formula"]
    ...
    checks["cross_formula_composition_meta_bundle_identity"] = identity_ok
    rows.append(
        _row(
            check_id="cross_formula_composition_meta_bundle_identity",
            layer="cross_formula_composition_meta_bundle_identity_lock",
            expected="5_compositions_meta_bundle",
            observed=(
                f"n={len(EXPECTED_CROSS_FORMULA_COMPOSITION_META_BUNDLE)};"
                f"sha={bundle_sha[:12]}"
            ),
            ok=identity_ok,
            notes="ok" if identity_ok else "cross_meta_bundle_drift",
        )
    )
    ...
    deflated = {
        k: dict(v)
        for k, v in EXPECTED_CROSS_FORMULA_COMPOSITION_META_BUNDLE.items()
        if k != "combined_dryrun_composition"
    }
    mutated = {
        k: dict(v)
        for k, v in EXPECTED_CROSS_FORMULA_COMPOSITION_META_BUNDLE.items()
    }
    mutated["risk_band_composition"] = {
        "formula": "75+14+12+4=105",
        "composition_sha256": EXPECTED_RISK_BAND_COMPOSITION_SHA256,
    }
    sample_denials = [
        evaluate_cross_formula_composition_meta_bundle_mutation(
    ...
    mut_ok = all(d["mutation_allowed"] is False for d in sample_denials)
    checks["cross_formula_composition_meta_bundle_mutation_denied"] = mut_ok
    rows.append(
        _row(
            check_id="cross_formula_composition_meta_bundle_mutation_denied",
            layer="cross_formula_composition_meta_bundle_identity_lock",
            expected="mutation_allowed=false",
            observed=f"samples={len(sample_denials)};all_denied={mut_ok}",
            ok=mut_ok,
            notes="ok" if mut_ok else "cross_meta_mutation_leak",
        )
    )
    ...
    checks["cross_formula_composition_meta_bundle_flags_locked"] = flags_ok
    rows.append(
        _row(
            check_id="cross_formula_composition_meta_bundle_flags_locked",
            layer="cross_formula_composition_meta_bundle_identity_lock",
            expected="deny_mutate/inflate/deflate+KEEP_EXECUTE_FALSE",
            observed=f"hold={doc['hold']}",
            ok=flags_ok,
            notes="ok" if flags_ok else "cross_meta_flags_drift",
        )
    )

    fp_ok = (
        fp == FROZEN_CROSS_FORMULA_COMPOSITION_META_BUNDLE_IDENTITY_LOCK_FP_SHA256
    )
    checks["cross_formula_composition_meta_bundle_fingerprint"] = fp_ok
    rows.append(
        _row(
            check_id="cross_formula_composition_meta_bundle_fingerprint",
            layer="cross_formula_composition_meta_bundle_identity_lock",
            expected=FROZEN_CROSS_FORMULA_COMPOSITION_META_BUNDLE_IDENTITY_LOCK_FP_SHA256,
            observed=fp,
            ok=fp_ok,
            notes="ok" if fp_ok else "cross_meta_fp_drift",
        )
    )

    all_ok = all(checks.values()) if checks else False
    checks["cross_formula_composition_meta_bundle_identity_lock_all_pass"] = all_ok
    rows.append(
        _row(
            check_id="cross_formula_composition_meta_bundle_identity_lock_all_pass",
            layer="cross_formula_composition_meta_bundle_identity_lock",
            expected="cross_formula_composition_meta_bundle_identity_lock+fp",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
            notes="ok" if all_ok else "cross_meta_incomplete",
        )
    )
    meta = {
        "fingerprint": fp,
...
def build_fm41_continuity_rows(
    paths: RiskBandTierDryrunCrossMetaBundlePaths, *, base_dir: str = BASE_DIR
) -> Tuple[List[Dict[str, str]], Dict[str, bool]]:
    """FM41 packet / fingerprint / gate / 四 ledger 零漂移。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}

    packet = load_json(_abs(paths.fm41_packet_rel, base_dir=base_dir))
    fp_doc = load_json(_abs(paths.fm41_fingerprint_rel, base_dir=base_dir))
    gate_doc = load_json(_abs(paths.fm41_gate_json_rel, base_dir=base_dir))
    add_led = load_json(_abs(paths.fm41_additive_rel, base_dir=base_dir))
    res_led = load_json(_abs(paths.fm41_residual_rel, base_dir=base_dir))
    resume_led = load_json(_abs(paths.fm41_resume_rel, base_dir=base_dir))
    cross_led = load_json(_abs(paths.fm41_cross_composition_rel, base_dir=base_dir))
    ...
    pkt_ok = (
        ...
        and packet.get("additive_composition_sha256")
        == EXPECTED_ADDITIVE_COMPOSITION_SHA256
        and packet.get("residual_composition_sha256")
        == EXPECTED_RESIDUAL_COMPOSITION_SHA256
        and packet.get("resume_composition_sha256")
        == EXPECTED_RESUME_COMPOSITION_SHA256
        and packet.get("cross_composition_bundle_sha256")
        == EXPECTED_CROSS_COMPOSITION_BUNDLE_SHA256
    ...
    checks["fm41_packet_zero_drift"] = pkt_ok
    rows.append(
        _row(
            check_id="fm41_packet_zero_drift",
            layer="fm41_continuity",
            path=paths.fm41_packet_rel,
            expected="PASS_OFFLINE+stable_metrics+compositions",
            observed=f"gate={packet.get('gate')};unique={packet.get('harvest_unique_union')}",
            ok=pkt_ok,
            notes="ok" if pkt_ok else "fm41_packet_drift",
        )
    )
    ...
    fp_ok = (
        ...
        and fp_doc.get("additive_composition_sha256")
        == EXPECTED_ADDITIVE_COMPOSITION_SHA256
        and fp_doc.get("residual_composition_sha256")
        == EXPECTED_RESIDUAL_COMPOSITION_SHA256
        and fp_doc.get("resume_composition_sha256")
        == EXPECTED_RESUME_COMPOSITION_SHA256
        and fp_doc.get("cross_composition_bundle_sha256")
        == EXPECTED_CROSS_COMPOSITION_BUNDLE_SHA256
        and fp_doc.get("seal_chain_extended") is False
    )
    checks["fm41_fingerprint_zero_drift"] = fp_ok
    rows.append(
        _row(
            check_id="fm41_fingerprint_zero_drift",
            layer="fm41_continuity",
            path=paths.fm41_fingerprint_rel,
            expected="stable_metrics+fm41_fps",
            observed=f"unique={fp_doc.get('harvest_unique_union')}",
            ok=fp_ok,
            notes="ok" if fp_ok else "fm41_fingerprint_drift",
        )
    )
    ...
    gate_ok = (
        ...
        and gate_doc.get("cross_composition_bundle_sha256")
        == EXPECTED_CROSS_COMPOSITION_BUNDLE_SHA256
        and gate_doc.get("seal_chain_extended") is False
    )
    checks["fm41_gate_zero_drift"] = gate_ok
    rows.append(
        _row(
            check_id="fm41_gate_zero_drift",
            layer="fm41_continuity",
            path=paths.fm41_gate_json_rel,
            expected="PASS_OFFLINE+KEEP_EXECUTE_FALSE",
            observed=f"gate={gate_doc.get('gate')}",
            ok=gate_ok,
            notes="ok" if gate_ok else "fm41_gate_drift",
        )
    )

    add_ok = (
        add_led.get("fingerprint_sha256") == FM41_FROZEN_ADDITIVE_FP
        and add_led.get("composition_sha256") == EXPECTED_ADDITIVE_COMPOSITION_SHA256
    )
    checks["fm41_additive_ledger_zero_drift"] = add_ok
    rows.append(
        _row(
            check_id="fm41_additive_ledger_zero_drift",
            layer="fm41_continuity",
            path=paths.fm41_additive_rel,
            expected=FM41_FROZEN_ADDITIVE_FP,
            observed=str(add_led.get("fingerprint_sha256")),
            ok=add_ok,
            notes="ok" if add_ok else "fm41_additive_ledger_drift",
        )
    )

    res_ok = (
        res_led.get("fingerprint_sha256") == FM41_FROZEN_RESIDUAL_FP
        and res_led.get("composition_sha256") == EXPECTED_RESIDUAL_COMPOSITION_SHA256
    )
    checks["fm41_residual_ledger_zero_drift"] = res_ok
    rows.append(
        _row(
            check_id="fm41_residual_ledger_zero_drift",
            layer="fm41_continuity",
            path=paths.fm41_residual_rel,
            expected=FM41_FROZEN_RESIDUAL_FP,
            observed=str(res_led.get("fingerprint_sha256")),
            ok=res_ok,
            notes="ok" if res_ok else "fm41_residual_ledger_drift",
        )
    )

    resume_ok = (
        resume_led.get("fingerprint_sha256") == FM41_FROZEN_RESUME_FP
        and resume_led.get("composition_sha256") == EXPECTED_RESUME_COMPOSITION_SHA256
    )
    checks["fm41_resume_ledger_zero_drift"] = resume_ok
    rows.append(
        _row(
            check_id="fm41_resume_ledger_zero_drift",
            layer="fm41_continuity",
            path=paths.fm41_resume_rel,
            expected=FM41_FROZEN_RESUME_FP,
            observed=str(resume_led.get("fingerprint_sha256")),
            ok=resume_ok,
            notes="ok" if resume_ok else "fm41_resume_ledger_drift",
        )
    )

    cross_ok = (
        cross_led.get("fingerprint_sha256") == FM41_FROZEN_CROSS_COMPOSITION_FP
        and cross_led.get("bundle_sha256") == EXPECTED_CROSS_COMPOSITION_BUNDLE_SHA256
    )
    checks["fm41_cross_composition_ledger_zero_drift"] = cross_ok
    rows.append(
        _row(
            check_id="fm41_cross_composition_ledger_zero_drift",
            layer="fm41_continuity",
            path=paths.fm41_cross_composition_rel,
            expected=FM41_FROZEN_CROSS_COMPOSITION_FP,
            observed=str(cross_led.get("fingerprint_sha256")),
            ok=cross_ok,
            notes="ok" if cross_ok else "fm41_cross_composition_ledger_drift",
        )
    )

    all_ok = all(checks.values()) if checks else False
    checks["fm41_continuity_all_pass"] = all_ok
    rows.append(
        _row(
            check_id="fm41_continuity_all_pass",
            layer="fm41_continuity",
            expected="packet+fp+gate+4ledgers",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
            notes="ok" if all_ok else "fm41_continuity_incomplete",
        )
    )
    return rows, checks
...
def build_output_root_protection_rows(
    ...
    """output-root 保护：resume/harvest 写拒绝 + MOCK44 放行。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    ...
    out_detail = ""
    try:
        assert_fm42_output_root(paths.output_root_rel, base_dir=base_dir)
        out_ok = True
        out_detail = "allowed"
    except RuntimeError as exc:
    ...
    rows.append(
        _row(
            ...
            expected="MOCK44_or_ephemeral_allowed",
            observed=out_detail,
            ok=out_ok,
...
def build_frozen_mock_isolation_rows(
    ...
    """冻结 mock cohort 写隔离：MOCK3–43 拒绝 · MOCK44 放行。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    load_frozen_mock_cohort_roots.cache_clear()
    ...
    try:
        assert_fm42_output_root(mock42_rel, base_dir=base_dir)
    except RuntimeError as exc:
    ...
    rows.append(
        _row(
            ...
            layer="frozen_mock_isolation",
            expected="MOCK3-43_block+MOCK44_allow",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
...
def build_protected_csv_registry_rows(
    *,
    ...
    """protected_output_roots.csv：MOCK44 已登记。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    ...
    rows.append(
        _row(
            ...
            layer="protected_csv_registry",
            expected="MOCK3–43+MOCK44+resume+auth+registered",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
...
def build_fm_gate_battery_rows(
    *, gates: Dict[str, Dict[str, Any]]
    ...
    """FM-01..05 + FM-12..41 gate battery（跳过 seal FM06–11）。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    specs = [
    ...
    for check_id, key in specs:
        ...
        if key in ("fm31", "fm32", "fm33", "fm34", "fm35", "fm36", "fm37", "fm38", "fm39", "fm40", "fm41"):
            ok = (
                ok
    ...
    rows.append(
        _row(
            check_id="fm01_05_12_41_battery_all_pass",
            layer="fm_gate_battery",
    ...
    checks["fm01_05_12_41_battery_all_pass"] = all_ok
    return rows, checks
...
def ensure_protected_roots_csv_fm42(
    *,
    csv_rel: str = PROTECTED_ROOTS_CSV_REL,
    base_dir: str = BASE_DIR,
) -> None:
    """注册 C-ROOT-MOCK44；加固 C-ROOT-002 risk_band/tier/dryrun/cross-meta 说明。"""
    ...
    _upsert(
        RESUME_HARVEST_ROOT_ID,
        {
            ...
            "notes": (
                ...
                "C-FM-24..41 scale/safety freezes + C-FM-42 risk_band/tier/dryrun/"
                "cross-meta-bundle; 只读直至人批重跑"
            ),
        },
    )
    _upsert(
        THIS_TASK_ROOT_ID,
        {
            ...
            "notes": (
                "C-FM-42 scale risk_band composition identity lock (75+14+12+5=106) + "
                "tier_coverage composition identity lock (tiers=7;coverage_sum=3314) + "
                "combined_dryrun composition identity lock (combined_dryrun=1053) + "
                "cross_formula_composition_meta_bundle identity lock + FM41 continuity; "
                "never production EXECUTE; must not overwrite MOCK3-43; "
...
def run_scale_risk_band_tier_dryrun_cross_meta_bundle_safety(
    *,
    paths: RiskBandTierDryrunCrossMetaBundlePaths | None = None,
    base_dir: str = BASE_DIR,
) -> Dict[str, Any]:
    """执行 C-FM-42 规模 risk_band/tier/dryrun/cross-meta-bundle 离线 QA。"""
    paths = paths or RiskBandTierDryrunCrossMetaBundlePaths()
    generated_at = _utc_now_iso()
    ensure_protected_roots_csv_fm42(
        csv_rel=paths.protected_roots_csv_rel, base_dir=base_dir
    )
    out_root = assert_fm42_output_root(paths.output_root_rel, base_dir=base_dir)

    matrix: List[Dict[str, str]] = []
    cont_rows, cont_checks = build_fm41_continuity_rows(paths, base_dir=base_dir)
    matrix.extend(cont_rows)
    rb_rows, rb_checks, rb_meta = build_risk_band_composition_identity_lock_rows(
        paths, base_dir=base_dir
    )
    matrix.extend(rb_rows)
    tier_rows, tier_checks, tier_meta = (
        build_tier_coverage_composition_identity_lock_rows(paths, base_dir=base_dir)
    )
    matrix.extend(tier_rows)
    dry_rows, dry_checks, dry_meta = (
        build_combined_dryrun_composition_identity_lock_rows(paths, base_dir=base_dir)
    )
    matrix.extend(dry_rows)
    bundle_rows, bundle_checks, bundle_meta = (
        build_cross_formula_composition_meta_bundle_identity_lock_rows(
            paths, base_dir=base_dir
        )
    )
    ...
    layer_gates = {
        "fm41_continuity": (
            "PASS_OFFLINE"
            if cont_checks.get("fm41_continuity_all_pass")
            else "FAIL_OFFLINE"
        ),
        "risk_band_composition_identity_lock": (
            "PASS_OFFLINE"
            if rb_checks.get("risk_band_composition_identity_lock_all_pass")
            else "FAIL_OFFLINE"
        ),
        "tier_coverage_composition_identity_lock": (
            "PASS_OFFLINE"
            if tier_checks.get("tier_coverage_composition_identity_lock_all_pass")
            else "FAIL_OFFLINE"
        ),
        "combined_dryrun_composition_identity_lock": (
            "PASS_OFFLINE"
            if dry_checks.get("combined_dryrun_composition_identity_lock_all_pass")
            else "FAIL_OFFLINE"
        ),
        "cross_formula_composition_meta_bundle_identity_lock": (
            "PASS_OFFLINE"
            if bundle_checks.get(
                "cross_formula_composition_meta_bundle_identity_lock_all_pass"
            )
            else "FAIL_OFFLINE"
        ),
        ...
        "fm_gate_battery": (
            "PASS_OFFLINE"
            if bat_checks.get("fm01_05_12_41_battery_all_pass")
            else "FAIL_OFFLINE"
        ),
        "execute_hold_seal": (
            "PASS_OFFLINE"
    ...
    dry_rel = _write_ledger(
        "combined_dryrun_composition_identity_lock_ledger.json",
        {
            ...
            "composition_sha256": dry_meta["composition_sha256"],
            "combined_dryrun_formula": dry_meta["combined_dryrun_formula"],
            "sample_denials": dry_meta["sample_denials"],
            "doc": dry_meta["doc"],
        },
    )
    bundle_rel = _write_ledger(
        "cross_formula_composition_meta_bundle_identity_lock_ledger.json",
        {
            "generated_at": generated_at,
            "task_id": TASK_ID,
    ...
    observed_fps = {
        "risk_band_composition_identity_lock": rb_meta["fingerprint"],
        "tier_coverage_composition_identity_lock": tier_meta["fingerprint"],
        "combined_dryrun_composition_identity_lock": dry_meta["fingerprint"],
        "cross_formula_composition_meta_bundle_identity_lock": bundle_meta[
            "fingerprint"
        ],
        "fm41_additive_composition_identity_lock": FM41_FROZEN_ADDITIVE_FP,
        "fm41_residual_composition_identity_lock": FM41_FROZEN_RESIDUAL_FP,
        "fm41_resume_composition_identity_lock": FM41_FROZEN_RESUME_FP,
        "fm41_cross_composition_bundle_identity_lock": FM41_FROZEN_CROSS_COMPOSITION_FP,
    ...
    packet_rel = _write_ledger(
        "scale_packet.json",
        {
            ...
            "notes": (
                ...
                "combined_dryrun composition identity lock (combined_dryrun=1053) + "
                "cross_formula_composition_meta_bundle identity lock + FM41 continuity "
                "+ MOCK44; EXECUTE remains human-held; does not overwrite MOCK3-43"
            ),
        },
    )
    ...
    with open(readme_path, "w", encoding="utf-8") as fh:
        fh.write(
            "\n".join(
                [
                    "# C-FM-42 mock scale risk_band/tier/dryrun/cross-meta-bundle safety root",
                    "",
                    f"gate: `{overall}`",
                    "seal_chain_extended: false",
                    ...
                    f"cross_formula_composition_meta_bundle_sha256: {EXPECTED_CROSS_FORMULA_COMPOSITION_META_BUNDLE_SHA256}",
                    "does_not_overwrite: _mock_c_fm02.._mock_c_fm41 / standard dryrun",
                    "",
                ]
            )
        )
    ...
    return {
        ...
        "fingerprint": fp,
        "risk_band_path": rb_rel,
        "tier_coverage_path": tier_rel,
        "combined_dryrun_path": dry_rel,
        "cross_formula_composition_meta_bundle_path": bundle_rel,
        "battery_path": battery_rel,
        "packet_path": packet_rel,
        "observed_fps": observed_fps,
        "inputs": {
            "fm41_packet": paths.fm41_packet_rel,
            "fm41_fingerprint": paths.fm41_fingerprint_rel,
            "fm41_gate": paths.fm41_gate_json_rel,
