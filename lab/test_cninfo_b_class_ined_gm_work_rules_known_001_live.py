"""
B-FM-37：独立非执行董事工作制度 / 总经理工作细则 known-document live allow-list 锁测（离线；不请求 CNINFO）。

覆盖：
- allow-list 仅含 independent_ned_work_system_known_001 与
  general_manager_work_rules_known_001（category 空）
- 字段与 fixtures ready 对齐
- mock live 路径可对两案得到 pass（predicted_document_type=announcement；非 other）
- 不包含 articles / raised_funds_management_system / incentive_esop 等已 LIVE_PASS 案

无真实 CNINFO · 不造 §7 FP。

运行：
    python lab/test_cninfo_b_class_ined_gm_work_rules_known_001_live.py
"""
...
LIVE_DIR = os.path.join(
    ...
    "outputs",
    "validation",
    "cninfo_b_class_ined_gm_work_rules_known_001_live_20260715",
)
...
ANN_GM = "1223997454"
# 近似毫秒时间戳（mock）
ANN_TS_INED = 1750982400000  # 2025-06-27
ANN_TS_GM = 1750896000000  # 2025-06-26
...
class TestInedGmWorkRulesKnown001LiveAllowList(unittest.TestCase):
