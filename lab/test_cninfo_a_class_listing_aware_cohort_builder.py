...
class ListingAwareCohortBuilderTests(unittest.TestCase):
    ...
    def test_s14_case_id_start_excludes_prior_codes(self) -> None:
        """S14 从 1201 起编；已占用码不得入选；1250 号可格式化为 AD2E1250。"""
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            profile_dir = tmp_path / "profiles"
            profile_dir.mkdir()
            ...
            result = builder.build_listing_aware_cohort(
                target_size=2,
                case_id_start=builder.CASE_ID_START_S14,
                a_exclude_csvs=[str(exclude_csv)],
                profile_dir=str(profile_dir),
                full_market_yaml=str(yaml_path),
                max_same_prefix=25,
            )
            self.assertEqual(result.selected[0].case_id, "AD2E1201")
            self.assertEqual(result.selected[1].case_id, "AD2E1202")
            self.assertEqual(
                [r.company_code for r in result.selected],
                ["000004", "000005"],
            )
    ...
    def test_s16_case_id_start_excludes_prior_codes(self) -> None:
        """S16 从 1301 起编；已占用码不得入选；1350 号可格式化为 AD2E1350。"""
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            profile_dir = tmp_path / "profiles"
            profile_dir.mkdir()
            ...
            result = builder.build_listing_aware_cohort(
                target_size=2,
                case_id_start=builder.CASE_ID_START_S16,
                a_exclude_csvs=[str(exclude_csv)],
                profile_dir=str(profile_dir),
                full_market_yaml=str(yaml_path),
                max_same_prefix=25,
            )
            self.assertEqual(result.selected[0].case_id, "AD2E1301")
            self.assertEqual(result.selected[1].case_id, "AD2E1302")
            self.assertEqual(
                [r.company_code for r in result.selected],
                ["000004", "000005"],
            )
            self.assertEqual(f"AD2E{1350:03d}", "AD2E1350")

    def test_s17_case_id_start_excludes_prior_codes(self) -> None:
        """S17 从 1351 起编；已占用码不得入选；1400 号可格式化为 AD2E1400。"""
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            profile_dir = tmp_path / "profiles"
            profile_dir.mkdir()
            ...
            result = builder.build_listing_aware_cohort(
                target_size=2,
                case_id_start=builder.CASE_ID_START_S17,
                a_exclude_csvs=[str(exclude_csv)],
                profile_dir=str(profile_dir),
                full_market_yaml=str(yaml_path),
                max_same_prefix=25,
            )
            self.assertEqual(result.selected[0].case_id, "AD2E1351")
            self.assertEqual(result.selected[1].case_id, "AD2E1352")
            self.assertEqual(
                [r.company_code for r in result.selected],
                ["000004", "000005"],
            )
            self.assertEqual(f"AD2E{1400:03d}", "AD2E1400")

    def test_s18_case_id_start_excludes_prior_codes(self) -> None:
        """S18 从 1401 起编；已占用码不得入选；1450 号可格式化为 AD2E1450。"""
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            profile_dir = tmp_path / "profiles"
            profile_dir.mkdir()
            ...
            result = builder.build_listing_aware_cohort(
                target_size=2,
                case_id_start=builder.CASE_ID_START_S18,
                a_exclude_csvs=[str(exclude_csv)],
                profile_dir=str(profile_dir),
                full_market_yaml=str(yaml_path),
                max_same_prefix=25,
            )
            self.assertEqual(result.selected[0].case_id, "AD2E1401")
            self.assertEqual(result.selected[1].case_id, "AD2E1402")
            self.assertEqual(
                [r.company_code for r in result.selected],
                ["000004", "000005"],
            )
            self.assertEqual(f"AD2E{1450:03d}", "AD2E1450")

    def test_s19_case_id_start_excludes_prior_codes(self) -> None:
        """S19 从 1451 起编；已占用码不得入选；1500 号可格式化为 AD2E1500。"""
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            profile_dir = tmp_path / "profiles"
            profile_dir.mkdir()
            ...
            result = builder.build_listing_aware_cohort(
                target_size=2,
                case_id_start=builder.CASE_ID_START_S19,
                a_exclude_csvs=[str(exclude_csv)],
                profile_dir=str(profile_dir),
                full_market_yaml=str(yaml_path),
                max_same_prefix=25,
            )
            self.assertEqual(result.selected[0].case_id, "AD2E1451")
            self.assertEqual(result.selected[1].case_id, "AD2E1452")
            self.assertEqual(
                [r.company_code for r in result.selected],
                ["000004", "000005"],
            )
            self.assertEqual(f"AD2E{1500:03d}", "AD2E1500")


    def test_s20_case_id_start_excludes_prior_codes(self) -> None:
        """S20 从 1501 起编；已占用码不得入选；1550 号可格式化为 AD2E1550。"""
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            profile_dir = tmp_path / "profiles"
            profile_dir.mkdir()
            ...
            result = builder.build_listing_aware_cohort(
                target_size=2,
                case_id_start=builder.CASE_ID_START_S20,
                a_exclude_csvs=[str(exclude_csv)],
                profile_dir=str(profile_dir),
                full_market_yaml=str(yaml_path),
                max_same_prefix=25,
            )
            self.assertEqual(result.selected[0].case_id, "AD2E1501")
            self.assertEqual(result.selected[1].case_id, "AD2E1502")
            self.assertEqual(
                [r.company_code for r in result.selected],
                ["000004", "000005"],
            )
            self.assertEqual(f"AD2E{1550:03d}", "AD2E1550")

    def test_s21_case_id_start_excludes_prior_codes(self) -> None:
        """S21 从 1551 起编；已占用码不得入选；1600 号可格式化为 AD2E1600。"""
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            profile_dir = tmp_path / "profiles"
            profile_dir.mkdir()
            ...
            result = builder.build_listing_aware_cohort(
                target_size=2,
                case_id_start=builder.CASE_ID_START_S21,
                a_exclude_csvs=[str(exclude_csv)],
                profile_dir=str(profile_dir),
                full_market_yaml=str(yaml_path),
                max_same_prefix=25,
            )
            self.assertEqual(result.selected[0].case_id, "AD2E1551")
            self.assertEqual(result.selected[1].case_id, "AD2E1552")
            self.assertEqual(
                [r.company_code for r in result.selected],
                ["000004", "000005"],
            )
            self.assertEqual(f"AD2E{1600:03d}", "AD2E1600")

    def test_s22_case_id_start_excludes_prior_codes(self) -> None:
        """S22 从 1601 起编；已占用码不得入选；1650 号可格式化为 AD2E1650。"""
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            profile_dir = tmp_path / "profiles"
            profile_dir.mkdir()
            ...
            result = builder.build_listing_aware_cohort(
                target_size=2,
                case_id_start=builder.CASE_ID_START_S22,
                a_exclude_csvs=[str(exclude_csv)],
                profile_dir=str(profile_dir),
                full_market_yaml=str(yaml_path),
                max_same_prefix=25,
            )
            self.assertEqual(result.selected[0].case_id, "AD2E1601")
            self.assertEqual(result.selected[1].case_id, "AD2E1602")
            self.assertEqual(
                [r.company_code for r in result.selected],
                ["000004", "000005"],
            )
            self.assertEqual(f"AD2E{1650:03d}", "AD2E1650")

    def test_s23_case_id_start_excludes_prior_codes(self) -> None:
        """S23 从 1651 起编；已占用码不得入选；1850 号可格式化为 AD2E1850。"""
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            profile_dir = tmp_path / "profiles"
            profile_dir.mkdir()
            ...
            result = builder.build_listing_aware_cohort(
                target_size=2,
                case_id_start=builder.CASE_ID_START_S23,
                a_exclude_csvs=[str(exclude_csv)],
                profile_dir=str(profile_dir),
                full_market_yaml=str(yaml_path),
                max_same_prefix=100,
            )
            self.assertEqual(result.selected[0].case_id, "AD2E1651")
            self.assertEqual(result.selected[1].case_id, "AD2E1652")
            self.assertEqual(
                [r.company_code for r in result.selected],
                ["000004", "000005"],
            )
            self.assertEqual(f"AD2E{1850:03d}", "AD2E1850")

    def test_s24_case_id_start_excludes_prior_codes(self) -> None:
        """S24 从 1851 起编；已占用码不得入选；2221 号可格式化为 AD2E2221。"""
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            profile_dir = tmp_path / "profiles"
            profile_dir.mkdir()
            ...
            result = builder.build_listing_aware_cohort(
                target_size=2,
                case_id_start=builder.CASE_ID_START_S24,
                a_exclude_csvs=[str(exclude_csv)],
                profile_dir=str(profile_dir),
                full_market_yaml=str(yaml_path),
                max_same_prefix=371,
            )
            self.assertEqual(result.selected[0].case_id, "AD2E1851")
            self.assertEqual(result.selected[1].case_id, "AD2E1852")
            self.assertEqual(
                [r.company_code for r in result.selected],
                ["000004", "000005"],
            )
            self.assertEqual(f"AD2E{2221:03d}", "AD2E2221")
            self.assertEqual(builder.DEFAULT_TARGET_SIZE_S24, 371)
