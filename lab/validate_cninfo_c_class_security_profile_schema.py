"""
CNINFO C-class security_profile fixture schema validation (Era C Phase 4).

Validates fixtures/c_class/security_profile/security_profile_fixtures.jsonl against
schemas/c_class/c_company_security_profile.schema.json.

No network, no database, no verified.

Usage:
    python lab/validate_cninfo_c_class_security_profile_schema.py
"""
...
DEFAULT_SCHEMA = os.path.join(
    BASE_DIR, "schemas", "c_class", "c_company_security_profile.schema.json"
)
DEFAULT_CSV = os.path.join(
    BASE_DIR, "outputs", "validation", "cninfo_c_class_security_profile_schema_validation_report.csv"
)
DEFAULT_MD = os.path.join(
    BASE_DIR, "outputs", "validation", "cninfo_c_class_security_profile_schema_validation_summary.md"
)
...
def write_summary_md(
    ...
    lines = [
        ...
        f"| 来源 | 路径 |",
        f"|------|------|",
        f"| Fixtures | `{os.path.relpath(fixtures_path, BASE_DIR)}` |",
        f"| Schema | `{os.path.relpath(schema_path, BASE_DIR)}` |",
        f"| 脚本 | `lab/validate_cninfo_c_class_security_profile_schema.py` |",
        "",
        "## 3. 总体结果",
        "",
        f"| 指标 | 数值 |",
    ...
    if fail_rows:
        for r in fail_rows:
            lines.append(
                f"- idx={r['fixture_index']} `{r['company_code']}`: {r['error_message']}"
            )
    else:
    ...
    lines.extend([
        ...
        "## 5. Schema 说明",
        "",
        "- 本轮新增 `c_company_security_profile.schema.json`（draft-07），"
        "对齐 marketOverview 字段与 lineage required 闭包。",
        "- YAML `expected_fields` 中 listing_date / listed_board 等未在 mapper v1 填充，"
        "schema 中为可选 null。",
        "",
        "## 6. 质量边界",
        "",
        ...
        "## 附录",
        "",
        "详见 [cninfo_c_class_security_profile_schema_validation_report.csv]"
        "(cninfo_c_class_security_profile_schema_validation_report.csv)。",
        "",
    ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate C-class security_profile fixtures")
