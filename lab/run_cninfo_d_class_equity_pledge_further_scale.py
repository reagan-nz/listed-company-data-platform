...
# -*- coding: utf-8 -*-
"""
D 类 equity_pledge further-scale（~50）孤立根 runner。

denser 策略：tdate_daily 多日并集（2026-07-02 + 2026-07-01）共享探针 + 离线 SECCODE 过滤。
禁止 2026-07-03 作 sole found 锚；不写入 EP next-slice / first-slice / SC / ESH / AT 冻结根。

用法：
  .venv/bin/python lab/run_cninfo_d_class_equity_pledge_further_scale.py --build-universe-lock
  .venv/bin/python lab/run_cninfo_d_class_equity_pledge_further_scale.py --dry-run \\
      --universe-csv outputs/validation/cninfo_d_class_equity_pledge_further_scale_universe_lock_20260716.csv \\
      --output-root outputs/validation/cninfo_d_class_equity_pledge_further_scale
  .venv/bin/python lab/run_cninfo_d_class_equity_pledge_further_scale.py --live \\
      --approve-d-class-equity-pledge-further-scale \\
      --universe-csv outputs/validation/cninfo_d_class_equity_pledge_further_scale_universe_lock_20260716.csv \\
      --output-root outputs/validation/cninfo_d_class_equity_pledge_further_scale
"""
...
SHARED_PROBE_KEY = "tdate_union_2026-07-02_2026-07-01"
EXPECTED_SIZE = 50
...
DEFAULT_OUTPUT_ROOT = os.path.join(
    VALIDATION, "cninfo_d_class_equity_pledge_further_scale"
)
...
EQUITY_PLEDGE_CITE_JSON = os.path.join(
    VALIDATION,
    "cninfo_d_class_equity_pledge_further_scale_equity_pledge_cite_20260716.json",
)
...
def validate_universe(rows: Sequence[Dict[str, str]]) -> List[str]:
    ...
    if len(rows) != EXPECTED_SIZE:
        issues.append(
            f"equity_pledge_further_scale_universe_size_must_equal_{EXPECTED_SIZE}:"
            f"got={len(rows)}"
        )
    ...
    for row in rows:
        ...
        if case_id not in expected_ids:
            issues.append(
                f"forbidden_case_id_in_equity_pledge_further_scale:{case_id}"
            )
        ...
        if anchor not in allowed_anchors:
            issues.append(f"anchor_tdate_not_in_ep_fs_compose:{case_id}:{anchor}")
        if include != "yes":
            issues.append(f"further_scale_include_must_be_yes:{case_id}")
    if seen_cases != expected_ids:
        issues.append("case_id_set_must_be_DEP201_DEP250")
    return issues
...
def build_universe_lock_from_cite() -> int:
    """两次 CNINFO denser-day cite（tdate_daily）→ DEP201–250 lock。"""
    ...
    primary = _cite_day(
        session, component_cfg, stats, PRIMARY_TDATE, "ep_fs_cite_primary"
    )
    adjacent = _cite_day(
        session, component_cfg, stats, ADJACENT_TDATE, "ep_fs_cite_adjacent"
    )
        ...
        rows.append(
            {
                ...
                "probe_key": f"ep_fs_shared_tdate_{tdate}_{code}",
                "company_code": code,
                "company_name": name,
                "component": COMPONENT,
                ...
                "exclude_flags": (
                    ...
                    "exclude_sc_esh_at_inflate"
                ),
                "notes": (
                    f"further-scale found-path from {cite_tag} tdate_daily cite; "
                    "NOT verified"
                ),
                "evidence_cite": f"D-FM-12_{cite_tag}_{tdate.replace('-', '')}",
    ...
    for code, name in selected_empty:
        rows.append(
            {
                "case_id": f"DEP{case_idx}",
                "probe_key": f"ep_fs_empty_control_{code}",
                "company_code": code,
                "company_name": name,
                "component": COMPONENT,
                "market": _infer_market(code),
                ...
                "exclude_flags": (
                    "exclude_688671;exclude_301259;"
                    "exclude_sparse_day_20260703_sole_found_anchor;"
                    "empty_control_not_in_ep_fs_compose;"
                    "exclude_dlc006r;exclude_ess_h3_h4;"
                    "exclude_sc_esh_at_inflate"
                ),
                "notes": (
                    "empty control retained; absent from denser multi-day "
                    f"tdate_daily union {PRIMARY_TDATE}+{ADJACENT_TDATE}"
                ),
    ...
    print(
        f"mode=equity_pledge_further_scale_universe_lock "
        ...
        dry.append(
            {
                ...
                "notes": (
                    ...
                    f"exclude_ep_next_slice_DEP101_105=yes"
                ),
            }
        )
    return dry
...
def write_dryrun_summary(
    ...
    lines = [
        ...
        f"- shared probe: **tdate={PRIMARY_TDATE} + {ADJACENT_TDATE}**",
...
def execute_live(
    ...
    quality_path = os.path.join(
        output_paths["reports"],
        "d_class_equity_pledge_further_scale_quality_report.csv",
    )
    ...
    summary_path = os.path.join(
        output_paths["reports"],
        "d_class_equity_pledge_further_scale_live_summary.md",
    )
    lines = [
        ...
        "d_class_equity_pledge_further_scale_live_path_gate = READY_FOR_APPROVAL",
        "d_class_equity_pledge_further_scale_live_gate = NOT_APPROVED",
        f"d_class_equity_pledge_further_scale_execution_gate = {gate}",
        ...
        "## Caveats",
        "",
        f"- denser multi-day tdate_daily（{PRIMARY_TDATE}+{ADJACENT_TDATE}）截面密度 ≠ 全市场股权质押覆盖。",
    ...
    print(
        f"mode=equity_pledge_further_scale_live cases={len(live_rows)} "
    ...
    print(f"gate=d_class_equity_pledge_further_scale_execution_gate={gate}")
...
def run_dry_run(args: argparse.Namespace) -> int:
    ...
    print(
        f"mode=equity_pledge_further_scale_dry_run cases={len(dry_rows)} "
        f"planned_request_count_total=2 cninfo_calls=0"
    )
    print(
        "gate=d_class_equity_pledge_further_scale_runner_extension_gate="
        "READY_FOR_APPROVAL"
    )
...
def run_live(args: argparse.Namespace) -> int:
    if not args.approve_d_class_equity_pledge_further_scale:
        print(
            "ERROR: approve_d_class_equity_pledge_further_scale_required",
            file=sys.stderr,
        )
        return 2
    ok_root, root_err = validate_output_root(args.output_root)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="D-class equity_pledge further-scale (~50) runner"
    )
