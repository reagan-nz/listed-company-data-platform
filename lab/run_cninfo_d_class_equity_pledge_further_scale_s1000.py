...
# -*- coding: utf-8 -*-
"""
D 类 equity_pledge further-scale S1000（~1000）孤立根 runner。

denser 策略：tdate_daily 多交易日并集（自适应 found + empty pad 凑满 1000）共享探针 + 离线 SECCODE 过滤。
禁止 2026-07-03 作 sole found 锚；不写入 EP s50/s200 / next-slice / first-slice / SC / ESH / AT 冻结根。

用法：
  .venv/bin/python lab/run_cninfo_d_class_equity_pledge_further_scale_s1000.py --build-universe-lock
  .venv/bin/python lab/run_cninfo_d_class_equity_pledge_further_scale_s1000.py --dry-run \\
      --universe-csv outputs/validation/cninfo_d_class_equity_pledge_further_scale_s1000_universe_lock_20260716.csv \\
      --output-root outputs/validation/cninfo_d_class_equity_pledge_further_scale_s1000
  .venv/bin/python lab/run_cninfo_d_class_equity_pledge_further_scale_s1000.py --live \\
      --approve-d-class-equity-pledge-further-scale-s1000 \\
      --universe-csv outputs/validation/cninfo_d_class_equity_pledge_further_scale_s1000_universe_lock_20260716.csv \\
      --output-root outputs/validation/cninfo_d_class_equity_pledge_further_scale_s1000
"""
...
# S50/S200 交易日保留为候选；S1000 扩展更多 denser 交易日；found 自适应 + empty pad
...
EXPECTED_SIZE = 1000
# excellence：acceptable >= 95% → ≥950/1000；且 fail/http=0
PASS_THRESHOLD = 950
EXCELLENCE_ACCEPTABLE_RATE = 0.95
CASE_ID_START = 501
CASE_ID_END = 1500
EMPTY_CONTROL_MIN = 10  # 至少保留若干 empty-control；found 自适应
MAX_COMPOSE_DAYS = 20

DEFAULT_OUTPUT_ROOT = os.path.join(
    VALIDATION, "cninfo_d_class_equity_pledge_further_scale_s1000"
)
DEFAULT_UNIVERSE_CSV = os.path.join(
    VALIDATION,
    "cninfo_d_class_equity_pledge_further_scale_s1000_universe_lock_20260716.csv",
)
EQUITY_PLEDGE_CITE_JSON = os.path.join(
    VALIDATION,
    "cninfo_d_class_equity_pledge_further_scale_s1000_equity_pledge_cite_20260716.json",
)
...
# D-FM-12 S50 found 槽公司码：不得再入 S1000 found 槽（空控可复用）
...
def validate_output_root(output_root: str) -> Tuple[bool, str]:
    ...
    if root != expected:
        return False, (
            ...
            "cninfo_d_class_equity_pledge_further_scale_s1000"
        )
...
def validate_universe(rows: Sequence[Dict[str, str]]) -> List[str]:
    ...
    if len(rows) != EXPECTED_SIZE:
        issues.append(
            f"equity_pledge_further_scale_s1000_universe_size_must_equal_{EXPECTED_SIZE}:"
            f"got={len(rows)}"
        )
    seen_cases: Set[str] = set()
    seen_codes: Set[str] = set()
    expected_ids = {f"DEP{i}" for i in range(CASE_ID_START, CASE_ID_END + 1)}
    ...
    for row in rows:
        ...
        if case_id not in expected_ids:
            issues.append(
                f"forbidden_case_id_in_equity_pledge_further_scale_s1000:{case_id}"
            )
        ...
        if component != COMPONENT:
            issues.append(f"component_must_be_equity_pledge:{case_id}")
        ...
        if anchor not in allowed_anchors:
            issues.append(f"anchor_tdate_not_in_ep_fs_s1000_compose:{case_id}:{anchor}")
        if include != "yes":
        ...
        if expected_behavior == "captured_normal":
            ...
            if code in prior_found:
                issues.append(f"found_slot_reuses_prior_ep_found_code:{code}")
        elif expected_behavior == "empty_but_valid":
            empty_count += 1
    if seen_cases != expected_ids:
        issues.append("case_id_set_must_be_DEP501_DEP1500")
...
def _excluded_prior_found_codes() -> Set[str]:
    """S50 + S200 found 码并集（CSV 优先，常量兜底）。"""
    codes = set(EXCLUDED_S50_FOUND_CODES) | set(EXCLUDED_S200_FOUND_CODES)
    codes |= _load_found_codes_from_lock(S50_UNIVERSE_CSV)
    codes |= _load_found_codes_from_lock(S200_UNIVERSE_CSV)
    return codes
...
def build_universe_lock_from_cite() -> int:
    """多日 tdate_daily denser cite（自适应 found + empty pad）→ DEP501–1500 lock。"""
    ...
    for tdate in COMPOSE_CANDIDATE_TDATES:
        ...
        if len(selected_found) >= max_found:
            break
        cite = _cite_day(
            session, component_cfg, stats, tdate, f"ep_fs_s1000_cite_{tdate}"
        )
    ...
    for code, rec, tdate in selected_found:
        ...
        rows.append(
            {
                "case_id": f"DEP{case_idx}",
                "probe_key": f"ep_fs_s1000_shared_desc_{tdate}_{code}",
                "company_code": code,
                "company_name": name,
                "component": COMPONENT,
                ...
                "exclude_flags": (
                    ...
                    "exclude_ep_next_slice_DEP101_105;"
                    "exclude_ep_first_slice_DEP001_005;"
                    "exclude_ep_fs_s50_DEP201_250_found;"
                    "exclude_ep_fs_s200_DEP301_500_found;"
                    "exclude_dlc006r;exclude_ess_h3_h4"
                ),
                "notes": (
                    f"further-scale s1000 found-path from tdate_daily cite day {tdate}; "
                    "NOT verified"
                ),
                "evidence_cite": f"D-FM-13_compose_{tdate.replace('-', '')}",
                "universe_lock_status": "locked",
                "lock_date": "2026-07-16",
                "approval_task_id": "D-FM-13",
    ...
    for code, name in selected_empty:
        rows.append(
            {
                "case_id": f"DEP{case_idx}",
                "probe_key": f"ep_fs_s1000_empty_control_{code}",
                "company_code": code,
                "company_name": name,
                "component": COMPONENT,
                "market": _infer_market(code),
                ...
                "exclude_flags": (
                    "exclude_688671;exclude_301259;"
                    "exclude_sparse_day_20260703_sole_found_anchor;"
                    "empty_control_not_in_ep_fs_s1000_compose;"
                    "exclude_dlc006r;exclude_ess_h3_h4"
                ),
                "notes": (
                ...
                "evidence_cite": "D-FM-13_empty_control_absent_from_compose",
                "universe_lock_status": "locked",
                "lock_date": "2026-07-16",
                "approval_task_id": "D-FM-13",
                "per_case_request_budget": "1",
    ...
    ordered = [rows_by_id[f"DEP{i}"] for i in range(CASE_ID_START, CASE_ID_END + 1)]
    ...
    cite_payload = {
        ...
        "excluded_s200_found_codes": sorted(EXCLUDED_S200_FOUND_CODES),
        "found_vs_empty_pad": {
            ...
            "note": (
                "empty pad fills remainder when multi-day union cannot supply "
                "990+ distinct new found codes"
            ),
        },
    }
    with open(EQUITY_PLEDGE_CITE_JSON, "w", encoding="utf-8") as f:
    ...
    print(
        f"mode=equity_pledge_further_scale_s1000_universe_lock "
    ...
    print(f"equity_pledge_cite={EQUITY_PLEDGE_CITE_JSON}")
...
def write_dryrun_report(
    ...
    path = os.path.join(
        ...
        "d_class_equity_pledge_further_scale_s1000_dryrun_report.csv",
    )
...
def write_dryrun_summary(
    ...
    path = os.path.join(
        output_paths["reports"],
        "d_class_equity_pledge_further_scale_s1000_dryrun_summary.md",
    )
...
def assess_case(
    ...
    if last_error in ("rate_limited",) or last_error.startswith("network_error"):
        ...
        empty_but_valid = "yes"
        notes.append(
            "equityPledge/list zero rows after SECCODE filter; "
...
def execute_live(
    ...
    quality_path = os.path.join(
        output_paths["reports"],
        "d_class_equity_pledge_further_scale_s1000_quality_report.csv",
    )
    ...
    summary_path = os.path.join(
        output_paths["reports"],
        "d_class_equity_pledge_further_scale_s1000_live_summary.md",
    )
    lines = [
        ...
        "d_class_equity_pledge_further_scale_s1000_live_path_gate = READY_FOR_APPROVAL",
        "d_class_equity_pledge_further_scale_s1000_live_gate = NOT_APPROVED",
        f"d_class_equity_pledge_further_scale_s1000_execution_gate = {gate}",
        ...
        "## Caveats",
        "",
        f"- denser multi-day tdate_daily（{'+'.join(compose_days)}）截面密度 ≠ 全市场增减持覆盖。",
    ...
    print(
        f"mode=equity_pledge_further_scale_s1000_live cases={len(live_rows)} "
    ...
    print(
        f"gate=d_class_equity_pledge_further_scale_s1000_execution_gate={gate}"
    )
...
def run_dry_run(args: argparse.Namespace) -> int:
    ...
    print(
        f"mode=equity_pledge_further_scale_s1000_dry_run cases={len(dry_rows)} "
        f"planned_request_count_total={len(_compose_days_from_rows(rows))} cninfo_calls=0"
    )
    print(
        "gate=d_class_equity_pledge_further_scale_s1000_runner_extension_gate="
        "READY_FOR_APPROVAL"
    )
...
def run_live(args: argparse.Namespace) -> int:
    if not args.approve_d_class_equity_pledge_further_scale_s1000:
        print(
            "ERROR: approve_d_class_equity_pledge_further_scale_s1000_required",
            file=sys.stderr,
        )
        return 2
    ok_root, root_err = validate_output_root(args.output_root)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="D-class equity_pledge further-scale S1000 (~1000) runner"
    )
