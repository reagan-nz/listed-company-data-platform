"""
CNINFO C-class — 非 seal Cross-FM mock cohort 二次完整性扩展（离线 · C-FM-18）。

在 C-FM-17（non-seal human decision readiness ledger）已 commit 且 EXECUTE 仍
human-held 之上，补齐非 seal 能力（不新增 seal 层 / 不新增 decision-await 文档层）：
  1) 扩大非 seal mock cohort 注册表：FM-01..05 + FM-12 + FM-13..17（MOCK15–19）
  2) 指纹链只读核验：extension/drift/boundary/attestation/readiness 矩阵零漂移重算
  3) 冻结 mock 写隔离：MOCK3–19 拒绝；本任务 MOCK20 / ephemeral 放行
  ...
  7) protected_output_roots.csv 注册一致性（含 MOCK20）

禁止：CNINFO live · production EXECUTE · 覆盖 MOCK3–19 / 权威 dual-layer 索引 ·
...
from cninfo_c_class_nonseal_extension_human_decision_readiness_ledger import (  # noqa: E402
    DEFAULT_MOCK_OUTPUT_ROOT_REL as FM17_MOCK_ROOT_REL,
...
DEFAULT_MOCK_OUTPUT_ROOT_REL = (
    "outputs/validation/_mock_c_fm18_nonseal_cross_fm_mock_cohort_second_extension"
)
...
# C-FM-17 冻结 readiness 指纹（二次扩展零漂移锚点）
FROZEN_READINESS_FP_SHA256 = (
    "a5e9f38528e691e91622f16b07280f0652438207084c64b4595857caf90964ea"
)

THIS_TASK_ROOT_ID = "C-ROOT-MOCK20"
...
def assert_fm18_output_root(
    output_root: str, *, base_dir: str = BASE_DIR
) -> str:
    """
    C-FM-18 写根：须 validation/_mock_*，不得覆盖 MOCK3–19，
    不得写权威 dual-layer 索引；允许本任务 MOCK20 或未登记 ephemeral。
    """
    ...
    if root_id is not None and root_id != THIS_TASK_ROOT_ID:
        raise RuntimeError(
            ...
            f"(C-FM-18 only writes {THIS_TASK_ROOT_ID} or ephemeral _mock_*)"
        )
...
def default_nonseal_second_extension_cohort_specs() -> Tuple[MockCohortSpec, ...]:
    """二次扩展：FM-01..05+FM-12 之上追加 FM-13..17。"""
    base = list(default_nonseal_cohort_specs())
    base.extend(
        [
            MockCohortSpec(
                cohort_id="fm13_nonseal_cross_fm_mock_cohort_extension",
                root_rel=FM13_MOCK_ROOT_REL,
                ...
                gate_json_rel=FM13_GATE_JSON_REL,
            ),
            MockCohortSpec(
                cohort_id="fm14_nonseal_extension_post_commit_drift_recheck",
                root_rel=FM14_MOCK_ROOT_REL,
                ...
                gate_json_rel=FM14_GATE_JSON_REL,
            ),
            MockCohortSpec(
                cohort_id="fm15_nonseal_extension_controller_commit_boundary",
                root_rel=FM15_MOCK_ROOT_REL,
                root_id="C-ROOT-MOCK17",
                required_files=(
                    "boundary_matrix.csv",
                    "boundary_fingerprint.json",
                    "controller_commit_boundary_packet.json",
                    "fm_gate_battery.json",
                    "README.md",
                ),
                fingerprint_kind="boundary_matrix",
                gate_json_rel=FM15_GATE_JSON_REL,
            ),
            MockCohortSpec(
                cohort_id="fm16_nonseal_extension_post_commit_seal_attestation",
                root_rel=FM16_MOCK_ROOT_REL,
                root_id="C-ROOT-MOCK18",
                required_files=(
                    "attestation_matrix.csv",
                    "attestation_fingerprint.json",
                    "post_commit_seal_packet.json",
                    "human_execute_decision_handoff_packet.json",
                    "fm_gate_battery.json",
                    "README.md",
                ),
                fingerprint_kind="attestation_matrix",
                gate_json_rel=FM16_GATE_JSON_REL,
            ),
            MockCohortSpec(
                cohort_id="fm17_nonseal_extension_human_decision_readiness_ledger",
                root_rel=FM17_MOCK_ROOT_REL,
                root_id="C-ROOT-MOCK19",
                required_files=(
                    "readiness_matrix.csv",
                    "readiness_fingerprint.json",
                    "human_decision_readiness_packet.json",
                    "human_execute_decision_checklist.json",
                    "fm_gate_battery.json",
                    "README.md",
                ),
                fingerprint_kind="readiness_matrix",
                gate_json_rel=FM17_GATE_JSON_REL,
            ),
        ]
    )
    return tuple(base)
...
def build_nonseal_chain_anchor_rows(
    *, base_dir: str = BASE_DIR
    ...
    """nonseal-chain 连续性锚点：冻结常量与 MOCK15–19 指纹文件对齐。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    ...
    anchors = [
        (
            "anchor_extension_fp",
            f"{FM13_MOCK_ROOT_REL}/extension_fingerprint.json",
            FROZEN_EXTENSION_FP_SHA256,
            "C-ROOT-MOCK15",
        ),
        (
            "anchor_drift_fp",
            f"{FM14_MOCK_ROOT_REL}/drift_fingerprint.json",
            FROZEN_DRIFT_FP_SHA256,
            "C-ROOT-MOCK16",
        ),
        (
            "anchor_boundary_fp",
            f"{FM15_MOCK_ROOT_REL}/boundary_fingerprint.json",
            FROZEN_BOUNDARY_FP_SHA256,
            "C-ROOT-MOCK17",
        ),
        (
            "anchor_attestation_fp",
            f"{FM16_MOCK_ROOT_REL}/attestation_fingerprint.json",
            FROZEN_ATTESTATION_FP_SHA256,
            "C-ROOT-MOCK18",
        ),
        (
            "anchor_readiness_fp",
            f"{FM17_MOCK_ROOT_REL}/readiness_fingerprint.json",
            FROZEN_READINESS_FP_SHA256,
            "C-ROOT-MOCK19",
        ),
    ]
    ...
    rows.append(
        _row(
            ...
            layer="nonseal_chain_continuity",
            expected="MOCK15-19_frozen_anchors_match",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
...
def build_frozen_mock_isolation_rows(
    paths: SecondExtensionPaths, *, base_dir: str = BASE_DIR
) -> Tuple[List[Dict[str, str]], Dict[str, bool]]:
    """冻结 mock cohort 写隔离：MOCK3–19 拒绝 · MOCK20 放行。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    load_frozen_mock_cohort_roots.cache_clear()
    ...
    mock20_prefix = frozen.get(THIS_TASK_ROOT_ID)
    mock20_listed = mock20_prefix is not None
    mock20_allowed = False
    if mock20_prefix:
        try:
            assert_frozen_mock_cohort_write_forbidden(
                mock20_prefix,
            ...
            mock20_allowed = False
    checks["frozen_allow_mock20"] = mock20_listed and mock20_allowed
    rows.append(
        _row(
            check_id="frozen_allow_mock20",
            layer="frozen_mock_isolation",
            root_id=THIS_TASK_ROOT_ID,
            path=_rel(mock20_prefix, base_dir=base_dir) if mock20_prefix else "",
            expected="listed_and_allowed_when_in_allowlist",
            observed=f"listed={mock20_listed};allowed={mock20_allowed}",
            ok=mock20_listed and mock20_allowed,
            notes="ok" if mock20_listed and mock20_allowed else "mock20_allow_fail",
        )
    )
    ...
    out_detail = ""
    try:
        assert_fm18_output_root(paths.output_root_rel, base_dir=base_dir)
        out_ok = True
        out_detail = "allowed"
    except RuntimeError as exc:
    ...
    rows.append(
        _row(
            ...
            expected="MOCK20_or_ephemeral_allowed",
            observed=out_detail,
            ok=out_ok,
        )
    )
    ...
    rows.append(
        _row(
            ...
            expected="seal_chain_not_writable_by_fm18",
    ...
    # 前序 nonseal 根 MOCK19 必须拒绝
    mock19 = frozen.get("C-ROOT-MOCK19")
    mock19_frozen = False
    if mock19:
        try:
            assert_frozen_mock_cohort_write_forbidden(
                mock19, allow_root_ids=(THIS_TASK_ROOT_ID,), base_dir=base_dir
            )
        except RuntimeError as exc:
            mock19_frozen = FROZEN_MOCK_COHORT_WRITE_FORBIDDEN in str(exc)
    checks["mock19_still_frozen"] = mock19_frozen
    rows.append(
        _row(
            check_id="mock19_still_frozen",
            layer="frozen_mock_isolation",
            root_id="C-ROOT-MOCK19",
            expected="prior_nonseal_not_writable_by_fm18",
            observed=f"refused={mock19_frozen}",
            ok=mock19_frozen,
            notes="ok" if mock19_frozen else "mock19_freeze_regressed",
        )
    )
    ...
    rows.append(
        _row(
            ...
            layer="frozen_mock_isolation",
            expected="MOCK3-19_blocked_MOCK20_ok",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
...
def build_protected_csv_registry_rows(
    *,
    ...
    """protected CSV：MOCK3–20 + AUTH1 注册一致性。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    ...
    mock20 = by_id.get(THIS_TASK_ROOT_ID) or {}
    mock20_path = (mock20.get("path_pattern") or "").strip().rstrip("/")
    expected_path = DEFAULT_MOCK_OUTPUT_ROOT_REL
    mock20_ok = mock20_path.endswith(
        "_mock_c_fm18_nonseal_cross_fm_mock_cohort_second_extension"
    ) or mock20_path == expected_path
    checks["protected_csv_mock20_path"] = mock20_ok
    rows.append(
        _row(
            check_id="protected_csv_mock20_path",
            layer="protected_csv_registry",
            root_id=THIS_TASK_ROOT_ID,
            path=mock20_path,
            expected=expected_path,
            observed=mock20_path or "missing",
            ok=mock20_ok,
            notes="ok" if mock20_ok else "mock20_path_mismatch",
        )
    )
    ...
    rows.append(
        _row(
            ...
            layer="protected_csv_registry",
            expected="MOCK3-20+AUTH1_registered",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
...
def build_fm_gate_battery_rows(
    *,
    ...
    """FM-01..05 + FM-12..17 既有 gate 只读聚合（跳过 seal FM06–11）。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    ...
    for check_id, key in specs:
        ...
        if key in ("fm12", "fm13", "fm14", "fm15", "fm16", "fm17"):
    ...
    rows.append(
        _row(
            check_id="fm01_05_12_17_battery_all_pass",
            layer="fm_gate_battery",
    ...
    checks["fm01_05_12_17_battery_all_pass"] = all_ok
    return rows, checks
...
def fingerprint_second_extension_matrix(
...
def run_nonseal_cross_fm_mock_cohort_second_extension(
    *,
    paths: SecondExtensionPaths = SecondExtensionPaths(),
    cohort_specs: Optional[Sequence[MockCohortSpec]] = None,
    base_dir: str = BASE_DIR,
) -> Dict[str, Any]:
    """执行 C-FM-18 非 seal Cross-FM mock cohort 二次扩展 QA（CNINFO=0）。"""
    generated_at = _utc_now_iso()
    out_root = assert_fm18_output_root(paths.output_root_rel, base_dir=base_dir)
    specs = (
        tuple(cohort_specs)
        if cohort_specs is not None
        else default_nonseal_second_extension_cohort_specs()
    )
    ...
    fp_rows, fp_checks = build_second_extension_fingerprint_chain_rows(
        ...
        json.dump(
            {
                ...
                "frozen_readiness_fp_sha256": FROZEN_READINESS_FP_SHA256,
                "fingerprint": fp,
            },
            fh,
            ensure_ascii=False,
        ...
        json.dump(
            {
                ...
                "gate": overall,
                "capability": [
                    "nonseal_cross_fm_mock_cohort_second_extension",
                    "fingerprint_chain_fm01_05_12_17",
                    "nonseal_chain_continuity_mock15_19",
                    "frozen_mock_cohort_write_isolation_mock3_19",
                    "harvest_exclusion_dual_layer_consistency",
                    "protected_output_root_mock20",
                ],
                "cninfo_calls": 0,
                "execute_production_snapshot_rebuild": False,
                ...
                "seal_chain_extended": False,
                "notes": (
                    "本包在 C-FM-17 commit 后做 non-seal 二次 cohort 完整性扩展："
                    "FM01-05+FM12-17 注册表 · MOCK15-19 锚点零漂移 · MOCK3-19 写隔离 · "
                    "MOCK20 保护根；不新增 seal/decision-await 层；"
                    "不覆盖 MOCK3-19；不翻转 approved_for_snapshot_rebuild；"
