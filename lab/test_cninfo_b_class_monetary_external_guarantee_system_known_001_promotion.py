"""
B-FM-38：货币资金管理制度 / 对外担保管理制度 known-document 晋升锁测（离线）。

覆盖：
- monetary_funds_management_system_known_001 与
  external_guarantee_management_system_known_001 已为 ready
- title_pattern 与裸「管理制度」/对外担保情况简报可区分
- harvest 标题经硬化路由预测 announcement → general（非 other）
- 既有 LIVE_PASS 路径不回退（含 B-FM-37 ined / gm_work_rules）
- 不重开 ined / gm / articles / raised_funds_management_system 等 LIVE_PASS

无 CNINFO · 无 live · 不造 §7 FP。

运行：
    python lab/test_cninfo_b_class_monetary_external_guarantee_system_known_001_promotion.py
"""
...
PATTERN_MONETARY = "货币资金管理制度"
PATTERN_GUARANTEE = "对外担保管理制度"
PATTERN_BARE = "管理制度"
...
class TestMonetaryExternalGuaranteeSystemKnown001Promotion(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.config = routing._load_yaml(routing.DEFAULT_CATEGORIES)
        cls.known = _load_cases(KNOWN_YAML)

    def test_monetary_funds_system_known_001_ready_fields(self) -> None:
        c = _by_id(self.known, "monetary_funds_management_system_known_001")
        self.assertEqual(c["case_status"], "ready")
        self.assertEqual(c["company_code"], "300513")
        self.assertEqual(c["company_name"], "恒实科技")
        self.assertEqual(c["title_pattern"], PATTERN_MONETARY)
        self.assertEqual(c["date_start"], "2025-05-28")
        self.assertEqual(c["date_end"], "2025-05-31")
        ...
        self.assertIn(PATTERN_MONETARY, HARVEST_MONETARY)
        self.assertNotEqual(c["title_pattern"], "管理制度")
        self.assertNotEqual(c["title_pattern"], "制度")

    def test_external_guarantee_system_known_001_ready_fields(self) -> None:
        c = _by_id(self.known, "external_guarantee_management_system_known_001")
        self.assertEqual(c["case_status"], "ready")
        self.assertEqual(c["company_code"], "603138")
        self.assertEqual(c["company_name"], "海量数据")
        self.assertEqual(c["title_pattern"], PATTERN_GUARANTEE)
        self.assertEqual(c["date_start"], "2025-06-25")
        self.assertEqual(c["date_end"], "2025-06-28")
        self.assertEqual(c["expected_document_type"], "announcement")
        self.assertEqual(c["expected_route_to"], "cninfo_general_announcement_pdf")
        self.assertIn(PATTERN_GUARANTEE, HARVEST_GUARANTEE)
        self.assertNotEqual(c["title_pattern"], "管理制度")
        self.assertFalse(retrieval._title_matches(GUARANTEE_BRIEF, PATTERN_GUARANTEE))

    def test_patterns_mutually_distinct(self) -> None:
        """两案 pattern 与裸管理制度/对外担保简报互斥。"""
        self.assertNotEqual(PATTERN_MONETARY, PATTERN_GUARANTEE)
        self.assertNotEqual(PATTERN_MONETARY, PATTERN_BARE)
        self.assertNotEqual(PATTERN_GUARANTEE, PATTERN_BARE)
        self.assertFalse(retrieval._title_matches(HARVEST_MONETARY, PATTERN_GUARANTEE))
        self.assertFalse(retrieval._title_matches(HARVEST_GUARANTEE, PATTERN_MONETARY))
        self.assertTrue(retrieval._title_matches(HARVEST_MONETARY, PATTERN_MONETARY))
        self.assertTrue(retrieval._title_matches(HARVEST_GUARANTEE, PATTERN_GUARANTEE))

    def test_harvest_titles_route_announcement(self) -> None:
        r_m = routing.route_title(HARVEST_MONETARY, self.config)
        self.assertEqual(r_m.predicted_document_type, "announcement")
        self.assertNotEqual(r_m.predicted_document_type, "other")
        self.assertEqual(r_m.predicted_route_to, "cninfo_general_announcement_pdf")
        r_g = routing.route_title(HARVEST_GUARANTEE, self.config)
        self.assertEqual(r_g.predicted_document_type, "announcement")
        self.assertNotEqual(r_g.predicted_document_type, "other")
        r_brief = routing.route_title(GUARANTEE_BRIEF, self.config)
        self.assertEqual(r_brief.predicted_document_type, "other")

    def test_prior_paths_not_regressed(self) -> None:
        r_ined = routing.route_title(
            "株洲中车时代电气股份有限公司独立非执行董事工作制度", self.config
        )
        self.assertEqual(r_ined.predicted_document_type, "announcement")
        r_gm = routing.route_title(
            "中盐内蒙古化工股份有限公司总经理工作细则（2025年6月修订）", self.config
        )
    ...
    def test_closed_live_pass_cases_still_ready(self) -> None:
        """已 LIVE_PASS 案仍为 ready（不降级）；含 B-FM-37 两案。"""
        for case_id in (
