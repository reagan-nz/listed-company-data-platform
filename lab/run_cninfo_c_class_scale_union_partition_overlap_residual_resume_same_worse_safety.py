...
write-boundary runner（离线 · C-FM-33）。

Usage:
    python3 lab/run_cninfo_c_class_scale_union_partition_overlap_residual_resume_same_worse_safety.py
    python3 lab/run_cninfo_c_class_scale_union_partition_overlap_residual_resume_same_worse_safety.py \\
      --output-root outputs/validation/_mock_c_fm33_scale_union_partition_overlap_residual_resume_same_worse_safety
"""
...
REPORT_ROOT_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_union_partition_overlap_residual_resume_same_worse_safety"
)
REPORT_MD_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_union_partition_overlap_residual_resume_same_worse_safety_20260715.md"
)
REPORT_JSON_REL = (
    "outputs/validation/"
...
TASK_REPORT_REL = (
    "outputs/validation/"
    "cninfo_c_class_fm33_scale_union_partition_overlap_residual_resume_same_worse_safety_20260715.md"
)
...
def write_reports(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    for key, name in (
        ("union_partition_path", "union_status_partition_cardinality_freeze_ledger.json"),
        ("overlap_delta_path", "overlap_delta_cardinality_freeze_ledger.json"),
        ("residual_coverage_path", "residual_safety_coverage_lock_ledger.json"),
        ("resume_same_worse_path", "resume_same_worse_write_boundary_ledger.json"),
    ):
    ...
    json_payload = {
        ...
        "union_partition_path": payload["union_partition_path"],
    ...
    md_lines = [
        ...
        *layer_lines,
        ...
        "## Scale / safety gain",
        "",
        "- FM32 连续：unique=**2249** · status=**2134/106/9** · surface=**2251** · "
        "additive=**2261** · tier=**7/3314** · resume-improved/surface/additive/tier fps",
        ...
        "- resume_same/worse write-boundary：**1/0**（301212）· force_improve/reclass/"
        "inject_worse 拒绝",
        ...
        "",
        "```",
        f"c_fm_33_scale_union_partition_overlap_residual_resume_same_worse_safety_gate = "
        f"{payload['gate']}",
        "execute_production_snapshot_rebuild = false",
        "approved_for_snapshot_rebuild = false",
        "cninfo_calls = 0",
    ...
    task_lines = [
        ...
        "## Task",
        "",
        "在 C-FM-32 已 commit 且 EXECUTE 仍 human-held 之上，继续 **非 seal** "
        "规模/安全能力（非 extension↔drift / seal-chain）：**union status partition "
        "cardinality freeze（2134/106/9）**、**overlap_delta cardinality freeze"
        "（12）**、**residual_safety_coverage lock（117）**、**resume_same/worse "
        "write-boundary（1/0）**、**FM32 连续 + MOCK35 隔离**；产物写入隔离 MOCK35"
        "（不覆盖 MOCK3–34）。",
        "",
        "## Capability gain",
        "",
        "1. FM32 packet/fingerprint/gate/ledger 零漂移连续（unique=2249 · 2134/106/9 · "
        "surface=2251 · additive=2261 · 7/3314）",
        ...
        "5. resume_same/worse write-boundary：1/0（301212）· force_improve/reclass/"
        "inject_worse 拒绝",
        ...
        "## Next",
        "",
        "- Controller 可 commit 本包（union-partition + overlap + residual + "
        "resume_same/worse only）",
    ...
    with open(readme, "w", encoding="utf-8") as fh:
        fh.write(
            "\n".join(
                [
                    "# C-FM-33 mock scale union partition overlap residual resume same/worse safety root",
                    "",
                    f"gate: `{payload['gate']}`",
                    "seal_chain_extended: false",
                    f"scale_tier_count: {payload['scale_tier_count']}",
                    ...
                    f"resume_taxonomy: {payload['resume_improved']}/"
                    f"{payload['resume_same']}/{payload['resume_worse']}",
                    "does_not_overwrite: _mock_c_fm02.._mock_c_fm32 / standard dryrun",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "C-FM-33 scale union-partition/overlap/residual/resume_same-worse "
            "safety (offline)"
        )
    )
    parser.add_argument(
    ...
    if args.execute:
        print(
            ...
            "C-FM-33 is offline QA only; KEEP_EXECUTE_FALSE",
            file=sys.stderr,
        )
        return 2

    payload = run_scale_union_partition_overlap_residual_resume_same_worse_safety(
        paths=UnionPartitionOverlapResidualResumePaths(output_root_rel=args.output_root)
    )
