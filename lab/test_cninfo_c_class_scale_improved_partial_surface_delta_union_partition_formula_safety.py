"""
CNINFO C-class — 规模 improved/partial/surface-delta membership + union-formula
单测（离线 · CNINFO=0 · C-FM-37）。

运行：
    python3 lab/test_cninfo_c_class_scale_improved_partial_surface_delta_union_partition_formula_safety.py
"""
...
from cninfo_c_class_scale_improved_partial_surface_delta_union_partition_formula_safety import (  # noqa: E402
    ...
    ImprovedPartialSurfaceDeltaUnionPartitionFormulaPaths,
    assert_fm37_output_root,
    build_fm36_continuity_rows,
    build_partial_codes_membership_freeze_rows,
    build_resume_improved_membership_freeze_rows,
    build_surface_harvest_delta_membership_freeze_rows,
    build_union_partition_formula_identity_lock_rows,
    ...
    fingerprint_resume_improved_membership_freeze,
    run_scale_improved_partial_surface_delta_union_partition_formula_safety,
)
...
_RUNNER = os.path.join(
    _LAB_DIR,
    "run_cninfo_c_class_scale_improved_partial_surface_delta_union_partition_formula_safety.py",
)
_TEST_SUMMARY_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_improved_partial_surface_delta_union_partition_formula_safety_test_summary_20260715.md"
)
...
def _write_test_summary(cases: list) -> None:
    ...
    lines = [
        "# C-FM-37 Scale Improved/Partial/Surface-Delta Union-Formula — Test Summary",
        "",
        "_offline · CNINFO=0_",
        "",
        "| case | result |",
    ...
    lines.extend(
        [
            "",
            "```",
            "c_fm_37_scale_improved_partial_surface_delta_union_partition_formula_safety_test_gate = "
            "PASS_OFFLINE",
            "cninfo_calls = 0",
...
class TestCFm37ImprovedPartialSurfaceDeltaUnionFormula(unittest.TestCase):
    ...
    def test_resume_improved_membership_mutation_denied(self) -> None:
        mut = evaluate_resume_improved_membership_mutation(
            proposed_n=29,
            proposed_sha256=EXPECTED_RESUME_IMPROVED_CODES_SHA256,
            action="inject",
        )
        self.assertIs(mut["mutation_allowed"], False)
        _rows, checks, meta = build_resume_improved_membership_freeze_rows(
            ImprovedPartialSurfaceDeltaUnionPartitionFormulaPaths()
        )
        self.assertTrue(checks["resume_improved_membership_freeze_all_pass"])
        self.assertEqual(
            meta["fingerprint"], FROZEN_RESUME_IMPROVED_MEMBERSHIP_FREEZE_FP_SHA256
        )
        fp, _doc = fingerprint_resume_improved_membership_freeze()
        self.assertEqual(fp, FROZEN_RESUME_IMPROVED_MEMBERSHIP_FREEZE_FP_SHA256)

    def test_partial_codes_membership_mutation_denied(self) -> None:
        mut = evaluate_partial_codes_membership_mutation(
            proposed_n=107,
            proposed_sha256=EXPECTED_PARTIAL_CODES_SHA256,
            action="inject",
        )
        self.assertIs(mut["mutation_allowed"], False)
        _rows, checks, meta = build_partial_codes_membership_freeze_rows(
            ImprovedPartialSurfaceDeltaUnionPartitionFormulaPaths()
        )
        self.assertTrue(checks["partial_codes_membership_freeze_all_pass"])
        self.assertEqual(
            meta["fingerprint"], FROZEN_PARTIAL_CODES_MEMBERSHIP_FREEZE_FP_SHA256
        )

    def test_surface_delta_membership_mutation_denied(self) -> None:
        mut = evaluate_surface_harvest_delta_membership_mutation(
            proposed_codes=["000037", "000055", "999999"], action="inject"
        )
        self.assertIs(mut["mutation_allowed"], False)
        _rows, checks, meta = build_surface_harvest_delta_membership_freeze_rows(
            ImprovedPartialSurfaceDeltaUnionPartitionFormulaPaths()
        )
        self.assertTrue(checks["surface_harvest_delta_membership_freeze_all_pass"])
        self.assertEqual(
            meta["fingerprint"],
            FROZEN_SURFACE_HARVEST_DELTA_MEMBERSHIP_FREEZE_FP_SHA256,
        )

    def test_union_partition_formula_mutation_denied(self) -> None:
        mut = evaluate_union_partition_formula_mutation(
        ...
        self.assertIs(mut["mutation_allowed"], False)
        _rows, checks, meta = build_union_partition_formula_identity_lock_rows(
            ImprovedPartialSurfaceDeltaUnionPartitionFormulaPaths()
        )
        self.assertTrue(checks["union_partition_formula_identity_lock_all_pass"])
        self.assertEqual(
            meta["fingerprint"], FROZEN_UNION_PARTITION_FORMULA_IDENTITY_LOCK_FP_SHA256
        )

    def test_fm36_continuity(self) -> None:
        rows, checks = build_fm36_continuity_rows(
            ImprovedPartialSurfaceDeltaUnionPartitionFormulaPaths()
        )
        self.assertTrue(checks.get("fm36_continuity_all_pass"), rows[-1])
    ...
    def test_output_root_guards(self) -> None:
        ...
        ok = assert_fm37_output_root(DEFAULT_MOCK_OUTPUT_ROOT_REL)
        self.assertTrue(ok)

    def test_full_offline_run(self) -> None:
        result = run_scale_improved_partial_surface_delta_union_partition_formula_safety()
...
if __name__ == "__main__":
    ...
    suite = loader.loadTestsFromTestCase(
        TestCFm37ImprovedPartialSurfaceDeltaUnionFormula
    )
    ...
    if result.wasSuccessful():
        for name in [
            ...
            "test_resume_improved_membership_mutation_denied",
            "test_partial_codes_membership_mutation_denied",
            "test_surface_delta_membership_mutation_denied",
            "test_union_partition_formula_mutation_denied",
            "test_fm36_continuity",
