"""
B-FM-26：非会议类法律意见书 known-document live allow-list 锁测（离线；不请求 CNINFO）。

覆盖：
- allow-list 仅含 legal_opinion_known_003 与 known_004（category 空）
- 字段与 fixtures ready 对齐
- mock live 路径可对两案得到 pass（predicted_document_type=announcement；非 other）
- 不包含 legal_opinion_known_001/002 及 supervisory / shareholder 已 LIVE_PASS 案

无真实 CNINFO · 不造 §7 FP。

运行：
    python lab/test_cninfo_b_class_legal_opinion_known_003_004_live.py
"""
...
LIVE_DIR = os.path.join(
    ...
    "outputs",
    "validation",
    "cninfo_b_class_legal_opinion_known_003_004_live_20260715",
)
...
ANN_003 = "1223973353"
ANN_004 = "1223900931"
# 2025-06-24 / 2025-06-17 00:00:00 UTC
ANN_TS_003_MS = 1750723200000
ANN_TS_004_MS = 1750118400000
...
class TestLegalOpinionKnown003004LiveAllowList(unittest.TestCase):
    def test_allowlist_scope_exactly_two_known_cases(self) -> None:
        ...
        self.assertEqual(
            ...
            ["legal_opinion_known_003", "legal_opinion_known_004"],
        )
    ...
    def test_allowlist_fields_match_fixtures(self) -> None:
        ...
        for case_id in ("legal_opinion_known_003", "legal_opinion_known_004"):
