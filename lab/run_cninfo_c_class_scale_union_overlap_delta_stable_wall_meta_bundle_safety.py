...
cross_stable_metrics_wall_meta_bundle identity lock runner（离线 · C-FM-43）。

Usage:
    python3 lab/run_cninfo_c_class_scale_union_overlap_delta_stable_wall_meta_bundle_safety.py
    python3 lab/run_cninfo_c_class_scale_union_overlap_delta_stable_wall_meta_bundle_safety.py \\
      --output-root outputs/validation/_mock_c_fm43_scale_union_overlap_delta_stable_wall_meta_bundle_safety
"""
...
REPORT_ROOT_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_union_overlap_delta_stable_wall_meta_bundle_safety"
)
REPORT_MD_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_union_overlap_delta_stable_wall_meta_bundle_safety_20260715.md"
)
REPORT_JSON_REL = (
    "outputs/validation/"
...
TASK_REPORT_REL = (
    "outputs/validation/"
    "cninfo_c_class_fm43_scale_union_overlap_delta_stable_wall_meta_bundle_safety_20260715.md"
)
...
def write_reports(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    for key, name in (
        ("union_status_path", "union_status_composition_identity_lock_ledger.json"),
        ("overlap_delta_path", "overlap_delta_composition_identity_lock_ledger.json"),
        ("surface_delta_path", "surface_delta_composition_identity_lock_ledger.json"),
        (
            "cross_stable_metrics_wall_meta_bundle_path",
    ...
    json_payload = {
        ...
        "union_status_composition_sha256": payload["union_status_composition_sha256"],
        "overlap_delta_composition_sha256": payload["overlap_delta_composition_sha256"],
        "surface_delta_composition_sha256": payload["surface_delta_composition_sha256"],
        "cross_stable_metrics_wall_meta_bundle_sha256": payload[
            "cross_stable_metrics_wall_meta_bundle_sha256"
        ],
        "cross_formula_composition_meta_bundle_sha256": payload.get(
            "cross_formula_composition_meta_bundle_sha256"
        ),
        "union_status_formula": payload["union_status_formula"],
        "overlap_delta_formula": payload["overlap_delta_formula"],
        "surface_delta_formula": payload["surface_delta_formula"],
        ...
        "cross_stable_metrics_wall_meta_bundle_path": payload[
            "cross_stable_metrics_wall_meta_bundle_path"
        ],
    ...
    md_lines = [
        ...
        f"| cross_stable_metrics_wall_meta_bundle_sha256 | `{payload['cross_stable_metrics_wall_meta_bundle_sha256'][:16]}…` |",
        ...
        *layer_lines,
        ...
        "## Scale / safety gain",
        "",
        "- FM42 连续：unique=**2249** · dryrun=**1053** · Δ2 · status=**2134/106/9** · "
        "resume=**28/1/0** · residual=**117** · risk=**75/14/12/5** · cross_meta",
        ...
        "",
        "```",
        f"c_fm_43_scale_union_overlap_delta_stable_wall_meta_bundle_safety_gate = "
        f"{payload['gate']}",
        "execute_production_snapshot_rebuild = false",
        "approved_for_snapshot_rebuild = false",
        "cninfo_calls = 0",
    ...
    task_lines = [
        ...
        "## Task",
        "",
        "在 C-FM-42 已 commit 且 EXECUTE 仍 human-held 之上，继续 **非 seal** "
        "规模/安全能力（非 extension↔drift / seal-chain）：**union_status composition "
        "identity lock（2134/106/9）**、**overlap_delta composition identity "
        "lock（Δ12）**、**surface_delta composition "
        "identity lock（Δ2）**、"
        "**cross_stable_metrics_wall_meta_bundle identity lock（稳定指标墙元捆绑）**、"
        "**FM42 连续 + MOCK45 隔离**；产物写入隔离 MOCK45（不覆盖 MOCK3–44）。",
        "",
        "## Capability gain",
        "",
        "1. FM42 packet/fingerprint/gate/ledger 零漂移连续（unique=2249 · 1053 · "
        "Δ2 · 2134/106/9 · resume 28/1/0 · residual 117 · risk 75/14/12/5 · cross_meta）",
        ...
        f"surface_formula = {payload['surface_formula']}",
        f"surface_delta_formula = {payload['surface_delta_formula']}",
        f"additive_formula = {payload['additive_formula']}",
        f"overlap_delta_formula = {payload['overlap_delta_formula']}",
        f"tier_coverage_formula = {payload['tier_coverage_formula']}",
        f"risk_band_formula = {payload['risk_band_formula']}",
        f"resume_formula = {payload['resume_formula']}",
        f"ready_for_commit = {'true' if payload['gate'] == 'PASS_OFFLINE' else 'false'}",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.execute:
        print(
            ...
            "C-FM-43 is offline QA only; KEEP_EXECUTE_FALSE",
            file=sys.stderr,
        )
        return 2

    payload = run_scale_union_overlap_delta_stable_wall_meta_bundle_safety(
        ...
        json.dumps(
            {
                ...
                "resume_formula": payload["resume_formula"],
                "union_status_composition_sha256": payload["union_status_composition_sha256"],
                "overlap_delta_composition_sha256": payload[
                    "overlap_delta_composition_sha256"
                ],
                "surface_delta_composition_sha256": payload[
                    "surface_delta_composition_sha256"
                ],
                "cross_stable_metrics_wall_meta_bundle_sha256": payload[
                    "cross_stable_metrics_wall_meta_bundle_sha256"
                ],
