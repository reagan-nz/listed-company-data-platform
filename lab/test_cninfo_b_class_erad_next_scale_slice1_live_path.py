"""
B-class Era D next-scale slice1 live path 测试（mock CNINFO · 不执行真实 live）。
...
QUALITY_REPORT = runner.ERAD_SLICE1_QUALITY_REPORT_CSV
DRYRUN_REPORT = os.path.join(
    OUTPUT_ROOT, "reports", "b_class_erad_next_scale_slice1_dryrun_report.csv"
)
...
class TestEradNextScaleSlice1LivePath(unittest.TestCase):
    ...
    def test_live_path_function_exists_and_wired(self) -> None:
        ...
        self.assertTrue(callable(runner.write_live_erad_next_scale_slice1_reports))
        with mock.patch(
            "run_cninfo_b_class_tiny_live_validation.execute_live_case",
    ...
    def test_request_cap_720_enforced(self) -> None:
        ...
        self.assertEqual(runner.MAX_ERAD_NEXT_SCALE_SLICE1_CNINFO_REQUESTS, 720)
    ...
    def test_execution_gate_pass_with_caveat_at_270_threshold(self) -> None:
        ...
        gate = runner.compute_erad_next_scale_slice1_execution_gate(rows, 300)
    ...
    def test_fresh_metadata_only_no_scale_200_rerun(self) -> None:
        ...
        try:
            ...
            report_path = os.path.join(
                tmp_root, "reports", "b_class_erad_next_scale_slice1_report.csv"
            )
            ...
            self.assertTrue(all(r["case_id"] >= "BD2E201" for r in rows))
            self.assertTrue(all(r["case_id"] <= "BD2E500" for r in rows))
            snap = os.path.join(tmp_root, "raw_metadata", "BD2E201_EP004.json")
    ...
    def test_session_split_case_range_session1(self) -> None:
        ...
        try:
            with mock.patch(
                ...
                rc = runner.main(
                    _live_args_for_output_root(
                        tmp_root, ["--case-range", "BD2E201:BD2E350"]
                    )
                )
            self.assertEqual(rc, 0)
            report_path = os.path.join(
            ...
            self.assertEqual(rows[0]["case_id"], "BD2E201")
            self.assertEqual(rows[-1]["case_id"], "BD2E350")
        finally:
            _cleanup_mock_live_output_root(tmp_root)
...
def main() -> None:
    ...
    summary_path = os.path.join(
        ...
        "outputs",
        "validation",
        "cninfo_b_class_erad_next_scale_slice1_live_path_summary.md",
    )
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        f.write(
            ...
            f"- gate: **b_class_erad_next_scale_slice1_live_path_gate = READY_FOR_APPROVAL**\n"
            ...
            f"- session split: `--case-range BD2E201:BD2E350` wired\n"
        )
