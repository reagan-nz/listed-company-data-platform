...
# -*- coding: utf-8 -*-
"""
D 类 restricted_shares_unlock next-slice runner 测试（无 CNINFO · 无 live 执行）。

运行：
    .venv/bin/python lab/test_cninfo_d_class_restricted_shares_unlock_next_slice_runner.py
"""
...
UNIVERSE_CSV = runner.DEFAULT_RESTRICTED_SHARES_UNLOCK_NEXT_SLICE_UNIVERSE_CSV
OUTPUT_ROOT = runner.DEFAULT_RESTRICTED_SHARES_UNLOCK_NEXT_SLICE_OUTPUT_ROOT
DRYRUN_REPORT = runner.RESTRICTED_SHARES_UNLOCK_NEXT_SLICE_DRYRUN_REPORT_CSV
DRYRUN_SUMMARY = runner.RESTRICTED_SHARES_UNLOCK_NEXT_SLICE_DRYRUN_SUMMARY_MD
FIRST_SLICE_OUTPUT_ROOT = runner.DEFAULT_RESTRICTED_SHARES_UNLOCK_FIRST_SLICE_OUTPUT_ROOT
...
RSU_NEXT_LOCK = UNIVERSE_CSV
RSU_FIRST_DRAFT = runner.DEFAULT_RESTRICTED_SHARES_UNLOCK_FIRST_SLICE_UNIVERSE_CSV
...
class TestRestrictedSharesUnlockNextSliceRunner(unittest.TestCase):
    ...
    def test_dry_run_planned_ok_five_of_five(self) -> None:
        ...
        self.assertEqual(
            ...
            set(runner.RESTRICTED_SHARES_UNLOCK_NEXT_SLICE_ALLOWED_CASE_IDS),
        )
        for row in rows:
            ...
            self.assertEqual(
                ...
                runner.RESTRICTED_SHARES_UNLOCK_NEXT_SLICE_SHARED_PROBE_KEY,
            )
            self.assertEqual(
                row["planned_endpoint"], runner.RESTRICTED_SHARES_UNLOCK_NEXT_SLICE_ENDPOINT
            )
    ...
    def test_planned_snapshots_written(self) -> None:
        ...
        for case_id in sorted(runner.RESTRICTED_SHARES_UNLOCK_NEXT_SLICE_ALLOWED_CASE_IDS):
            path = os.path.join(snap_dir, f"{case_id}_restricted_shares_unlock.json")
            ...
            self.assertEqual(
                payload["forbidden_sole_found_anchor"], "2026-06-08"
            )
    ...
    def test_forbidden_anchor_20260608_blocked(self) -> None:
        rows = _read_universe_rows()
        rows[2]["anchor_tdate"] = "2026-06-08"
        with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_live_path_execute_function_exists(self) -> None:
        self.assertTrue(hasattr(runner, "execute_restricted_shares_unlock_next_slice_live"))
        self.assertTrue(callable(runner.execute_restricted_shares_unlock_next_slice_live))

    def test_shared_plan_equals_one(self) -> None:
        self.assertEqual(
            runner.compute_restricted_shares_unlock_next_slice_planned_shared(), 1
        )
        self.assertEqual(
            runner.build_restricted_shares_unlock_next_slice_plan(),
            ["tdate_daily_2026-07-03"],
        )
    ...
    def test_empty_but_valid_acceptable_rules(self) -> None:
        ...
        rows = runner.load_restricted_shares_unlock_next_slice_universe(UNIVERSE_CSV)
        by_id = {r.case_id: r for r in rows}
        for case_id in ("DRU101", "DRU102", "DRU103", "DRU104", "DRU105"):
            self.assertTrue(
                runner.is_restricted_shares_unlock_next_slice_acceptable(
    ...
    def test_live_with_approval_mock_shared_path_cninfo_zero(self) -> None:
        ...
        def _fake_cninfo_request(session, source_cfg, params_override, stats, case_id):
            ...
            self.assertEqual(
                ...
                runner.RESTRICTED_SHARES_UNLOCK_NEXT_SLICE_ANCHOR_TDATE,
            )
            return (
                {
                    "data": {
        ...
        with tempfile.TemporaryDirectory() as tmp:
            out_root = os.path.join(
                tmp, "cninfo_d_class_restricted_shares_unlock_next_slice"
            )
            ...
            with mock.patch(
                ...
                rc = runner.execute_restricted_shares_unlock_next_slice_live(
                    rows, output_paths
                )
                get_mock.assert_not_called()
                post_mock.assert_not_called()
            ...
            self.assertEqual(
                call_ids, [runner.RESTRICTED_SHARES_UNLOCK_NEXT_SLICE_SHARED_PROBE_KEY]
            )
            ...
            for case_id in ("DRU101", "DRU102", "DRU103", "DRU104"):
                self.assertEqual(live_rows[case_id]["acceptable"], "yes")
                self.assertEqual(live_rows[case_id]["retrieval_status"], "found")
            self.assertEqual(live_rows["DRU105"]["acceptable"], "yes")
            self.assertEqual(
                live_rows["DRU105"]["retrieval_status"], "empty_but_valid"
            )
    ...
    def test_plan_helpers_and_fixtures(self) -> None:
        rows = runner.load_restricted_shares_unlock_next_slice_universe(UNIVERSE_CSV)
        self.assertEqual(len(rows), 5)
        issues = runner.validate_restricted_shares_unlock_next_slice_universe(rows)
        self.assertEqual(issues, [])
        for row in rows:
            refs = runner.resolve_restricted_shares_unlock_next_slice_fixture_refs(row.case_id)
