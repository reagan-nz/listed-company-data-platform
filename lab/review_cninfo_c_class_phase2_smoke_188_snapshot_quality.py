#!/usr/bin/env python3
"""
CNINFO C-class Phase 2 Smoke 188 Snapshot Quality Review（Era C Phase 4）。

离线只读 phase2_smoke_188 snapshot JSON 做 QA 分析。
允许重写 quality/company_snapshot_status.csv（dry-run 遗留校正）。
不修改 snapshot JSON · normalized · field_inventory · 不请求 CNINFO。

Usage:
    python lab/review_cninfo_c_class_phase2_smoke_188_snapshot_quality.py
"""
...
STATUS_CSV = os.path.join(
    PHASE2_SNAPSHOT_DIR, "quality/company_snapshot_status.csv"
)

COMPLETENESS_CSV = os.path.join(
    BASE_DIR,
    "outputs/validation/cninfo_c_class_phase2_smoke_188_snapshot_completeness_report.csv",
)
MODULE_COVERAGE_CSV = os.path.join(
    BASE_DIR,
    "outputs/validation/cninfo_c_class_phase2_smoke_188_snapshot_module_coverage.csv",
)
QUALITY_FLAGS_CSV = os.path.join(
    BASE_DIR,
    "outputs/validation/cninfo_c_class_phase2_smoke_188_snapshot_quality_flags.csv",
)
QA_SUMMARY_MD = os.path.join(
    BASE_DIR,
    "outputs/validation/cninfo_c_class_phase2_smoke_188_snapshot_qa_summary.md",
)

EXPECTED_COMPANY_COUNT = 188

EXCLUDED_CODES = frozenset({
    "000038", "000616", "000956", "002087", "002231", "300023", "300356",
    "600005", "600290", "600634", "600646", "600696",
})
...
def load_all_valid_snapshots(
    snapshot_dir: str = PHASE2_SNAPSHOT_DIR,
...
def derive_qa_gate(stats: Dict[str, Any]) -> str:
    if (
        ...
        and stats.get("full_snapshot_json_count", 0) == 863
    ):
...
def write_qa_summary(
    ...
    lines = [
        ...
        "# Module Coverage",
        "",
        "与 863 full snapshot QA 模式一致：`technology_profile` 预期 `not_available`；"
        "`shareholder_profile` / `dividend_profile` / `risk_profile` / `market_behavior` "
        "等多为 `partial` 或 `available` 混合。",
        "",
    ...
    lines.extend([
        ...
        "",
        "`outputs/snapshot/cninfo_c_class/phase2_smoke_188/quality/company_snapshot_status.csv` "
        ...
        "# Output Isolation",
        "",
        f"- phase2 snapshot dir: `outputs/snapshot/cninfo_c_class/phase2_smoke_188/`",
        f"- full snapshot dir JSON count: **{stats.get('full_snapshot_json_count', 0)}**（未写入）",
        "",
        "# Gate",
        "",
        "```",
        f"phase2_smoke_188_snapshot_qa_gate = {gate}",
        "```",
        "",
        "# Next Step",
        "",
...
def run_phase2_smoke_188_snapshot_quality_review(
    snapshot_dir: str = PHASE2_SNAPSHOT_DIR,
) -> Dict[str, Any]:
    """执行 Phase 2 smoke 188 snapshot QA review 流水线。"""
...
def main() -> int:
    result = run_phase2_smoke_188_snapshot_quality_review()
    stats = result["stats"]
    ...
    print(f"phase2_smoke_188_snapshot_qa_gate: {result['gate']}")
