...
# -*- coding: utf-8 -*-
"""
D 类 shareholder_change further-scale S1000（~1000）孤立根 runner。

denser 策略：type=desc 多交易日并集（自适应 found + empty pad 凑满 1000）共享探针 + 离线 SECCODE 过滤。
禁止 type=inc+2026-07-03 作 sole found 锚；不写入 SC s50/s200 / next-slice / first-slice / ESH / AT 冻结根。
...
DEFAULT_OUTPUT_ROOT = os.path.join(
    VALIDATION, "cninfo_d_class_shareholder_change_further_scale_s1000"
)
DEFAULT_UNIVERSE_CSV = os.path.join(
    VALIDATION,
    "cninfo_d_class_shareholder_change_further_scale_s1000_universe_lock_20260716.csv",
)
SHAREHOLDER_DETAIL_CITE_JSON = os.path.join(
    VALIDATION,
    "cninfo_d_class_shareholder_change_further_scale_s1000_shareholder_detail_cite_20260716.json",
)
...
# D-FM-09 S50 found 槽公司码：不得再入 S1000 found 槽（空控可复用）
...
def validate_output_root(output_root: str) -> Tuple[bool, str]:
    ...
    if root != expected:
        return False, (
            "shareholder_change_further_scale_s1000_output_root_must_be_"
            "cninfo_d_class_shareholder_change_further_scale_s1000"
        )
...
def validate_universe(rows: Sequence[Dict[str, str]]) -> List[str]:
    ...
    if len(rows) != EXPECTED_SIZE:
        issues.append(
            f"shareholder_change_further_scale_s1000_universe_size_must_equal_{EXPECTED_SIZE}:"
            f"got={len(rows)}"
        )
    ...
    for row in rows:
        ...
        if case_id not in expected_ids:
            issues.append(
                f"forbidden_case_id_in_shareholder_change_further_scale_s1000:{case_id}"
            )
        ...
        if anchor not in allowed_anchors:
            issues.append(f"anchor_tdate_not_in_sc_fs_s1000_compose:{case_id}:{anchor}")
        if include != "yes":
        ...
        if expected_behavior == "captured_normal":
            ...
            if code in prior_found:
                issues.append(f"found_slot_reuses_prior_sc_found_code:{code}")
        elif expected_behavior == "empty_but_valid":
            empty_count += 1
    if seen_cases != expected_ids:
        issues.append("case_id_set_must_be_DSC501_DSC1500")
...
def build_universe_lock_from_cite() -> int:
    """多日 type=desc denser cite（自适应 found + empty pad）→ DSC501–1500 lock。"""
    ...
    for tdate in COMPOSE_CANDIDATE_TDATES:
        ...
        if len(selected_found) >= max_found:
            break
        cite = _cite_day(
            session, component_cfg, stats, tdate, f"sc_fs_s1000_cite_{tdate}"
        )
        ...
        rows.append(
            {
                ...
                "probe_key": f"sc_fs_s1000_shared_desc_{tdate}_{code}",
                "company_code": code,
                "company_name": name,
                "component": COMPONENT,
                ...
                "exclude_flags": (
                    ...
                    "exclude_sc_fs_s200_DSC301_500_found;"
                    "exclude_dlc006r;exclude_ess_h3_h4"
                ),
                "notes": (
                    f"further-scale s1000 found-path from type=desc cite day {tdate}; "
                    "NOT verified"
                ),
                "evidence_cite": f"D-FM-11_compose_{tdate.replace('-', '')}",
                "universe_lock_status": "locked",
                "lock_date": "2026-07-16",
                "approval_task_id": "D-FM-11",
    ...
    for code, name in selected_empty:
        rows.append(
            {
                "case_id": f"DSC{case_idx}",
                "probe_key": f"sc_fs_s1000_empty_control_{code}",
                "company_code": code,
                "company_name": name,
                "component": COMPONENT,
                "market": _infer_market(code),
                ...
                "exclude_flags": (
                    "exclude_688671;exclude_301259;"
                    "exclude_sparse_inc_20260703_sole_found_anchor;"
                    "empty_control_not_in_sc_fs_s1000_compose;"
                    "exclude_dlc006r;exclude_ess_h3_h4"
                ),
                "notes": (
                ...
                "evidence_cite": "D-FM-11_empty_control_absent_from_compose",
                "universe_lock_status": "locked",
                "lock_date": "2026-07-16",
                "approval_task_id": "D-FM-11",
                "per_case_request_budget": "1",
    ...
    cite_payload = {
        ...
        "found_vs_empty_pad": {
            ...
            "note": "empty pad fills remainder when multi-day union cannot supply 990+ distinct new found codes",
        },
    }
    ...
    print(
        f"mode=shareholder_change_further_scale_s1000_universe_lock "
        ...
        dry.append(
            {
                ...
                "notes": (
                    ...
                    f"exclude_sc_fs_s50_found=yes; exclude_sc_fs_s200_found=yes"
                ),
            }
        )
    return dry
...
def write_dryrun_report(
    ...
    path = os.path.join(
        ...
        "d_class_shareholder_change_further_scale_s1000_dryrun_report.csv",
    )
...
def write_dryrun_summary(
    ...
    path = os.path.join(
        output_paths["reports"],
        "d_class_shareholder_change_further_scale_s1000_dryrun_summary.md",
    )
...
def execute_live(
    ...
    live_path = os.path.join(
        output_paths["reports"],
        "d_class_shareholder_change_further_scale_s1000_live_report.csv",
    )
    ...
    quality_path = os.path.join(
        output_paths["reports"],
        "d_class_shareholder_change_further_scale_s1000_quality_report.csv",
    )
    ...
    summary_path = os.path.join(
        output_paths["reports"],
        "d_class_shareholder_change_further_scale_s1000_live_summary.md",
    )
    lines = [
        ...
        "d_class_shareholder_change_further_scale_s1000_live_path_gate = READY_FOR_APPROVAL",
        "d_class_shareholder_change_further_scale_s1000_live_gate = NOT_APPROVED",
        f"d_class_shareholder_change_further_scale_s1000_execution_gate = {gate}",
    ...
    print(
        f"mode=shareholder_change_further_scale_s1000_live cases={len(live_rows)} "
    ...
    print(
        f"gate=d_class_shareholder_change_further_scale_s1000_execution_gate={gate}"
    )
...
def run_dry_run(args: argparse.Namespace) -> int:
    ...
    print(
        f"mode=shareholder_change_further_scale_s1000_dry_run cases={len(dry_rows)} "
        f"planned_request_count_total={len(_compose_days_from_rows(rows))} cninfo_calls=0"
    )
    print(
        "gate=d_class_shareholder_change_further_scale_s1000_runner_extension_gate="
        "READY_FOR_APPROVAL"
    )
...
def run_live(args: argparse.Namespace) -> int:
    if not args.approve_d_class_shareholder_change_further_scale_s1000:
        print(
            "ERROR: approve_d_class_shareholder_change_further_scale_s1000_required",
            file=sys.stderr,
        )
        return 2
    ok_root, root_err = validate_output_root(args.output_root)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="D-class shareholder_change further-scale S1000 (~1000) runner"
    )
