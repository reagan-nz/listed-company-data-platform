"""
B-FM-03（R19）：~50 known-doc residual scale 晋升锁测（离线）。

覆盖：
- allow-list 50 案均为 ready
...
运行：
    python lab/test_cninfo_b_class_residual_scale50_promotion.py
"""
...
class TestResidualScale50Promotion(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.config = routing._load_yaml(routing.DEFAULT_CATEGORIES)
        cls.known = _load_cases(KNOWN_YAML)

    def test_allowlist_size_fifty(self) -> None:
        self.assertEqual(len(ALLOW_IDS), 50)
        self.assertEqual(len(set(ALLOW_IDS)), 50)
