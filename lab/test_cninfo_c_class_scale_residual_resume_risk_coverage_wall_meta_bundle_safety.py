"""
CNINFO C-class — 规模 residual/resume/risk/coverage-wall-meta-bundle
单测（离线 · CNINFO=0 · C-FM-44）。

运行：
    python3 lab/test_cninfo_c_class_scale_residual_resume_risk_coverage_wall_meta_bundle_safety.py
"""
...
from cninfo_c_class_scale_residual_resume_risk_coverage_wall_meta_bundle_safety import (  # noqa: E402
    ...
    ResidualResumeRiskCoverageWallMetaBundlePaths,
    assert_fm44_output_root,
    build_cross_residual_resume_risk_coverage_wall_meta_bundle_identity_lock_rows,
    build_fm43_continuity_rows,
    build_residual_formula_composition_identity_lock_rows,
    build_resume_taxonomy_composition_identity_lock_rows,
    ...
    run_scale_residual_resume_risk_coverage_wall_meta_bundle_safety,
)
...
_RUNNER = os.path.join(
    _LAB_DIR,
    "run_cninfo_c_class_scale_residual_resume_risk_coverage_wall_meta_bundle_safety.py",
)
_TEST_SUMMARY_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_residual_resume_risk_coverage_wall_meta_bundle_safety_test_summary_20260715.md"
)
...
def _write_test_summary(cases: list) -> None:
    ...
    lines = [
        "# C-FM-44 Scale Residual/Resume/Risk/Coverage-Wall-Meta-Bundle — Test Summary",
        "",
        "_offline · CNINFO=0_",
        "",
        "| case | result |",
    ...
    lines.extend(
        [
            "",
            "```",
            "c_fm_44_scale_residual_resume_risk_coverage_wall_meta_bundle_safety_test_gate = "
            "PASS_OFFLINE",
            "cninfo_calls = 0",
            ...
            f"cross_residual_resume_risk_coverage_wall_meta_bundle_sha256 = "
            f"{EXPECTED_CROSS_RESIDUAL_RESUME_RISK_COVERAGE_WALL_META_BUNDLE_SHA256}",
            "```",
            "",
        ]
    )
...
class TestCFm44ResidualResumeRiskCoverageWallMetaBundle(unittest.TestCase):
    def test_constants_frozen(self) -> None:
        ...
        self.assertEqual(
            len(EXPECTED_CROSS_RESIDUAL_RESUME_RISK_COVERAGE_WALL_META_BUNDLE), 5
        )
    ...
    def test_residual_formula_composition_mutation_denied(self) -> None:
        ...
        fp, _doc = fingerprint_residual_formula_composition_identity_lock()
        self.assertEqual(fp, FROZEN_RESIDUAL_FORMULA_COMPOSITION_IDENTITY_LOCK_FP_SHA256)

    def test_resume_taxonomy_composition_mutation_denied(self) -> None:
        mut = evaluate_resume_taxonomy_composition_mutation(
        ...
        self.assertIs(mut["mutation_allowed"], False)
        _rows, checks, meta = build_resume_taxonomy_composition_identity_lock_rows(
            ResidualResumeRiskCoverageWallMetaBundlePaths()
        )
        self.assertTrue(checks["resume_taxonomy_composition_identity_lock_all_pass"])
        self.assertEqual(
            meta["fingerprint"],
            FROZEN_RESUME_TAXONOMY_COMPOSITION_IDENTITY_LOCK_FP_SHA256,
        )
    ...
    def test_wall_meta_bundle_mutation_denied(self) -> None:
        ...
        mut = evaluate_cross_residual_resume_risk_coverage_wall_meta_bundle_mutation(
    ...
    def test_fm43_continuity(self) -> None:
        rows, checks = build_fm43_continuity_rows(
            ResidualResumeRiskCoverageWallMetaBundlePaths()
        )
        self.assertTrue(checks.get("fm43_continuity_all_pass"), rows[-1])
    ...
    def test_output_root_guards(self) -> None:
        ...
        ok = assert_fm44_output_root(DEFAULT_MOCK_OUTPUT_ROOT_REL)
        self.assertTrue(ok)

    def test_full_offline_run(self) -> None:
        result = run_scale_residual_resume_risk_coverage_wall_meta_bundle_safety()
        ...
        self.assertEqual(result["resume_taxonomy_formula"], "28/1/0")
        self.assertEqual(result["risk_band_status_formula"], "75/14/12/5")
        self.assertEqual(result["coverage_formula"], "coverage=117")
        self.assertEqual(
            result["cross_residual_resume_risk_coverage_wall_meta_bundle_sha256"],
            EXPECTED_CROSS_RESIDUAL_RESUME_RISK_COVERAGE_WALL_META_BUNDLE_SHA256,
        )
        packet = os.path.join(BASE_DIR, result["packet_path"])
        with open(packet, encoding="utf-8") as fh:
            doc = json.load(fh)
        self.assertEqual(doc["task_id"], "C-FM-44")
        self.assertFalse(doc["approved_for_snapshot_rebuild"])

    def test_cli_runner_offline(self) -> None:
        tmp = os.path.join(
            BASE_DIR,
            "outputs/validation/_mock_c_fm44_cli_test_tmp",
        )
        proc = subprocess.run(
...
if __name__ == "__main__":
    ...
    suite = loader.loadTestsFromTestCase(
        TestCFm44ResidualResumeRiskCoverageWallMetaBundle
    )
