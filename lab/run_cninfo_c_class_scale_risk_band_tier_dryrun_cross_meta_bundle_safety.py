...
cross_formula_composition_meta_bundle identity lock runner（离线 · C-FM-42）。

Usage:
    python3 lab/run_cninfo_c_class_scale_risk_band_tier_dryrun_cross_meta_bundle_safety.py
    python3 lab/run_cninfo_c_class_scale_risk_band_tier_dryrun_cross_meta_bundle_safety.py \\
      --output-root outputs/validation/_mock_c_fm42_scale_risk_band_tier_dryrun_cross_meta_bundle_safety
"""
...
REPORT_ROOT_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_risk_band_tier_dryrun_cross_meta_bundle_safety"
)
REPORT_MD_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_risk_band_tier_dryrun_cross_meta_bundle_safety_20260715.md"
)
REPORT_JSON_REL = (
    "outputs/validation/"
...
TASK_REPORT_REL = (
    "outputs/validation/"
    "cninfo_c_class_fm42_scale_risk_band_tier_dryrun_cross_meta_bundle_safety_20260715.md"
)
...
def write_reports(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    for key, name in (
        ("risk_band_path", "risk_band_composition_identity_lock_ledger.json"),
        ("tier_coverage_path", "tier_coverage_composition_identity_lock_ledger.json"),
        ("combined_dryrun_path", "combined_dryrun_composition_identity_lock_ledger.json"),
        (
            "cross_formula_composition_meta_bundle_path",
    ...
    json_payload = {
        ...
        "risk_band_composition_sha256": payload["risk_band_composition_sha256"],
        "tier_coverage_composition_sha256": payload["tier_coverage_composition_sha256"],
        "combined_dryrun_composition_sha256": payload[
            "combined_dryrun_composition_sha256"
        ],
        "cross_formula_composition_meta_bundle_sha256": payload[
    ...
    md_lines = [
        ...
        f"| cross_formula_composition_meta_bundle_sha256 | `{payload['cross_formula_composition_meta_bundle_sha256'][:16]}…` |",
        ...
        *layer_lines,
        ...
        "## Scale / safety gain",
        "",
        "- FM41 连续：unique=**2249** · dryrun=**1053** · Δ2 · status=**2134/106/9** · "
        "resume=**28/1/0** · residual=**117=106+9+2** · cross_composition_bundle",
        ...
        "",
        "```",
        f"c_fm_42_scale_risk_band_tier_dryrun_cross_meta_bundle_safety_gate = "
        f"{payload['gate']}",
        "execute_production_snapshot_rebuild = false",
        "approved_for_snapshot_rebuild = false",
        "cninfo_calls = 0",
    ...
    task_lines = [
        ...
        "## Task",
        "",
        "在 C-FM-41 已 commit 且 EXECUTE 仍 human-held 之上，继续 **非 seal** "
        "规模/安全能力（非 extension↔drift / seal-chain）：**risk_band composition "
        "identity lock（75+14+12+5=106）**、**tier_coverage composition identity "
        "lock（tiers=7;coverage_sum=3314）**、**combined_dryrun composition "
        "identity lock（combined_dryrun=1053）**、"
        "**cross_formula_composition_meta_bundle identity lock（五组成元捆绑）**、"
        "**FM41 连续 + MOCK44 隔离**；产物写入隔离 MOCK44（不覆盖 MOCK3–43）。",
        "",
        "## Capability gain",
        "",
        "1. FM41 packet/fingerprint/gate/ledger 零漂移连续（unique=2249 · 1053 · "
        "Δ2 · 2134/106/9 · resume 28/1/0 · residual 117=106+9+2 · cross_composition_bundle）",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "C-FM-42 scale risk_band/tier/dryrun/cross-meta-bundle "
            "safety (offline)"
        )
    )
    parser.add_argument(
    ...
    if args.execute:
        print(
            ...
            "C-FM-42 is offline QA only; KEEP_EXECUTE_FALSE",
            file=sys.stderr,
        )
        return 2

    payload = run_scale_risk_band_tier_dryrun_cross_meta_bundle_safety(
        ...
        json.dumps(
            {
                ...
                "cross_formula_bundle_sha256": payload["cross_formula_bundle_sha256"],
                "risk_band_composition_sha256": payload["risk_band_composition_sha256"],
                "tier_coverage_composition_sha256": payload[
