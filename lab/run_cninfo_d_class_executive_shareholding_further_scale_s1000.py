...
# -*- coding: utf-8 -*-
"""
D 类 executive_shareholding further-scale S1000（~1000）孤立根 runner。

共享 denser-mode 探针 timeMark=threeMonth + varyType=b；离线 SECCODE 过滤。
组成：cite 剩余 found 并集 + 文档化 empty-control pad 至 ~1000。
不写入 ESH S200（DES251–450）/ S50（DES201–250）/ next-slice（DES101–105）/
first-slice / AT / SC / EP / RSU / FIA 冻结根。
...
EXPECTED_SIZE = 1000
CASE_ID_START = 451
CASE_ID_END = 1450  # inclusive → 1000 cases
...
DEFAULT_OUTPUT_ROOT = os.path.join(
    VALIDATION, "cninfo_d_class_executive_shareholding_further_scale_s1000"
)
DEFAULT_UNIVERSE_CSV = os.path.join(
    VALIDATION,
    "cninfo_d_class_executive_shareholding_further_scale_s1000_universe_lock_20260716.csv",
)
LEADER_DETAIL_CITE_JSON = os.path.join(
    VALIDATION,
    "cninfo_d_class_executive_shareholding_further_scale_s1000_leader_detail_cite_20260716.json",
)
...
# D-FM-06 S50 found 槽公司码：不得再入 S1000 found 槽（空控可复用）
...
# D-FM-07 S200 found 槽公司码：不得再入 S1000 found 槽（空控可复用）
...
# 空控种子：优先 next-slice / S50 / S200 已证空控与大盘蓝筹
EMPTY_CONTROL_SEEDS = [
    ("000895", "双汇发展"),
    ("601988", "中国银行"),
    ("600519", "贵州茅台"),
...
def validate_universe(rows: Sequence[Dict[str, str]]) -> List[str]:
    ...
    if len(rows) != EXPECTED_SIZE:
        issues.append(
            f"executive_shareholding_further_scale_s1000_universe_size_must_equal_{EXPECTED_SIZE}:"
            f"got={len(rows)}"
        )
    ...
    for row in rows:
        ...
        if case_id not in expected_ids:
            issues.append(
                f"forbidden_case_id_in_executive_shareholding_further_scale_s1000:{case_id}"
            )
        ...
        if eb == "captured_normal" and code in prior_found:
            issues.append(f"prior_found_code_reuse_forbidden:{code}")
    ...
    if seen_cases != expected_ids:
        issues.append("case_id_set_must_be_DES451_DES1450")
    return issues


def build_universe_lock_from_cite() -> int:
    """一次 CNINFO denser-mode cite → DES451–1450 lock（剩余 found + empty pad）。"""
    ...
    payload, http_status, last_error = core._cninfo_request(
        session, component_cfg, params, stats, "esh_fs_s1000_leader_detail_cite"
    )
    ...
    for rec in records:
        ...
        if not code:
            continue
        code = code.zfill(6)
        present_union.add(code)
        if (
            code in FORBIDDEN_CODES
            or code in EXCLUDED_PRIOR_SLICE_CODES
            or code in prior_found
        ):
            continue
        if code not in by_code:
            by_code[code] = rec

    found_available = sorted(by_code.keys())
    max_found = EXPECTED_SIZE - EMPTY_CONTROL_MIN
    selected_found = found_available[: min(len(found_available), max_found)]
    ...
    if len(selected_found) < 1:
        print("ERROR: denser-mode cite yielded zero new found codes", file=sys.stderr)
        return 2
    selected_empty = empty_candidates[:empty_needed]

    rows: List[Dict[str, str]] = []
    case_idx = CASE_ID_START
    for code in selected_found:
        rec = by_code[code]
        name = (
            str(rec.get("SECNAME") or rec.get("secName") or "").strip() or code
        )
        rows.append(
            {
                "case_id": f"DES{case_idx}",
                "probe_key": f"esh_fs_s1000_shared_threeMonth_b_{code}",
                "company_code": code,
                "company_name": name,
                "component": COMPONENT,
                ...
                "exclude_flags": (
                    ...
                    "exclude_esh_further_scale_s200_DES251_450;"
                    "exclude_dlc006r;exclude_ess_h3_h4"
                ),
                "notes": (
                    "further-scale S1000 found-path from denser-mode leader/detail cite "
                    "after S50/S200 exclusions; NOT verified"
                ),
                "evidence_cite": "D-FM-08_leader_detail_cite_threeMonth_b",
                "universe_lock_status": "locked",
                "lock_date": "2026-07-16",
                "approval_task_id": "D-FM-08",
                "per_case_request_budget": "1",
                "total_request_cap": str(EXPECTED_SIZE),
                "shared_probe_prefer": "1",
                "dense_mode_cite_strength": "denser_cite_plus_empty_pad",
            }
        )
        case_idx += 1
    ...
    for code, name in selected_empty:
        rows.append(
            {
                "case_id": f"DES{case_idx}",
                "probe_key": f"esh_fs_s1000_empty_control_{code}",
                "company_code": code,
                "company_name": name,
                "component": COMPONENT,
                "market": _infer_market(code),
                ...
                "exclude_flags": (
                    "exclude_688671;exclude_301259;"
                    "exclude_sparse_oneMonth_b_sole_found_anchor;"
                    "empty_control_pad_documented;"
                    "exclude_esh_further_scale_s50_s200_mutation;"
                    "exclude_dlc006r;exclude_ess_h3_h4"
                ),
                "notes": (
                    "empty-control pad: absent from denser-mode threeMonth+b cite "
                    "present_union after SECCODE filter; denser found pool exhausted"
                ),
                "evidence_cite": "D-FM-08_empty_pad_absent_from_denser_cite",
                "universe_lock_status": "locked",
                "lock_date": "2026-07-16",
                "approval_task_id": "D-FM-08",
                "per_case_request_budget": "1",
    ...
    cite_payload = {
        ...
        "present_union_size": len(present_union),
        "unique_codes_after_exclusions": len(found_available),
        "selected_found_codes": selected_found,
        "selected_found_count": len(selected_found),
        "selected_empty_count": len(selected_empty),
        "selected_empty_codes": [c for c, _ in selected_empty],
        ...
        "compose_note": "denser_cite_plus_empty_pad",
    ...
    print(
        f"mode=executive_shareholding_further_scale_s1000_universe_lock "
        f"cases={len(ordered)} cninfo_calls={stats.cninfo_requests} "
        f"leader_detail_total={total} selected_found={len(selected_found)} "
        f"empty_pad={len(selected_empty)}"
    )
        ...
        dry.append(
            {
                ...
                "notes": (
                    ...
                    f"exclude_esh_next_slice_DES101_105=yes; exclude_esh_further_scale_s50_DES201_250=yes; exclude_esh_further_scale_s200_DES251_450=yes; compose=denser_cite_plus_empty_pad"
                ),
            }
        )
    return dry
...
def write_dryrun_report(
    ...
    path = os.path.join(
        ...
        "d_class_executive_shareholding_further_scale_s1000_dryrun_report.csv",
    )
...
def write_dryrun_summary(
    ...
    path = os.path.join(
        output_paths["reports"],
        "d_class_executive_shareholding_further_scale_s1000_dryrun_summary.md",
    )
...
def execute_live(
    ...
    quality_path = os.path.join(
        output_paths["reports"],
        "d_class_executive_shareholding_further_scale_s1000_quality_report.csv",
    )
    ...
    summary_path = os.path.join(
        output_paths["reports"],
        "d_class_executive_shareholding_further_scale_s1000_live_summary.md",
    )
    lines = [
        ...
        "d_class_executive_shareholding_further_scale_s1000_live_path_gate = READY_FOR_APPROVAL",
        "d_class_executive_shareholding_further_scale_s1000_live_gate = NOT_APPROVED",
        f"d_class_executive_shareholding_further_scale_s1000_execution_gate = {gate}",
    ...
    print(
        f"mode=executive_shareholding_further_scale_s1000_live cases={len(live_rows)} "
    ...
    print(f"gate=d_class_executive_shareholding_further_scale_s1000_execution_gate={gate}")
...
def run_dry_run(args: argparse.Namespace) -> int:
    ...
    print(
        f"mode=executive_shareholding_further_scale_s1000_dry_run cases={len(dry_rows)} "
        f"planned_request_count_total=1 cninfo_calls=0"
    )
    print(
        "gate=d_class_executive_shareholding_further_scale_s1000_runner_extension_gate="
        "READY_FOR_APPROVAL"
    )
...
def run_live(args: argparse.Namespace) -> int:
    if not args.approve_d_class_executive_shareholding_further_scale_s1000:
        print(
            "ERROR: approve_d_class_executive_shareholding_further_scale_s1000_required",
            file=sys.stderr,
        )
        return 2
    ok_root, root_err = validate_output_root(args.output_root)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="D-class executive_shareholding further-scale S1000 (~1000) runner"
    )
