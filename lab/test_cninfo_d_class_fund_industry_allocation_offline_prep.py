...
# -*- coding: utf-8 -*-
"""
CNINFO D 类 fund_industry_allocation — D-FM-10 offline prep smoke（Tier-0 mapper + universe sketch）。

离线 only · 无 CNINFO · 无 live · 无 runner · 不 claim verified。

运行：
    .venv/bin/python lab/test_cninfo_d_class_fund_industry_allocation_offline_prep.py
"""
...
SAMPLE_RAW = BASE_DIR / "fixtures" / "d_class" / "fund_industry_allocation" / "sample_raw.json"
SCHEMA_PATH = BASE_DIR / "schemas" / "d_class" / "d_industry_aggregate.schema.json"
UNIVERSE_SKETCH = (
    BASE_DIR
    / "outputs"
    / "validation"
    / "cninfo_d_class_fund_industry_allocation_first_slice_universe_draft_sketch_20260715.csv"
)
...
class FundIndustryAllocationOfflinePrepTests(unittest.TestCase):
    """D-FM-10：Tier-0 sample_raw 映射 + universe sketch 结构校验。"""
    ...
    def test_sample_raw_exists_and_component_match(self) -> None:
        ...
        self.assertEqual(payload.get("query_mode"), "default")
        self.assertEqual(payload.get("expected_logical_table"), "d_industry_aggregate")
        raw = payload["raw_record"]
        self.assertEqual(str(raw.get("F001V")), "C26")
        self.assertEqual(str(raw.get("F002V")), "化学原料和化学制品制造业")

    def test_mapper_emits_three_metrics(self) -> None:
        payload = _load_sample()
    ...
    def test_universe_sketch_five_cases_and_excludes(self) -> None:
        ...
        for row in rows:
            ...
            # 行业聚合：不得把排除码误写成 industry_code
            self.assertNotIn(row["industry_code"], EXCLUDED_CODES)
        self.assertEqual(rows[0]["industry_code"], "C26")
    ...
    def test_registry_fund_industry_allocation_block_present(self) -> None:
    ...
    def test_no_network_imports_for_cninfo(self) -> None:
        ...
        import lab.test_cninfo_d_class_fund_industry_allocation_offline_prep as self_mod
