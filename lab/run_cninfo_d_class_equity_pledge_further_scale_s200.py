...
# -*- coding: utf-8 -*-
"""
D 类 equity_pledge further-scale S200（~200）孤立根 runner。

denser 策略：tdate_daily 多交易日并集（候选日自适应 cite 至 found=198）共享探针 + 离线 SECCODE 过滤。
禁止 2026-07-03 作 sole found 锚；不写入 EP s50 / next-slice / first-slice / SC / ESH / AT 冻结根。

用法：
  .venv/bin/python lab/run_cninfo_d_class_equity_pledge_further_scale_s200.py --build-universe-lock
  .venv/bin/python lab/run_cninfo_d_class_equity_pledge_further_scale_s200.py --dry-run \\
      --universe-csv outputs/validation/cninfo_d_class_equity_pledge_further_scale_s200_universe_lock_20260716.csv \\
      --output-root outputs/validation/cninfo_d_class_equity_pledge_further_scale_s200
  .venv/bin/python lab/run_cninfo_d_class_equity_pledge_further_scale_s200.py --live \\
      --approve-d-class-equity-pledge-further-scale-s200 \\
      --universe-csv outputs/validation/cninfo_d_class_equity_pledge_further_scale_s200_universe_lock_20260716.csv \\
      --output-root outputs/validation/cninfo_d_class_equity_pledge_further_scale_s200
"""
...
DEFAULT_OUTPUT_ROOT = os.path.join(
    VALIDATION, "cninfo_d_class_equity_pledge_further_scale_s200"
)
...
EQUITY_PLEDGE_CITE_JSON = os.path.join(
    VALIDATION,
    "cninfo_d_class_equity_pledge_further_scale_s200_equity_pledge_cite_20260716.json",
)
...
def validate_output_root(output_root: str) -> Tuple[bool, str]:
    ...
    if root != expected:
        return False, (
            "equity_pledge_further_scale_s200_output_root_must_be_"
...
def validate_universe(rows: Sequence[Dict[str, str]]) -> List[str]:
    ...
    if len(rows) != EXPECTED_SIZE:
        issues.append(
            f"equity_pledge_further_scale_s200_universe_size_must_equal_{EXPECTED_SIZE}:"
            f"got={len(rows)}"
        )
    ...
    for row in rows:
        ...
        if case_id not in expected_ids:
            issues.append(
                f"forbidden_case_id_in_equity_pledge_further_scale_s200:{case_id}"
            )
        ...
        if anchor not in allowed_anchors:
            issues.append(f"anchor_tdate_not_in_ep_fs_s200_compose:{case_id}:{anchor}")
        if include != "yes":
    ...
    if seen_cases != expected_ids:
        issues.append("case_id_set_must_be_DEP301_DEP500")
...
def build_universe_lock_from_cite() -> int:
    """多日 tdate_daily denser cite（自适应至 found=198）→ DEP301–500 lock。"""
    ...
    for tdate in COMPOSE_CANDIDATE_TDATES:
        ...
        if len(selected_found) >= FOUND_SLOTS:
            break
        cite = _cite_day(
            session, component_cfg, stats, tdate, f"ep_fs_s200_cite_{tdate}"
        )
        ...
        rows.append(
            {
                ...
                "probe_key": f"ep_fs_s200_shared_desc_{tdate}_{code}",
                "company_code": code,
                "company_name": name,
                "component": COMPONENT,
                ...
                "notes": (
                    f"further-scale s200 found-path from tdate_daily cite day {tdate}; "
                    "NOT verified"
                ),
                "evidence_cite": f"D-FM-12_compose_{tdate.replace('-', '')}",
                "universe_lock_status": "locked",
                "lock_date": "2026-07-16",
                "approval_task_id": "D-FM-12",
    ...
    for code, name in selected_empty:
        rows.append(
            {
                "case_id": f"DEP{case_idx}",
                "probe_key": f"ep_fs_s200_empty_control_{code}",
                "company_code": code,
                "company_name": name,
                "component": COMPONENT,
                "market": _infer_market(code),
                ...
                "exclude_flags": (
                    "exclude_688671;exclude_301259;"
                    "exclude_sparse_day_20260703_sole_found_anchor;"
                    "empty_control_not_in_ep_fs_s200_compose;"
                    "exclude_dlc006r;exclude_ess_h3_h4"
                ),
                "notes": (
                ...
                "evidence_cite": "D-FM-12_empty_control_absent_from_compose",
                "universe_lock_status": "locked",
                "lock_date": "2026-07-16",
                "approval_task_id": "D-FM-12",
                "per_case_request_budget": "1",
    ...
    print(
        f"mode=equity_pledge_further_scale_s200_universe_lock "
...
def write_dryrun_report(
    ...
    path = os.path.join(
        ...
        "d_class_equity_pledge_further_scale_s200_dryrun_report.csv",
    )
...
def write_dryrun_summary(
    ...
    path = os.path.join(
        output_paths["reports"],
        "d_class_equity_pledge_further_scale_s200_dryrun_summary.md",
    )
...
def execute_live(
    ...
    quality_path = os.path.join(
        output_paths["reports"],
        "d_class_equity_pledge_further_scale_s200_quality_report.csv",
    )
    ...
    summary_path = os.path.join(
        output_paths["reports"],
        "d_class_equity_pledge_further_scale_s200_live_summary.md",
    )
    lines = [
        "# CNINFO D 类 equity_pledge Further-Scale S200 Live Summary",
        "",
        f"_生成时间：{datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S')} UTC_",
        "",
        "> **性质：** equity_pledge further-scale s200 live · **R19 standing bounded** · **NOT verified**",
        "",
        "## Counts",
        "",
        "| 指标 | 值 |",
        ...
        "d_class_equity_pledge_further_scale_s200_live_path_gate = READY_FOR_APPROVAL",
        "d_class_equity_pledge_further_scale_s200_live_gate = NOT_APPROVED",
        f"d_class_equity_pledge_further_scale_s200_execution_gate = {gate}",
        ...
        "- EP s50 / next-slice / first-slice / ESH / AT 冻结根未 mutate。",
        "",
    ]
    ...
    print(
        f"mode=equity_pledge_further_scale_s200_live cases={len(live_rows)} "
    ...
    print(
        f"gate=d_class_equity_pledge_further_scale_s200_execution_gate={gate}"
    )
...
def run_dry_run(args: argparse.Namespace) -> int:
    ...
    print(
        f"mode=equity_pledge_further_scale_s200_dry_run cases={len(dry_rows)} "
        f"planned_request_count_total={len(_compose_days_from_rows(rows))} cninfo_calls=0"
    )
    print(
        "gate=d_class_equity_pledge_further_scale_s200_runner_extension_gate="
        "READY_FOR_APPROVAL"
    )
...
def run_live(args: argparse.Namespace) -> int:
    if not args.approve_d_class_equity_pledge_further_scale_s200:
        print(
            "ERROR: approve_d_class_equity_pledge_further_scale_s200_required",
            file=sys.stderr,
        )
        return 2
    ok_root, root_err = validate_output_root(args.output_root)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="D-class equity_pledge further-scale S200 (~200) runner"
    )
