#!/usr/bin/env python3
"""
CNINFO C-class — 规模 dry863/surface-delta codeset freeze + combined dryrun coverage
cardinality freeze + cross-metric identity lock + partition codeset
SHA256 lock runner（离线 · C-FM-34）。

Usage:
    python3 lab/run_cninfo_c_class_scale_surface_delta_combined_dryrun_cross_identity_partition_codeset_safety.py
    python3 lab/run_cninfo_c_class_scale_surface_delta_combined_dryrun_cross_identity_partition_codeset_safety.py \\
      --output-root outputs/validation/_mock_c_fm34_scale_surface_delta_combined_dryrun_cross_identity_partition_codeset_safety
"""
...
REPORT_ROOT_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_surface_delta_combined_dryrun_cross_identity_partition_codeset_safety"
)
REPORT_MD_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_surface_delta_combined_dryrun_cross_identity_partition_codeset_safety_20260715.md"
)
REPORT_JSON_REL = (
    "outputs/validation/"
...
TASK_REPORT_REL = (
    "outputs/validation/"
    "cninfo_c_class_fm34_scale_surface_delta_combined_dryrun_cross_identity_partition_codeset_safety_20260715.md"
)
...
def write_reports(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    for key, name in (
        ("dry863_path", "dry863_surface_delta_codeset_freeze_ledger.json"),
        ("combined_dryrun_path", "combined_dryrun_coverage_cardinality_freeze_ledger.json"),
        ("cross_identity_path", "cross_metric_identity_lock_ledger.json"),
        ("partition_codeset_path", "partition_codeset_sha256_lock_ledger.json"),
    ):
    ...
    json_payload = {
        ...
        "cross_identity_path": payload["cross_identity_path"],
    ...
    md_lines = [
        ...
        *layer_lines,
        ...
        "## Scale / safety gain",
        "",
        "- FM33 连续：unique=**2249** · status=**2134/106/9** · Δ12 · coverage=**117** · "
        "resume=**28/1/0** · union/overlap/residual/same-worse fps",
        ...
        "",
        "```",
        f"c_fm_34_scale_surface_delta_combined_dryrun_cross_identity_partition_codeset_safety_gate = "
        f"{payload['gate']}",
        "execute_production_snapshot_rebuild = false",
        "approved_for_snapshot_rebuild = false",
        "cninfo_calls = 0",
    ...
    task_lines = [
        ...
        "## Task",
        "",
        "在 C-FM-33 已 commit 且 EXECUTE 仍 human-held 之上，继续 **非 seal** "
        "规模/安全能力（非 extension↔drift / seal-chain）：**dry863/surface-delta "
        "codeset freeze（Δ2）**、**combined_dryrun_coverage cardinality freeze"
        "（1053）**、**cross_metric_identity_lock**、**partition_codeset_sha256_lock**、"
        "**FM33 连续 + MOCK36 隔离**；产物写入隔离 MOCK36（不覆盖 MOCK3–35）。",
        "",
        "## Capability gain",
        "",
        "1. FM33 packet/fingerprint/gate/ledger 零漂移连续（unique=2249 · 2134/106/9 · "
        "Δ12 · coverage=117 · resume 28/1/0）",
        ...
        "## Next",
        "",
        "- Controller 可 commit 本包（dry863/surface-delta + combined-dryrun + "
        "cross-identity + partition-codeset only）",
    ...
    with open(readme, "w", encoding="utf-8") as fh:
        fh.write(
            "\n".join(
                [
                    "# C-FM-34 mock scale surface-delta combined-dryrun cross-identity partition-codeset safety root",
                    "",
                    f"gate: `{payload['gate']}`",
                    "seal_chain_extended: false",
                    f"scale_tier_count: {payload['scale_tier_count']}",
                    ...
                    f"resume_taxonomy: {payload['resume_improved']}/"
                    f"{payload['resume_same']}/{payload['resume_worse']}",
                    "does_not_overwrite: _mock_c_fm02.._mock_c_fm33 / standard dryrun",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.execute:
        print(
            ...
            "C-FM-34 is offline QA only; KEEP_EXECUTE_FALSE",
            file=sys.stderr,
        )
        return 2

    payload = run_scale_surface_delta_combined_dryrun_cross_identity_partition_codeset_safety(
        paths=SurfaceDeltaCombinedDryrunCrossIdentityPaths(output_root_rel=args.output_root)
    )
