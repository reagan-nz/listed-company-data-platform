"""
CNINFO C-class — 规模 additive/residual/resume/cross-composition-bundle
单测（离线 · CNINFO=0 · C-FM-41）。

运行：
    python3 lab/test_cninfo_c_class_scale_additive_residual_resume_cross_composition_bundle_safety.py
"""
...
from cninfo_c_class_scale_additive_residual_resume_cross_composition_bundle_safety import (  # noqa: E402
    ...
    assert_fm41_output_root,
    build_additive_composition_identity_lock_rows,
    build_cross_composition_bundle_identity_lock_rows,
    build_fm40_continuity_rows,
    build_residual_composition_identity_lock_rows,
    build_resume_composition_identity_lock_rows,
    ...
    fingerprint_additive_composition_identity_lock,
...
_RUNNER = os.path.join(
    _LAB_DIR,
    "run_cninfo_c_class_scale_additive_residual_resume_cross_composition_bundle_safety.py",
)
_TEST_SUMMARY_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_additive_residual_resume_cross_composition_bundle_safety_test_summary_20260715.md"
)
...
def _write_test_summary(cases: list) -> None:
    ...
    lines = [
        "# C-FM-41 Scale Additive/Residual/Resume/Cross-Composition-Bundle — Test Summary",
        "",
        "_offline · CNINFO=0_",
        "",
        "| case | result |",
    ...
    lines.extend(
        [
            "",
            "```",
            "c_fm_41_scale_additive_unique_surface_cross_formula_bundle_safety_test_gate = "
            "PASS_OFFLINE",
            "cninfo_calls = 0",
            ...
            f"additive_composition_sha256 = {EXPECTED_ADDITIVE_COMPOSITION_SHA256}",
            f"residual_composition_sha256 = {EXPECTED_RESIDUAL_COMPOSITION_SHA256}",
            f"resume_composition_sha256 = {EXPECTED_RESUME_COMPOSITION_SHA256}",
...
class TestCFm41AdditiveResidualResumeCrossCompositionBundle(unittest.TestCase):
    def test_constants_frozen(self) -> None:
        ...
        self.assertEqual(len(EXPECTED_CROSS_COMPOSITION_BUNDLE), 5)

    def test_additive_composition_mutation_denied(self) -> None:
        mut = evaluate_additive_composition_mutation(
            proposed_additive=2262, proposed_unique=2249, proposed_overlap=12
        )
        self.assertIs(mut["mutation_allowed"], False)
        _rows, checks, meta = build_additive_composition_identity_lock_rows(
            AdditiveResidualResumeCrossCompositionBundlePaths()
        )
        self.assertTrue(checks["additive_composition_identity_lock_all_pass"])
        self.assertEqual(
            meta["fingerprint"], FROZEN_ADDITIVE_COMPOSITION_IDENTITY_LOCK_FP_SHA256
        )
        fp, _doc = fingerprint_additive_composition_identity_lock()
        self.assertEqual(fp, FROZEN_ADDITIVE_COMPOSITION_IDENTITY_LOCK_FP_SHA256)

    def test_residual_composition_mutation_denied(self) -> None:
        mut = evaluate_residual_composition_mutation(
            proposed_residual=118,
            proposed_partial=106,
            proposed_failed=9,
            proposed_delta_n=2,
        )
        self.assertIs(mut["mutation_allowed"], False)
        _rows, checks, meta = build_residual_composition_identity_lock_rows(
            AdditiveResidualResumeCrossCompositionBundlePaths()
        )
        self.assertTrue(checks["residual_composition_identity_lock_all_pass"])
        self.assertEqual(
            meta["fingerprint"], FROZEN_RESIDUAL_COMPOSITION_IDENTITY_LOCK_FP_SHA256
        )

    def test_resume_composition_mutation_denied(self) -> None:
        mut = evaluate_resume_composition_mutation(
            proposed_total=30, proposed_improved=28, proposed_same=1, proposed_worse=0
        )
        self.assertIs(mut["mutation_allowed"], False)
        _rows, checks, meta = build_resume_composition_identity_lock_rows(
            AdditiveResidualResumeCrossCompositionBundlePaths()
        )
        self.assertTrue(checks["resume_composition_identity_lock_all_pass"])
        self.assertEqual(
            meta["fingerprint"], FROZEN_RESUME_COMPOSITION_IDENTITY_LOCK_FP_SHA256
        )

    def test_cross_composition_bundle_mutation_denied(self) -> None:
        mutated = {k: dict(v) for k, v in EXPECTED_CROSS_COMPOSITION_BUNDLE.items()}
        mutated["additive_composition"] = {
            "formula": "2249+11=2260",
            "composition_sha256": EXPECTED_ADDITIVE_COMPOSITION_SHA256,
        }
        mut = evaluate_cross_composition_bundle_mutation(proposed_compositions=mutated)
        self.assertIs(mut["mutation_allowed"], False)
        _rows, checks, meta = build_cross_composition_bundle_identity_lock_rows(
            AdditiveResidualResumeCrossCompositionBundlePaths()
        )
        self.assertTrue(checks["cross_composition_bundle_identity_lock_all_pass"])
        self.assertEqual(
            meta["fingerprint"], FROZEN_CROSS_COMPOSITION_BUNDLE_IDENTITY_LOCK_FP_SHA256
        )

    def test_fm40_continuity(self) -> None:
        rows, checks = build_fm40_continuity_rows(
            AdditiveResidualResumeCrossCompositionBundlePaths()
        )
        self.assertTrue(checks.get("fm40_continuity_all_pass"), rows[-1])
    ...
    def test_output_root_guards(self) -> None:
        ...
        ok = assert_fm41_output_root(DEFAULT_MOCK_OUTPUT_ROOT_REL)
        self.assertTrue(ok)

    def test_full_offline_run(self) -> None:
        result = run_scale_additive_residual_resume_cross_composition_bundle_safety()
...
if __name__ == "__main__":
    ...
    suite = loader.loadTestsFromTestCase(
        TestCFm41AdditiveResidualResumeCrossCompositionBundle
    )
    ...
    if result.wasSuccessful():
        for name in [
            ...
            "test_additive_composition_mutation_denied",
            "test_residual_composition_mutation_denied",
            "test_resume_composition_mutation_denied",
            "test_cross_composition_bundle_mutation_denied",
            "test_fm40_continuity",
