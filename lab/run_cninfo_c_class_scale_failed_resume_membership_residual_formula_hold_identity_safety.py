...
membership freeze + residual_formula identity lock + hold_decision identity
lock runner（离线 · C-FM-36）。

Usage:
    python3 lab/run_cninfo_c_class_scale_failed_resume_membership_residual_formula_hold_identity_safety.py
    python3 lab/run_cninfo_c_class_scale_failed_resume_membership_residual_formula_hold_identity_safety.py \\
      --output-root outputs/validation/_mock_c_fm36_scale_failed_resume_membership_residual_formula_hold_identity_safety
"""
...
REPORT_ROOT_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_failed_resume_membership_residual_formula_hold_identity_safety"
)
REPORT_MD_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_failed_resume_membership_residual_formula_hold_identity_safety_20260715.md"
)
REPORT_JSON_REL = (
    "outputs/validation/"
...
TASK_REPORT_REL = (
    "outputs/validation/"
    "cninfo_c_class_fm36_scale_failed_resume_membership_residual_formula_hold_identity_safety_20260715.md"
)
...
def write_reports(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    for key, name in (
        ("failed_codes_path", "failed_codes_membership_freeze_ledger.json"),
        ("resume_same_worse_path", "resume_same_worse_membership_freeze_ledger.json"),
        ("residual_formula_path", "residual_formula_identity_lock_ledger.json"),
        ("hold_decision_path", "hold_decision_identity_lock_ledger.json"),
    ):
    ...
    json_payload = {
        ...
        "failed_codes_path": payload["failed_codes_path"],
        "resume_same_worse_path": payload["resume_same_worse_path"],
        "residual_formula_path": payload["residual_formula_path"],
        "hold_decision_path": payload["hold_decision_path"],
    ...
    md_lines = [
        ...
        *layer_lines,
        ...
        "## Scale / safety gain",
        "",
        "- FM35 连续：unique=**2249** · dryrun=**1053** · Δ2 · status=**2134/106/9** · "
        "resume=**28/1/0** · winner/resume-taxonomy/batch/risk fps",
        ...
        "",
        "```",
        f"c_fm_36_scale_failed_resume_membership_residual_formula_hold_identity_safety_gate = "
        f"{payload['gate']}",
        "execute_production_snapshot_rebuild = false",
        "approved_for_snapshot_rebuild = false",
        "cninfo_calls = 0",
    ...
    task_lines = [
        ...
        "## Task",
        "",
        "在 C-FM-35 已 commit 且 EXECUTE 仍 human-held 之上，继续 **非 seal** "
        "规模/安全能力（非 extension↔drift / seal-chain）：**failed_codes membership "
        "freeze（9）**、**resume_same/worse membership freeze（1/0）**、"
        "**residual_formula identity lock（117=106+9+2）**、**hold_decision "
        "identity lock**、**FM35 连续 + MOCK38 隔离**；产物写入隔离 MOCK38"
        "（不覆盖 MOCK3–37）。",
        "",
        "## Capability gain",
        "",
        "1. FM35 packet/fingerprint/gate/ledger 零漂移连续（unique=2249 · 1053 · "
        "Δ2 · 2134/106/9 · resume 28/1/0 · risk 75/14/12/5）",
        ...
        "5. hold_decision_identity_lock：KEEP_EXECUTE_FALSE + AWAITING + "
        "approved=false + seal=false · 翻转拒绝",
        ...
        "## Next",
        "",
        "- Controller 可 commit 本包（failed/resume membership + residual-formula + "
        "hold-identity only）",
    ...
    with open(readme, "w", encoding="utf-8") as fh:
        fh.write(
            "\n".join(
                [
                    "# C-FM-36 mock scale failed/resume membership residual-formula hold-identity safety root",
                    "",
                    f"gate: `{payload['gate']}`",
                    "seal_chain_extended: false",
                    f"scale_tier_count: {payload['scale_tier_count']}",
                    ...
                    f"resume_taxonomy: {payload['resume_improved']}/"
                    f"{payload['resume_same']}/{payload['resume_worse']}",
                    "does_not_overwrite: _mock_c_fm02.._mock_c_fm35 / standard dryrun",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "C-FM-36 scale failed/resume membership + residual-formula + "
            "hold-identity safety (offline)"
        )
    )
    parser.add_argument(
        "--output-root",
    ...
    if args.execute:
        print(
            ...
            "C-FM-36 is offline QA only; KEEP_EXECUTE_FALSE",
            file=sys.stderr,
        )
        return 2

    payload = run_scale_failed_resume_membership_residual_formula_hold_identity_safety(
        paths=FailedResumeMembershipResidualFormulaHoldIdentityPaths(
