#!/usr/bin/env python3
"""
CNINFO C-class — 非 seal Cross-FM 二次扩展 post-commit 漂移复核 runner（离线 · C-FM-19）。

Usage:
    python3 lab/run_cninfo_c_class_nonseal_second_extension_post_commit_drift_recheck.py
    python3 lab/run_cninfo_c_class_nonseal_second_extension_post_commit_drift_recheck.py \\
      --output-root outputs/validation/_mock_c_fm19_nonseal_second_extension_post_commit_drift_recheck
"""
...
REPORT_ROOT_REL = (
    "outputs/validation/"
    "cninfo_c_class_nonseal_second_extension_post_commit_drift_recheck"
)
REPORT_MD_REL = (
    "outputs/validation/"
    "cninfo_c_class_nonseal_second_extension_post_commit_drift_recheck_20260715.md"
)
REPORT_JSON_REL = (
    "outputs/validation/"
...
TASK_REPORT_REL = (
    "outputs/validation/"
    "cninfo_c_class_fm19_nonseal_second_extension_post_commit_drift_recheck_20260715.md"
)
...
def write_reports(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    json_payload = {
        ...
        "frozen_second_extension_fp_sha256": payload[
            "frozen_second_extension_fp_sha256"
        ],
        "recomputed_second_extension_fp_sha256": payload[
            "recomputed_second_extension_fp_sha256"
        ],
    ...
    md_lines = [
        ...
        *layer_lines,
        ...
        f"| frozen second-extension fp | `{payload['frozen_second_extension_fp_sha256']}` |",
        f"| recomputed second-extension fp | `{payload['recomputed_second_extension_fp_sha256']}` |",
    ...
    md_lines.extend(
        [
            ...
            "",
            "```",
            f"c_fm_19_nonseal_second_extension_post_commit_drift_recheck_gate = {payload['gate']}",
            "execute_production_snapshot_rebuild = false",
            "approved_for_snapshot_rebuild = false",
            "cninfo_calls = 0",
            "ready_for_execute = false",
    ...
    task_lines = [
        ...
        "## Task",
        "",
        "在 C-FM-18 已 commit 且 EXECUTE 仍 human-held 之上，补齐 **非 seal-chain** 能力："
        "**Cross-FM mock cohort 二次扩展 post-commit 漂移复核**、"
        "**MOCK20 冻结产物零漂移**、**MOCK3–20 写隔离**；"
        "产物写入隔离 MOCK21（不覆盖 MOCK3–20；不新增 seal / decision-await 层）。",
        "",
        "## Capability gain",
        "",
    ...
    with open(readme, "w", encoding="utf-8") as fh:
        fh.write(
            "\n".join(
                [
                    "# C-FM-19 mock non-seal second-extension post-commit drift recheck root",
                    "",
                    f"gate: `{payload['gate']}`",
                    "seal_chain_extended: false",
                    "does_not_overwrite: _mock_c_fm02.._mock_c_fm18 / standard dryrun",
...
def main(argv: Any = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "C-FM-19 non-seal second-extension post-commit drift recheck QA (offline)"
        )
    )
    parser.add_argument(
        "--output-root",
        default=DEFAULT_MOCK_OUTPUT_ROOT_REL,
        help="隔离 mock 写根（须 validation/_mock_*；不得覆盖 MOCK3–20）",
    )
    parser.add_argument(
        "--execute",
        action="store_true",
    ...
    paths = SecondExtensionDriftPaths(output_root_rel=args.output_root)
    payload = run_nonseal_second_extension_post_commit_drift_recheck(paths=paths)
