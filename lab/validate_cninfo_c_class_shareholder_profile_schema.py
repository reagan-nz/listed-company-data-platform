"""
CNINFO C-class shareholder_profile fixture schema validation (Era C Phase 4).

Validates fixtures/c_class/shareholder_profile/shareholder_profile_fixtures.jsonl
against schemas/c_class/c_shareholder_profile.schema.json.

No network, no database, no verified.

Usage:
    python lab/validate_cninfo_c_class_shareholder_profile_schema.py
"""
...
DEFAULT_SCHEMA = os.path.join(
    BASE_DIR, "schemas", "c_class", "c_shareholder_profile.schema.json"
)
DEFAULT_CSV = os.path.join(
    BASE_DIR, "outputs", "validation", "cninfo_c_class_shareholder_profile_schema_validation_report.csv"
)
DEFAULT_MD = os.path.join(
    BASE_DIR, "outputs", "validation", "cninfo_c_class_shareholder_profile_schema_validation_summary.md"
)
...
def run_validation(fixtures_path: str, schema_path: str) -> List[Dict[str, str]]:
    ...
    for idx, record in enumerate(records):
        ...
        out.append({
            ...
            "shareholder_scope": str(record.get("shareholder_scope", "")),
            "source_id": str(record.get("source_id", "")),
            "report_period": str(record.get("report_period", "")),
            "rank": str(record.get("rank", "")),
            "shareholder_name": str(record.get("shareholder_name", "")),
    ...
    if fail_rows:
        for r in fail_rows:
            lines.append(
                ...
                f"{r['shareholder_scope']} rank={r['rank']}: {r['error_message']}"
            )
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate C-class shareholder_profile fixtures")
...
def main() -> None:
    ...
    write_summary_md(args.output_md, rows)
