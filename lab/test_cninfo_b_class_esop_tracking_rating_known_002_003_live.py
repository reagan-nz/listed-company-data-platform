"""
B-FM-46：员工持股计划 known_002 / 跟踪评级 known_003 live allow-list 锁测（离线）。

覆盖：
- allow-list 仅含 employee_stock_ownership_plan_known_002 与
  tracking_rating_report_known_003（category 空）
- 字段与 fixtures ready 对齐
- mock live 路径可对两案得到 pass（predicted_document_type=announcement；非 other）
- 不包含 ESOP/tracking known_001/002 及 B-FM-45 等已 LIVE_PASS 案

无真实 CNINFO · 不造 §7 FP。

运行：
    python lab/test_cninfo_b_class_esop_tracking_rating_known_002_003_live.py
"""
...
LIVE_DIR = os.path.join(
    ...
    "outputs",
    "validation",
    "cninfo_b_class_esop_tracking_rating_known_002_003_live_20260715",
)
...
HARVEST_ESOP = "2025年员工持股计划第一次持有人会议决议公告"
HARVEST_RATING = (
    "中国宝安集团股份有限公司2022年面向合格投资者公开发行公司债券(第一期)定期跟踪评级报告"
)
ANN_ESOP = "1223951294"
ANN_RATING = "1223886549"
# 2025-06-20 / 2025-06-16 00:00:00 UTC
ANN_TS_ESOP_MS = 1750377600000
ANN_TS_RATING_MS = 1750032000000
...
class TestEsopTrackingRatingKnown002003LiveAllowList(unittest.TestCase):
