"""
CNINFO C-class — 规模 unique_surface_additive/overlap_delta/surface_delta/repro-wall-meta-bundle
单测（离线 · CNINFO=0 · C-FM-52）。

运行：
    python3 lab/test_cninfo_c_class_scale_full_market_unique_surface_additive_overlap_delta_surface_delta_wall_meta_bundle_safety.py
"""
...
from cninfo_c_class_scale_full_market_unique_surface_additive_overlap_delta_surface_delta_wall_meta_bundle_safety import (  # noqa: E402
    ...
    EXPECTED_SURFACE_DELTA_PRIMARY_COMPOSITION_SHA256,
    EXPECTED_SURFACE_DELTA_PRIMARY_FORMULA,
    EXPECTED_COMPANY_COVERAGE_SUM,
    EXPECTED_CROSS_FULL_MARKET_UNIQUE_SURFACE_ADDITIVE_OVERLAP_DELTA_SURFACE_DELTA_WALL_META_BUNDLE,
    EXPECTED_CROSS_FULL_MARKET_UNIQUE_SURFACE_ADDITIVE_OVERLAP_DELTA_SURFACE_DELTA_WALL_META_BUNDLE_SHA256,
    ...
    EXPECTED_UNIQUE_SURFACE_ADDITIVE_PRIMARY_COMPOSITION_SHA256,
    EXPECTED_UNIQUE_SURFACE_ADDITIVE_PRIMARY_FORMULA,
    EXPECTED_OVERLAP_DELTA_PRIMARY_COMPOSITION_SHA256,
    EXPECTED_OVERLAP_DELTA_PRIMARY_FORMULA,
    ...
    FROZEN_CROSS_FULL_MARKET_UNIQUE_SURFACE_ADDITIVE_OVERLAP_DELTA_SURFACE_DELTA_WALL_META_BUNDLE_IDENTITY_LOCK_FP_SHA256,
    ...
    THIS_TASK_ROOT_ID,
    assert_fm52_output_root,
    build_surface_delta_composition_identity_lock_rows,
    build_cross_full_market_unique_surface_additive_overlap_delta_surface_delta_wall_meta_bundle_identity_lock_rows,
    build_fm51_continuity_rows,
    build_unique_surface_additive_composition_identity_lock_rows,
    build_overlap_delta_composition_identity_lock_rows,
...
_RUNNER = os.path.join(
    _LAB_DIR,
    "run_cninfo_c_class_scale_full_market_unique_surface_additive_overlap_delta_surface_delta_wall_meta_bundle_safety.py",
)
_TEST_SUMMARY_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_full_market_unique_surface_additive_overlap_delta_surface_delta_wall_meta_bundle_safety_test_summary_20260716.md"
)
...
def _write_test_summary(cases: list) -> None:
    ...
    lines = [
        "# C-FM-52 Scale Unique-Surface-Additive/Overlap-Delta/Surface-Delta-Wall-Meta-Bundle — Test Summary",
        "",
        "_offline · CNINFO=0_",
        "",
        "| case | result |",
    ...
    lines.extend(
        [
            "",
            "```",
            "c_fm_52_scale_full_market_unique_surface_additive_overlap_delta_surface_delta_wall_meta_bundle_safety_test_gate = "
            "PASS_OFFLINE",
            "cninfo_calls = 0",
            ...
            f"unique_surface_additive_formula = {EXPECTED_UNIQUE_SURFACE_ADDITIVE_PRIMARY_FORMULA}",
            f"overlap_delta_formula = {EXPECTED_OVERLAP_DELTA_PRIMARY_FORMULA}",
            f"surface_delta_formula = {EXPECTED_SURFACE_DELTA_PRIMARY_FORMULA}",
            ...
            f"unique_surface_additive_composition_sha256 = {EXPECTED_UNIQUE_SURFACE_ADDITIVE_PRIMARY_COMPOSITION_SHA256}",
            f"overlap_delta_composition_sha256 = {EXPECTED_OVERLAP_DELTA_PRIMARY_COMPOSITION_SHA256}",
            f"surface_delta_composition_sha256 = {EXPECTED_SURFACE_DELTA_PRIMARY_COMPOSITION_SHA256}",
            f"cross_full_market_unique_surface_additive_overlap_delta_surface_delta_wall_meta_bundle_sha256 = "
            f"{EXPECTED_CROSS_FULL_MARKET_UNIQUE_SURFACE_ADDITIVE_OVERLAP_DELTA_SURFACE_DELTA_WALL_META_BUNDLE_SHA256}",
            "```",
            "",
        ]
    )
...
class TestCFm52FullMarketUniqueSurfaceAdditiveOverlapDeltaSurfaceDeltaWallMetaBundle(unittest.TestCase):
    def test_constants_frozen(self) -> None:
        ...
        self.assertEqual(EXPECTED_UNIQUE_SURFACE_ADDITIVE_PRIMARY_FORMULA, "2249/2251/2261")
        self.assertEqual(EXPECTED_OVERLAP_DELTA_PRIMARY_FORMULA, "overlap_delta=12")
        self.assertEqual(EXPECTED_SURFACE_DELTA_PRIMARY_FORMULA, "surface_delta=2")
        ...
        self.assertEqual(
            len(EXPECTED_CROSS_FULL_MARKET_UNIQUE_SURFACE_ADDITIVE_OVERLAP_DELTA_SURFACE_DELTA_WALL_META_BUNDLE), 5
        )
    ...
    def test_unique_surface_additive_composition_mutation_denied(self) -> None:
        ...
        self.assertTrue(checks["unique_surface_additive_composition_identity_lock_all_pass"])
        self.assertEqual(
            meta["fingerprint"], FROZEN_UNIQUE_SURFACE_ADDITIVE_COMPOSITION_IDENTITY_LOCK_FP_SHA256
        )

    def test_overlap_delta_composition_mutation_denied(self) -> None:
        mut = evaluate_overlap_delta_composition_mutation(
            proposed_overlap_delta=EXPECTED_OVERLAP_DELTA + 1,
            proposed_unique=EXPECTED_HARVEST_UNIQUE_UNION,
            proposed_additive=EXPECTED_HARVEST_ADDITIVE,
        )
        self.assertIs(mut["mutation_allowed"], False)
        _rows, checks, meta = build_overlap_delta_composition_identity_lock_rows(
            FullMarketUniqueSurfaceAdditiveOverlapDeltaSurfaceDeltaWallMetaBundlePaths()
        )
        self.assertTrue(checks["overlap_delta_composition_identity_lock_all_pass"])
        self.assertEqual(
            meta["fingerprint"], FROZEN_OVERLAP_DELTA_COMPOSITION_IDENTITY_LOCK_FP_SHA256
        )

    def test_surface_delta_composition_mutation_denied(self) -> None:
        mut = evaluate_surface_delta_composition_mutation(
            proposed_delta_n=EXPECTED_SURFACE_HARVEST_DELTA_N + 1,
            proposed_unique=EXPECTED_HARVEST_UNIQUE_UNION,
            proposed_surface=EXPECTED_SURFACE_UNIQUE,
        )
        self.assertIs(mut["mutation_allowed"], False)
        _rows, checks, meta = build_surface_delta_composition_identity_lock_rows(
            FullMarketUniqueSurfaceAdditiveOverlapDeltaSurfaceDeltaWallMetaBundlePaths()
        )
        self.assertTrue(checks["surface_delta_composition_identity_lock_all_pass"])
        self.assertEqual(
            meta["fingerprint"], FROZEN_SURFACE_DELTA_COMPOSITION_IDENTITY_LOCK_FP_SHA256
        )

    def test_wall_meta_bundle_mutation_denied(self) -> None:
        mut = evaluate_cross_full_market_unique_surface_additive_overlap_delta_surface_delta_wall_meta_bundle_mutation(
            proposed_compositions={}
        )
        self.assertIs(mut["mutation_allowed"], False)
        _rows, checks, meta = build_cross_full_market_unique_surface_additive_overlap_delta_surface_delta_wall_meta_bundle_identity_lock_rows(
            FullMarketUniqueSurfaceAdditiveOverlapDeltaSurfaceDeltaWallMetaBundlePaths()
        )
        self.assertTrue(
            checks[
                "cross_full_market_unique_surface_additive_overlap_delta_surface_delta_wall_meta_bundle_identity_lock_all_pass"
            ]
        )
        self.assertEqual(
            meta["fingerprint"],
    ...
    def test_fm51_continuity(self) -> None:
        rows, checks = build_fm51_continuity_rows(
            FullMarketUniqueSurfaceAdditiveOverlapDeltaSurfaceDeltaWallMetaBundlePaths()
        )
        self.assertTrue(checks["fm51_continuity_all_pass"])
        self.assertGreaterEqual(len(rows), 7)

    def test_output_root_guards(self) -> None:
        with self.assertRaises(RuntimeError):
            assert_fm52_output_root(
                "outputs/validation/_mock_c_fm51_scale_full_market_residual_formula_resume_formula_coverage_wall_meta_bundle_safety"
            )
        with self.assertRaises(RuntimeError):
            assert_fm52_output_root(
                "outputs/validation/_mock_c_fm50_scale_full_market_risk_band_status_tier_coverage_combined_dryrun_wall_meta_bundle_safety"
            )
        ok = assert_fm52_output_root(DEFAULT_MOCK_OUTPUT_ROOT_REL)
...
if __name__ == "__main__":
    suite = unittest.defaultTestLoader.loadTestsFromTestCase(
        TestCFm52FullMarketUniqueSurfaceAdditiveOverlapDeltaSurfaceDeltaWallMetaBundle
    )
    ...
    names = [
        ...
        "test_overlap_delta_composition_mutation_denied",
        "test_surface_delta_composition_mutation_denied",
        "test_wall_meta_bundle_mutation_denied",
        "test_fm51_continuity",
