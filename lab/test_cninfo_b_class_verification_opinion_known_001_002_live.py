"""
B-FM-27：核查意见 known-document live allow-list 锁测（离线；不请求 CNINFO）。

覆盖：
- allow-list 仅含 verification_opinion_known_001 与 known_002（category 空）
- 字段与 fixtures ready 对齐
- mock live 路径可对两案得到 pass（predicted_document_type=announcement；非 other）
- 不包含 legal_opinion / supervisory / shareholder 已 LIVE_PASS 案

无真实 CNINFO · 不造 §7 FP。

运行：
    python lab/test_cninfo_b_class_verification_opinion_known_001_002_live.py
"""
...
LIVE_DIR = os.path.join(
    ...
    "outputs",
    "validation",
    "cninfo_b_class_verification_opinion_known_001_002_live_20260715",
)
...
ANN_001 = "1224013030"
ANN_002 = "1223974498"
# 2025-06-27 / 2025-06-24 00:00:00 UTC
ANN_TS_001_MS = 1750982400000
ANN_TS_002_MS = 1750723200000
...
class TestVerificationOpinionKnown001002LiveAllowList(unittest.TestCase):
    def test_allowlist_scope_exactly_two_known_cases(self) -> None:
        ...
        self.assertEqual(
            ...
            ["verification_opinion_known_001", "verification_opinion_known_002"],
        )
    ...
    def test_allowlist_fields_match_fixtures(self) -> None:
        ...
        for case_id in (
            ...
            self.assertIn("核查意见", ak["title_pattern"])
