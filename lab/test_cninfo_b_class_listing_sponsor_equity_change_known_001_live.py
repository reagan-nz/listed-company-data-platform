"""
B-FM-28：上市保荐书 / 权益变动报告书 known-document live allow-list 锁测（离线；不请求 CNINFO）。

覆盖：
- allow-list 仅含 listing_sponsor_known_001 与 equity_change_report_known_001（category 空）
- 字段与 fixtures ready 对齐
- mock live 路径可对两案得到 pass（predicted_document_type=announcement；非 other）
- 不包含 verification_opinion / legal_opinion / supervisory / shareholder 已 LIVE_PASS 案

无真实 CNINFO · 不造 §7 FP。

运行：
    python lab/test_cninfo_b_class_listing_sponsor_equity_change_known_001_live.py
"""
...
LIVE_DIR = os.path.join(
    ...
    "outputs",
    "validation",
    "cninfo_b_class_listing_sponsor_equity_change_known_001_live_20260715",
)
...
ANN_SPONSOR = "1223824291"
ANN_EQUITY = "1223980482"
# 2025-06-09 / 2025-06-25 00:00:00 UTC
ANN_TS_SPONSOR_MS = 1749427200000
ANN_TS_EQUITY_MS = 1750809600000
...
class TestListingSponsorEquityChangeKnown001LiveAllowList(unittest.TestCase):
    def test_allowlist_scope_exactly_two_known_cases(self) -> None:
        ...
        self.assertEqual(
            ...
            ["listing_sponsor_known_001", "equity_change_report_known_001"],
        )
