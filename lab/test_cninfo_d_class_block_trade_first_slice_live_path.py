"""
D-class block_trade first-slice live path 测试（mock CNINFO · 不执行真实 live）。

运行：
    python lab/test_cninfo_d_class_block_trade_first_slice_live_path.py
"""
...
def _mock_cninfo_block_trade(
    ...
    code = runner.BLOCK_TRADE_FIRST_SLICE_EXPECTED_COMPANY_CODES.get(case_id, "000895")
    return (
        {
            "data": {
                "records": [
                    {"SECCODE": code, "SECNAME": "mock", "TRADEDATE": "2026-07-03"}
                ]
            }
        },
        200,
...
def _run_live_inprocess(
    ...
    with mock.patch.object(
        runner, "_cninfo_request", side_effect=_mock_cninfo_block_trade
    ) as mock_req, mock.patch("time.sleep"):
        rc = runner.main(_live_args(root, extra=extra))
    return rc, mock_req, root


class TestBlockTradeFirstSliceLivePath(unittest.TestCase):
    @classmethod
    ...
    def test_wrong_approval_flag_rejected_before_cninfo(self) -> None:
        with mock.patch("requests.get") as get_mock, mock.patch("requests.post") as post_mock:
            result = _run(
                _live_args(OUTPUT_ROOT, extra=["--approve-d-class-margin-trading-first-slice"])
            )
    ...
    def test_live_path_function_exists_and_wired(self) -> None:
        ...
        self.assertEqual(called_cases, runner.BLOCK_TRADE_FIRST_SLICE_ALLOWED_CASE_IDS)
    ...
    def test_component_must_be_block_trade(self) -> None:
        rows = _read_universe_rows()
        rows = [
            {**r, "component": "margin_trading"} if r["case_id"] == "DBT002" else r
            for r in rows
        ]
        with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_688671_rejected(self) -> None:
        rows = _read_universe_rows()
        rows = [
            {**r, "company_code": "688671"} if r["case_id"] == "DBT003" else r
            for r in rows
        ]
        with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_301259_rejected(self) -> None:
        rows = _read_universe_rows()
        rows = [
            {**r, "company_code": "301259"} if r["case_id"] == "DBT004" else r
            for r in rows
        ]
        with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_known_event_margin_and_disclosure_roots_write_blocked(self) -> None:
    ...
    def test_request_cap_lte_20_in_planning(self) -> None:
        ...
        for r in rows:
            ...
            total += runner.compute_block_trade_first_slice_planned_requests(row)
        self.assertLessEqual(total, runner.BLOCK_TRADE_FIRST_SLICE_TOTAL_MAX_REQUESTS)

    def test_empty_but_valid_semantics_dbt001(self) -> None:
        row = runner.BlockTradeFirstSliceRow(
            case_id="DBT001",
            company_code="601988",
            company_name="中国银行",
            ...
            reason="DLC002-style control",
            dlc002_reference="yes",
        )
        summary = {
        ...
        self.assertTrue(runner.is_block_trade_first_slice_acceptable(row, summary))

    def test_captured_normal_candidate_semantics_dbt002(self) -> None:
        row = runner.BlockTradeFirstSliceRow(
            case_id="DBT002",
            company_code="000895",
            company_name="双汇发展",
            ...
            reason="captured normal candidate",
            dlc002_reference="no",
        )
        summary = {
