"""
B-FM-43：持续督导年度 / 公司章程 known_002 live allow-list 锁测（离线；不请求 CNINFO）。

覆盖：
- allow-list 仅含 continuous_supervision_annual_known_002 与 company_articles_known_002
  （category 空）
- 字段与 fixtures ready 对齐
- mock live 路径可对两案得到 pass（predicted_document_type=announcement；
  非 annual_report / other）
- 不包含 continuous_supervision / company_articles known_001 及更早 LIVE_PASS 案
  （含 B-FM-42）

无真实 CNINFO · 不造 §7 FP。

运行：
    python lab/test_cninfo_b_class_supervision_articles_known_002_live.py
"""
...
LIVE_DIR = os.path.join(
    ...
    "outputs",
    "validation",
    "cninfo_b_class_supervision_articles_known_002_live_20260715",
)
...
HARVEST_ARTICLES = "江苏秀强玻璃工艺股份有限公司章程（2025年6月修订）"
ANN_CSA = "1223348104"
ANN_ARTICLES = "1223952124"
# 2025-04-27 / 2025-06-20 00:00:00 UTC
ANN_TS_CSA_MS = 1745712000000
ANN_TS_ARTICLES_MS = 1750377600000
...
class TestSupervisionArticlesKnown002LiveAllowList(unittest.TestCase):
