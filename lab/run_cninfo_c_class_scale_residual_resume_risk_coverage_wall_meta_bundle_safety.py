...
identity lock runner（离线 · C-FM-44）。

Usage:
    python3 lab/run_cninfo_c_class_scale_residual_resume_risk_coverage_wall_meta_bundle_safety.py
    python3 lab/run_cninfo_c_class_scale_residual_resume_risk_coverage_wall_meta_bundle_safety.py \\
      --output-root outputs/validation/_mock_c_fm44_scale_residual_resume_risk_coverage_wall_meta_bundle_safety
"""
...
REPORT_ROOT_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_residual_resume_risk_coverage_wall_meta_bundle_safety"
)
REPORT_MD_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_residual_resume_risk_coverage_wall_meta_bundle_safety_20260715.md"
)
REPORT_JSON_REL = (
    "outputs/validation/"
...
TASK_REPORT_REL = (
    "outputs/validation/"
    "cninfo_c_class_fm44_scale_residual_resume_risk_coverage_wall_meta_bundle_safety_20260715.md"
)
...
def write_reports(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    for key, name in (
        ...
        (
            "cross_residual_resume_risk_coverage_wall_meta_bundle_path",
    ...
    json_payload = {
        ...
        "residual_formula_composition_sha256": payload[
            "residual_formula_composition_sha256"
        ],
        "resume_taxonomy_composition_sha256": payload[
            "resume_taxonomy_composition_sha256"
        ],
        "risk_band_status_composition_sha256": payload[
            "risk_band_status_composition_sha256"
        ],
        "cross_residual_resume_risk_coverage_wall_meta_bundle_sha256": payload[
            "cross_residual_resume_risk_coverage_wall_meta_bundle_sha256"
        ],
        "cross_stable_metrics_wall_meta_bundle_sha256": payload[
        ...
        "residual_formula_path": payload["residual_formula_path"],
        "resume_taxonomy_path": payload["resume_taxonomy_path"],
        "risk_band_status_path": payload["risk_band_status_path"],
        "cross_residual_resume_risk_coverage_wall_meta_bundle_path": payload[
            "cross_residual_resume_risk_coverage_wall_meta_bundle_path"
        ],
    ...
    md_lines = [
        ...
        f"| resume_taxonomy_formula | **{payload['resume_taxonomy_formula']}** |",
        f"| risk_band_status_formula | **{payload['risk_band_status_formula']}** |",
        f"| coverage_formula | **{payload['coverage_formula']}** |",
        ...
        *layer_lines,
        ...
        "## Scale / safety gain",
        "",
        "- FM43 连续：unique=**2249** · dryrun=**1053** · Δ2 · status=**2134/106/9** · "
        "resume=**28/1/0** · residual=**117** · risk=**75/14/12/5** · wall_meta",
        "- residual_formula_composition_identity_lock：**106+9+2=117**",
        "- resume_taxonomy_composition_identity_lock：**28/1/0**",
        "- risk_band_status_composition_identity_lock：**75/14/12/5**",
        "- cross_residual_resume_risk_coverage_wall_meta_bundle_identity_lock："
        "残差/resume/风险/覆盖墙元捆绑身份锁",
        "- MOCK3–45 冻结 · MOCK46 放行",
        "",
        "## Hold",
        "",
        "```",
        f"c_fm_44_scale_residual_resume_risk_coverage_wall_meta_bundle_safety_gate = "
        f"{payload['gate']}",
        "execute_production_snapshot_rebuild = false",
        "approved_for_snapshot_rebuild = false",
        "cninfo_calls = 0",
    ...
    task_lines = [
        ...
        "## Task",
        "",
        "在 C-FM-43 已 commit 且 EXECUTE 仍 human-held 之上，继续 **非 seal** "
        "规模/安全能力（非 extension↔drift / seal-chain）：**residual_formula "
        "composition identity lock（106+9+2=117）**、**resume_taxonomy composition "
        "identity lock（28/1/0）**、**risk_band_status composition identity "
        "lock（75/14/12/5）**、**cross_residual_resume_risk_coverage_wall_meta_bundle "
        "identity lock（残差/resume/风险/覆盖墙元捆绑）**、**FM43 连续 + MOCK46 隔离**；"
        "产物写入隔离 MOCK46（不覆盖 MOCK3–45）。",
        "",
        "## Capability gain",
        "",
        "1. FM43 packet/fingerprint/gate/ledger 零漂移连续（unique=2249 · 1053 · "
        "Δ2 · 2134/106/9 · resume 28/1/0 · residual 117 · risk 75/14/12/5 · wall_meta）",
        ...
        "5. cross_residual_resume_risk_coverage_wall_meta_bundle_identity_lock："
        "残差/resume/风险/覆盖墙元捆绑 · 组成变异拒绝",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.execute:
        print(
            ...
            "C-FM-44 is offline QA only; KEEP_EXECUTE_FALSE",
            file=sys.stderr,
        )
        return 2

    payload = run_scale_residual_resume_risk_coverage_wall_meta_bundle_safety(
