...
- 分子公司管理制度 / 薪酬 / 名单 / 简报 / ESG 仍落 other（货币资金/对外担保管理制度由 B-FM-38 承接）
...
BARE_ZHIDU = "分、子公司管理制度（2025年6月）"  # 分子公司管理制度仍 other（B-FM-38 未推）
...
class TestArticlesRaisedFundsSystemRoutingEdge(unittest.TestCase):
    ...
    def test_remaining_low_value_edges_still_other(self) -> None:
        """薪酬/名单/简报/ESG 仍落 other（本包不硬推）。"""
        for title in (XINCHOU, MINGDAN, JIANBAO, ESG):
