"""
CNINFO C-class — 规模 dry863/surface-harvest-delta codeset freeze + combined
dryrun coverage cardinality freeze + cross-metric identity lock + partition
codeset SHA256 lock（离线 · C-FM-34）。

在 C-FM-33（union status partition cardinality freeze + overlap_delta
cardinality freeze + residual_safety_coverage lock + resume_same/worse
write-boundary）已 commit 且 EXECUTE 仍 human-held 之上，继续非 seal 规模/
安全能力（不新增 seal / decision-await / commit-boundary；非 extension↔drift
循环）：
  1) FM33 packet / fingerprint / gate / union·overlap·residual·same-worse
     ledgers 零漂移连续
  2) dry863 extras / surface_harvest_delta codeset freeze：Δ2（000037/000055）
     禁止 inject/drop / delta 基数变异
  3) combined_dryrun_coverage cardinality freeze：1053 禁止 inflate/deflate
  4) cross_metric_identity_lock：unique=partition；surface=unique+delta；
     additive=unique+overlap；residual=failed+partial+delta
  5) partition_codeset_sha256_lock：complete/partial/failed SHA256 禁止变异
  6) output-root：MOCK3–35 冻结 · MOCK36 放行
  7) FM-01..05 + FM-12..33 gate battery（跳过 seal FM06–11）

禁止：CNINFO live · production EXECUTE · 覆盖 MOCK3–35 / 权威 dual-layer 索引 ·
...
FM33_PACKET_REL = f"{FM33_MOCK_ROOT_REL}/scale_packet.json"
FM33_FINGERPRINT_REL = f"{FM33_MOCK_ROOT_REL}/scale_fingerprint.json"
FM33_UNION_REL = (
    f"{FM33_MOCK_ROOT_REL}/union_status_partition_cardinality_freeze_ledger.json"
)
FM33_OVERLAP_REL = (
    f"{FM33_MOCK_ROOT_REL}/overlap_delta_cardinality_freeze_ledger.json"
)
FM33_RESIDUAL_REL = (
    f"{FM33_MOCK_ROOT_REL}/residual_safety_coverage_lock_ledger.json"
)
FM33_RESUME_SAME_WORSE_REL = (
    f"{FM33_MOCK_ROOT_REL}/resume_same_worse_write_boundary_ledger.json"
)

# C-FM-34 本包冻结指纹
FROZEN_DRY863_SURFACE_DELTA_CODESET_FP_SHA256 = (
    "2c74735022d758c306c53c1510152fde20f99301abb338329b0cb43874858e64"
)
FROZEN_COMBINED_DRYRUN_COVERAGE_CARDINALITY_FP_SHA256 = (
    "354363f4f2c0809cdb0720ac5a366746ef8fa326fa6102cd0f451e2142b07f39"
)
FROZEN_CROSS_METRIC_IDENTITY_LOCK_FP_SHA256 = (
    "7e2d3d3a517e09e8eac77204c1b0832e87d001f8a3d2cdbd8bbee51d95109842"
)
FROZEN_PARTITION_CODESET_SHA256_LOCK_FP_SHA256 = (
    "44c1f92225b49df762e458cd5b1617888aa873b3e5723191bab4dd1927e47ade"
)
...
THIS_TASK_ROOT_ID = "C-ROOT-MOCK36"
PRIOR_TASK_ROOT_ID = "C-ROOT-MOCK35"
RESUME_HARVEST_ROOT_ID = "C-ROOT-002"

FROZEN_ROOT_IDS_MUST_BLOCK = tuple(f"C-ROOT-MOCK{i}" for i in range(3, 36))
...
class SurfaceDeltaCombinedDryrunCrossIdentityPaths:
    """只读输入与隔离写根路径规格。"""
    ...
    fm33_packet_rel: str = FM33_PACKET_REL
    fm33_fingerprint_rel: str = FM33_FINGERPRINT_REL
    fm33_union_rel: str = FM33_UNION_REL
    fm33_overlap_rel: str = FM33_OVERLAP_REL
    fm33_residual_rel: str = FM33_RESIDUAL_REL
    fm33_resume_same_worse_rel: str = FM33_RESUME_SAME_WORSE_REL
...
def assert_fm34_output_root(
    output_root: str, *, base_dir: str = BASE_DIR
) -> str:
    """
    C-FM-34 写根：须 validation/_mock_*，不得覆盖 MOCK3–35，
    不得写权威 dual-layer 索引；允许本任务 MOCK36 或未登记 ephemeral。
    """
    ...
    if root_id is not None and root_id != THIS_TASK_ROOT_ID:
        raise RuntimeError(
            ...
            f"(C-FM-34 only writes {THIS_TASK_ROOT_ID} or ephemeral _mock_*)"
        )
...
def fingerprint_dry863_surface_delta_codeset_freeze() -> Tuple[str, Dict[str, Any]]:
    """dry863 extras / surface_harvest_delta codeset freeze 指纹。"""
    doc = {
        "kind": "dry863_surface_delta_codeset_freeze",
        "surface_harvest_delta_n": EXPECTED_SURFACE_HARVEST_DELTA_N,
        "dry863_extras": sorted(EXPECTED_DRY863_EXTRA),
        "deny_extras_inject": True,
        "deny_extras_drop": True,
        "deny_delta_cardinality_mutation": True,
...
def fingerprint_combined_dryrun_coverage_cardinality_freeze() -> Tuple[str, Dict[str, Any]]:
    """combined_dryrun_coverage cardinality freeze 指纹。"""
    doc = {
        "kind": "combined_dryrun_coverage_cardinality_freeze",
...
def fingerprint_cross_metric_identity_lock() -> Tuple[str, Dict[str, Any]]:
    """cross-metric identity lock 指纹。"""
    doc = {
        "kind": "cross_metric_identity_lock",
        "partition_sum_eq_unique": True,
        "surface_eq_unique_plus_delta": True,
        "additive_eq_unique_plus_overlap": True,
        "residual_eq_failed_plus_partial_plus_delta": True,
...
def fingerprint_partition_codeset_sha256_lock() -> Tuple[str, Dict[str, Any]]:
    """partition codeset SHA256 lock 指纹。"""
    doc = {
        "kind": "partition_codeset_sha256_lock",
        ...
        "deny_complete_codeset_mutation": True,
        "deny_partial_codeset_mutation": True,
        "deny_failed_codeset_mutation": True,
        "hold": "KEEP_EXECUTE_FALSE",
    }
    fp = hashlib.sha256(
        json.dumps(doc, ensure_ascii=False, sort_keys=True).encode("utf-8")
...
def evaluate_dry863_extras_mutation(
    *, proposed_extras: Sequence[str], action: str
) -> Dict[str, Any]:
    """评估 dry863 extras 变异；inject/drop 一律拒绝。"""
    if action not in ("inject", "drop", "replace"):
        raise ValueError(f"unknown dry863 action: {action}")
    proposed = sorted(proposed_extras)
    frozen = sorted(EXPECTED_DRY863_EXTRA)
    return {
        "action": action,
        "proposed_extras": proposed,
        "frozen_extras": frozen,
        "matches_frozen": proposed == frozen,
        "mutation_allowed": False,
        "reason": "dry863_surface_delta_codeset_frozen_pre_execute",
        "hold": "KEEP_EXECUTE_FALSE",
    }
...
def evaluate_combined_dryrun_coverage_mutation(
    *, proposed_coverage: int
    ...
    """评估 combined_dryrun_coverage 基数变异；mutation 一律拒绝。"""
    ...
    return {
        ...
        "mutation_allowed": False,
        "reason": "combined_dryrun_coverage_cardinality_frozen_pre_execute",
        "hold": "KEEP_EXECUTE_FALSE",
    }


def evaluate_cross_metric_identity_break(
    *, broken_identity: str
) -> Dict[str, Any]:
    """评估跨指标恒等式破坏；一律拒绝。"""
    allowed = {
        "partition_sum_ne_unique",
        "surface_ne_unique_plus_delta",
        "additive_ne_unique_plus_overlap",
        "residual_ne_failed_plus_partial_plus_delta",
    }
    if broken_identity not in allowed:
        raise ValueError(f"unknown identity break: {broken_identity}")
    return {
        "broken_identity": broken_identity,
        "mutation_allowed": False,
        "reason": "cross_metric_identity_frozen_pre_execute",
        "hold": "KEEP_EXECUTE_FALSE",
    }


def evaluate_partition_codeset_mutation(
    *, bucket: str, proposed_sha256: str
) -> Dict[str, Any]:
    """评估 partition codeset SHA256 变异；一律拒绝。"""
    frozen_map = {
        "complete": EXPECTED_COMPLETE_CODES_SHA256,
        "partial": EXPECTED_PARTIAL_CODES_SHA256,
        "failed": EXPECTED_FAILED_CODES_SHA256,
    }
    ...
    return {
        ...
        "mutation_allowed": False,
        "reason": "partition_codeset_sha256_frozen_pre_execute",
        "hold": "KEEP_EXECUTE_FALSE",
    }


def build_fm33_continuity_rows(
    paths: SurfaceDeltaCombinedDryrunCrossIdentityPaths, *, base_dir: str = BASE_DIR
) -> Tuple[List[Dict[str, str]], Dict[str, bool]]:
    """FM33 packet / fingerprint / gate / 四 ledger 零漂移。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}

    packet = load_json(_abs(paths.fm33_packet_rel, base_dir=base_dir))
    fp_doc = load_json(_abs(paths.fm33_fingerprint_rel, base_dir=base_dir))
    gate_doc = load_json(_abs(paths.fm33_gate_json_rel, base_dir=base_dir))
    uni_led = load_json(_abs(paths.fm33_union_rel, base_dir=base_dir))
    ov_led = load_json(_abs(paths.fm33_overlap_rel, base_dir=base_dir))
    res_led = load_json(_abs(paths.fm33_residual_rel, base_dir=base_dir))
    sw_led = load_json(_abs(paths.fm33_resume_same_worse_rel, base_dir=base_dir))
    ...
    checks["fm33_packet_continuity"] = pkt_ok
    rows.append(
        _row(
            check_id="fm33_packet_continuity",
            layer="fm33_continuity",
            path=paths.fm33_packet_rel,
            expected="PASS_OFFLINE;unique=2249;2134/106/9;28/1/0;delta2;KEEP",
            observed=(
            ...
            ok=pkt_ok,
            notes="ok" if pkt_ok else "fm33_packet_drift",
        )
    )
    ...
    fp_ok = (
        ...
        and frozen.get("union_status_partition_cardinality_freeze")
        == FROZEN_UNION_STATUS_PARTITION_CARDINALITY_FP_SHA256
        and frozen.get("overlap_delta_cardinality_freeze")
        == FROZEN_OVERLAP_DELTA_CARDINALITY_FP_SHA256
        and frozen.get("residual_safety_coverage_lock")
        == FROZEN_RESIDUAL_SAFETY_COVERAGE_LOCK_FP_SHA256
        and frozen.get("resume_same_worse_write_boundary")
        == FROZEN_RESUME_SAME_WORSE_WRITE_BOUNDARY_FP_SHA256
    ...
    checks["fm33_fingerprint_continuity"] = fp_ok
    rows.append(
        _row(
            check_id="fm33_fingerprint_continuity",
            layer="fm33_continuity",
            path=paths.fm33_fingerprint_rel,
            expected="unique2249+fm33_frozen_fps",
            observed=(
                f"unique={fp_doc.get('harvest_unique_union')};"
                f"overlap={fp_doc.get('overlap_delta')};"
                f"residual={fp_doc.get('residual_safety_coverage')}"
            ),
            ok=fp_ok,
            notes="ok" if fp_ok else "fm33_fingerprint_drift",
        )
    )
    ...
    checks["fm33_gate_continuity"] = gate_ok
    rows.append(
        _row(
            check_id="fm33_gate_continuity",
            layer="fm33_continuity",
            path=paths.fm33_gate_json_rel,
            expected="PASS_OFFLINE;KEEP;2134/106/9;Δ12;117;1/0",
            observed=(
                f"gate={gate_doc.get('gate')};"
                f"status={gate_doc.get('union_complete')}/"
                f"{gate_doc.get('union_partial')}/{gate_doc.get('union_failed')};"
                f"overlap={gate_doc.get('overlap_delta')};"
                f"residual={gate_doc.get('residual_safety_coverage')}"
            ),
            ok=gate_ok,
            notes="ok" if gate_ok else "fm33_gate_drift",
        )
    )

    uni_ok = (
        uni_led.get("fingerprint_sha256")
        == FROZEN_UNION_STATUS_PARTITION_CARDINALITY_FP_SHA256
        and uni_led.get("union_complete") == EXPECTED_UNION_COMPLETE
        and uni_led.get("union_partial") == EXPECTED_UNION_PARTIAL
        and uni_led.get("union_failed") == EXPECTED_UNION_FAILED
    )
    checks["fm33_union_ledger_continuity"] = uni_ok
    rows.append(
        _row(
            check_id="fm33_union_ledger_continuity",
            layer="fm33_continuity",
            path=paths.fm33_union_rel,
            expected=FROZEN_UNION_STATUS_PARTITION_CARDINALITY_FP_SHA256,
            observed=str(uni_led.get("fingerprint_sha256") or ""),
            ok=uni_ok,
            notes="ok" if uni_ok else "fm33_union_drift",
        )
    )

    ov_ok = (
        ov_led.get("fingerprint_sha256") == FROZEN_OVERLAP_DELTA_CARDINALITY_FP_SHA256
        and ov_led.get("overlap_delta") == EXPECTED_OVERLAP_DELTA
    )
    checks["fm33_overlap_ledger_continuity"] = ov_ok
    rows.append(
        _row(
            check_id="fm33_overlap_ledger_continuity",
            layer="fm33_continuity",
            path=paths.fm33_overlap_rel,
            expected=FROZEN_OVERLAP_DELTA_CARDINALITY_FP_SHA256,
            observed=str(ov_led.get("fingerprint_sha256") or ""),
            ok=ov_ok,
            notes="ok" if ov_ok else "fm33_overlap_drift",
        )
    )

    res_ok = (
        res_led.get("fingerprint_sha256")
        == FROZEN_RESIDUAL_SAFETY_COVERAGE_LOCK_FP_SHA256
        and res_led.get("residual_safety_coverage") == EXPECTED_RESIDUAL_SAFETY_COVERAGE
    )
    checks["fm33_residual_ledger_continuity"] = res_ok
    rows.append(
        _row(
            check_id="fm33_residual_ledger_continuity",
            layer="fm33_continuity",
            path=paths.fm33_residual_rel,
            expected=FROZEN_RESIDUAL_SAFETY_COVERAGE_LOCK_FP_SHA256,
            observed=str(res_led.get("fingerprint_sha256") or ""),
            ok=res_ok,
            notes="ok" if res_ok else "fm33_residual_drift",
        )
    )

    sw_ok = (
        sw_led.get("fingerprint_sha256")
        == FROZEN_RESUME_SAME_WORSE_WRITE_BOUNDARY_FP_SHA256
        and sw_led.get("resume_same") == EXPECTED_RESUME_SAME
        and sw_led.get("resume_worse") == EXPECTED_RESUME_WORSE
        and sw_led.get("same_codes_sha256") == EXPECTED_RESUME_SAME_CODES_SHA256
    )
    checks["fm33_resume_same_worse_ledger_continuity"] = sw_ok
    rows.append(
        _row(
            check_id="fm33_resume_same_worse_ledger_continuity",
            layer="fm33_continuity",
            path=paths.fm33_resume_same_worse_rel,
            expected=FROZEN_RESUME_SAME_WORSE_WRITE_BOUNDARY_FP_SHA256,
            observed=str(sw_led.get("fingerprint_sha256") or ""),
            ok=sw_ok,
            notes="ok" if sw_ok else "fm33_resume_same_worse_drift",
        )
    )

    all_ok = all(checks.values()) if checks else False
    checks["fm33_continuity_all_pass"] = all_ok
    rows.append(
        _row(
            check_id="fm33_continuity_all_pass",
            layer="fm33_continuity",
            expected="packet+fp+gate+4ledgers",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
            notes="ok" if all_ok else "fm33_continuity_incomplete",
        )
    )
    return rows, checks


def build_dry863_surface_delta_codeset_freeze_rows(
    paths: SurfaceDeltaCombinedDryrunCrossIdentityPaths, *, base_dir: str = BASE_DIR
) -> Tuple[List[Dict[str, str]], Dict[str, bool], Dict[str, Any]]:
    """dry863 extras / surface_harvest_delta codeset freeze（Δ2）。"""
    del paths, base_dir
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    fp, doc = fingerprint_dry863_surface_delta_codeset_freeze()

    counts_ok = (
        EXPECTED_SURFACE_HARVEST_DELTA_N == 2
        and sorted(EXPECTED_DRY863_EXTRA) == ["000037", "000055"]
        and len(EXPECTED_DRY863_EXTRA) == EXPECTED_SURFACE_HARVEST_DELTA_N
        and _sha256_codes(sorted(EXPECTED_DRY863_EXTRA)) == EXPECTED_DRY863_EXTRAS_SHA256
        and EXPECTED_SURFACE_UNIQUE
        == EXPECTED_HARVEST_UNIQUE_UNION + EXPECTED_SURFACE_HARVEST_DELTA_N
    )
    checks["dry863_surface_delta_exact"] = counts_ok
    rows.append(
        _row(
            check_id="dry863_surface_delta_exact",
            layer="dry863_surface_delta_codeset_freeze",
            expected="delta=2;000037,000055",
            observed=(
                f"n={EXPECTED_SURFACE_HARVEST_DELTA_N};"
                f"{','.join(sorted(EXPECTED_DRY863_EXTRA))}"
            ),
            ok=counts_ok,
            notes="ok" if counts_ok else "dry863_surface_delta_drift",
        )
    )

    mut = evaluate_dry863_extras_mutation(
        proposed_extras=list(EXPECTED_DRY863_EXTRA) + ["999999"], action="inject"
    )
    mut2 = evaluate_dry863_extras_mutation(
        proposed_extras=["000037"], action="drop"
    )
    mut3 = evaluate_dry863_extras_mutation(
        proposed_extras=sorted(EXPECTED_DRY863_EXTRA), action="replace"
    )
    mut_ok = (
        mut["mutation_allowed"] is False
        and mut2["mutation_allowed"] is False
        and mut3["mutation_allowed"] is False
    )
    checks["dry863_surface_delta_mutation_denied"] = mut_ok
    rows.append(
        _row(
            check_id="dry863_surface_delta_mutation_denied",
            layer="dry863_surface_delta_codeset_freeze",
            expected="mutation_allowed=false",
            observed=f"inject/drop/exact_denied={mut_ok}",
            ok=mut_ok,
    ...
    flags_ok = (
        doc["deny_extras_inject"] is True
        and doc["deny_extras_drop"] is True
        and doc["deny_delta_cardinality_mutation"] is True
        and doc["hold"] == "KEEP_EXECUTE_FALSE"
    )
    checks["dry863_surface_delta_flags_locked"] = flags_ok
    rows.append(
        _row(
            check_id="dry863_surface_delta_flags_locked",
            layer="dry863_surface_delta_codeset_freeze",
            expected="deny_inject+drop+delta_mut+KEEP",
            observed=(
                f"inject={doc['deny_extras_inject']};drop={doc['deny_extras_drop']};"
                f"delta={doc['deny_delta_cardinality_mutation']};hold={doc['hold']}"
            ),
            ok=flags_ok,
    ...
    fp_ok = fp == FROZEN_DRY863_SURFACE_DELTA_CODESET_FP_SHA256
    checks["dry863_surface_delta_fingerprint"] = fp_ok
    rows.append(
        _row(
            check_id="dry863_surface_delta_fingerprint",
            layer="dry863_surface_delta_codeset_freeze",
            expected=FROZEN_DRY863_SURFACE_DELTA_CODESET_FP_SHA256,
            observed=fp,
            ok=fp_ok,
    ...
    all_ok = all(checks.values()) if checks else False
    checks["dry863_surface_delta_codeset_freeze_all_pass"] = all_ok
    rows.append(
        _row(
            check_id="dry863_surface_delta_codeset_freeze_all_pass",
            layer="dry863_surface_delta_codeset_freeze",
            expected="delta2_codeset_freeze+fp",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
    ...
    meta = {
        ...
        "sample_denials": [mut, mut2, mut3],
        "doc": doc,
    }
    return rows, checks, meta


def build_combined_dryrun_coverage_cardinality_freeze_rows(
    paths: SurfaceDeltaCombinedDryrunCrossIdentityPaths, *, base_dir: str = BASE_DIR
) -> Tuple[List[Dict[str, str]], Dict[str, bool], Dict[str, Any]]:
    """combined_dryrun_coverage cardinality freeze（1053）。"""
    del paths, base_dir
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    fp, doc = fingerprint_combined_dryrun_coverage_cardinality_freeze()

    counts_ok = EXPECTED_COMBINED_DRYRUN_COVERAGE == 1053
    checks["combined_dryrun_coverage_exact"] = counts_ok
    rows.append(
        _row(
            check_id="combined_dryrun_coverage_exact",
            layer="combined_dryrun_coverage_cardinality_freeze",
            expected="1053",
            observed=str(EXPECTED_COMBINED_DRYRUN_COVERAGE),
            ok=counts_ok,
            notes="ok" if counts_ok else "combined_dryrun_drift",
        )
    )

    mut = evaluate_combined_dryrun_coverage_mutation(proposed_coverage=1054)
    mut2 = evaluate_combined_dryrun_coverage_mutation(
    ...
    flags_ok = (
        doc["deny_coverage_inflate"] is True
    ...
    rows.append(
        _row(
            ...
            observed=(
                f"inflate={doc['deny_coverage_inflate']};"
                f"deflate={doc['deny_coverage_deflate']};"
    ...
    fp_ok = fp == FROZEN_COMBINED_DRYRUN_COVERAGE_CARDINALITY_FP_SHA256
    checks["combined_dryrun_coverage_fingerprint"] = fp_ok
    rows.append(
        _row(
            check_id="combined_dryrun_coverage_fingerprint",
            layer="combined_dryrun_coverage_cardinality_freeze",
            expected=FROZEN_COMBINED_DRYRUN_COVERAGE_CARDINALITY_FP_SHA256,
            observed=fp,
            ok=fp_ok,
    ...
    all_ok = all(checks.values()) if checks else False
    checks["combined_dryrun_coverage_cardinality_freeze_all_pass"] = all_ok
    rows.append(
        _row(
            check_id="combined_dryrun_coverage_cardinality_freeze_all_pass",
            layer="combined_dryrun_coverage_cardinality_freeze",
            expected="1053_freeze+fp",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
...
def build_cross_metric_identity_lock_rows(
    paths: SurfaceDeltaCombinedDryrunCrossIdentityPaths, *, base_dir: str = BASE_DIR
) -> Tuple[List[Dict[str, str]], Dict[str, bool], Dict[str, Any]]:
    """cross-metric identity lock（四恒等式）。"""
    del paths, base_dir
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    fp, doc = fingerprint_cross_metric_identity_lock()
    ...
    id_ok = (
        ...
        and EXPECTED_RESIDUAL_SAFETY_COVERAGE
        == (
            EXPECTED_UNION_FAILED
            + EXPECTED_UNION_PARTIAL
            + EXPECTED_SURFACE_HARVEST_DELTA_N
    ...
    checks["cross_metric_identity_exact"] = id_ok
    rows.append(
        _row(
            check_id="cross_metric_identity_exact",
            layer="cross_metric_identity_lock",
            expected="4_identities_hold",
            observed=(
                f"part={EXPECTED_UNION_COMPLETE + EXPECTED_UNION_PARTIAL + EXPECTED_UNION_FAILED};"
                f"surf={EXPECTED_SURFACE_UNIQUE};add={EXPECTED_HARVEST_ADDITIVE};"
                f"res={EXPECTED_RESIDUAL_SAFETY_COVERAGE}"
            ),
            ok=id_ok,
            notes="ok" if id_ok else "cross_metric_identity_drift",
        )
    )

    sample_denials = [
        evaluate_cross_metric_identity_break(broken_identity=k)
        for k in (
            "partition_sum_ne_unique",
            "surface_ne_unique_plus_delta",
            "additive_ne_unique_plus_overlap",
    ...
    checks["cross_metric_identity_break_denied"] = mut_ok
    rows.append(
        _row(
            check_id="cross_metric_identity_break_denied",
            layer="cross_metric_identity_lock",
            expected="mutation_allowed=false",
            observed=f"samples={len(sample_denials)};all_denied={mut_ok}",
            ok=mut_ok,
            notes="ok" if mut_ok else "identity_break_leak",
        )
    )

    flags_ok = (
        doc["partition_sum_eq_unique"] is True
        and doc["surface_eq_unique_plus_delta"] is True
        and doc["additive_eq_unique_plus_overlap"] is True
        and doc["residual_eq_failed_plus_partial_plus_delta"] is True
        and doc["mutation_allowed"] is False
        and doc["hold"] == "KEEP_EXECUTE_FALSE"
    )
    checks["cross_metric_identity_flags_locked"] = flags_ok
    rows.append(
        _row(
            check_id="cross_metric_identity_flags_locked",
            layer="cross_metric_identity_lock",
            expected="4_eq+mutation_false+KEEP",
    ...
    fp_ok = fp == FROZEN_CROSS_METRIC_IDENTITY_LOCK_FP_SHA256
    checks["cross_metric_identity_fingerprint"] = fp_ok
    rows.append(
        _row(
            check_id="cross_metric_identity_fingerprint",
            layer="cross_metric_identity_lock",
            expected=FROZEN_CROSS_METRIC_IDENTITY_LOCK_FP_SHA256,
            observed=fp,
            ok=fp_ok,
            notes="ok" if fp_ok else "cross_metric_fp_drift",
        )
    )

    all_ok = all(checks.values()) if checks else False
    checks["cross_metric_identity_lock_all_pass"] = all_ok
    rows.append(
        _row(
            check_id="cross_metric_identity_lock_all_pass",
            layer="cross_metric_identity_lock",
            expected="4_identities_lock+fp",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
            notes="ok" if all_ok else "cross_metric_incomplete",
        )
    )
    meta = {
        "fingerprint": fp,
...
def build_partition_codeset_sha256_lock_rows(
    paths: SurfaceDeltaCombinedDryrunCrossIdentityPaths, *, base_dir: str = BASE_DIR
) -> Tuple[List[Dict[str, str]], Dict[str, bool], Dict[str, Any]]:
    """partition codeset SHA256 lock（complete/partial/failed）。"""
    del paths, base_dir
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    fp, doc = fingerprint_partition_codeset_sha256_lock()

    sha_ok = (
        len(EXPECTED_COMPLETE_CODES_SHA256) == 64
        and len(EXPECTED_PARTIAL_CODES_SHA256) == 64
        and len(EXPECTED_FAILED_CODES_SHA256) == 64
        and EXPECTED_FAILED_CODES_SHA256 == FM31_EXPECTED_FAILED_CODES_SHA256
        and EXPECTED_PARTIAL_CODES_SHA256 == FM31_EXPECTED_PARTIAL_CODES_SHA256
    )
    checks["partition_codeset_sha_exact"] = sha_ok
    rows.append(
        _row(
            check_id="partition_codeset_sha_exact",
            layer="partition_codeset_sha256_lock",
            expected="complete+partial+failed_sha256",
            observed=(
                f"c={EXPECTED_COMPLETE_CODES_SHA256[:12]};"
                f"p={EXPECTED_PARTIAL_CODES_SHA256[:12]};"
                f"f={EXPECTED_FAILED_CODES_SHA256[:12]}"
            ),
            ok=sha_ok,
            notes="ok" if sha_ok else "partition_codeset_sha_drift",
        )
    )

    sample_denials = [
        evaluate_partition_codeset_mutation(
            bucket="complete", proposed_sha256="0" * 64
        ),
        evaluate_partition_codeset_mutation(
            bucket="partial", proposed_sha256="1" * 64
        ),
        evaluate_partition_codeset_mutation(
            bucket="failed", proposed_sha256="2" * 64
        ),
        evaluate_partition_codeset_mutation(
            bucket="complete", proposed_sha256=EXPECTED_COMPLETE_CODES_SHA256
        ),
    ]
    mut_ok = all(d["mutation_allowed"] is False for d in sample_denials)
    checks["partition_codeset_mutation_denied"] = mut_ok
    rows.append(
        _row(
            check_id="partition_codeset_mutation_denied",
            layer="partition_codeset_sha256_lock",
            expected="mutation_allowed=false",
            observed=f"samples={len(sample_denials)};all_denied={mut_ok}",
            ok=mut_ok,
            notes="ok" if mut_ok else "partition_codeset_mutation_leak",
        )
    )

    flags_ok = (
        doc["deny_complete_codeset_mutation"] is True
        and doc["deny_partial_codeset_mutation"] is True
        and doc["deny_failed_codeset_mutation"] is True
        and doc["hold"] == "KEEP_EXECUTE_FALSE"
        and doc["complete_codes_sha256"] == EXPECTED_COMPLETE_CODES_SHA256
        and doc["partial_codes_sha256"] == EXPECTED_PARTIAL_CODES_SHA256
        and doc["failed_codes_sha256"] == EXPECTED_FAILED_CODES_SHA256
    )
    checks["partition_codeset_flags_locked"] = flags_ok
    rows.append(
        _row(
            check_id="partition_codeset_flags_locked",
            layer="partition_codeset_sha256_lock",
            expected="deny_c+p+f+KEEP",
            observed=f"flags_ok={flags_ok};hold={doc['hold']}",
            ok=flags_ok,
            notes="ok" if flags_ok else "flags_unlocked",
        )
    ...
    fp_ok = fp == FROZEN_PARTITION_CODESET_SHA256_LOCK_FP_SHA256
    checks["partition_codeset_fingerprint"] = fp_ok
    rows.append(
        _row(
            check_id="partition_codeset_fingerprint",
            layer="partition_codeset_sha256_lock",
            expected=FROZEN_PARTITION_CODESET_SHA256_LOCK_FP_SHA256,
            observed=fp,
            ok=fp_ok,
            notes="ok" if fp_ok else "partition_codeset_fp_drift",
        )
    )

    all_ok = all(checks.values()) if checks else False
    checks["partition_codeset_sha256_lock_all_pass"] = all_ok
    rows.append(
        _row(
            check_id="partition_codeset_sha256_lock_all_pass",
            layer="partition_codeset_sha256_lock",
            expected="c_p_f_sha256_lock+fp",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
            notes="ok" if all_ok else "partition_codeset_incomplete",
        )
    )
    meta = {
        "fingerprint": fp,
...
def build_output_root_protection_rows(
    ...
    """output-root 保护：resume/harvest 写拒绝 + MOCK36 放行。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    ...
    for check_id, probe_rel in (
        (
            ...
            f"{HARVEST_PHASE3_ROOT_REL}/quality/probe_write_forbidden_fm34.csv",
        ),
        (
            "write_guard_phase2_harvest_refused",
            f"{HARVEST_PHASE2_ROOT_REL}/quality/probe_write_forbidden_fm34.csv",
        ),
        (
            "write_guard_fuller_harvest_refused",
            f"{HARVEST_FULLER_ROOT_REL}/quality/probe_write_forbidden_fm34.csv",
        ),
        (
            "write_guard_phase35_harvest_refused",
            f"{HARVEST_PHASE35_ROOT_REL}/quality/probe_write_forbidden_fm34.csv",
        ),
        (
            "write_guard_phase35_resume_refused",
            f"{HARVEST_PHASE35_RESUME_ROOT_REL}/quality/probe_write_forbidden_fm34.csv",
        ),
    ):
        refused = False
        msg = ""
    ...
    out_detail = ""
    try:
        assert_fm34_output_root(paths.output_root_rel, base_dir=base_dir)
        out_ok = True
        out_detail = "allowed"
    except RuntimeError as exc:
    ...
    rows.append(
        _row(
            ...
            expected="MOCK36_or_ephemeral_allowed",
            observed=out_detail,
            ok=out_ok,
    ...
    rows.append(
        _row(
            ...
            layer="output_root_protection",
            expected="harvest+resume_refused;mock36_ok",
            observed=(
...
def build_frozen_mock_isolation_rows(
    ...
    """冻结 mock cohort 写隔离：MOCK3–35 拒绝 · MOCK36 放行。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    load_frozen_mock_cohort_roots.cache_clear()
    ...
    mock35_blocked = False
    try:
        assert_fm34_output_root(mock35_rel, base_dir=base_dir)
    except RuntimeError as exc:
        mock35_blocked = FROZEN_MOCK_COHORT_WRITE_FORBIDDEN in str(exc)
    checks["mock35_still_frozen"] = mock35_blocked
    rows.append(
        _row(
            check_id="mock35_still_frozen",
            layer="frozen_mock_isolation",
            root_id=PRIOR_TASK_ROOT_ID,
            path=mock35_rel,
            expected="write_forbidden",
            observed="blocked" if mock35_blocked else "allowed",
            ok=mock35_blocked,
            notes="ok" if mock35_blocked else "mock35_write_leak",
        )
    )
    ...
    try:
        ...
        allow_ok = False
    checks["frozen_allow_mock36"] = allow_ok
    rows.append(
        _row(
            check_id="frozen_allow_mock36",
            layer="frozen_mock_isolation",
    ...
    rows.append(
        _row(
            ...
            layer="frozen_mock_isolation",
            expected="MOCK3-35_block+MOCK36_allow",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
...
def build_protected_csv_registry_rows(
    *,
    ...
    """protected_output_roots.csv：MOCK36 已登记。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    ...
    mock36 = by_id.get(THIS_TASK_ROOT_ID) or {}
    path_ok = DEFAULT_MOCK_OUTPUT_ROOT_REL in str(mock36.get("path_pattern") or "")
    checks["protected_csv_mock36_path"] = path_ok
    rows.append(
        _row(
            check_id="protected_csv_mock36_path",
            layer="protected_csv_registry",
            root_id=THIS_TASK_ROOT_ID,
            expected=DEFAULT_MOCK_OUTPUT_ROOT_REL,
            observed=str(mock36.get("path_pattern") or ""),
            ok=path_ok,
            notes="ok" if path_ok else "mock36_path_mismatch",
        )
    )
    ...
    rows.append(
        _row(
            ...
            layer="protected_csv_registry",
            expected="MOCK3–35+resume+auth+fuller_registered",
            observed=f"pass={sum(1 for v in checks.values() if v)}/{len(checks)}",
            ok=all_ok,
...
def build_fm_gate_battery_rows(
    *, gates: Dict[str, Dict[str, Any]]
    ...
    """FM-01..05 + FM-12..33 gate battery（跳过 seal FM06–11）。"""
    rows: List[Dict[str, str]] = []
    checks: Dict[str, bool] = {}
    specs = [
    ...
    for check_id, key in specs:
        ...
        if key in (
            "fm25", "fm26", "fm27", "fm28", "fm29", "fm30", "fm31", "fm32", "fm33"
        ):
            ok = (
                ok
                and payload.get("harvest_unique_union") == EXPECTED_HARVEST_UNIQUE_UNION
        ...
        if key in ("fm26", "fm27", "fm28", "fm29", "fm30", "fm31", "fm32", "fm33"):
            ok = (
                ok
        ...
        if key in ("fm27", "fm28", "fm29", "fm30", "fm31", "fm32", "fm33"):
            ok = (
                ok
                and payload.get("partial_risk_bands") == EXPECTED_PARTIAL_RISK_BANDS
            )
        if key in ("fm28", "fm29", "fm30", "fm31", "fm32", "fm33"):
            ok = (
                ok
                and payload.get("residual_safety_coverage")
                == EXPECTED_RESIDUAL_SAFETY_COVERAGE
            )
        if key in ("fm29", "fm30", "fm31", "fm32", "fm33"):
            ok = (
                ok
    ...
    rows.append(
        _row(
            check_id="fm01_05_12_33_battery_all_pass",
            layer="fm_gate_battery",
    ...
    checks["fm01_05_12_33_battery_all_pass"] = all_ok
    return rows, checks
...
def ensure_protected_roots_csv_fm34(
    *,
    csv_rel: str = PROTECTED_ROOTS_CSV_REL,
    base_dir: str = BASE_DIR,
) -> None:
    """注册 C-ROOT-MOCK36；加固 C-ROOT-002 surface-delta/dryrun/identity 说明。"""
    ...
    _upsert(
        RESUME_HARVEST_ROOT_ID,
        {
            ...
            "notes": (
                ...
                "C-FM-24..33 scale/safety freezes + C-FM-34 dry863/surface-delta/"
                "combined-dryrun/cross-identity/partition-codeset; "
                "只读直至人批重跑"
            ),
        },
    )
    _upsert(
        THIS_TASK_ROOT_ID,
        {
            ...
            "notes": (
                "C-FM-34 scale dry863/surface-delta codeset freeze (Δ2) + "
                "combined_dryrun_coverage cardinality freeze (1053) + "
                "cross_metric_identity_lock + partition_codeset_sha256_lock + "
                "FM33 continuity; never production EXECUTE; must not overwrite "
                "MOCK3-35; seal_chain_extended=false"
            ),
        },
    )
...
def run_scale_surface_delta_combined_dryrun_cross_identity_partition_codeset_safety(
    *,
    paths: SurfaceDeltaCombinedDryrunCrossIdentityPaths | None = None,
    base_dir: str = BASE_DIR,
) -> Dict[str, Any]:
    """执行 C-FM-34 规模 dry863/combined-dryrun/cross-identity/partition-codeset 离线 QA。"""
    paths = paths or SurfaceDeltaCombinedDryrunCrossIdentityPaths()
    generated_at = _utc_now_iso()
    ensure_protected_roots_csv_fm34(
        csv_rel=paths.protected_roots_csv_rel, base_dir=base_dir
    )
    out_root = assert_fm34_output_root(paths.output_root_rel, base_dir=base_dir)

    matrix: List[Dict[str, str]] = []
    cont_rows, cont_checks = build_fm33_continuity_rows(paths, base_dir=base_dir)
    matrix.extend(cont_rows)
    d863_rows, d863_checks, d863_meta = build_dry863_surface_delta_codeset_freeze_rows(
        paths, base_dir=base_dir
    )
    matrix.extend(d863_rows)
    cdr_rows, cdr_checks, cdr_meta = (
        build_combined_dryrun_coverage_cardinality_freeze_rows(
            paths, base_dir=base_dir
        )
    )
    matrix.extend(cdr_rows)
    xid_rows, xid_checks, xid_meta = build_cross_metric_identity_lock_rows(
        paths, base_dir=base_dir
    )
    matrix.extend(xid_rows)
    pcs_rows, pcs_checks, pcs_meta = build_partition_codeset_sha256_lock_rows(
        paths, base_dir=base_dir
    )
    matrix.extend(pcs_rows)
    ...
    layer_gates = {
        "fm33_continuity": (
            "PASS_OFFLINE"
            if cont_checks.get("fm33_continuity_all_pass")
            else "FAIL_OFFLINE"
        ),
        "dry863_surface_delta_codeset_freeze": (
            "PASS_OFFLINE"
            if d863_checks.get("dry863_surface_delta_codeset_freeze_all_pass")
            else "FAIL_OFFLINE"
        ),
        "combined_dryrun_coverage_cardinality_freeze": (
            "PASS_OFFLINE"
            if cdr_checks.get("combined_dryrun_coverage_cardinality_freeze_all_pass")
            else "FAIL_OFFLINE"
        ),
        "cross_metric_identity_lock": (
            "PASS_OFFLINE"
            if xid_checks.get("cross_metric_identity_lock_all_pass")
            else "FAIL_OFFLINE"
        ),
        "partition_codeset_sha256_lock": (
            "PASS_OFFLINE"
            if pcs_checks.get("partition_codeset_sha256_lock_all_pass")
            else "FAIL_OFFLINE"
        ),
        ...
        "fm_gate_battery": (
            "PASS_OFFLINE"
            if bat_checks.get("fm01_05_12_33_battery_all_pass")
            else "FAIL_OFFLINE"
        ),
        "execute_hold_seal": (
            "PASS_OFFLINE"
    ...
    d863_rel = _rel(
        os.path.join(out_root, "dry863_surface_delta_codeset_freeze_ledger.json"),
        base_dir=base_dir,
    )
    with open(_abs(d863_rel, base_dir=base_dir), "w", encoding="utf-8") as fh:
        json.dump(
            {
                "generated_at": generated_at,
                "task_id": TASK_ID,
                "fingerprint_sha256": d863_meta["fingerprint"],
                "surface_harvest_delta_n": d863_meta["surface_harvest_delta_n"],
                "dry863_extras_sha256": d863_meta["dry863_extras_sha256"],
                "sample_denials": d863_meta["sample_denials"],
                "doc": d863_meta["doc"],
            },
            fh,
            ensure_ascii=False,
    ...
    cdr_rel = _rel(
        os.path.join(
            out_root, "combined_dryrun_coverage_cardinality_freeze_ledger.json"
        ),
        base_dir=base_dir,
    )
    with open(_abs(cdr_rel, base_dir=base_dir), "w", encoding="utf-8") as fh:
        json.dump(
            {
                "generated_at": generated_at,
                "task_id": TASK_ID,
                "fingerprint_sha256": cdr_meta["fingerprint"],
                "combined_dryrun_coverage": cdr_meta["combined_dryrun_coverage"],
                "sample_denials": cdr_meta["sample_denials"],
                "doc": cdr_meta["doc"],
            },
            fh,
            ensure_ascii=False,
    ...
    xid_rel = _rel(
        os.path.join(out_root, "cross_metric_identity_lock_ledger.json"),
        base_dir=base_dir,
    )
    with open(_abs(xid_rel, base_dir=base_dir), "w", encoding="utf-8") as fh:
        json.dump(
            {
                "generated_at": generated_at,
                "task_id": TASK_ID,
                "fingerprint_sha256": xid_meta["fingerprint"],
                "sample_denials": xid_meta["sample_denials"],
                "doc": xid_meta["doc"],
            },
            fh,
            ensure_ascii=False,
    ...
    pcs_rel = _rel(
        os.path.join(out_root, "partition_codeset_sha256_lock_ledger.json"),
        base_dir=base_dir,
    )
    with open(_abs(pcs_rel, base_dir=base_dir), "w", encoding="utf-8") as fh:
        json.dump(
            {
                "generated_at": generated_at,
                "task_id": TASK_ID,
                "fingerprint_sha256": pcs_meta["fingerprint"],
                "complete_codes_sha256": pcs_meta["complete_codes_sha256"],
                "partial_codes_sha256": pcs_meta["partial_codes_sha256"],
                "failed_codes_sha256": pcs_meta["failed_codes_sha256"],
                "sample_denials": pcs_meta["sample_denials"],
                "doc": pcs_meta["doc"],
            },
            fh,
            ensure_ascii=False,
        ...
        json.dump(
            {
                ...
                "notes": (
                    "dry863/surface-delta codeset freeze (Δ2) + combined_dryrun_"
                    "coverage cardinality freeze (1053) + cross_metric_identity_"
                    "lock + partition_codeset_sha256_lock + FM33 continuity + "
                    "MOCK36; EXECUTE remains human-held; does not overwrite "
                    "MOCK3-35"
                ),
            },
            fh,
    ...
    observed_fps = {
        "dry863_surface_delta_codeset_freeze": d863_meta["fingerprint"],
        "combined_dryrun_coverage_cardinality_freeze": cdr_meta["fingerprint"],
        "cross_metric_identity_lock": xid_meta["fingerprint"],
        "partition_codeset_sha256_lock": pcs_meta["fingerprint"],
    }
    fp_rel = _rel(
        ...
        json.dump(
            {
                ...
                "frozen_fps": {
                    ...
                    "fm33_union_status_partition_cardinality_freeze": (
                        FROZEN_UNION_STATUS_PARTITION_CARDINALITY_FP_SHA256
                    ),
                    "fm33_overlap_delta_cardinality_freeze": (
                        FROZEN_OVERLAP_DELTA_CARDINALITY_FP_SHA256
                    ),
                    "fm33_residual_safety_coverage_lock": (
                        FROZEN_RESIDUAL_SAFETY_COVERAGE_LOCK_FP_SHA256
                    ),
                    "fm33_resume_same_worse_write_boundary": (
                        FROZEN_RESUME_SAME_WORSE_WRITE_BOUNDARY_FP_SHA256
                    ),
                },
    ...
    return {
        ...
        "fingerprint": fp,
        "dry863_path": d863_rel,
        "combined_dryrun_path": cdr_rel,
        "cross_identity_path": xid_rel,
        "partition_codeset_path": pcs_rel,
        "battery_path": battery_rel,
        "packet_path": packet_rel,
        "observed_fps": observed_fps,
        "inputs": {
            "fm33_packet": paths.fm33_packet_rel,
            "fm33_gate": paths.fm33_gate_json_rel,
        },
