"""
B-FM-48：公司章程 known_003 / 董事会决议 known_002 live allow-list 锁测（离线）。

覆盖：
- allow-list 仅含 company_articles_known_003 与 board_resolution_known_002（category 空）
- 字段与 fixtures ready 对齐
- mock live 路径可对两案得到 pass（章程→announcement；董事会→board_resolution）
- 不包含 articles/board known_001 及 B-FM-47 等已 LIVE_PASS 案

无真实 CNINFO · 不造 §7 FP。

运行：
    python lab/test_cninfo_b_class_articles_board_known_002_003_live.py
"""
...
LIVE_DIR = os.path.join(
    ...
    "outputs",
    "validation",
    "cninfo_b_class_articles_board_known_002_003_live_20260715",
)
...
HARVEST_ARTICLES = "【通程控股】公司章程"
HARVEST_BOARD = "第三届董事会第四次会议决议公告"
ANN_ARTICLES = "1223954488"
ANN_BOARD = "1223953622"
# 2025-06-20 00:00:00 UTC
ANN_TS_MS = 1750377600000
...
class TestArticlesBoardKnown002003LiveAllowList(unittest.TestCase):
    ...
    def test_allowlist_fields_match_fixtures(self) -> None:
        ...
        for case_id in (
            ...
            if case_id == "company_articles_known_003":
    ...
    def test_mock_live_both_cases_pass(self) -> None:
        ...
        for case_id, title, ann_id, ts_ms, code, date_str, expected_dtype in fixtures:
            ...
            self.assertEqual(row_k["predicted_document_type"], expected_dtype, case_id)
