...
def _general_document_type(title: str, patterns: List[str]) -> str:
    ...
    B-FM-38：「货币资金管理制度」/「对外担保管理制度」无 general
    positive_patterns，旧逻辑落 other。窄 pattern 一律 announcement（勿裸
    「管理制度」/「制度」；分子公司管理制度与薪酬/名单/简报/ESG 仍可 other；
    与「对外担保的情况简报」可区分；不扩 schema）。
    ...
    if "权益变动报告书" in title:
        return "announcement"
    # 受托管理事务报告：债券受托管理人年度事务报告，保持 announcement（勿落 other）
    if "受托管理事务报告" in title:
        return "announcement"
    ...
    if "总经理工作细则" in title:
        return "announcement"
    # 货币资金管理制度：资金内控制度文本（窄；勿裸「管理制度」）
    if "货币资金管理制度" in title:
        return "announcement"
    # 对外担保管理制度：担保内控制度文本（窄；勿裸「管理制度」；≠对外担保简报）
    if "对外担保管理制度" in title:
        return "announcement"
    # 子公司管理制度：分/子公司内控制度（窄；覆盖「分、子公司管理制度」；勿裸「管理制度」）
    if "子公司管理制度" in title:
        return "announcement"
