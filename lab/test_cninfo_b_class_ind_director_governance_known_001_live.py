"""
B-FM-33：独立董事治理披露 known-document live allow-list 锁测（离线；不请求 CNINFO）。

覆盖：
- allow-list 仅含 independent_director_meeting_review_known_001 与
  independent_director_nominee_declaration_known_001（category 空）
- 字段与 fixtures ready 对齐
- mock live 路径可对两案得到 pass（predicted_document_type=announcement；非 other）
- 不包含 legal_opinion / continuous_supervision / nonstandard_audit 等已 LIVE_PASS 案

无真实 CNINFO · 不造 §7 FP。

运行：
    python lab/test_cninfo_b_class_ind_director_governance_known_001_live.py
"""
...
LIVE_DIR = os.path.join(
    ...
    "outputs",
    "validation",
    "cninfo_b_class_ind_director_governance_known_001_live_20260715",
)
...
ANN_MR = "1223951163"
ANN_ND = "1223973877"
# 2025-06-20 / 2025-06-24 00:00:00 UTC 近似；mock 用固定 ms
ANN_TS_MR = 1750377600000  # 2025-06-20
ANN_TS_ND = 1750723200000  # 2025-06-24
...
class TestIndDirectorGovernanceKnown001LiveAllowList(unittest.TestCase):
