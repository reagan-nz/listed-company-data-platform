#!/usr/bin/env python3
"""
CNINFO C-class — 非 seal Cross-FM 三次扩展 post-commit 漂移复核 runner（离线 · C-FM-21）。

Usage:
    python3 lab/run_cninfo_c_class_nonseal_third_extension_post_commit_drift_recheck.py
    python3 lab/run_cninfo_c_class_nonseal_third_extension_post_commit_drift_recheck.py \\
      --output-root outputs/validation/_mock_c_fm21_nonseal_third_extension_post_commit_drift_recheck
"""
...
REPORT_ROOT_REL = (
    "outputs/validation/"
    "cninfo_c_class_nonseal_third_extension_post_commit_drift_recheck"
)
REPORT_MD_REL = (
    "outputs/validation/"
    "cninfo_c_class_nonseal_third_extension_post_commit_drift_recheck_20260715.md"
)
REPORT_JSON_REL = (
    "outputs/validation/"
...
TASK_REPORT_REL = (
    "outputs/validation/"
    "cninfo_c_class_fm21_nonseal_third_extension_post_commit_drift_recheck_20260715.md"
)
...
def write_reports(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    json_payload = {
        ...
        "frozen_third_extension_fp_sha256": payload[
            "frozen_third_extension_fp_sha256"
        ],
        "recomputed_third_extension_fp_sha256": payload[
            "recomputed_third_extension_fp_sha256"
        ],
    ...
    md_lines = [
        "# C-FM-21 Non-seal Third Extension Post-Commit Drift Recheck",
        "",
        ...
        *layer_lines,
        ...
        f"| frozen third-extension fp | `{payload['frozen_third_extension_fp_sha256']}` |",
        f"| recomputed third-extension fp | `{payload['recomputed_third_extension_fp_sha256']}` |",
        f"| drift_detected | **{payload['drift_detected']}** |",
        f"| drift fingerprint | `{payload['fingerprint'].get('fingerprint_sha256', '')}` |",
        "",
        "## Capability",
        "",
        "1. FM-01..05 + FM-12..20 gate battery（跳过 seal FM06–11）",
        "2. C-FM-20 MOCK22 冻结产物存在性",
        "3. 三次扩展矩阵指纹零漂移（冻结常量 · gate JSON · 矩阵文件 · 重算）",
        "4. 冻结 mock 写隔离（MOCK3–22 拒绝；MOCK23 / ephemeral 放行）",
        "5. harvest/exclusion dual-layer 一致性（FM-03）",
        "6. EXECUTE hold seal（KEEP_EXECUTE_FALSE · AWAITING · idle_not_required）",
        "7. protected CSV MOCK23 注册一致性",
        "",
        "## Inputs (read-only)",
        "",
        "| 输入 | 路径 |",
    ...
    md_lines.extend(
        [
            ...
            "",
            "```",
            f"c_fm_21_nonseal_third_extension_post_commit_drift_recheck_gate = {payload['gate']}",
            "execute_production_snapshot_rebuild = false",
            "approved_for_snapshot_rebuild = false",
            "cninfo_calls = 0",
            "ready_for_execute = false",
    ...
    task_lines = [
        ...
        "## Task",
        "",
        "在 C-FM-20 已 commit 且 EXECUTE 仍 human-held 之上，补齐 **非 seal-chain** 能力："
        "**Cross-FM mock cohort 三次扩展 post-commit 漂移复核**、"
        "**MOCK22 冻结产物零漂移**、**MOCK3–22 写隔离**；"
        "产物写入隔离 MOCK23（不覆盖 MOCK3–22；不新增 seal / decision-await 层）。",
        "",
        "## Capability gain",
        "",
        "1. FM-01..05 + FM-12..20 gate battery（显式跳过 seal FM06–11）",
        "2. MOCK22 冻结产物存在性（matrix / fingerprint / registry / battery / packet）",
        "3. 三次扩展指纹零漂移：常量 · gate JSON · 冻结矩阵 · builder 重算对齐",
        "4. 冻结写隔离扩展至 MOCK22；本任务 MOCK23 放行",
        "5. harvest/exclusion FM-03 一致性层",
        "6. EXECUTE hold seal（不得因 AWAITING 而 IDLE）",
        "7. protected CSV：MOCK23 注册",
        "",
        "## Files",
        "",
        "| 路径 | 变更 |",
        "|------|------|",
        "| `lab/cninfo_c_class_nonseal_third_extension_post_commit_drift_recheck.py` | **新增** 核心 |",
        "| `lab/run_cninfo_c_class_nonseal_third_extension_post_commit_drift_recheck.py` | **新增** runner |",
        "| `lab/test_cninfo_c_class_nonseal_third_extension_post_commit_drift_recheck.py` | **新增** 测试 |",
        "| `outputs/validation/cninfo_c_class_erad_protected_output_roots.csv` | C-ROOT-MOCK23 |",
        ...
        "| 只读 FM01–05 / FM12–20 gate JSON / MOCK22 产物 / harvest / protected CSV | CNINFO live |",
        "| offline QA · 三次扩展指纹重算（不覆盖 MOCK3–22） | 覆盖 MOCK3–22 |",
        "| | 覆盖权威 dual-layer 索引 |",
        "| | commit/push（本包未执行） |",
        ...
        "## Next",
        "",
        "- Controller 可 commit 本包（non-seal third-extension post-commit drift recheck only）",
    ...
    with open(readme, "w", encoding="utf-8") as fh:
        fh.write(
            "\n".join(
                [
                    "# C-FM-21 mock non-seal third-extension post-commit drift recheck root",
                    "",
                    f"gate: `{payload['gate']}`",
                    "seal_chain_extended: false",
                    "does_not_overwrite: _mock_c_fm02.._mock_c_fm20 / standard dryrun",
...
def main(argv: Any = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "C-FM-21 non-seal third-extension post-commit drift recheck QA (offline)"
        )
    )
    parser.add_argument(
        "--output-root",
        default=DEFAULT_MOCK_OUTPUT_ROOT_REL,
        help="隔离 mock 写根（须 validation/_mock_*；不得覆盖 MOCK3–22）",
    )
    parser.add_argument(
        "--execute",
        action="store_true",
    ...
    paths = ThirdExtensionDriftPaths(output_root_rel=args.output_root)
    payload = run_nonseal_third_extension_post_commit_drift_recheck(paths=paths)
