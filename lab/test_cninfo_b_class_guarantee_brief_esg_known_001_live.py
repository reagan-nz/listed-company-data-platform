"""
B-FM-40：对外担保情况简报 / 英文 ESG known-document live allow-list 锁测（离线；不请求 CNINFO）。

覆盖：
- allow-list 仅含 external_guarantee_situation_brief_known_001 与
  esg_report_known_001（category 空）
- 字段与 fixtures ready 对齐
- mock live 路径可对两案得到 pass（predicted_document_type=announcement；非 other）
- 不包含 subsidiary / compensation / monetary 等已 LIVE_PASS 案

无真实 CNINFO · 不造 §7 FP。

运行：
    python lab/test_cninfo_b_class_guarantee_brief_esg_known_001_live.py
"""
...
LIVE_DIR = os.path.join(
    ...
    "outputs",
    "validation",
    "cninfo_b_class_guarantee_brief_esg_known_001_live_20260715",
)
...
ANN_BRIEF = "1223951749"
ANN_ESG = "1224040090"
ANN_TS_BRIEF = 1750377600000  # 2025-06-20
ANN_TS_ESG = 1751241600000  # 2025-06-30
...
class TestGuaranteeBriefEsgKnown001LiveAllowList(unittest.TestCase):
