"""
CNINFO C-class — 非 seal Cross-FM mock cohort 三次扩展单测（离线 · CNINFO=0 · C-FM-20）。

运行：
    python3 lab/test_cninfo_c_class_nonseal_cross_fm_mock_cohort_third_extension.py
"""
...
_RUNNER = os.path.join(
    _LAB_DIR, "run_cninfo_c_class_nonseal_cross_fm_mock_cohort_third_extension.py"
)
_TEST_SUMMARY_REL = (
    "outputs/validation/"
    "cninfo_c_class_nonseal_cross_fm_mock_cohort_third_extension_test_summary_20260715.md"
)
...
def _write_test_summary(cases: list) -> None:
    ...
    lines = [
        "# C-FM-20 Non-seal Cross-FM Mock Cohort Third Extension — Test Summary",
        "",
        "_offline · CNINFO=0_",
        "",
        "| case | result |",
    ...
    lines.extend(
        [
            "",
            "```",
            "c_fm_20_nonseal_cross_fm_mock_cohort_third_extension_test_gate = PASS_OFFLINE",
            "cninfo_calls = 0",
...
class TestNonsealCrossFmMockCohortThirdExtension(unittest.TestCase):
    def test_output_root_requires_mock_and_not_frozen(self) -> None:
        with self.assertRaises(RuntimeError):
            assert_fm20_output_root("outputs/validation/cninfo_c_class_not_mock")
        with self.assertRaises(RuntimeError):
            assert_fm20_output_root(AUTHORITATIVE_DUAL_LAYER_INDEX_ROOT_REL)
        ...
        norm = assert_fm20_output_root(DEFAULT_MOCK_OUTPUT_ROOT_REL)
        self.assertIn("_mock_c_fm20", norm)
    ...
    def test_third_extension_specs_include_fm18_to_fm19(self) -> None:
        ...
        self.assertIn("fm18_nonseal_cross_fm_mock_cohort_second_extension", ids)
        self.assertIn("fm19_nonseal_second_extension_post_commit_drift_recheck", ids)
        self.assertEqual(len(specs), 13)

    def test_nonseal_chain_anchors_pass(self) -> None:
        _rows, checks = build_nonseal_chain_anchor_rows()
        self.assertTrue(checks.get("anchor_second_extension_fp"))
        self.assertTrue(checks.get("anchor_second_extension_drift_fp"))
        self.assertTrue(checks.get("nonseal_chain_continuity_all_pass"))

    def test_frozen_isolation_blocks_mock21_allows_mock22(self) -> None:
        rows, checks = build_frozen_mock_isolation_rows(
            ThirdExtensionPaths(output_root_rel=DEFAULT_MOCK_OUTPUT_ROOT_REL)
        )
        self.assertTrue(checks.get("frozen_block_C-ROOT-MOCK21"))
        self.assertTrue(checks.get("frozen_block_C-ROOT-MOCK8"))
        self.assertTrue(checks.get("frozen_allow_mock22"))
        self.assertTrue(checks.get("mock21_still_frozen"))
    ...
    def test_fingerprint_matrix_stable(self) -> None:
        ...
        fp1 = fingerprint_third_extension_matrix(rows)
        fp2 = fingerprint_third_extension_matrix(rows)
        self.assertEqual(fp1["fingerprint_sha256"], fp2["fingerprint_sha256"])

    def test_full_extension_pass_isolated_mock(self) -> None:
        result = run_nonseal_cross_fm_mock_cohort_third_extension(
            paths=ThirdExtensionPaths(
                output_root_rel="outputs/validation/_mock_c_fm20_cli_test_tmp"
            )
        )
        self.assertEqual(result["gate"], "PASS_OFFLINE")
        self.assertEqual(result["cninfo_calls"], 0)
        self.assertEqual(result["cohort_count"], 13)
    ...
    def test_cninfo_not_called(self) -> None:
        with mock.patch("requests.get") as get_mock, mock.patch(
            ...
            run_nonseal_cross_fm_mock_cohort_third_extension(
                paths=ThirdExtensionPaths(
                    output_root_rel=(
                        "outputs/validation/_mock_c_fm20_unit_cninfo_probe"
                    )
                )
            )
...
if __name__ == "__main__":
    ...
    suite = loader.loadTestsFromTestCase(TestNonsealCrossFmMockCohortThirdExtension)
    ...
    cases = [
        ...
        {
            "case": "test_third_extension_specs_include_fm18_to_fm19",
            "result": "PASS",
        },
        {"case": "test_nonseal_chain_anchors_pass", "result": "PASS"},
        {
            "case": "test_frozen_isolation_blocks_mock21_allows_mock22",
            "result": "PASS",
        },
