...
identity lock runner（离线 · C-FM-39）。

Usage:
    python3 lab/run_cninfo_c_class_scale_risk_band_combined_dryrun_resume_formula_safety.py
    python3 lab/run_cninfo_c_class_scale_risk_band_combined_dryrun_resume_formula_safety.py \\
      --output-root outputs/validation/_mock_c_fm39_scale_risk_band_combined_dryrun_resume_formula_safety
"""
...
from cninfo_c_class_scale_risk_band_combined_dryrun_resume_formula_safety import (  # noqa: E402
    ...
    TASK_ID,
    RiskBandCombinedDryrunResumeFormulaPaths,
    run_scale_risk_band_combined_dryrun_resume_formula_safety,
)
from cninfo_c_class_erad_cleanup_guard import BASE_DIR  # noqa: E402

REPORT_ROOT_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_risk_band_combined_dryrun_resume_formula_safety"
)
REPORT_MD_REL = (
    "outputs/validation/"
    "cninfo_c_class_scale_risk_band_combined_dryrun_resume_formula_safety_20260715.md"
)
REPORT_JSON_REL = (
    "outputs/validation/"
...
TASK_REPORT_REL = (
    "outputs/validation/"
    "cninfo_c_class_fm39_scale_risk_band_combined_dryrun_resume_formula_safety_20260715.md"
)
...
def write_reports(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    for key, name in (
        ("partial_risk_band_path", "partial_risk_band_membership_freeze_ledger.json"),
        ("combined_dryrun_path", "combined_dryrun_coverage_identity_lock_ledger.json"),
        ("risk_band_formula_path", "risk_band_formula_identity_lock_ledger.json"),
        ("resume_formula_path", "resume_formula_identity_lock_ledger.json"),
    ):
    ...
    json_payload = {
        ...
        "partial_risk_band_membership_sha256": payload[
        ...
        "risk_band_formula_path": payload["risk_band_formula_path"],
        "resume_formula_path": payload["resume_formula_path"],
    ...
    md_lines = [
        ...
        *layer_lines,
        ...
        "## Scale / safety gain",
        "",
        "- FM38 连续：unique=**2249** · dryrun=**1053** · Δ2 · status=**2134/106/9** · "
        "resume=**28/1/0** · residual=**117=106+9+2** · additive/tier formulas · "
        "complete/overlap/additive/tier fps",
        "- partial_risk_band_membership_freeze：**75/14/12/5** · membership sha256 锁 · "
        "inject/drop/reclass 拒绝",
        ...
        "",
        "```",
        f"c_fm_39_scale_risk_band_combined_dryrun_resume_formula_safety_gate = "
        f"{payload['gate']}",
        "execute_production_snapshot_rebuild = false",
        "approved_for_snapshot_rebuild = false",
        "cninfo_calls = 0",
    ...
    task_lines = [
        ...
        "## Task",
        "",
        "在 C-FM-38 已 commit 且 EXECUTE 仍 human-held 之上，继续 **非 seal** "
        "规模/安全能力（非 extension↔drift / seal-chain）：**partial_risk_band membership "
        "freeze（75/14/12/5）**、**combined_dryrun_coverage identity lock（1053）**、"
        "**risk_band_formula identity lock（75+14+12+5=106）**、**resume_formula "
        "identity lock（28+1+0=29）**、**FM38 连续 + MOCK41 隔离**；"
        "产物写入隔离 MOCK41（不覆盖 MOCK3–40）。",
        "",
        "## Capability gain",
        "",
        "1. FM38 packet/fingerprint/gate/ledger 零漂移连续（unique=2249 · 1053 · "
        "Δ2 · 2134/106/9 · resume 28/1/0 · residual 117=106+9+2 · risk 75/14/12/5 · "
        "additive/tier formulas）",
        "2. partial_risk_band_membership_freeze：75/14/12/5 · membership sha256 锁 · "
        "inject/drop/reclass 拒绝",
        "3. combined_dryrun_coverage_identity_lock：1053 · inflate/deflate 拒绝",
        "4. risk_band_formula_identity_lock：75+14+12+5=106 · 公式变异拒绝",
        "5. resume_formula_identity_lock：28+1+0=29 · 公式变异拒绝",
        "6. output-root：MOCK3–40 冻结 · MOCK41 放行；harvest/resume 写拒绝",
        ...
        "| 路径 | 变更 |",
        "|------|------|",
        "| `lab/cninfo_c_class_scale_risk_band_combined_dryrun_resume_formula_safety.py` | **新增** 核心 |",
        "| `lab/run_cninfo_c_class_scale_risk_band_combined_dryrun_resume_formula_safety.py` | **新增** runner |",
        "| `lab/test_cninfo_c_class_scale_risk_band_combined_dryrun_resume_formula_safety.py` | **新增** 测试 |",
        "| `outputs/validation/cninfo_c_class_erad_protected_output_roots.csv` | C-ROOT-MOCK41 + C-ROOT-002 说明 |",
        ...
        "| 允许 | 禁止 |",
        "|------|------|",
        "| validation/_mock_* risk-band membership / combined-dryrun/resume formula ledger / battery / packet | 生产 snapshot EXECUTE |",
        "| 只读 FM01–05 / FM12–38 gate JSON / harvest / resume / dryrun / exclusion | CNINFO live |",
        "| offline QA · risk-band membership / combined-dryrun/resume formula 重算（不覆盖 MOCK3–40） | 覆盖 MOCK3–40 |",
        ...
        "## Next",
        "",
        "- Controller 可 commit 本包（risk-band membership + combined-dryrun/resume formula only）",
    ...
    with open(readme, "w", encoding="utf-8") as fh:
        fh.write(
            "\n".join(
                [
                    "# C-FM-39 mock scale risk-band/combined-dryrun/resume formula safety root",
                    "",
                    f"gate: `{payload['gate']}`",
                    "seal_chain_extended: false",
                    f"scale_tier_count: {payload['scale_tier_count']}",
                    ...
                    f"resume_taxonomy: {payload['resume_improved']}/"
                    f"{payload['resume_same']}/{payload['resume_worse']}",
                    "does_not_overwrite: _mock_c_fm02.._mock_c_fm38 / standard dryrun",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "C-FM-39 scale complete/overlap membership + "
    ...
    if args.execute:
        print(
            ...
            "C-FM-39 is offline QA only; KEEP_EXECUTE_FALSE",
            file=sys.stderr,
        )
        return 2

    payload = run_scale_risk_band_combined_dryrun_resume_formula_safety(
        paths=RiskBandCombinedDryrunResumeFormulaPaths(
