"""
CNINFO C-class Phase 2 smoke 188 snapshot quality review 测试（只读 · 无 CNINFO）。

运行：
    python lab/test_cninfo_c_class_phase2_smoke_188_snapshot_quality_review.py
"""
...
from review_cninfo_c_class_phase2_smoke_188_snapshot_quality import (  # noqa: E402
    BASE_DIR,
    COMPLETENESS_CSV,
    ...
    run_phase2_smoke_188_snapshot_quality_review,
)

TEST_SUMMARY_PATH = os.path.join(
    BASE_DIR,
    "outputs/validation/cninfo_c_class_phase2_smoke_188_snapshot_quality_review_test_summary.md",
)


class TestPhase2Smoke188SnapshotQualityReview(unittest.TestCase):
  def test_case1_188_json_files_detected(self) -> None:
      snapshots, stats = load_all_valid_snapshots(PHASE2_SNAPSHOT_DIR)
  ...
  def test_case2_excluded_codes_absent(self) -> None:
      _, stats = load_all_valid_snapshots(PHASE2_SNAPSHOT_DIR)
      self.assertEqual(stats["excluded_code_present_count"], 0)
      paths = {
          os.path.basename(p)[:-5]
          for p in os.listdir(PHASE2_SNAPSHOT_DIR)
          if p.endswith(".json")
      }
      self.assertFalse(paths & set(EXCLUDED_CODES))

  def test_case3_invalid_json_count_zero(self) -> None:
      _, stats = load_all_valid_snapshots(PHASE2_SNAPSHOT_DIR)
  ...
  def test_case4_status_csv_regenerated_from_json(self) -> None:
  ...
  def test_case5_quality_reports_generated(self) -> None:
      ...
      with open(QA_SUMMARY_MD, encoding="utf-8") as fh:
          content = fh.read()
      self.assertIn("phase2_smoke_188_snapshot_qa_gate = PASS_WITH_CAVEAT", content)


def write_test_summary(results: list) -> None:
    passed = sum(1 for r in results if r["status"] == "PASS")
    lines = [
        "# CNINFO C-Class Phase 2 Smoke 188 Snapshot Quality Review Test Summary",
        "",
...
def main() -> int:
    ...
    suite = loader.loadTestsFromTestCase(TestPhase2Smoke188SnapshotQualityReview)
    result = unittest.TextTestRunner(verbosity=2).run(suite)

    cases = [
        ("case_1_188_json_files_detected", "test_case1_188_json_files_detected"),
        ("case_2_excluded_codes_absent", "test_case2_excluded_codes_absent"),
        ("case_3_invalid_json_count_zero", "test_case3_invalid_json_count_zero"),
        ("case_4_status_csv_regenerated_from_json", "test_case4_status_csv_regenerated_from_json"),
