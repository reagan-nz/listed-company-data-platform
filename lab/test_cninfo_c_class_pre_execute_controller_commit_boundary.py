"""
CNINFO C-class — Pre-EXECUTE controller commit-boundary 单测（离线 · CNINFO=0 · C-FM-08）。

运行：
    python3 lab/test_cninfo_c_class_pre_execute_controller_commit_boundary.py
"""
...
from cninfo_c_class_pre_execute_controller_commit_boundary import (  # noqa: E402
    ...
    CommitBoundaryPaths,
    assert_commit_boundary_output_root,
...
_RUNNER = os.path.join(
    _LAB_DIR, "run_cninfo_c_class_pre_execute_controller_commit_boundary.py"
)
_TEST_SUMMARY_REL = (
    "outputs/validation/"
    "cninfo_c_class_pre_execute_controller_commit_boundary_test_summary_20260715.md"
)
...
def _write_test_summary(cases: list) -> None:
    ...
    lines = [
        "# C-FM-08 Pre-EXECUTE Controller Commit-Boundary — Test Summary",
        "",
        "_offline · CNINFO=0_",
        "",
        "| case | result |",
    ...
    lines.extend(
        [
            "",
            "```",
            "c_fm_08_pre_execute_controller_commit_boundary_test_gate = PASS_OFFLINE",
            "cninfo_calls = 0",
...
class TestPreExecuteControllerCommitBoundary(unittest.TestCase):
    def test_output_root_requires_mock_and_not_fm06_fm07(self) -> None:
        ...
        with self.assertRaises(RuntimeError):
            assert_commit_boundary_output_root(FM06_MOCK_ROOT_REL)
        with self.assertRaises(RuntimeError):
            assert_commit_boundary_output_root(FM07_MOCK_ROOT_REL)
        norm = assert_commit_boundary_output_root(DEFAULT_MOCK_OUTPUT_ROOT_REL)
        self.assertIn("_mock_c_fm08", norm)
    ...
    def test_fm_battery_requires_all_seven(self) -> None:
        good = {
            "gate": "PASS_OFFLINE",
            "cninfo_calls": 0,
            "execute_production_snapshot_rebuild": False,
        ...
        bad = dict(good)
        bad["gate"] = "FAIL"
        _rows, checks = build_fm01_to_07_gate_battery_rows(
            fm01=good,
            fm02=good,
            fm03=good,
            fm04=good,
        ...
        self.assertFalse(checks["fm01_to_07_battery_all_pass"])
    ...
    def test_full_commit_boundary_pass_isolated_mock(self) -> None:
        result = run_pre_execute_controller_commit_boundary(
            paths=CommitBoundaryPaths(
                output_root_rel="outputs/validation/_mock_c_fm08_cli_test_tmp"
            )
        )
    ...
    def test_cninfo_not_called(self) -> None:
        with mock.patch("requests.get") as get_mock, mock.patch(
            ...
            run_pre_execute_controller_commit_boundary(
                paths=CommitBoundaryPaths(
                    output_root_rel=(
                        "outputs/validation/_mock_c_fm08_unit_cninfo_probe"
                    )
                )
            )
...
if __name__ == "__main__":
    ...
    suite = loader.loadTestsFromTestCase(TestPreExecuteControllerCommitBoundary)
    result = unittest.TextTestRunner(verbosity=2).run(suite)
    cases = [
        {
            "case": "test_output_root_requires_mock_and_not_fm06_fm07",
            "result": "PASS",
        },
        {"case": "test_auth_index_write_still_forbidden", "result": "PASS"},
        {"case": "test_fm_battery_requires_all_seven", "result": "PASS"},
        {"case": "test_boundary_matrix_fingerprint_stable", "result": "PASS"},
        {
            "case": "test_full_commit_boundary_pass_isolated_mock",
            "result": "PASS",
        },
