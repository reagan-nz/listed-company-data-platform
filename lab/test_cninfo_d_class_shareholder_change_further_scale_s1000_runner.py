...
# -*- coding: utf-8 -*-
"""
D 类 shareholder_change further-scale S1000 runner 测试（无真实 CNINFO · mock live 允许）。
...
import run_cninfo_d_class_shareholder_change_further_scale_s1000 as s1000  # noqa: E402
import run_cninfo_d_class_tiny_live_validation as core  # noqa: E402

BASE_DIR = s1000.BASE_DIR
RUNNER = os.path.join(
    _LAB_DIR, "run_cninfo_d_class_shareholder_change_further_scale_s1000.py"
)
...
S200_LOCK_SHA256 = (
    "e653b9624056eec86516d75000ffc4c6e14fcfacd37a65eb49eb1f0084324823"
)
S200_LIVE_SHA256 = (
    "825ef1ac405896ea183b03e19b3c6b140025884c8eaed720eb07e9aa0232c02a"
)
SC_NEXT_LOCK_SHA256 = (
...
def _synthetic_universe(
    ...
    for i in range(found_n):
        case_id = f"DSC{s1000.CASE_ID_START + i}"
        code = f"{910000 + i:06d}"
    ...
    for j in range(empty_n):
        case_id = f"DSC{s1000.CASE_ID_START + found_n + j}"
        code = f"{1 + j:06d}"
        # 避开 present 合成 found：使用低号段空控
        rows.append(
            {
                "case_id": case_id,
                "company_code": code,
...
class TestShareholderChangeFurtherScaleS1000Runner(unittest.TestCase):
    def test_validate_universe_size_and_ids(self) -> None:
        rows = _synthetic_universe()
        self.assertEqual(len(rows), 1000)
        self.assertEqual(rows[0]["case_id"], "DSC501")
        self.assertEqual(rows[-1]["case_id"], "DSC1500")
    ...
    def test_live_without_approve_blocked(self) -> None:
        ...
        self.assertEqual(result.returncode, 2)
        self.assertTrue(
            "approve_d_class_shareholder_change_further_scale_s1000_required"
            in result.stderr
    ...
    def test_frozen_s50_s200_esh_at_untouched(self) -> None:
    ...
    def test_universe_lock_when_present(self) -> None:
        ...
        for code in ("002415", "000550", "600000"):
            self.assertNotIn(code, found_codes)
        issues = s1000.validate_universe(rows)
        self.assertEqual(issues, [])
