"""
B-FM-23：监事会决议 known-document live allow-list 锁测（离线；不请求 CNINFO）。

覆盖：
- allow-list 仅含 supervisory_board_known_001（category 空）
- 字段与 fixtures ready 对齐
- mock live 路径可对 known_001 得到 pass（predicted_document_type=announcement）
- 不包含 shareholder_meeting_known_001–007 / board_resolution_known_001（不重开）

无真实 CNINFO · 不造 §7 FP。

运行：
    python lab/test_cninfo_b_class_supervisory_board_known_001_live.py
"""
...
LIVE_DIR = os.path.join(
    ...
    "outputs",
    "validation",
    "cninfo_b_class_supervisory_board_known_001_live_20260715",
)
...
class TestSupervisoryBoardKnown001LiveAllowList(unittest.TestCase):
    ...
    def test_allowlist_fields_match_fixtures(self) -> None:
        ak = _by_id(_load_cases(KNOWN_ALLOW), "supervisory_board_known_001")
        fk = _by_id(_load_cases(FIXTURE_KNOWN), "supervisory_board_known_001")
        for key in (
            "company_code",
            "company_name",
            "title_pattern",
            ...
            self.assertEqual(ak[key], fk[key], f"supervisory_board_known_001.{key}")
    ...
    def test_mock_live_known_001_pass(self) -> None:
        ...
        known_case = _by_id(_load_cases(KNOWN_ALLOW), "supervisory_board_known_001")
        fake_known = [
            {
