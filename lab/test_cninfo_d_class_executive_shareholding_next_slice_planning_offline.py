...
# -*- coding: utf-8 -*-
"""
CNINFO D 类 executive_shareholding — D-FM-53 next-slice offline planning smoke。
...
ESH_CASE_IDS = ("DES101", "DES102", "DES103", "DES104", "DES105")
EXCLUDED_CODES = {"688671", "301259"}
FORBIDDEN_TIME_MARK = {"oneMonth"}
EXPECTED_TIME_MARK = "threeMonth"
EXPECTED_VARY_TYPE = "b"

PLANNING_MD = (
    BASE_DIR
    / "plans"
    / "cninfo_d_class_executive_shareholding_next_slice_planning_20260716.md"
)
MATRIX_CSV = (
    BASE_DIR
    / "outputs"
    / "validation"
    / "cninfo_d_class_executive_shareholding_next_slice_candidate_matrix_20260716.csv"
)
SKETCH_CSV = (
    BASE_DIR
    / "outputs"
...
RECOMMENDATION_MD = (
    ...
    / "outputs"
    / "validation"
    / "cninfo_d_class_executive_shareholding_next_slice_recommendation_20260716.md"
)
SUMMARY_MD = (
    BASE_DIR
    / "outputs"
    / "validation"
    / "cninfo_d_class_executive_shareholding_next_slice_planning_summary_20260716.md"
)
NEXT_STEP_MD = (
    BASE_DIR
    / "outputs"
    / "validation"
    / "cninfo_d_class_executive_shareholding_next_slice_next_step_recommendation_20260716.md"
)
CAVEAT_LEDGER_CSV = (
    BASE_DIR
    / "outputs"
    / "validation"
    / "cninfo_d_class_executive_shareholding_next_slice_final_caveat_ledger.csv"
)
EVIDENCE_MD = (
    BASE_DIR
    / "outputs"
    / "validation"
    / "cninfo_d_class_executive_shareholding_dfm53_next_slice_planning_20260716.md"
)
...
class ExecutiveShareholdingNextSlicePlanningOfflineTests(unittest.TestCase):
    """D-FM-53：ESH next-slice sketch + frozen-root sha256。"""
    ...
    def test_universe_sketch_five_cases_dense_mode(self) -> None:
        ...
        self.assertEqual(tuple(r["case_id"] for r in rows), ESH_CASE_IDS)
        for row in rows:
            self.assertEqual(row["component"], "executive_shareholding")
            self.assertEqual(row["next_slice_include"], "yes")
            self.assertEqual(row["universe_lock_status"], "draft_not_locked")
            self.assertEqual(row["time_mark"], EXPECTED_TIME_MARK)
            self.assertEqual(row["vary_type"], EXPECTED_VARY_TYPE)
            self.assertNotIn(row["time_mark"], FORBIDDEN_TIME_MARK)
            ...
            self.assertIn("exclude_ess_h3_h4", row["exclude_flags"])
        ...
        self.assertEqual(rows[0]["company_code"], "002415")
        self.assertEqual(rows[0]["structure_cite_reference"], "yes")
        self.assertEqual(rows[3]["company_code"], "000550")
        self.assertEqual(rows[4]["expected_behavior"], "empty_but_valid")

    def test_dc006_is_structure_cite_not_dense_threeMonth(self) -> None:
        """DC006 为 envelope 结构 cite；不得误导 denser threeMonth company found。"""
        payload = json.loads(DC006.read_text(encoding="utf-8"))
        self.assertEqual(
            payload["_fixture_meta"]["component"], "executive_shareholding"
        )
        self.assertEqual(payload["executive_shareholding"]["company_code"], "999106")
        lineage = payload["market_event"]["lineage"]
        self.assertNotEqual(lineage.get("query_mode"), "timeMark_threeMonth_varyType_b")

    def test_priority2_threeMonth_dense_cite_present(self) -> None:
        rows = _load_csv(PRIORITY2_STABILITY)
        by_id = {r["test_case_id"]: r for r in rows}
        three = by_id["esh_threeMonth_varyType_b"]
        one = by_id["esh_oneMonth_varyType_b"]
        self.assertEqual(three["source_id"], "executive_shareholding")
        self.assertEqual(int(three["observed_total_rows"]), 1862)
        self.assertEqual(int(one["observed_total_rows"]), 842)
        self.assertGreater(
            int(three["observed_total_rows"]), int(one["observed_total_rows"])
        )

    def test_candidate_matrix_ranks_esh_primary_and_forbids_live(self) -> None:
        rows = _load_csv(MATRIX_CSV)
        by_id = {r["option_id"]: r for r in rows}
        self.assertEqual(
            by_id["EXECUTIVE_SHAREHOLDING_NEXT_SLICE_OFFLINE"]["recommendation_rank"],
            "1",
        )
        self.assertEqual(
            by_id["EXECUTIVE_SHAREHOLDING_NEXT_SLICE_OFFLINE"]["status"],
            "primary_executed",
        )
        ...
        self.assertEqual(
            by_id["EXECUTIVE_SHAREHOLDING_FIRST_SLICE_RELIVE"]["status"], "forbidden"
        )
        self.assertEqual(by_id["SC_FIRST_OR_NEXT_MUTATE"]["status"], "forbidden")
        self.assertEqual(by_id["RSU_FIRST_OR_NEXT_MUTATE"]["status"], "forbidden")
        self.assertEqual(
            by_id["EP_FIA_AT_SD_FROZEN_ROOT_MUTATE"]["status"], "forbidden"
        )
        self.assertEqual(
            by_id["SC_NEXT_SLICE_BOUNDED_LIVE"]["status"], "forbidden_this_round"
        )
        self.assertEqual(
            by_id["ESS_DEVTOOLS_PAUSE_HOLD"]["status"], "hold_documented"
        )
    ...
    def test_planning_and_gates_text(self) -> None:
        ...
        self.assertIn("DES101", planning)
        self.assertIn("threeMonth", planning)
        self.assertIn("varyType=b", planning)
        self.assertIn(
        ...
        self.assertIn("primary_component = executive_shareholding", recommendation)
    ...
    def test_validation_rules_cover_core_ids(self) -> None:
        ...
        self.assertIn("threeMonth", text)
        self.assertIn("oneMonth", text)
        self.assertIn("leader/detail", text)

    def test_caveat_ledger_has_esh_ns_entries(self) -> None:
        rows = _load_csv(CAVEAT_LEDGER_CSV)
        ids = {r["caveat_id"] for r in rows}
        self.assertIn("CAV-ESH-NS-001", ids)
        self.assertIn("CAV-ESH-NS-002", ids)
        self.assertIn("CAV-ESH-NS-003", ids)
        self.assertIn("CAV-ESH-NS-007", ids)
        self.assertIn("CAV-ESH-NS-010", ids)
    ...
    def test_checklist_blocks_live_and_runner(self) -> None:
        ...
        self.assertEqual(by_id["ESH-NS-GATE-01"]["status"], "READY_FOR_APPROVAL")
        self.assertEqual(by_id["ESH-NS-SAFE-20"]["status"], "policy")
        self.assertEqual(by_id["ESH-NS-SAFE-21"]["status"], "policy")

    def test_registry_executive_shareholding_still_present(self) -> None:
        self.assertTrue(REGISTRY.is_file())
        text = REGISTRY.read_text(encoding="utf-8")
        self.assertIn("source_id: executive_shareholding", text)
        self.assertIn("leader/detail", text)
        self.assertIn("threeMonth_varyType_b", text)

    def test_esh_first_slice_live_report_still_present(self) -> None:
        rows = _load_csv(ESH_FIRST_LIVE_REPORT)
        self.assertEqual(len(rows), 5)
        self.assertEqual(rows[0]["case_id"], "DES001")
