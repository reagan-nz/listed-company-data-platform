...
- **金融公司字段体系 v1 文档**（Issue #3）：`docs/financial_company_schema.md` 定义银行/券商/保险三类子 schema 与评估分开报告建议（文档设计，未改抽取代码）
...
3. **金融公司 schema 实现**：v1 设计文档已完成（[docs/financial_company_schema.md](docs/financial_company_schema.md)）；`field_schema.py` 仍为 generic `FINANCIAL_FIELD_SPECS`，bank/broker/insurer 子 schema 尚未写入代码。
...
2. 金融公司子 schema 实现（Issue #4）：按 [docs/financial_company_schema.md](docs/financial_company_schema.md) 写入 `bank_v1` / `broker_v1` / `insurer_v1` 至 `field_schema.py`
