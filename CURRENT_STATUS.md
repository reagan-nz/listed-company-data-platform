...
| **Gate** | `a_class_phase1_boundary_gate = PASS_WITH_CAVEAT` · `a_class_phase2_metadata_execution_gate = FAIL_REVIEW_REQUIRED` · `a_class_phase2_failed_retry_planning_gate = READY_FOR_APPROVAL` · `a_class_phase2_failed_retry_execution_gate = FAIL_REVIEW_REQUIRED` · `a_class_phase2_metadata_closure_gate = PASS_WITH_CAVEAT_NETWORK_UNRESOLVED` |
| **下一步** | 网络恢复后 Option A/B retry 8 unresolved cases（**仍无 PDF · 无 verified · 不扩展 50 家**） |
...
| **Gate** | `b_class_phase25_failed_retry_package_gate = READY_FOR_APPROVAL` · `b_class_phase25_expansion_closure_gate = PASS_WITH_CAVEAT` · `b_class_phase25_expansion_execution_gate = PASS_WITH_CAVEAT` · `b_class_phase25_expansion_runner_gate = READY_FOR_APPROVAL` · `b_class_phase25_expansion_planning_gate = READY_FOR_APPROVAL` · `b_class_phase2_expansion_closure_gate = PASS_WITH_CAVEAT` · `b_class_phase2_expansion_execution_gate = PASS_WITH_CAVEAT` · `b_class_phase1_tiny_live_closure_gate = PASS_WITH_CAVEAT` |
| **下一步** | 人工批准 5-case isolated retry → `--approve-b-class-phase25-failed-retry` live（**仍无 PDF** · **无 verified** · **不重跑 45**） |
...
| **Candidate intake gate** | `d_class_known_event_candidate_intake_gate = WAITING_FOR_HUMAN_INPUT`（**不是 ready_for_live**） |
| **下一步** | 人工填写 DLC003R/DLC006R candidate template · 重跑 intake validator（**均不自动执行**） |
