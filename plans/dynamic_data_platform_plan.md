...
> 本文件是当前阶段的总体计划。它**不是**已经做出来的功能，而是架构方向 + 小范围试点准备。  
> **已完成工作**与**未来计划**分开记录：下方「演进现实」描述仓库已发生的路径；第 9 节及以后仍是未来试点，**不等于已交付**。
...
```text
静态年报数据库（Era B 底座 · 已交付）
    ↓
动态数据平台方向（文档规划 · 存储设计暂缓全量落地）
    ↓
CNINFO A–D 数据采集能力研究（进行中 · PASS_WITH_CAVEAT 为主）
    ↓
多 agent 数据收集工作流（Controller + executors/reviewers · 本地自治 / push 人控）
```

| 层 | 状态 | 说明 |
|----|------|------|
| 静态年报底座 | **已完成** | SQLite 字段级记录 + 质量审计（见 CHANGELOG Stage 1–3） |
| 动态平台产品 | **未完成** | RAG / Wiki / 推送仍属未来能力；不写 production_ready |
| CNINFO 能力研究 | **进行中** | 见 CURRENT_STATUS / eraC / eraD；gate 多为 `PASS_WITH_CAVEAT` |
| 多 agent 工作流 | **本地已落地策略与 agent 定义** | `0f63a90` / `d385bb6` / `4a62f78` / `PROJECT_CONTROL`；**无 verified** · **无 push** |
| 三层存储部署 | **仍暂缓** | 见 storage_schema_design_plan；不接生产 DB |

---

## 一句话

把现有的「2024 年报静态数据库」逐步升级为「动态上市公司数据平台」。本阶段先把架构方向想清楚，再做小范围试点，**不直接全量开发、不直接全量迁移**。数据采集侧已进入多 agent 编排，但平台产品能力仍未交付。
