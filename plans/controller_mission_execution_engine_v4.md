...
_修订：Run 14 回归修复 — 恢复 queue-driven agent ownership（保留异步，禁止 Controller 代执行轨能力）_
...
5. **能力任务必须由轨 agent 执行**（§1.3）；异步 ≠ Controller 亲自写轨代码。

---

# 1.3 Controller Ownership vs Track Agent Ownership（Run 14 regression fix）

Run 14 证明了异步有效，但引入执行所有权回归：Controller 会话直接完成 A/C 能力实现。

**目标组合：**

| 来源 | 保留 |
|------|------|
| Run 12 | queue depth · multi-agent dispatch · continuous refill |
| Run 14 | independent track loops · **no** Global Wave barrier |
...
candidate discovery
priority scoring
queue generation / refill
agent dispatch
evidence / review coordination
git-boundary coordination + explicit-path commit gate
```

### Controller FORBID

```text
implementing track capability code itself
performing A/B/C/D capability work directly
substituting for a/b/c/d-class-executor on queued tasks
```

### Required task ownership fields

每个生成并入队的任务**必须**含：

```yaml
track: A|B|C|D
executor: a-class-executor | b-class-executor | c-class-executor | d-class-executor
controller_execution_allowed: false   # 恒为 false；禁止省略
```

`EXECUTING` 语义 = **对应 executor agent 已派发且在工作**，不是 Controller 正在手写该轨产物。

### Queue depth（与 Track Queue v2 对齐）

活跃轨（有有价值安全任务时）：

```text
queue_depth >= 1
prefer queue_depth = 2   # current + successor
```

完成一任务后：头任务出队 → successor 晋升 → 立刻 generate 新候选 refill（不等其他轨）。

### Review routing（不变，重申）

```text
executor result
  → evidence-auditor
  → regression-reviewer   # if shared runner / core path changed
  → git-boundary-reviewer
  → commit
```

审阅与 commit 由 Controller **协调**；实现仍属 executor。
...
TrackAsyncState:                         # v4 NEW
  ...
  active_executor: a-class-executor|...  # 与 active_task.executor 一致
  queue_depth: <int>                     # prefer 2；活跃轨 >= 1
  ...
  while budget_ok and not safety_stop:
    ...
    for done in poll_completed_agents():
      ...
      if evidence.ok and boundary.ready and commit_budget_remaining():
        ...
        # 3. 立刻派发轨 agent（若有价值）— Controller 不亲自执行能力工作
        if T.queue.has_valuable_ready():
          T.lifecycle = READY
          assert task.controller_execution_allowed == false
          dispatch_agent(task.executor, task)     # a/b/c/d-class-executor only
          T.lifecycle = EXECUTING
          refill_track_queue_to_prefer_depth_2(T) # successor ready while current runs
        else:
...
- 启用需人类确认（与 v3 相同）。  
- 验证建议：至少一次 Run 证明 (a) 异步不阻塞；(b) **≥2 个轨 agent 真实执行**；(c) Controller 仅调度/审阅/commit，`controller_execution_allowed=false`。  
- Push 仍人控。
...
- 不是多进程/多机编排实现规范（仍可由单 Controller 会话用**后台 track agent** 模拟异步）  
- 不是取消 evidence/regression/boundary  
- 不是提高 commit 噪音（仍禁止 `git add .`；仍按域分包）  
- 不是放宽 push / 破坏性动作门控  
- **不是**允许 Controller 为“赶进度”而代写轨能力代码（§1.3）
