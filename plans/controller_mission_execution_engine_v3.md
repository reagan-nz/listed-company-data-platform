...
8. 一个 **Approval Guidance Layer**（§10.1）：`HUMAN_GATE_BLOCKED` 时的纯呈现层，把已有的 blocked action / exact phrase / scope / successor 信息拼装展示，**不推断批准、不接受 paraphrase、不绕过 exact-phrase 要求**——只读附加物，human interrupt v2 §6 仍是唯一判定权威。
...
| 层级 | 允许的停机值 | 触发条件 |
|------|--------------|----------|
| **Track T** | `HUMAN_GATE_BLOCKED` | T 的下一个高价值动作需要审批，且 Autonomous Queue 为空或只剩低价值残留 |
...
v3 不改变 [mission objective v2 §5](controller_mission_objective_v2.md)（"Approval is a synchronization point, not a global workflow stop"）或 [human interrupt v2](controller_human_interrupt_policy_v2.md) 的分级规则。v3 唯一要做的是把这条原则显式接到 §2.2 `TrackCapabilityState.stop_reason` 与 §3.2 停机表上：


```text
Example — D shareholder_change 需要审批：

D.TrackCapabilityState.human_blocked_actions = [shareholder_change live execute]
D.TrackCapabilityState.available_autonomous_actions =
  [schema refinement, event taxonomy, sample preparation, validation rules, offline evidence mapping]
D.stop_reason = HUMAN_GATE_BLOCKED   （仅当 available_autonomous_actions 为空或已耗尽时才成立——task generator v2 §3.1）
...
## 10.1 Approval Guidance Layer（NEW · 纯呈现层，不改变任何审批规则）


### 目的


当某轨 `stop_reason = HUMAN_GATE_BLOCKED` 时，Controller 目前只输出"被卡住"这一事实（track stop reason v2 §2）。人类要采取行动，仍需自己去翻 [human interrupt v2 §6](controller_human_interrupt_policy_v2.md) 的 exact-phrase 表、去 `PROJECT_CONTROL.md` 找当前 approval 状态、再去 task memory 找"批准后具体执行什么"。**Approval Guidance Layer** 只是把这三处已存在的信息**拼装展示**在同一处，减少人类的查找成本——它不引入新的判断，不产生新的审批状态，不做任何自动化的"猜测批准"。


### 硬性边界（不可违反）


1. **不推断批准。** 本层永不将"用户描述了大致意图"「should be treated as approved」「已经批准了差不多的东西」视为满足审批。批准判定继续 100% 由 [human interrupt v2 §6](controller_human_interrupt_policy_v2.md) 的 exact-phrase 匹配决定——本层不参与该判定，只在判定**尚未满足**时生成提示。
2. **不接受 paraphrase。** 若人类输入的批准语句与 §6 表中的 exact phrase 不逐字匹配（或不满足该 track 的具体 scope），本层必须继续展示"仍需以下 exact phrase"，不得因为语义相近就静默放行。
3. **不绕过 exact-phrase 要求。** 本层是 [human interrupt v2](controller_human_interrupt_policy_v2.md) 的**只读呈现附加物**，不是审批机制的替代实现；若本层与 §6 冲突，**§6 权威优先**。
4. 本层只在 `stop_reason = HUMAN_GATE_BLOCKED` 时触发；不适用于 `NO_SAFE_AUTONOMOUS_TASK` / `LOW_PRIORITY_DEFERRED` / 其余 stop_reason（那些不是审批缺口，展示 exact phrase 无意义）。


### 输出 schema（`ApprovalGuidance`，附加于 §2.2 `TrackCapabilityState.human_blocked_actions` 之上）


```yaml
ApprovalGuidance:                        # 仅当 stop_reason == HUMAN_GATE_BLOCKED 时生成
  track: A | B | C | D
  blocked_action: "<一句话，来自 human_blocked_actions 的具体条目>"
  reason: "<为什么这一步需要人类，引用 PROJECT_CONTROL / human interrupt v2 §3 分类>"
  required_approval_phrase: "<原文抄录 human interrupt v2 §6 对应的 exact phrase 模板；scope 未定的字段（如 <exact live scope id>/<exact universe>）保留占位符，不由 Controller 自行填空>"
  approval_scope: "<该 phrase 仅授权的确切范围，抄录 human interrupt v2 §6 末行：不外溢到 push / 其他 track / 其他 scope>"
  expected_next_execution: "<批准后将执行的具体动作，来自 task memory / continuation policy 的已知 successor，而非新发明>"
  phrase_match_status: NOT_YET_SUPPLIED | SUPPLIED_BUT_MISMATCHED | EXACT_MATCH_CONFIRMED
```


`phrase_match_status` 的判定规则：


| 状态 | 判定条件 |
|---|---|
| `NOT_YET_SUPPLIED` | 本轮对话中尚未出现任何声称批准的语句 |
| `SUPPLIED_BUT_MISMATCHED` | 出现了批准意图的语句，但与 §6 exact phrase **不逐字匹配**，或缺少必需的 scope 占位符填值（如未指明 `<exact universe>`）——**必须视为未批准**，不得部分放行 |
| `EXACT_MATCH_CONFIRMED` | 逐字匹配 §6 模板 **且** scope 占位符已被人类显式填写为具体值 |


只有 `EXACT_MATCH_CONFIRMED` 才允许 Controller 把该 track 从 `HUMAN_GATE_BLOCKED` 移出并进入 §7 execution loop 的下一步；`SUPPLIED_BUT_MISMATCHED` 必须继续展示 `ApprovalGuidance`（而不是静默假定批准），并明确指出不匹配的具体原因（例如"缺少 exact universe"或"措辞与模板不同"）。


### Worked example（对齐用户提供的示例，验证与既有政策一致）


```text
D:
Blocked:
  shareholder_change component progression

Required approval:
  "I approve D-class shareholder_change as the next Era D component."
  （逐字抄录 human interrupt v2 §6 "D component" 行，非 Controller 改写）

Approval scope:
  仅授权 D-class shareholder_change 作为下一个 Era D component 的启动决定；
  不授权 S4 runner 的 live 执行、不授权 push（human interrupt v2 §6 末行）。

After approval:
  Execute S4 implementation workflow.
  （来自 task memory D-GEN-20260714-08 S4 runner design 的既定 successor，非新发明）

phrase_match_status: NOT_YET_SUPPLIED（本轮尚无人类逐字给出该短语）
```


这与 [human interrupt v2 §6](controller_human_interrupt_policy_v2.md) 表中的 "D component" 行、以及本文件此前验证周期中对 A/C/D 三个模糊批准声明的处理方式（拒绝笼统描述，要求逐字短语）完全一致——Approval Guidance Layer 只是把当时人工写出的那段分析，标准化成一个每次 `HUMAN_GATE_BLOCKED` 都会自动生成的固定结构。
...
| 字段来源 | 权威（不变） |
|---|---|
| `blocked_action` / `reason` | track stop reason v2 §2/§3.1 · PROJECT_CONTROL Active Approvals |
| `required_approval_phrase` / `approval_scope` | **human interrupt v2 §6**（唯一权威，逐字抄录，不改写） |
| `expected_next_execution` | task memory v2 §2 · task continuation v2 §4（已记录的 successor，不新发明） |
| `phrase_match_status` 判定 | human interrupt v2 §6（"Phrases authorize only the named scope"）——本层仅执行该判定，不修改判定标准 |


### Reporting 集成（附加式，不改变 §11 必填字段）


```markdown
### Approval guidance（v3 §10.1，附加式，仅在存在 HUMAN_GATE_BLOCKED 轨道时出现）
- A: blocked_action / required_approval_phrase / approval_scope / expected_next_execution / phrase_match_status
- C: 同上
- D: 同上
```
...
- 把"用户表达了批准意图"直接置为 `EXACT_MATCH_CONFIRMED`
- 在 `required_approval_phrase` 里自行填入占位符的具体值（scope 必须由人类显式给出，不得 Controller 代填）
- 因为 `SUPPLIED_BUT_MISMATCHED` 状态出现多次而"放宽"匹配标准
- 把本层输出当作审批记录本身（审批记录仍以 human interrupt v2 + PROJECT_CONTROL Active Approvals 表为准）
- 对非 `HUMAN_GATE_BLOCKED` 的 stop_reason 生成本层输出
...
| 条件 | 层级 | 触发前必须完成 |
|------|------|----------------|
| `HUMAN_GATE_BLOCKED` | Track | Autonomous Queue audit（task generator v2 §3.1） |
...
| `HUMAN_INTERRUPT` | Global（S3/S4）或 Track（S2） | human interrupt v2 §8 escalation severity |
...
**禁止：**
- push / force-push（push policy v1 · human interrupt v2 §3.1）
- approval bypass（commit autonomy v2 §4 · task generator v2 §5）
- 未获批的 live CNINFO（task generator v2 §3.2 · §5）
- production mutation（human interrupt v2 §3.4）
- destructive operations（`reset --hard`、清空受保护目录等，human interrupt v2 §3.4）
- 未经批准的范围扩张（human interrupt v2 §3.5）
