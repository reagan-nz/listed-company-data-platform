...
不是"把巨潮所有栏目都抓下来"，而是：**对 CNINFO 主要栏目给出"能不能稳定拿到"的结论**，形成一份可交差的能力盘点。达到下面 3 条即算完成：

1. **类年报（年报/半年报/季报）**结论明确：能稳定检索、能解析报告期、字段可得性有数字。✅ 基本已完成（`validate_cninfo_report_announcements.py`）。
2. **盘点表状态列更新到最新**：`plans/cninfo_data_source_value_inventory.md` 里每个已验证栏目标上真实 `recommended_status`（`testing`/`partial`），未验证的保持 `candidate`。
3. **一份 Era C 总结**：`outputs/validation/cninfo_capability_final_summary.md`，一页说清"CNINFO 现在能稳定拿到什么、拿不到什么、下一步"。

> 做完这 3 条，Era C 就是一个完整闭环。第 4 节的扩展任务是"有余力再做"，不做也不影响交差。
