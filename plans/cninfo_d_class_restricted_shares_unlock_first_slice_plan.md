# CNINFO D 类 restricted_shares_unlock First-Slice Plan
...
| registry path | `restricted_shares_unlock/liftBan/detail`（[registry draft](../config/cninfo_d_class_source_registry_draft.yaml)） |
...
- **主端点：** `liftBan/detail` — 按解禁日 `tdate` 返回当日解禁记录列表
- **公司过滤：** 响应行以 `SECCODE` / `SECNAME` 匹配 universe `company_code`
- **multi-probe（未来 runner）：** base runner 已有 `restricted_shares_unlock` anchor 窗口逻辑（±trade-day 探测 · early_stop）；本 approval package **不实现** first-slice mode
- **字段映射（registry）：** `F003D` unlock_date · `F004N` unlock_amount · `F005N` unlock_ratio · `DECLAREDATE` announcement_date
- **Phase1 先例：** DLC003（300009 安科生物）→ calibrated **`empty_but_valid`**（稀疏解禁日合法零行）
...
| 项 | 值 |
|----|-----|
| approval phrase | **I approve D-class restricted_shares_unlock as the next Era D component.** |
...
| universe draft | [cninfo_d_class_restricted_shares_unlock_first_slice_universe_draft.csv](../outputs/validation/cninfo_d_class_restricted_shares_unlock_first_slice_universe_draft.csv) |

### Anchor `tdate` Policy（离线文档化）

- 全案例共享 **单一** `tdate = 2026-06-08`
- 依据：registry `default_params.tdate` 与 Phase1 DLC003 稀疏日 `empty_but_valid` 先例
- **本任务不调用 CNINFO** 验证该日是否有解禁行；稀疏日零行属 **可接受** 结果
...
DRU001 引用 DLC003（300009 安科生物）Phase1 **acceptable · empty_but_valid** 先例，使用 **独立 DRU case_id**。

---

## 4. Expected Behavior Mix（吸收 block_trade 教训）
...
**禁止：** 将唯一案例标为 sole `captured_normal_candidate` 且绑定可能稀疏的单一 anchor（block_trade DBT002 caveat 教训）。
...
| 项 | 值 |
|----|-----|
| per-case requests | **≤ 4**（multi-probe · anchor 窗口） |
...
- 产物落盘 `outputs/validation/cninfo_d_class_restricted_shares_unlock_first_slice/`
...
- **不** 扩展 margin_trading / disclosure_schedule / block_trade
...
- **本任务不实现** `--restricted-shares-unlock-first-slice` runner 扩展
...
| restricted_shares_unlock Phase1 path | **exists**（DLC003 · multi-probe anchor logic） |
| first-slice mode extension | **required later** — **本任务不实现** |
| planned flags | `--restricted-shares-unlock-first-slice` · `--approve-d-class-restricted-shares-unlock-first-slice` |
...
**禁止写入：** known-event · margin_trading · disclosure_schedule · block_trade · tiny-live v1/v2 原始报告目录。
...
|------|------|
| universe draft | [cninfo_d_class_restricted_shares_unlock_first_slice_universe_draft.csv](../outputs/validation/cninfo_d_class_restricted_shares_unlock_first_slice_universe_draft.csv) |
| approval checklist | [cninfo_d_class_restricted_shares_unlock_first_slice_approval_checklist.md](../outputs/validation/cninfo_d_class_restricted_shares_unlock_first_slice_approval_checklist.md) |
| command draft | [cninfo_d_class_restricted_shares_unlock_first_slice_command_draft.md](cninfo_d_class_restricted_shares_unlock_first_slice_command_draft.md) |
| approval summary | [cninfo_d_class_restricted_shares_unlock_first_slice_approval_summary.md](../outputs/validation/cninfo_d_class_restricted_shares_unlock_first_slice_approval_summary.md) |
| prior sketch | [cninfo_d_class_restricted_shares_unlock_first_slice_plan_draft.md](cninfo_d_class_restricted_shares_unlock_first_slice_plan_draft.md) |
