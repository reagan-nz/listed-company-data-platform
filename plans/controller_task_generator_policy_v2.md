...
# 3.1 Approval Split（normative · 2026-07-15 收窄，见下方修订说明）


**2026-07-15 Scope-Driven Execution Amendment**（[human interrupt v2 §12](controller_human_interrupt_policy_v2.md) 为判定权威）：一旦某 track/component 的**方向**已被人类授权（scope_authorized=true），该 scope 内的 CNINFO live 执行不再算作"Approval Queue"项——它和 implementation/test/dry-run 一样进入 Autonomous Queue。Approval Queue 现在只保留两类：push 相关、以及 scope 本身尚未授权的方向。
...
| Queue | Contents |
|-------|----------|
| **Autonomous Queue** | Tasks executable **without** new human approval — 现包括：offline_safe · spent-scope only · **以及已授权 scope 内的 CNINFO live 执行（NEW）** |
| **Approval Queue** | Tasks blocked by missing **push** approval，或该 track/component **方向尚未被人类授权**（不再包括"scope 已授权但某个执行阶段是 live"这种情况） |
...
5. **（2026-07-15 NEW）** 若 track 的 scope 已被人类授权，**不得**把该 scope 内的 CNINFO live 任务放入 Approval Queue——直接放入 Autonomous Queue，执行状态用 `READY`/`RUNNING`/`FAILED`/`WAITING_RETRY`（见 [track stop reason v2 §2.1](controller_track_stop_reason_policy_v2.md)）。


### Example — D shareholder_change（更新）


| Queue | Examples |
|-------|----------|
| Approval | **仅当** shareholder_change 方向尚未被人类授权时：整个 component 的启动决定 |
| Autonomous（scope 已授权后） | event taxonomy · schema refinement · sample preparation · validation rules · offline evidence mapping · **runner implementation** · **CNINFO live execution** · evidence packaging · commit |
